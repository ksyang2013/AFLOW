  ...
  bool InitMachine(bool INIT_VERBOSE,vector<string>& argv,vector<string>& cmds,std::ostream& oss) {
   ...
   // XHOST.User="boot";
    ...
    if(!(XHOST.vflag_control.flag("AFLOW_HELP") || 
          ...
          XHOST.vflag_control.flag("README_HTRESOURCES") )){  // CO 180306 - need to catch --readme such that it doesn't interfere with --readme=
...
namespace init {
  string InitLoadString(string str2load,bool LVERBOSE) {
    ...
    if(LDEBUG) cerr << "init::InitLoadString:  length=" << out.length() << endl;
    if(LDEBUG) cerr.flush();
    //    if(LDEBUG) exit(0);
    return out;
  }
