...
AFLOWRuntimeError::AFLOWRuntimeError(const std::string& function,const std::string& message) : std::runtime_error(message),f_name(function) {}  // I/O or computer type errors (no entries loaded)
AFLOWRuntimeError::AFLOWRuntimeError(const std::string& function,std::stringstream& message) : std::runtime_error(message.str()),f_name(function) {message.str("");}  // I/O or computer type errors (no entries loaded)
string AFLOWRuntimeError::where(){return f_name;}
AFLOWLogicError::AFLOWLogicError(const std::string& function,const std::string& message) : std::logic_error(message),f_name(function) {}    //errors in logic, unintended (and insurmountable) use of functionality
AFLOWLogicError::AFLOWLogicError(const std::string& function,std::stringstream& message) : std::logic_error(message.str()),f_name(function) {message.str("");}    //errors in logic, unintended (and insurmountable) use of functionality
string AFLOWLogicError::where(){return f_name;}
...
int main(int _argc,char **_argv) {
 ...
 if(!Arun && aurostd::args2flag(argv,cmds,"--test=CHGCAR|--test=CHGCAR.relax1"+DEFAULT_KZIP_EXT+"|--test=CHGCAR.relax2"+DEFAULT_KZIP_EXT+"|--test=CHGCAR.static"+DEFAULT_KZIP_EXT+"|--test=CHGCAR.bands"+DEFAULT_KZIP_EXT+"")) {
  ...
  return (int) !Arun; //Arun==TRUE is 1, so flip because return 0 is normal
    ...
    pflow::logger(soliloquy, "AFLOWRuntimeError detected. Report on the AFLOW Forum: aflow.org/forum.", oss, _LOGGER_ERROR_);
    pflow::logger(re.where(), re.what(), oss, _LOGGER_ERROR_);
    return 1;
  }
  catch(AFLOWLogicError& le){
    pflow::logger(soliloquy, "AFLOWLogicError detected. Adjust your inputs accordingly.", oss, _LOGGER_ERROR_);
    pflow::logger(le.where(), le.what(), oss, _LOGGER_ERROR_);
    return 1;
  }
}
...
******* BEGIN RUNNING MODE ************************************************************************* \n\
 ...
 \n\
  HOST ORIENTED OPTION \n\
   --machine=beta | beta_openmpi | qrats | qflow | eos | materials | habana | aflowlib | ranger | kraken \n\
...
namespace aflow {
  string Banner(string type) {
    ...
    if(type=="BANNER_BIG") {
      ...
      oss << "*                                                                                                  *" << endl;
      oss << "*  Oses et al.      submitted (2018)                  (AFLOW-CloudHull)                            *" << endl;
