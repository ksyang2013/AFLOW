...
bool prepare_CHGCAR_4_Jmol(const string& _chgcar_file, string& species_header, bool zip_file, ostream& oss) {
  ...
  any_file_exists|=(aurostd::EFileExist(chgcar_file,chgcar_file_compressed) && aurostd::FileNotEmpty(chgcar_file_compressed));
  any_file_exists|=(aurostd::FileExist(chgcar_file) && aurostd::FileNotEmpty(chgcar_file));
  if(!any_file_exists){
    oss << endl;
  ...
  if(LDEBUG) oss << soliloquy << "UNCOMPRESSED VARIANT OF CHGCAR_FILE IS " << chgcar_file_uncompressed << endl;
...
bool PLOT_PHDISP(vector<string>& argv) {
  ...
  string phondisdat = directory + "/PDIS",phondisdat_compressed;
  string phondosdat = directory + "/PDOS",phondosdat_compressed;
  string phonthrmdat = directory + "/THERMO",phonthrmdat_compressed;
  ...
  if(aurostd::EFileExist(phondisdat,phondisdat_compressed) && aurostd::FileNotEmpty(phondisdat_compressed)){
    cout << "pflow::PLOT_PHDISP:  Decompressing PDIS.EXT . . . " << endl;
  ...
  else{cout << "pflow::PLOT_PHDISP:  AFLOW found no file named PDIS in " << directory << "." << endl;}

  // look for PDOS
  if(aurostd::EFileExist(phondosdat,phondosdat_compressed) && aurostd::FileNotEmpty(phondosdat_compressed)){
    cout << "pflow::PLOT_PHDISP:  Decompressing PDOS.EXT . . . " << endl;
  ...
  else{cout << "pflow::PLOT_PHDISP:  AFLOW found no file named PDOS in " << directory << "." << endl;}

  // look for THERMO
  if(aurostd::EFileExist(phonthrmdat,phonthrmdat_compressed) && aurostd::FileNotEmpty(phonthrmdat_compressed)){
    cout << "pflow::PLOT_PHDISP:  Decompressing THERMO.EXT . . . " << endl;
  ...
  else{cout << "pflow::PLOT_PHDISP:  AFLOW found no file named THERMO in " << directory << "." << endl;}
