...
ostream& operator<<(ostream& oss,const xstructure& a) { // operator<<
  ...
  if(a_iomode==IOVASP_AUTO || a_iomode==IOVASP_POSCAR || a_iomode==IOVASP_ABCCAR || a_iomode==IOVASP_WYCKCAR) { // VASP POSCAR
    ...
    if(a.partial_occupation_flag==TRUE) {
      ...
      else{oss << a.partial_occupation_stoich_tol;}
      if(a.scale_third.isentry){  // CO 170803 - site tol
        oss << " "; 
        oss << a.scale_third.content_double;
      }
      oss << std::fixed;
    }
    oss << endl;
    ...
    if(a.partial_occupation_flag==TRUE) {
      for(uint i=0;i<a.num_each_type.size();i++){
        ...
        //oss << std::defaultfloat;
        oss.unsetf(ios_base::floatfield);
        oss << a.atoms[i].partial_occupation_value << std::fixed << " ";
      }
    }else{
      for(uint i=0;i<a.num_each_type.size();i++){
      oss << a.num_each_type.at(i) << " ";
...
#define oss cout
  ...
  if(vinput.size()==0) {cerr << "XSTRUCTURE>>: No input..." << endl; exit(0);}  // CO 180420
      ...
      if(lat_found_anywhere || frac_found_anywhere){
        ...
        for(uint i=0;i<vinput.size();i++){
          ...
          if(tokens_line.size() && tokens_line[0]=="lattice_vector"){
            lat_count++;
            if(LDEBUG){cerr << "XSTRUCTURE>>: AIMS lat_count=" << lat_count << " line[" << i << "," << vinput[i] << "]" << endl;}
  ...
  if(a.iomode==IOVASP_AUTO || a.iomode==IOVASP_POSCAR || a.iomode==IOVASP_ABCCAR || a.iomode==IOVASP_WYCKCAR) { // VASP POSCAR
    ...
    if(a.partial_occupation_flag){ //&& a.neg_scale_second) { // CO 180409
      ...
      if(a.neg_scale_second){a.partial_occupation_HNF=(int) (-a.scale_second);} // CO 180409
        ...
        a.partial_occupation_stoich_tol=a.partial_occupation_site_tol=a.scale_second; // CO 180409
        if(a.scale_third.isentry){a.partial_occupation_site_tol=a.scale_third.content_double;} // CO 170803 - site tol
      }
      if(LDEBUG){
...
// GetSpaceGroupHall
// ***************************************************************************
string GetSpaceGroupHall(int spacegroupnumber, string directory) {
