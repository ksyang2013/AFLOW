...
uint PflowARGs(vector<string> &argv,vector<string> &cmds,aurostd::xoption &vpflow) {  // vpflow is really XHOST.vflag_pflow
  ...
  if(vpflow.flag("CHULL::INIT")) {
    ...
    vpflow.flag("CHULL::SKIP_STABILITY_CRITERION_ANALYSIS",aurostd::args2flag(argv,cmds,"--skip_stability_criterion_analysis|--skipstabilitycriterionanalysis|--skipscriterion|--sscriterion"));    //use entropic temperature instead of enthalpy of formation
    ...
    if(vpflow.flag("CHULL::LATEX_DOC")) {   //latex specific options
      ...
      vpflow.flag("CHULL::NO_DOC",aurostd::args2flag(argv,cmds,"--image_only|--imageonly|--image|--no_document|--nodocument|--no_doc|--nodoc|--full_page_image|--fullpageimage"));   //no convex hull picture
      ...
      vpflow.flag("CHULL::IMAGE_ONLY",aurostd::args2flag(argv,cmds,"--picture_only|--pictureonly|--picture|--pic"));    //image only, no doc or links, special compilation
  ...
  if(vpflow.flag("COMPARE_MATERIAL") || vpflow.flag("COMPARE_STRUCTURE")) {
    ...
    for(uint i=0;i<vinput.size();i++) {
      ...
      vpflow.push_attached(scheme_name,vinput[i]);
...
******* BEGIN POSTPROCESSING MODE ****************************************************************** \n\
  ...
  "<< x<<" --convex_hull=|--chull --alloy=MnPdPt[,AlCuZn,...] [chull_options] [--destination=[DIRECTORY]] \n\
    chull_options: \n\
                 ...
                 \n\
                 PDF/LATEX OPTIONS: \n\
                 --image_only|--imageonly|--image|--no_document|--nodocument|--no_doc|--nodoc|--full_page_image|--fullpageimage \n\
...
namespace pflow {
  bool SYMMETRY_GROUPS(_aflags &aflags,istream& input, aurostd::xoption& vpflow, ostream& oss){
	...
	if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_PGROUP_OUT+vext.at(i)) ||
      ...
      for(uint i=0;i<vext.size();i++) {
     	if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_PGROUP_OUT+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_PGROUP_OUT)){ aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_PGROUP_OUT+"*");}
	if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_PGROUP_XTAL_OUT+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_PGROUP_XTAL_OUT)){ aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_PGROUP_XTAL_OUT+"*");}
	if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_PGROUPK_OUT+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_PGROUPK_OUT)){ aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_PGROUPK_OUT+"*");}
	if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_PGROUPK_XTAL_OUT+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_PGROUPK_XTAL_OUT)){ aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_PGROUPK_XTAL_OUT+"*");} // DX 1/18/18 - added pgroupk_xtal
	if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_FGROUP_OUT+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_FGROUP_OUT)){ aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_FGROUP_OUT+"*");}
	if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_AGROUP_OUT+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_AGROUP_OUT)){ aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_AGROUP_OUT+"*");}
	if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_SGROUP_OUT+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_SGROUP_OUT)){ aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_SGROUP_OUT+"*");}
	if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_IATOMS_OUT+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_IATOMS_OUT)){ aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_IATOMS_OUT+"*");}
      }
      for(uint i=0;i<vext.size();i++) {
 	if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_PGROUP_JSON+vext.at(i)) ||
      ...
      for(uint i=0;i<vext.size();i++) {
 	if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_PGROUP_JSON+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_PGROUP_JSON)){   aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_PGROUP_JSON+"*");}
	if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_PGROUP_XTAL_JSON+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_PGROUP_XTAL_JSON)){   aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_PGROUP_XTAL_JSON+"*");}
	if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_PGROUPK_JSON+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_PGROUPK_JSON)){ aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_PGROUPK_JSON+"*");}
	if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_PGROUPK_XTAL_JSON+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_PGROUPK_XTAL_JSON)){ aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_PGROUPK_XTAL_JSON+"*");} // DX 1/18/18 - added pgroupk_xtal
	if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_FGROUP_JSON+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_FGROUP_JSON)){ aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_FGROUP_JSON+"*");}
	if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_AGROUP_JSON+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_AGROUP_JSON)){ aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_AGROUP_JSON+"*");}
	if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_SGROUP_JSON+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_SGROUP_JSON)){ aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_SGROUP_JSON+"*");}
	if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_IATOMS_JSON+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_IATOMS_JSON)){ aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_IATOMS_JSON+"*");}
      }
    }
...
namespace pflow {
  ...
  bool CalculateFullSymmetry(_aflags &aflags, _kflags& kflags, xstructure& _a, aurostd::xoption& vpflow, bool osswrite,ostream& oss){ //main function
	...
	if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_PGROUP_XTAL_OUT+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_PGROUP_XTAL_OUT)){ aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_PGROUP_XTAL_OUT+"*");}
	if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_PGROUPK_OUT+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_PGROUPK_OUT)){ aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_PGROUPK_OUT+"*");}
	if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_PGROUPK_XTAL_OUT+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_PGROUPK_XTAL_OUT)){ aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_PGROUPK_XTAL_OUT+"*");} // DX 12/5/17 - Added pgroupk_xtal
	if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_FGROUP_OUT+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_FGROUP_OUT)){ aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_FGROUP_OUT+"*");}
	if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_AGROUP_OUT+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_AGROUP_OUT)){ aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_AGROUP_OUT+"*");}
	if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_SGROUP_OUT+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_SGROUP_OUT)){ aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_SGROUP_OUT+"*");}
	if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_IATOMS_OUT+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_IATOMS_OUT)){ aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_IATOMS_OUT+"*");}
	...
	if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_PGROUP_XTAL_JSON+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_PGROUP_XTAL_JSON)){ aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_PGROUP_XTAL_JSON+"*");}
	if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_PGROUPK_JSON+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_PGROUPK_JSON)){ aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_PGROUPK_JSON+"*");}
	if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_PGROUPK_XTAL_JSON+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_PGROUPK_XTAL_JSON)){ aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_PGROUPK_XTAL_JSON+"*");} //DX 5/16/18 - added pgroupk_xtal
	if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_FGROUP_JSON+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_FGROUP_JSON)){ aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_FGROUP_JSON+"*");}
	if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_AGROUP_JSON+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_AGROUP_JSON)){ aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_AGROUP_JSON+"*");}
	if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_SGROUP_JSON+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_SGROUP_JSON)){ aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_SGROUP_JSON+"*");}
	if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_IATOMS_JSON+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_IATOMS_JSON)){ aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_IATOMS_JSON+"*");}
...
namespace pflow {
  string EXTRACT_Symmetry(_aflags &aflags,vector<string> argv) {
    ...
    for(uint i=0;i<vext.size();i++) {
      ...
      if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_PGROUPK_OUT+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_PGROUPK_OUT)) aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_PGROUPK_OUT+"*");
      if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_PGROUP_XTAL_OUT+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_PGROUP_XTAL_OUT)) aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_PGROUP_XTAL_OUT+"*");
      if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_PGROUPK_XTAL_OUT+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_PGROUPK_XTAL_OUT)) aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_PGROUPK_XTAL_OUT+"*");
      if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_FGROUP_OUT+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_FGROUP_OUT)) aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_FGROUP_OUT+"*");
      if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_AGROUP_OUT+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_AGROUP_OUT)) aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_AGROUP_OUT+"*");
      if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_SGROUP_OUT+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_SGROUP_OUT)) aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_SGROUP_OUT+"*");
      if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_IATOMS_OUT+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_IATOMS_OUT)) aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_IATOMS_OUT+"*");
      ...
      if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_PGROUPK_JSON+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_PGROUPK_JSON)) aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_PGROUPK_JSON+"*");
      if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_PGROUP_XTAL_JSON+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_PGROUP_XTAL_JSON)) aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_PGROUP_XTAL_JSON+"*");
      if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_PGROUPK_XTAL_JSON+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_PGROUPK_XTAL_JSON)) aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_PGROUPK_XTAL_JSON+"*");
      if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_FGROUP_JSON+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_FGROUP_JSON)) aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_FGROUP_JSON+"*");
      if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_AGROUP_JSON+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_AGROUP_JSON)) aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_AGROUP_JSON+"*");
      if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_SGROUP_JSON+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_SGROUP_JSON)) aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_SGROUP_JSON+"*");
      if(aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_IATOMS_JSON+vext.at(i)) || aurostd::FileExist(directory+"/"+DEFAULT_AFLOW_IATOMS_JSON)) aurostd::execute("rm -f "+directory+"/"+DEFAULT_AFLOW_IATOMS_JSON+"*");
...
namespace pflow {
  ...
  void getCombination(const vector<string>& velements, vector<string>& combination,
...
namespace pflow {
  bool PROTO_AFLOW(aurostd::xoption vpflow,bool flag_REVERSE) { // too many options
    ...
    if(vpflow.flag("PROTO_AFLOW::MODULE")) PARAMS.vparams.push_attached("AFLOWIN_FLAG::MODULE",vpflow.getattachedscheme("PROTO_AFLOW::MODULE"));
...
namespace pflow {
  bool QMVASP(vector<string> argv) {
    ...
    for(uint i=0;i<vCARS_relax.size();i++) {
      ...
      for(uint iext=0;iext<vext.size();iext++) {
	if(aurostd::FileExist(directory+"/"+vCARS_relax.at(i)+vext.at(iext)))
	  aurostd::execute(XHOST.command(vcmd.at(iext))+" "+directory+"/"+vCARS_relax.at(i)+vext.at(iext)+ " > "+directory+"/"+vCARS.at(i));
      }
    }
    ...
    deque<string> vcmd; aurostd::string2tokens("bzip2,xz,gzip",vcmd,",");
    for(uint iext=0;iext<vext.size();iext++) {
      if(aurostd::FileExist(directory+"/"+vCARS_relax.at(0)+vext.at(iext))) {
	aurostd::execute("rm -f "+xvasp.Directory+"/"+DEFAULT_AFLOW_QMVASP_OUT+vext.at(iext));    
	aurostd::execute(XHOST.command(vcmd.at(iext))+" -9q "+xvasp.Directory+"/"+DEFAULT_AFLOW_QMVASP_OUT);
      }
    }
    //  cout << a << endl;
    return TRUE;
