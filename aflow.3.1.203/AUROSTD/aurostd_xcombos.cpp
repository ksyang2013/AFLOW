...
xcombos::xcombos(int choice_count,int choose_count, bool rpt, bool seq) {reset(choice_count,choose_count, rpt, seq);}
...
void xcombos::free() {
  ...
  m_x=0;
  m_y=0;
  repeat = false; // ME180509
  sequence = false; // ME180516
}
...
void xcombos::copy(const xcombos& b) {  //copy PRIVATE
  ...
  m_x=b.m_x;
  m_y=b.m_y;
  sequence = b.sequence; // ME 180509
  repeat = b.repeat; // ME 180509
}
...
void xcombos::reset() {  //clear PUBLIC
  ...
  else {reset(n_choices,m_choose, repeat, sequence);}
}
...
void xcombos::reset(int choice_count,int choose_count, bool rpt, bool seq) {
  free();
  ...
  repeat = rpt;
  sequence = seq;
  if(n_choices<=0 || m_choose<=0 || ((!repeat) && (m_choose>n_choices))) {m_exhausted=TRUE; return;}  //safe
  initialize();
}
...
void xcombos::initialize() {
  ...
  if(m_permutation) {
    ...
    if(LDEBUG) {
      cerr << "combinations: " << n_choices << " choose " << m_choose << ((repeat)? " with " : " without ") << "repetitions.";
      if (sequence) {cerr << " Sequence important.";}
      cerr << endl;
    }
    if (repeat) {
...
void xcombos::incrementCombinations() {
  if(m_exhausted) {return;}
  if (repeat) { getNextRepeatCombination(); }
  else {
...
// ME 180509
// Creates the next combination with repetitions in lexicographical order
void xcombos::getNextRepeatCombination() {
  bool LDEBUG=(FALSE || XHOST.DEBUG);
  if (m_current[m_choose-1] == n_choices - 1) {
    ...
    if (LDEBUG) {cerr << "xcombos::getNextRepeatCombination: p(0) = " << p << endl;}
    if (p == -1)  {
      m_exhausted = true;
      return;
    } else {
  ...
  if (LDEBUG) {
    cerr << "xcombos::getNextRepeatCombination: m_current(0): ";
    for (uint i = 0; i < m_current.size(); i++) {
      cerr << m_current[i] << " ";
    }
    cerr << endl;
