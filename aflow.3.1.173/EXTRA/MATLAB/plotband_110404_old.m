...
if(pin==-1) disp(msg)
 ...
 while(~feof(pin))
  ...
  if(length(sLS)>0) LS=true; break; end
  sLS=strfind(sline,'LSCOUPLING =ON');
  if(length(sLS)>0) LS=true; break; end
  sLS=strfind(sline,'LSCOUPLING= ON');
  if(length(sLS)>0) LS=true; break; end
...
if(isMetal) vbmin=ybot;
 ...
 if(ispin==1)
  vbminup=vbmin;%initialization
  for ib=Nband:-1:2
    testbandup=up(1:Nk,ib);
	maxup=max(testbandup);
	if(maxup-vbminup > (-tolDeg))
...
xdosmax=0;
for i=1:nE
    if(EDOS(i,1)>(vbmin+maxEv) && EDOS(i,1)<(0+maxEv))
