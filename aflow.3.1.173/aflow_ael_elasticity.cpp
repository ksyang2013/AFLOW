  ...
  uint RunElastic_AEL(_xvasp& xvasp, string AflowIn, _aflags& aflags, _kflags& kflags, _vflags& vflags, _AEL_data& AEL_data, ofstream& FileMESSAGE) {
	...
	if(AEL_data.relax_static || AEL_data.static_only) {
	  if( aurostd::FileExist( vaspRuns.at(idVaspRun).Directory + "/" + _AFLOWLOCK_ ) ||
	      ...
	      aurostd::FileExist( vaspRuns.at(idVaspRun).Directory + string("/OUTCAR.static.bz2") ) ||
	      aurostd::FileExist( vaspRuns.at(idVaspRun).Directory + string("/OUTCAR.static.gz") ) ) continue; 	 
	} else {
	  if( aurostd::FileExist( vaspRuns.at(idVaspRun).Directory + "/" + _AFLOWLOCK_ ) ||
	      aurostd::FileExist( vaspRuns.at(idVaspRun).Directory + string("/OUTCAR.relax2") ) ||
	      aurostd::FileExist( vaspRuns.at(idVaspRun).Directory + string("/OUTCAR.relax2.bz2") ) ||
	      aurostd::FileExist( vaspRuns.at(idVaspRun).Directory + string("/OUTCAR.relax2.gz") ) ) continue; 	 
	}
	...
	for (uint k = 0; k < vaspRunsPressures.size(); k++) {
	  ...
	  for(uint idVaspRun = 0; idVaspRun < vaspRunsPressures.at(k).size(); idVaspRun++) {
	    ...
	    if(AEL_data.relax_static || AEL_data.static_only) {
		  ...
		  aurostd::FileExist( vaspRunsPressures.at(k).at(idVaspRun).Directory + string("/OUTCAR.static.bz2") ) ||
		  aurostd::FileExist( vaspRunsPressures.at(k).at(idVaspRun).Directory + string("/OUTCAR.static.gz") ) ) continue; 	 
	    } else {
	      if( aurostd::FileExist( vaspRunsPressures.at(k).at(idVaspRun).Directory + "/" + _AFLOWLOCK_ ) ||
		  aurostd::FileExist( vaspRunsPressures.at(k).at(idVaspRun).Directory + string("/OUTCAR.relax2") ) ||
		  aurostd::FileExist( vaspRunsPressures.at(k).at(idVaspRun).Directory + string("/OUTCAR.relax2.bz2") ) ||
		  aurostd::FileExist( vaspRunsPressures.at(k).at(idVaspRun).Directory + string("/OUTCAR.relax2.gz") ) ) continue; 	 
	    }
