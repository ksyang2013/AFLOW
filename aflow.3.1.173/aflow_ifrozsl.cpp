...
//const string FROZSL_RELATIVE_ENERGY_DATA = _AFLOW_FROZSL_MODES_FILE_;
...
namespace KBIN {
  void VASP_RunPhonons_FROZSL( _xvasp&  xvasp,
	...
	for(uint k=0;k<(uint) xvasp_aus.str.num_each_type.at(j);k++) {
      ...
      xvasp_aus.AVASP_flag_ALGO_scheme=vflags.KBIN_VASP_FORCE_OPTION_ALGO.scheme;
...
namespace FROZSL {
  bool WGET_OUTPUT(ofstream &FileMESSAGE,_aflags &aflags,_kflags &kflags) {
	...
	vfiles.push_back("./ARUN."+vdirectories.at(i)+"/aflow.qmvasp.out.bz2");
	vcommands.push_back(XHOST.command("bzgrep")+" \"H=\" "+aflags.Directory+"/ARUN."+vdirectories.at(i)+"/aflow.qmvasp.out.bz2");// >> aflow.frozsl.out");
      }
	...
	aflow_frozsl_out_stringstream << vdirectories.at(i) << " : " << aurostd::execute2string(vcommands.at(i));// << endl;
      ...
      string _energies; 
      if(aurostd::FileExist(_AFLOW_FROZSL_MODES_FILE_)) aurostd::file2string(_AFLOW_FROZSL_MODES_FILE_,_energies);
      if(aurostd::FileExist(_AFLOW_FROZSL_MODES_FILE_+".gz")) aurostd::gzfile2string(_AFLOW_FROZSL_MODES_FILE_+".gz",_energies);
      if(aurostd::FileExist(_AFLOW_FROZSL_MODES_FILE_+".bz2")) aurostd::bz2file2string(_AFLOW_FROZSL_MODES_FILE_+".bz2",_energies);
      ...
      aurostd::stringstream2file(aflow_frozsl_eigen_stringstream,aflags.Directory+"/"+_AFLOW_FROZSL_EIGEN_FILE_);
...
namespace FROZSL {
  bool Extract_INPUT(const string& AflowIn,ofstream &FileMESSAGE,stringstream &input_file,_aflags &aflags,_kflags &kflags) {
    ...
    aurostd::stringstream2file(input_file,string(aflags.Directory+"/"+_AFLOW_FROZSL_POSCAR_FILE_));
...
namespace FROZSL {
  bool Setup_frozsl_init_input(const string& AflowIn,ofstream &FileMESSAGE,stringstream &input_file,_aflags &aflags,_kflags &kflags) {
    ...
    //   aurostd::stringstream2file(input_file,string(aflags.Directory+"/"+_AFLOW_FROZSL_INPUT_FILE_));
...
namespace FROZSL {
  bool File_INPUT(const string& AflowIn,ofstream &FileMESSAGE,stringstream &input_file,_aflags &aflags,_kflags &kflags) {
    ...
    if(aurostd::substring2bool(AflowIn,"[AFLOW_FROZSL]FILE=",TRUE)) {
      ...
      kflags.KBIN_FROZSL_FILE_NAME=_AFLOW_FROZSL_INPUT_FILE_;
    }
