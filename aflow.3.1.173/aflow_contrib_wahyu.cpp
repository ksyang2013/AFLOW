...
void AConvaspBandgap(vector<string>& argv) {
  ...
  //  grep E-fermi from OUTCAR.bands.bz2
  //  determine the occupied/unocc bands in the EIGENVAL.bands.bz2
  //  Egap = CBM-VBM
  //in the bandsdir, there must be OUTCAR.bands.bz2 and EIGENVAL.bands.bz2
  char gaptype;
  float Egap,Efermi;
  string s,stmp,tag="";
  stringstream straus;
  
  s=argv.at(2); //directory
  //cerr<<"directory: "<<s<<endl;
  //OUTCAR.bands
  
  string file_tmp=aurostd::TmpFileCreate("AConvaspBandgap");
  aurostd::RemoveFile(file_tmp);

  if(aurostd::FileExist(s+"/OUTCAR.bands")) {
    aurostd::execute("grep E-fermi "+s+"/OUTCAR.bands > "+file_tmp);
  } else{
    if(!aurostd::FileExist(s+"/OUTCAR.bands.bz2")) {cerr<<"OUTCAR.bands or OUTCAR.bands.bz2 not found in the directory, aborting."<<endl;}
    aurostd::execute(XHOST.command("bzcat")+" "+s+"/OUTCAR.bands.bz2 | grep E-fermi > "+file_tmp);
  }
  ...
  //EIGENVAL.bands
  if(aurostd::FileExist(s+"/EIGENVAL.bands")) {
    aurostd::execute("cp "+s+"/EIGENVAL.bands "+file_tmp);
  }
  else{
    if(!aurostd::FileExist(s+"/EIGENVAL.bands.bz2")) {cerr<<"EIGENVAL.bands or EIGENVAL.bands.bz2 not found, aborting.";abort();}
    aurostd::execute(XHOST.command("bzcat")+" "+s+"/EIGENVAL.bands.bz2 > "+file_tmp);
  }
  straus.clear();straus.str(std::string());
  aurostd::file2stringstream(file_tmp,straus);
  Egap=-1.0;
...
void AConvaspBandgaps(istream& bandsdir, ostringstream& oss) {
  ...
  while(bandsdir.good()) {
    ...
    if(aurostd::FileExist(s+"/OUTCAR.bands")) {
      ...
      if(!aurostd::FileExist(s+"/OUTCAR.bands.bz2")) {cerr << "OUTCAR.bands or OUTCAR.bands.bz2 not found in the directory, aborting." << endl;break;}
      aurostd::execute(XHOST.command("bzcat")+" "+s+"/OUTCAR.bands.bz2 | grep E-fermi > "+file_tmp);
    }
    straus.clear();straus.str(std::string());
    aurostd::file2stringstream(file_tmp,straus);
    ...
    //EIGENVAL.bands
    //loading EIGENVAL.bands into stringstream strauss
...
void AConvaspBandgapListFromDOS(istream& doscar) {
  ...
  float f;
  bool fgz=false,fbz=false,fbz2=false;
  size_t ibz,ibz2,igz,idoscar;
  ...
  while(doscar.good()) {
    ...
    fgz=fbz=fbz2=false;
    igz=s.find(".gz");
    if(igz!=s.npos)	{fbz2=false; fbz=false; fgz=true; fext=".gz";}
    ibz=s.find(".bz");
    if(ibz!=s.npos)	{fbz2=false; fbz=true; fgz=false; fext=".bz";}
    ibz2=s.find(".bz2");
    if(ibz2!=s.npos) {fbz2=true; fbz=false; fgz=false; fext=".bz2";}
    if(fgz || fbz || fbz2) {
      stmp="";
      ...
      if(fgz) aurostd::execute(XHOST.command("gunzip")+" -fq "+stmp+fext);
      if(fbz) aurostd::execute(XHOST.command("bunzip2")+" -fq "+stmp+fext);
      if(fbz2) aurostd::execute(XHOST.command("bunzip2")+" -fq "+stmp+fext);
    }
    else stmp=s;
    dos.open(stmp.data());
    if(dos.good()) {
...
namespace pflow {
  void ICSD_CheckRaw(vector<string> argv) {
    ...
    */
    ...
    if(!iftmp) {cerr<<"ERROR can not open wliblist.tmp. Aborted"; exit(1);}
    while(!iftmp.eof()) {
      iftmp>>stmp;
    ...
    if(!iftmp) {cerr<<"ERROR can not open wrawlist.tmp. Aborted"; exit(1);}
    while(!iftmp.eof()) {
      iftmp>>stmp;
    ...
    //output LIBnotRAW
    for(i=0;i<Nlibnotraw;i++) cout<<vAFLOW_PROJECTS_DIRECTORIES.at(XHOST_LIBRARY_ICSD)<<"LIB/"<<LIBnotRAW[i]<<" NotInRaw"<<endl;
    //output RAWnotLIB
    for(i=0;i<Nrawnotlib;i++) cout<<vAFLOW_PROJECTS_DIRECTORIES.at(XHOST_LIBRARY_ICSD)<<"RAW/"<<RAWnotLIB[i]<<" NotInLib"<<endl;	
	
    // processing LIBRAW
    int itmp=0;
    string sdir,sraw,slib;
    for(i=0;i<Nlibraw;i++) {
      sdir = LIBRAW[i]+"/";
      slib = vAFLOW_PROJECTS_DIRECTORIES.at(XHOST_LIBRARY_ICSD)+"LIB/"+sdir;
      sraw = vAFLOW_PROJECTS_DIRECTORIES.at(XHOST_LIBRARY_ICSD)+"RAW/"+sdir;
      cerr << "Processing " << sraw << endl;
      // 1. check if exist
      ifound = aurostd::FileExist(sraw+"POSCAR.bands");
      if(ifound) ifound = aurostd::FileExist(sraw+"KPOINTS.bands");
      if(ifound) ifound = aurostd::FileExist(sraw+"EIGENVAL.bands");
      if(ifound) ifound = aurostd::FileExist(sraw+"DOSCAR.static.bz2");
      if(!ifound) {
	cout<<sraw<<" ERROR "<<"POSCAR.bands KPOINTS.bands EIGENVAL.bands DOSCAR.static.bz2 NOTEXIST"<<endl;
	continue;
      }
      // 2. diff POSCAR.bands
      aurostd::execute(XHOST.command("bzcat")+" "+slib+"POSCAR.bands.bz2 > wlib.tmp");
      aurostd::execute("diff -w -B wlib.tmp "+sraw+"POSCAR.bands | wc -l > wdiff.tmp");
      iftmp.open("wdiff.tmp");
      iftmp>>itmp;
      if(itmp>0) ifound=false;
      iftmp.close();
      if(!ifound) {
	cout<<sraw<<" ERROR "<<"POSCAR.bands"<<endl;
	continue;
      }
      // 3. diff KPOINTS.bands
      aurostd::execute(XHOST.command("bzcat")+" "+slib+"KPOINTS.bands.bz2 > wlib.tmp");
      aurostd::execute("diff -w -B wlib.tmp "+sraw+"KPOINTS.bands | wc -l > wdiff.tmp");
      iftmp.open("wdiff.tmp");
      iftmp>>itmp;
      if(itmp>0) ifound=false;
      iftmp.close();
      if(!ifound) {
	cout<<sraw<<" ERROR "<<"KPOINTS.bands"<<endl;
	continue;
      }
      /*
      // 4. diff EIGENVAL.bands
      aurostd::execute(XHOST.command("bzcat")+" "+slib+"EIGENVAL.bands.bz2 > wlib.tmp");
      aurostd::execute("diff -w -B wlib.tmp "+sraw+"EIGENVAL.bands | wc -l > wdiff.tmp");
      iftmp.open("wdiff.tmp");
      iftmp>>itmp;
      if(itmp>0) ifound=false;
      iftmp.close();
      if(!ifound) {
      cout<<sraw<<" ERROR "<<"EIGENVAL.bands"<<endl;
      continue;
      }
      */
      // 5. diff DOSCAR.static.bz2
      aurostd::execute("diff -w -B "+slib+"DOSCAR.static.bz2 "+sraw+"DOSCAR.static.bz2 | wc -l > wdiff.tmp");
      iftmp.open("wdiff.tmp");
      iftmp>>itmp;
      if(itmp>0) ifound=false;
      iftmp.close();
      if(!ifound) {
	cout<<sraw<<" ERROR "<<"DOSCAR.static.bz2"<<endl;
	continue;
      }
      cout<<sraw<<" OK"<<endl;
    }
    //delete tmp files
    aurostd::execute("rm -f wrawlist.tmp* wliblist.tmp* wlib.tmp* wdiff.tmp*");
  }
}
...
vector<string> GetMetalsList(bool v) {
  ...
  */
  ...
  if(v) {
    cout<<"Alkali Metals:";
    for(i=j;i<(int)vmetal.size();i++) cout<<" "<<vmetal[i];
    cout<<'\n';
  }
  j=(int)vmetal.size();
...
float GetBandGap_WAHYU(stringstream& ein,float Efermi,char& gaptype) {
  ...
  if(count==2) ispin=1;
  if(count==3) ispin=2;
  if(ispin==0) {cerr<<"ERROR ispin = 0, aborted"<<endl; abort();}
