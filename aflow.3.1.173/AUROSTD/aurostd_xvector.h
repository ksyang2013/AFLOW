...
namespace aurostd {
  ...
  template<class utype> bool
    isinteger(const xvector<utype>&,const utype& tol=(utype)0.01) __xprototype;
  ...
  template<class utype> xvector<utype>
    vector2xvector(const vector<utype>&,int lrows=1) __xprototype;
  ...
  template<class utype> xvector<utype>                  // clear values too small
    roundoff(const xvector<utype>&,utype tol=(utype)_AUROSTD_XVECTOR_TOLERANCE_ROUNDOFF_) __xprototype; // claar values too small

  int gcd(const xvector<int>&);                         // get GCD of vector

  template<class utype> xvector<utype>                                       // simply divide by GCD (useful for compounds)
    reduceByGCD(const xvector<utype>& in_V,                                  // simply divide by GCD (useful for compounds)
        const utype& tol=_AUROSTD_XVECTOR_TOLERANCE_IDENTITY_) __xprototype; // simply divide by GCD (useful for compounds)

  template<class utype> void                                   // swap
    swap(xvector<utype>&,const int&,const int&) __xprototype;  // swap
  
  template<class utype> void                              //shift lrows so first index is i
    shiftlrows(xvector<utype>&,const int&) __xprototype;  //shift lrows so first index is i
  ...
  template<class utype> xvector<double> 
    getGeneralAngles(const xvector<utype>& vec,const utype& tol=_AUROSTD_XVECTOR_TOLERANCE_IDENTITY_);

  template<class utype> double
    getGeneralAngle(const xvector<utype>& _vec,int _i,const utype& tol=_AUROSTD_XVECTOR_TOLERANCE_IDENTITY_);
