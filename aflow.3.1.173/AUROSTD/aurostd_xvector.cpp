...
// [[BUG]    if(nh==0 && nl==0) {
    ...
    if(vsize>0) {
      ...
      if(!corpus) {
	cerr << _AUROSTD_XLIBS_ERROR_ << " allocation failure in xvector default constructor" << endl;
	exit(0);
      }
      corpus+= -lrows+XXEND;
...
namespace aurostd {  // namespace aurostd
  ...
  xvector<utype>::xvector(const xvector<utype>& a) {
    ...
    if(vsize>0) {
      corpus=new utype[rows+XXEND];
      if(!corpus) {
	cerr << _AUROSTD_XLIBS_ERROR_ << " allocation failure in xvector copy constructor" << endl;
	exit(0);
      }
      corpus+= -lrows+XXEND;
      for(i=lrows;i<=urows;i++)
...
namespace aurostd {  // namespace aurostd
  ...
  xvector<utype>& xvector<utype>::operator=(const xvector<utype>& a) {
	...
	if(vsize>0) {
	  corpus=new utype[rows+XXEND];
	  if(!corpus) {
	    cerr << _AUROSTD_XLIBS_ERROR_ << " allocation failure in xvector ASSIGNEMENT" << endl;
	    exit(0);
	  }
	  corpus+= -lrows+XXEND;
	}
...
#endif
    if(this->rows!=r.rows) {
      cerr << _AUROSTD_XLIBS_ERROR_ << " failure in xvector operator+=:" << " (this->rows!=r.rows)=FALSE" << endl;
      exit(0);
    }
...
namespace aurostd {  // namespace aurostd
  ...
  operator*(const xvector<utype>& a,const xvector<utype>& b) {
    if(a.rows!=b.rows) {
      cerr << _AUROSTD_XLIBS_ERROR_ << " failure in xvector operator*" << endl;
      exit(0);
    }
    utype out=(utype) 0.0;
...
namespace aurostd {  // namespace aurostd
  ...
  operator>(const xvector<utype>& a,const xvector<utype>& b) {
    if(a.rows!=b.rows) {
      cerr << _AUROSTD_XLIBS_ERROR_ << " failure in xvector operator> (xvector > xvector)" << endl;
      exit(0);
    }
    xvector<char> c(a.lrows,a.urows);
    for(int i=a.lrows,ii=b.lrows;i<=a.urows;i++,ii++) {
...
namespace aurostd {  // namespace aurostd
  ...
  identical(const xvector<utype>& a,const xvector<utype>& b,const utype& _tol_) {
    if(a.rows!=b.rows)  {
      cerr << _AUROSTD_XLIBS_ERROR_ << " failure in xvector function identical (xvector == xvector)[1]" << endl;
      exit(0);
    }
    bool output=TRUE;
...
namespace aurostd {  // namespace aurostd
  ...
  isinteger(const xvector<utype>& a,const utype& tol) {
    for(int i=a.lrows;i<=a.urows;i++)
      if(isinteger(a[i],tol)==FALSE) return FALSE;
    return TRUE;
  }
...
namespace aurostd {                   // conversion to xvector<utype>
  template<class utype> xvector<utype>
  vector2xvector(const vector<utype> & vec,int lrows) {
    int isize=vec.size();
    xvector<utype> xvec(isize+lrows-1,lrows);
    for(int i=lrows;i<=isize+lrows-1;i++)
      xvec(i)=vec.at(i-lrows);
    return xvec;
  }
}
...
namespace aurostd {  // namespace aurostd
  ...
  xvector<float>  _aurostd_initialize_roundoff(const xvector<float>& a) { return roundoff(a);}
  xvector<double> _aurostd_initialize_roundoff(const xvector<double>& a) { return roundoff(a);}
...
namespace aurostd {
  template<class utype> bool  //determine if two vectors are collinear
...
namespace aurostd {
  template<class utype> xvector<utype> //get centroid of data points
...
namespace aurostd {
  template<class utype> xvector<double> 
    getGeneralAngles(const xvector<utype>& vec,const utype& tol) {
  ...
  template<class utype> double 
    getGeneralAngle(const xvector<utype>& _vec,int _i,const utype& tol) {
    //https://en.wikipedia.org/wiki/N-sphere#Spherical_coordinates
...
namespace aurostd {
  template<class utype> xvector<utype>
    getGeneralNormal(const vector<xvector<utype> >& _directive_vectors){
      ...
      //normal in n-dimensions is wedge product
...
namespace aurostd {
  template<class utype> void 
    getQuartiles(const xvector<utype>& _a,utype& q1,utype& q2,utype& q3){
...
namespace aurostd {
  template<class utype> utype
    getMAD(const xvector<utype>& _a,utype median){  //absolute deviation around the median (MAD)
