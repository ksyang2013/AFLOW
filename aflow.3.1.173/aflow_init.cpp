...
namespace init {
  ...
  bool InitMachine(bool INIT_VERBOSE,vector<string>& argv,vector<string>& cmds,std::ostream& oss) {
   ...
   // XHOST.User="boot";
    ...
    found=FALSE;
    if(!found&&(XHOST.Progname=="aflow"||XHOST.Progname=="aflowd"||XHOST.Progname=="aconvasp"||XHOST.Progname=="apennsy")) {XHOST.vcmd.push_back("aflow_data");found=TRUE;}
    if(!found&&(XHOST.Progname=="./aflow"||XHOST.Progname=="./aflowd"||XHOST.Progname=="./aconvasp"||XHOST.Progname=="./apennsy")) {XHOST.vcmd.push_back("./aflow_data");found=TRUE;}
    if(!found&&(XHOST.Progname=="/usr/local/bin/aflow"||XHOST.Progname=="/usr/local/bin/aflowd"||XHOST.Progname=="/usr/local/bin/aconvasp"||XHOST.Progname=="/usr/local/bin/apennsy")) {XHOST.vcmd.push_back("/usr/local/bin/aflow_data");found=TRUE;}
    if(!found) {
      aurostd::string2tokens(XHOST.argv.at(0),tokens,"/");
      string aflow_data="";
    ...
    if(INIT_VERBOSE) {
      ...
      if(XHOST.is_command("bunzip2")) {oss << aurostd::PaddedPOST("XHOST.is_command(\"bunzip2\")=TRUE",depth_long) << "[" << XHOST.command("bunzip2") << "]" << endl;} else {oss << "XHOST.is_command(\"bunzip2\")=FALSE" << endl;}
      if(XHOST.is_command("bzip2")) {oss << aurostd::PaddedPOST("XHOST.is_command(\"bzip2\")=TRUE",depth_long) << "[" << XHOST.command("bzip2") << "]" << endl;} else {oss << "XHOST.is_command(\"bzip2\")=FALSE" << endl;}
      ...
      if(XHOST.is_command("epstopdf")) {oss << aurostd::PaddedPOST("XHOST.is_command(\"epstopdf\")=TRUE",depth_long) << "[" << XHOST.command("epstopdf") << "]" << endl;} else {oss << "XHOST.is_command(\"epstopdf\")=FALSE" << endl;}
      if(XHOST.is_command("eps2pdf")) {oss << aurostd::PaddedPOST("XHOST.is_command(\"eps2pdf\")=TRUE",depth_long) << "[" << XHOST.command("eps2pdf") << "]" << endl;} else {oss << "XHOST.is_command(\"eps2pdf\")=FALSE" << endl;}
      ...
      if(XHOST.is_command("gunzip")) {oss << aurostd::PaddedPOST("XHOST.is_command(\"gunzip\")=TRUE",depth_long) << "[" << XHOST.command("gunzip") << "]" << endl;} else {oss << "XHOST.is_command(\"gunzip\")=FALSE" << endl;}
      if(XHOST.is_command("gzip")) {oss << aurostd::PaddedPOST("XHOST.is_command(\"gzip\")=TRUE",depth_long) << "[" << XHOST.command("gzip") << "]" << endl;} else {oss << "XHOST.is_command(\"gzip\")=FALSE" << endl;}
      ...
      if(XHOST.is_command("mpivasp53s")) {oss << aurostd::PaddedPOST("XHOST.is_command(\"mpivasp53s\")=TRUE",depth_long) << "[" << XHOST.command("mpivasp53s") << "]" << endl;} else {oss << "XHOST.is_command(\"mpivasp53s\")=FALSE" << endl;}
      ...
      if(XHOST.is_command("vasp53s")) {oss << aurostd::PaddedPOST("XHOST.is_command(\"vasp53s\")=TRUE",depth_long) << "[" << XHOST.command("vasp53s") << "]" << endl;} else {oss << "XHOST.is_command(\"vasp53s\")=FALSE" << endl;}
    ...
    if(INIT_VERBOSE) {
      ...
      // CO - START
      oss << "MPI_COMMAND_DUKE_QUSER_OPENMPI=" << MPI_COMMAND_DUKE_QUSER_OPENMPI << "\"" << endl;
      oss << "MPI_BINARY_DIR_DUKE_QUSER_OPENMPI=" << MPI_BINARY_DIR_DUKE_QUSER_OPENMPI << "\"" << endl;
      oss << "MPI_COMMAND_MPCDF_EOS_MPIIFORT=" << MPI_COMMAND_MPCDF_EOS_MPIIFORT << "\"" << endl;
      oss << "MPI_BINARY_DIR_MPCDF_EOS_MPIIFORT=" << MPI_BINARY_DIR_MPCDF_EOS_MPIIFORT << "\"" << endl;
      oss << "MPI_COMMAND_MPCDF_DRACO_MPIIFORT=" << MPI_COMMAND_MPCDF_DRACO_MPIIFORT << "\"" << endl;
      oss << "MPI_BINARY_DIR_MPCDF_DRACO_MPIIFORT=" << MPI_BINARY_DIR_MPCDF_DRACO_MPIIFORT << "\"" << endl;
      oss << "MPI_COMMAND_MPCDF_HYDRA_MPIIFORT=" << MPI_COMMAND_MPCDF_HYDRA_MPIIFORT << "\"" << endl;
      oss << "MPI_BINARY_DIR_MPCDF_HYDRA_MPIIFORT=" << MPI_BINARY_DIR_MPCDF_HYDRA_MPIIFORT << "\"" << endl;
      // CO - END
    ...
    XHOST.vflag_control.flag("MULTISH",aurostd::args2flag(argv,cmds,"--multish"));
    XHOST.vflag_control.flag("MULTIBZIP2",aurostd::args2flag(argv,cmds,"--multibzip2"));
    XHOST.vflag_control.flag("MULTIBUNZIP2",aurostd::args2flag(argv,cmds,"--multibunzip2"));
    XHOST.vflag_control.flag("MULTIGZIP",aurostd::args2flag(argv,cmds,"--multigzip"));
    XHOST.vflag_control.flag("MULTIGUNZIP",aurostd::args2flag(argv,cmds,"--multigunzip"));
    XHOST.vflag_control.flag("MULTIZIP",aurostd::args2flag(argv,cmds,"--multizip"));
    ...
    XHOST.vflag_control.addattachedscheme("DIRECTORY",dir,XHOST.vflag_control.flag("DIRECTORY"));
    ...
    XHOST.vflag_control.addattachedscheme("FILE",file,XHOST.vflag_control.flag("FILE"));
    ...
    XHOST.vflag_control.addattachedscheme("VDIR",dirs,XHOST.vflag_control.flag("VDIR")); 
    ...
    XHOST.vflag_control.addattachedscheme("VFILES",files,XHOST.vflag_control.flag("VFILES")); 
    ...
    XHOST.vflag_control.flag("README_QHA",aurostd::args2flag(argv,cmds,"--readme=qha|--readme_qha"));
    ...
    XHOST.vflag_control.addattachedscheme("XPLUG_PREFIX",aurostd::args2attachedstring(argv,"--prefix=",""),XHOST.vflag_control.flag("XPLUG_PREFIX"));
    XHOST.vflag_control.flag("XPLUG_NUM_ZIP",aurostd::args2attachedflag(argv,"--nzip="));
    XHOST.vflag_control.addattachedscheme("XPLUG_NUM_ZIP",aurostd::args2attachedstring(argv,"--nzip=",""),XHOST.vflag_control.flag("XPLUG_NUM_ZIP"));
    if(!XHOST.vflag_control.flag("XPLUG_NUM_ZIP")) XHOST.vflag_control.addattachedscheme("XPLUG_NUM_ZIP",aurostd::utype2string(1),TRUE);
    XHOST.vflag_control.flag("XPLUG_NUM_SIZE",aurostd::args2attachedflag(argv,"--nsize="));
    XHOST.vflag_control.addattachedscheme("XPLUG_NUM_SIZE",aurostd::args2attachedstring(argv,"--nsize=",""),XHOST.vflag_control.flag("XPLUG_NUM_SIZE"));
    if(!XHOST.vflag_control.flag("XPLUG_NUM_SIZE")) XHOST.vflag_control.addattachedscheme("XPLUG_NUM_SIZE",aurostd::utype2string(128),TRUE);
    ...
    XHOST.vflag_control.addattachedscheme("XPLUG_NUM_THREADS",aurostd::args2attachedstring(argv,"--np=",""),XHOST.vflag_control.flag("XPLUG_NUM_THREADS"));
    if(!XHOST.vflag_control.flag("XPLUG_NUM_THREADS")){
      if(XHOST.vflag_control.flag("XPLUG_NUM_THREADS_MAX")){XHOST.vflag_control.addattachedscheme("XPLUG_NUM_THREADS",aurostd::utype2string(XHOST.CPU_Cores),TRUE);}  //CO 180124
      else{XHOST.vflag_control.addattachedscheme("XPLUG_NUM_THREADS",aurostd::utype2string(XHOST.CPU_Cores/2),TRUE);}
    }

    // AFLOWLIB_SERVER
    XHOST.vflag_control.flag("AFLOWLIB_SERVER",aurostd::args2attachedflag(argv,"--server="));
    XHOST.vflag_control.addattachedscheme("AFLOWLIB_SERVER",aurostd::args2attachedstring(argv,"--server=",""),XHOST.vflag_control.flag("AFLOWLIB_SERVER"));
    if(XHOST.vflag_control.getattachedscheme("AFLOWLIB_SERVER")=="default" || XHOST.vflag_control.getattachedscheme("AFLOWLIB_SERVER")=="aflowlib") {
      XHOST.vflag_control.purgeattachedscheme("AFLOWLIB_SERVER");
      XHOST.vflag_control.addattachedscheme("AFLOWLIB_SERVER","aflowlib.duke.edu",TRUE);
    }
    ...
    XHOST.vflag_control.addattachedscheme("CV::AUTHOR",aurostd::args2attachedstring(argv,"--author=",""),XHOST.vflag_control.flag("CV::AUTHOR"));
    XHOST.vflag_control.flag("PHP::PUBS_ALLOY",aurostd::args2attachedflag(argv,"--php_pubs_alloy="));
    XHOST.vflag_control.addattachedscheme("PHP::PUBS_ALLOY",aurostd::args2attachedstring(argv,"--php_pubs_alloy=",""),XHOST.vflag_control.flag("PHP::PUBS_ALLOY"));
    XHOST.vflag_control.flag("PHP::PUBS_KEYWORD",aurostd::args2attachedflag(argv,"--php_pubs_keyword="));
    XHOST.vflag_control.addattachedscheme("PHP::PUBS_KEYWORD",aurostd::args2attachedstring(argv,"--php_pubs_keyword=",""),XHOST.vflag_control.flag("PHP::PUBS_KEYWORD"));
    XHOST.vflag_control.flag("GRANTS",aurostd::args2flag(argv,cmds,"--grant|--grants"));
    XHOST.vflag_control.addattachedscheme("GRANTS",aurostd::args2attachedstring(argv,"--grant=|--grants=",""),XHOST.vflag_control.flag("GRANTS"));
    if(LDEBUG) cout << "OUTREACH OPTIONS: scheme=" << XHOST.vflag_control.scheme << endl;
    if(LDEBUG) cout << "OUTREACH OPTIONS: vscheme.size()=" << XHOST.vflag_control.vscheme.size() << endl;
    if(LDEBUG) cout << "OUTREACH OPTIONS: vsghost.size()=" << XHOST.vflag_control.vsghost.size() << endl;
...
namespace init {
  string InitLoadString(string str2load,bool LVERBOSE) {
    ...
    if(LDEBUG) cerr << "init::InitLoadString: str2load=" << str2load << endl; 
	...
	if(LDEBUG) cerr << "init::InitLoadString: FOUND " << XHOST.Home << "/work/AFLOW3/aflow_data" << endl;
	if(LDEBUG) cerr << "init::InitLoadString: out=" << out << endl; 
	if(LDEBUG) cerr << "init::InitLoadString: str2load=" << str2load << endl; 
      } else {
    ...
    if(LDEBUG) cerr << " length=" << out.length();// << " " << endl;
    if(LDEBUG) cerr.flush();
    if(LDEBUG) exit(0);
    return out;
  }
...
namespace init {
  string InitGlobalObject(string str,string grep,bool LVERBOSE) {
    ...
    if(str=="README_AFLOW_SYMMETRY_TXT") { if(XHOST_README_AFLOW_SYMMETRY_TXT.empty()) { return XHOST_README_AFLOW_SYMMETRY_TXT=init::InitLoadString(str,LVERBOSE);} else { return XHOST_README_AFLOW_SYMMETRY_TXT;}} // LOADED TXTS
