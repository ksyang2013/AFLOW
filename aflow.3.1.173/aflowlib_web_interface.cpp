...
#include "aflow.h"
#include "aflow_entry.cpp"  //CO 170622 - bob hanson JMOL stuff
...
namespace aflowlib {
  ...
  uint _aflowlib_entry::file2aflowlib(const string& file,ostream& oss) {
    if(!aurostd::FileExist(file)) {cerr << "ERROR - _aflowlib_entry::file2aflowlib: " << _FILE_AFLOWLIB_ENTRY_OUT_ << " not found =" << file << endl;return 0;} //exit(0);} //CO 170609, this is a dud
    string entry;
  ...
  uint _aflowlib_entry::url2aflowlib(const string& _url,ostream& oss,bool verbose) {
    ...
    if(aurostd::substring2bool(url,"index") || aurostd::substring2bool(url,"format")) {
      aurostd::StringSubst(url,"/"+_FILE_AFLOWLIB_ENTRY_OUT_,"");
      if(!aurostd::url2string(url,entry,verbose)){return 0;}   //corey, this is a dud
    } else {
      aurostd::StringSubst(url,"/"+_FILE_AFLOWLIB_ENTRY_OUT_,"");
      if(!aurostd::url2string(url+"/"+_FILE_AFLOWLIB_ENTRY_OUT_,entry,verbose)){return 0;}  //corey, this is a dud
    }
    return Load(entry,oss);
  }
  ...
  uint _aflowlib_entry::Load(const string& _entry,ostream& oss) {
    ...
    for(uint i=0;i<ventry.size();i++) {
      ...
      if(tokens.size()>0) {
	keyword=tokens.at(0);
	if(tokens.size()>1) {content=tokens.at(1);} else {content="";}
	aurostd::string2tokens(content,stokens,",");
	if(keyword=="auid") {auid=content;}
	if(keyword=="aurl") {aurl=content;aurostd::string2tokens(content,stokens,":");for(uint j=0;j<stokens.size();j++) vaurl.push_back(stokens.at(j));}
	if(keyword=="keywords") {keywords=content;aurostd::string2tokens(content,stokens,",");for(uint j=0;j<stokens.size();j++) vkeywords.push_back(stokens.at(j));}
	if(keyword=="aflowlib_date") {aflowlib_date=content;}
 	if(keyword=="aflowlib_version") {aflowlib_version=content;}
 	if(keyword=="aflowlib_entries") {aflowlib_entries=content;aurostd::string2tokens(content,stokens,",");for(uint j=0;j<stokens.size();j++) vaflowlib_entries.push_back(stokens.at(j));}
 	if(keyword=="aflowlib_entries_number") {aflowlib_entries_number=aurostd::string2utype<int>(content);}
	if(keyword=="aflow_version") {aflow_version=content;}
	if(keyword=="catalog") {catalog=content;}
	if(keyword=="data_api") {data_api=content;}
	if(keyword=="data_source") {data_source=content;}
	if(keyword=="data_language") {data_language=content;}
	if(keyword=="error_status") {error_status=content;}
	if(keyword=="author") {author=content;aurostd::string2tokens(content,stokens,",");for(uint j=0;j<stokens.size();j++) vauthor.push_back(stokens.at(j));}
	if(keyword=="calculation_cores") {calculation_cores=aurostd::string2utype<int>(content);}
	if(keyword=="calculation_memory") {calculation_memory=aurostd::string2utype<double>(content);}
	if(keyword=="calculation_time") {calculation_time=aurostd::string2utype<double>(content);}
	if(keyword=="corresponding") {corresponding=content;aurostd::string2tokens(content,stokens,",");for(uint j=0;j<stokens.size();j++) vcorresponding.push_back(stokens.at(j));}
	if(keyword=="loop") {vloop.push_back(content);}  // CHECK THIS OUT IN THE FITURE
	if(keyword=="node_CPU_Cores") {node_CPU_Cores=aurostd::string2utype<int>(content);}
	if(keyword=="node_CPU_MHz") {node_CPU_MHz=aurostd::string2utype<double>(content);}
	if(keyword=="node_CPU_Model") {node_CPU_Model=content;}
	if(keyword=="node_RAM_GB") {node_RAM_GB=aurostd::string2utype<double>(content);}
	if(keyword=="Bravais_lattice_orig") {Bravais_lattice_orig=content;}
	if(keyword=="Bravais_lattice_relax") {Bravais_lattice_relax=content;}
	if(keyword=="code") {code=content;}
	if(keyword=="composition") {composition=content;for(uint j=0;j<stokens.size();j++) vcomposition.push_back(aurostd::string2utype<double>(stokens.at(j)));}
	if(keyword=="compound") {compound=content;}
	if(keyword=="density") {density=aurostd::string2utype<double>(content);}
	if(keyword=="dft_type") {dft_type=content;aurostd::string2tokens(content,stokens,",");for(uint j=0;j<stokens.size();j++) vdft_type.push_back(stokens.at(j));}
	if(keyword=="eentropy_cell") {eentropy_cell=aurostd::string2utype<double>(content);}
	if(keyword=="eentropy_atom") {eentropy_atom=aurostd::string2utype<double>(content);}
	if(keyword=="Egap") {Egap=aurostd::string2utype<double>(content);}
	if(keyword=="Egap_fit") {Egap_fit=aurostd::string2utype<double>(content);}
	if(keyword=="energy_cell") {energy_cell=aurostd::string2utype<double>(content);}
	if(keyword=="energy_atom") {energy_atom=aurostd::string2utype<double>(content);energy_atom_relax1=aurostd::string2utype<double>(content);}
	if(keyword=="energy_cutoff") {energy_cutoff=aurostd::string2utype<double>(content);}
	if(keyword=="delta_electronic_energy_convergence") {delta_electronic_energy_convergence=aurostd::string2utype<double>(content);}
	if(keyword=="delta_electronic_energy_threshold") {delta_electronic_energy_threshold=aurostd::string2utype<double>(content);}
	if(keyword=="nkpoints") {nkpoints=aurostd::string2utype<uint>(content);}
	if(keyword=="nkpoints_irreducible") {nkpoints_irreducible=aurostd::string2utype<uint>(content);}
	if(keyword=="kppra") {kppra=aurostd::string2utype<uint>(content);}
	if(keyword=="kpoints") {kpoints=content;}
	if(keyword=="enthalpy_cell") {enthalpy_cell=aurostd::string2utype<double>(content);}
	if(keyword=="enthalpy_atom") {enthalpy_atom=aurostd::string2utype<double>(content);}
	if(keyword=="enthalpy_formation_cell") {enthalpy_formation_cell=aurostd::string2utype<double>(content);}
	if(keyword=="enthalpy_formation_atom") {enthalpy_formation_atom=aurostd::string2utype<double>(content);}
	if(keyword=="entropic_temperature") {entropic_temperature=aurostd::string2utype<double>(content);}
	if(keyword=="files") {files=content;for(uint j=0;j<stokens.size();j++) vfiles.push_back(stokens.at(j));}
	if(keyword=="files_LIB") {files_LIB=content;for(uint j=0;j<stokens.size();j++) vfiles_LIB.push_back(stokens.at(j));}
	if(keyword=="files_RAW") {files_RAW=content;for(uint j=0;j<stokens.size();j++) vfiles_RAW.push_back(stokens.at(j));}
	if(keyword=="files_WEB") {files_WEB=content;for(uint j=0;j<stokens.size();j++) vfiles_WEB.push_back(stokens.at(j));}
	if(keyword=="forces") {forces=content;for(uint j=0;j<stokens.size();j++) vforces.push_back(aurostd::string2utype<double>(stokens.at(j)));}  // FIX
	if(keyword=="geometry") {
	  geometry=content; 
	...
	if(keyword=="lattice_system_orig") {lattice_system_orig=content;}
	if(keyword=="lattice_variation_orig") {lattice_variation_orig=content;}
	if(keyword=="lattice_system_relax") {lattice_system_relax=content;}
	if(keyword=="lattice_variation_relax") {lattice_variation_relax=content;}
	if(keyword=="ldau_TLUJ") {ldau_TLUJ=content;}
	if(keyword=="natoms") {natoms=aurostd::string2utype<int>(content);}
	if(keyword=="nbondxx") {nbondxx=content;for(uint j=0;j<stokens.size();j++) vnbondxx.push_back(aurostd::string2utype<double>(stokens.at(j)));}
	if(keyword=="nspecies") {nspecies=aurostd::string2utype<int>(content);}
	if(keyword=="Pearson_symbol_orig") {Pearson_symbol_orig=content;}
	if(keyword=="Pearson_symbol_relax") {Pearson_symbol_relax=content;}
	if(keyword=="positions_cartesian") {positions_cartesian=content;for(uint j=0;j<stokens.size();j++) vpositions_cartesian.push_back(aurostd::string2utype<double>(stokens.at(j)));}  // FIX
	if(keyword=="positions_fractional") {positions_fractional=content;for(uint j=0;j<stokens.size();j++) vpositions_fractional.push_back(aurostd::string2utype<double>(stokens.at(j)));}  // FIX
	if(keyword=="pressure") {pressure=aurostd::string2utype<double>(content);}
	if(keyword=="stress_tensor") {
	...
	if(keyword=="pressure_residual") {pressure_residual=aurostd::string2utype<double>(content);}
	if(keyword=="Pulay_stress") {Pulay_stress=aurostd::string2utype<double>(content);}
	if(keyword=="prototype") {prototype=content;}  // apennsy
	if(keyword=="PV_cell") {PV_cell=aurostd::string2utype<double>(content);}
	if(keyword=="PV_atom") {PV_atom=aurostd::string2utype<double>(content);}
	if(keyword=="scintillation_attenuation_length") {scintillation_attenuation_length=aurostd::string2utype<double>(content);}
	if(keyword=="sg") {sg=content;for(uint j=0;j<stokens.size();j++) vsg.push_back(stokens.at(j));} //CO 180101
	if(keyword=="sg2") {sg2=content;for(uint j=0;j<stokens.size();j++) vsg2.push_back(stokens.at(j));} //CO 180101
	if(keyword=="spacegroup_orig") {spacegroup_orig=content;}
	if(keyword=="spacegroup_relax") {spacegroup_relax=content;}
	if(keyword=="species") {species=content;for(uint j=0;j<stokens.size();j++) vspecies.push_back(stokens.at(j));}
	if(keyword=="species_pp") {species_pp=content;for(uint j=0;j<stokens.size();j++) vspecies_pp.push_back(stokens.at(j));}
	if(keyword=="species_pp_version") {species_pp_version=content;for(uint j=0;j<stokens.size();j++) vspecies_pp_version.push_back(stokens.at(j));}
	if(keyword=="species_pp_ZVAL") {species_pp_ZVAL=content;for(uint j=0;j<stokens.size();j++) vspecies_pp_ZVAL.push_back(aurostd::string2utype<double>(stokens.at(j)));}
	if(keyword=="spin_cell") {spin_cell=aurostd::string2utype<double>(content);}
	if(keyword=="spin_atom") {spin_atom=aurostd::string2utype<double>(content);}
	if(keyword=="spinD") {spinD=content;for(uint j=0;j<stokens.size();j++) vspinD.push_back(aurostd::string2utype<double>(stokens.at(j)));}
	if(keyword=="spinD_magmom_orig") {spinD_magmom_orig=content;for(uint j=0;j<stokens.size();j++) vspinD_magmom_orig.push_back(aurostd::string2utype<double>(stokens.at(j)));}
	if(keyword=="spinF") {spinF=aurostd::string2utype<double>(content);}
	if(keyword=="sponsor") {sponsor=content;aurostd::string2tokens(content,stokens,",");for(uint j=0;j<stokens.size();j++) vsponsor.push_back(stokens.at(j));}
	if(keyword=="stoichiometry") {stoichiometry=content;for(uint j=0;j<stokens.size();j++) vstoichiometry.push_back(aurostd::string2utype<double>(stokens.at(j)));}
	if(keyword=="Egap_type") {Egap_type=content;}
	if(keyword=="valence_cell_std") {valence_cell_std=aurostd::string2utype<double>(content);}
	if(keyword=="valence_cell_iupac") {valence_cell_iupac=aurostd::string2utype<double>(content);}
	if(keyword=="volume_cell") {volume_cell=aurostd::string2utype<double>(content);}
	if(keyword=="volume_atom") {volume_atom=aurostd::string2utype<double>(content);}
 	// legacy
	if(keyword=="server") {vserver.push_back(content);}
	if(keyword=="stoich") {aurostd::string2tokens(ventry.at(i),tokens,"=");stoich=tokens.at(1);aurostd::string2tokens(stoich,stokens);for(uint j=0;j<stokens.size();j++) vstoich.push_back(aurostd::string2utype<double>(stokens.at(j)));}
	// AGL/AEL
	if(keyword=="agl_thermal_conductivity_300K") {agl_thermal_conductivity_300K=aurostd::string2utype<double>(content);}
	if(keyword=="agl_debye") {agl_debye=aurostd::string2utype<double>(content);}
	if(keyword=="agl_acoustic_debye") {agl_acoustic_debye=aurostd::string2utype<double>(content);}
	if(keyword=="agl_gruneisen") {agl_gruneisen=aurostd::string2utype<double>(content);}
	if(keyword=="agl_heat_capacity_Cv_300K") {agl_heat_capacity_Cv_300K=aurostd::string2utype<double>(content);}
	if(keyword=="agl_heat_capacity_Cp_300K") {agl_heat_capacity_Cp_300K=aurostd::string2utype<double>(content);}
	if(keyword=="agl_thermal_expansion_300K") {agl_thermal_expansion_300K=aurostd::string2utype<double>(content);}
	if(keyword=="agl_bulk_modulus_static_300K") {agl_bulk_modulus_static_300K=aurostd::string2utype<double>(content);}
	if(keyword=="agl_bulk_modulus_isothermal_300K") {agl_bulk_modulus_isothermal_300K=aurostd::string2utype<double>(content);}
	if(keyword=="ael_poisson_ratio") {ael_poisson_ratio=aurostd::string2utype<double>(content);}
	if(keyword=="ael_bulk_modulus_voigt") {ael_bulk_modulus_voigt=aurostd::string2utype<double>(content);}
	if(keyword=="ael_bulk_modulus_reuss") {ael_bulk_modulus_reuss=aurostd::string2utype<double>(content);}
	if(keyword=="ael_shear_modulus_voigt") {ael_shear_modulus_voigt=aurostd::string2utype<double>(content);}
	if(keyword=="ael_shear_modulus_reuss") {ael_shear_modulus_reuss=aurostd::string2utype<double>(content);}
	if(keyword=="ael_bulk_modulus_vrh") {ael_bulk_modulus_vrh=aurostd::string2utype<double>(content);}
	if(keyword=="ael_shear_modulus_vrh") {ael_shear_modulus_vrh=aurostd::string2utype<double>(content);}
	if(keyword=="ael_elastic_anistropy") {ael_elastic_anistropy=aurostd::string2utype<double>(content);}
	// BADER
	if(keyword=="bader_net_charges") {bader_net_charges=content;aurostd::string2tokens<double>(content,vbader_net_charges,",");}
	if(keyword=="bader_atomic_volumes") {bader_atomic_volumes=content;aurostd::string2tokens<double>(content,vbader_atomic_volumes,",");}
      }
    }
    // FIX LOOP
    loop="";
...
namespace aflowlib {
  ...
  void _aflowlib_entry::correctBadDatabase(ofstream& FileMESSAGE,bool verbose,ostream& oss){
    ...
    //APENNSY fix - binaries only
    string soliloquy="_aflowlib_entry::correctBadDatabase():";
    stringstream message;
    if(vspecies_pp.size()==2) {
      string pseudoA = vspecies_pp[0];
      string pseudoB = vspecies_pp[1];
      // tiny corrections
      //gamma_IrV
...
namespace aflowlib {
  ...
  string _aflowlib_entry::getPathAURL(ofstream& FileMESSAGE,ostream& oss, bool load_from_common){
    ...
    if(load_from_common){return "/www/"+tokens.at(1);}
    else{
      string server;
...
namespace aflowlib {
  string directory2auid(string directory,string aurl) {
    ...
    while (conflict) {
      ...
      vfiles2.push_back("OUTCAR.relax1.bz2");vfiles2.push_back("OUTCAR.relax2.bz2");vfiles2.push_back("OUTCAR.relax3.bz2");vfiles2.push_back("OUTCAR.relax4.bz2");
      vfiles2.push_back("OUTCAR.static.bz2");vfiles2.push_back("OUTCAR.bands.bz2");vfiles2.push_back("OUTCAR.bz2");
      for(uint i=0;i<vfiles2.size();i++) 
	if(aurostd::FileExist(directory+"/"+vfiles2.at(i)))
	  crc=aurostd::crc64(crc,aurostd::bz2file2string(directory+"/"+vfiles2.at(i))); // DONT TOUCH THIS
	...
	if(vAURL.at(j)!=aurl) { // avoid conflict with yourself
	  ...
	  aurostd::StringSubst(file,".bz2","");
	  stringstream sss;aurostd::bz2file2stringstream(directory+"/"+file+".bz2",sss); sss << endl << salt << endl;
	  aurostd::execute("mv "+directory+"/"+file+".bz2"+" "+directory+"/"+file+".conflict_auid.bz2");
	  aurostd::stringstream2file(sss,directory+"/"+file);
	  aurostd::BzipFile(directory+"/"+file);
...
namespace aflowlib {
  ...
  void _aflowlib_entry::GetSGROUP(string aflowlibentry) {
    ...
    if(vaflowlib_entry.size()==0) {cerr << "ERROR - aflowlib_entry::GetSGROUP(): " << _FILE_AFLOWLIB_ENTRY_OUT_ << " file not found " << endl;exit(0);}
...
namespace aflowlib {
  uint WEB_Aflowlib_Entry_PHP(string options,ostream& oss) {
    ...
    string art058_link=" [<a href=http://dx.doi.org/10.1016/j.commatsci.2010.05.010><font color=black><i>info</i></font></a>]";
    string art064_link=" [<a href=http://dx.doi.org/10.1021/co200012w><font color=black><i>info</i></font></a>]";
    string icsd_link=" [<a href=http://www.fiz-karlsruhe.com/icsd.html><font color=black><i>info</i></font></a>]";
    string art096_link=" [<a href=http://dx.doi.org/10.1103/PhysRevB.90.174107><font color=black><i>info</i></font></a>]";
    string art100_link=" [<a href=http://www.nature.com/articles/sdata20159><font color=black><i>info</i></font></a>]";
    ...
    if(!directory.empty()) { // play with aentry.entry
      aentry.file2aflowlib(directory_RAW+"/"+_FILE_AFLOWLIB_ENTRY_OUT_,oss);  //   oss << aentry.entry << endl;
    ...
    // ***************************************************************************
    // NEW JSMOL BEGIN
    if(vflags.flag("FLAG::FOUND") && !directory.empty() && aurostd::substring2bool(aentry.vfiles_WEB,label+".cif")) {
    ...
    if(vflags.flag("FLAG::FOUND") && vflags.flag("FLAG::CALCULATION") && !directory.empty()) {
      ...
      oss << "<span class=\"title\"><FONT SIZE=+3>Calculation details</font></span>" << endl;
      oss << line_rule << endl;
      //      oss << "<div class=\"calculation_details\">" << endl;
      oss << "<div>" << endl;
      oss << "<ul>" << endl;
      if(!aentry.code.empty()) {
	oss << "<li><span class=\"description\">aflowlib.out:</span>" << "[" << "<a href=\"" << url_WEB << "/\"" << html_TAB << ">entry</a>|" << "<a href=\"" << url_WEB << "/aflowlib.out\"" << html_TAB << ">raw</a>]" << "</li>" << endl;
	oss << "<li><span class=\"description\">aflowlib.json:</span>" << "[" << "<a href=\"" << url_WEB << "/?format=json\"" << html_TAB << ">entry</a>|" << "<a href=\"" << url_WEB << "/aflowlib.json\"" << html_TAB << ">raw</a>]" << "</li>" << endl;
      }
      if(!aentry.auid.empty()) 
	oss << "<li><span class=\"description\">AFLOW-UID [<a href=\"" << url_WEB << "/?auid\">auid</a>]:</span>" << aentry.auid << "</li>" << endl;
      if(!aentry.aurl.empty()) 
	oss << "<li><span class=\"description\">AFLOW-URL [<a href=\"" << url_WEB << "/?aurl\">aurl</a>]:</span>" << aentry.aurl << "</li>" << endl;
      if(!aentry.code.empty()) 
	oss << "<li><span class=\"description\"><i>Ab-initio</i> code [<a href=\"" << url_WEB << "/?code\">code</a>]:</span>" << aentry.code << (aentry.calculation_cores>1?" (MPI) ":"") << "</li>" << endl;
      if(!aentry.compound.empty()) 
	oss << "<li><span class=\"description\">Composition per cell [<a href=\"" << url_WEB << "/?compound\">compound</a>]:</span>" << aentry.compound << "</li><!br>" << endl;
      if(!aentry.icsd.empty()) 
	oss << "<li><span class=\"description\">ICSD entry" << icsd_link << ":</span>#" << aentry.icsd << "</li><!br>" << endl;
      if(!aentry.dft_type.empty()) 
	oss << "<li><span class=\"description\">Pseudopotentials type [<a href=\"" << url_WEB << "/?pp_type\">pp_type</a>]:</span>" << aentry.dft_type << "</li><!br>" << endl;
      for(uint i=0;i<aentry.vspecies_pp.size();i++)  
	oss << "<li><span class=\"description\">PP - Species&middot;Version [<a href=\"" << url_WEB << "/?species_pp_version\">species_pp_version</a>]:</span>" << aentry.vspecies_pp_version.at(i) << "</li><!br>" << endl;
      for(uint i=0;i<aentry.vspecies_pp_ZVAL.size();i++)  
	oss << "<li><span class=\"description\">PP - Species&middot;ZVAL [<a href=\"" << url_WEB << "/?species_pp_ZVAL\">species_pp_ZVAL</a>]:</span>" << aentry.vspecies_pp_ZVAL.at(i) << "</li><!br>" << endl;
      oss << "<li><span class=\"description\">LDAU [T;{L};{U};{J}] [<a href=\"" << url_WEB << "/?ldau_TLUJ\">ldau_TLUJ</a>]:</span>" << (!aentry.ldau_TLUJ.empty()?aentry.ldau_TLUJ+"  (type;{angular-1};{eV};{eV}) ":"no-LDAU") << "</li>" << endl;
      if(aentry.calculation_time*aentry.calculation_cores>0.0) 
	oss << "<li><span class=\"description\">Total CPU&middot;hours [<a href=\"" << url_WEB << "/?calculation_time\">calculation_time</a>*<a href=\"" << url_WEB << "/?calculation_cores\">calculation_cores</a>]/3600:</span>" << aentry.calculation_time*aentry.calculation_cores/3600 << " hours</li>" << endl;
      if(aentry.calculation_time>0.0) 
	oss << "<li><span class=\"description\">Total Wall-time [<a href=\"" << url_WEB << "/?calculation_time\"> calculation_time/3600</a>]:</span>" << aentry.calculation_time/3600 << " hours</li>" << endl;
      if(aentry.calculation_memory>0.0)
	oss << "<li><span class=\"description\">Memory Used [<a href=\"" << url_WEB << "/?calculation_memory\">calculation_memory</a>]:</span>" << aentry.calculation_memory << " MB</li>" << endl;
      if(aentry.calculation_cores>0) 
	oss << "<li><span class=\"description\">Number of cores [<a href=\"" << url_WEB << "/?calculation_cores\">calculation_cores</a>]:</span>" << aentry.calculation_cores << (aentry.calculation_cores>1?" (MPI) ":"(serial)") << "</li>" << endl;
      if(!aentry.node_CPU_Model.empty()) 
	oss << "<li><span class=\"description\">Processor - CPU Model [<a href=\"" << url_WEB << "/?node_CPU_Model\">node_CPU_Model</a>]:</span>" << (!aentry.node_CPU_Model.empty()?aentry.node_CPU_Model:"unavailable") << "</li>" << endl;
      if(aentry.node_CPU_MHz>0.0) 
	oss << "<li><span class=\"description\">Processor - CPU MHz [<a href=\"" << url_WEB << "/?node_CPU_MHz\">node_CPU_MHz</a>]:</span>" << (aentry.node_CPU_MHz>1.0?aurostd::utype2string(aentry.node_CPU_MHz)+"MHz ":"unavailable") << " </li>" << endl;
      if(!aentry.aflow_version.empty()) 
	oss << "<li><span class=\"description\">AFLOW Version [<a href=\"" << url_WEB << "/?aflow_version\">aflow_version</a>]:</span>" << (!aentry.aflow_version.empty()?aentry.aflow_version:"unavailable") << "</li>" << endl;
      if(!aentry.catalog.empty()) 
	oss << "<li><span class=\"description\">AFLOW Version [<a href=\"" << url_WEB << "/?catalog\">catalog</a>]:</span>" << (!aentry.catalog.empty()?aentry.catalog:"unavailable") << "</li>" << endl;
      if(!aentry.data_api.empty()) 
	oss << "<li><span class=\"description\">AFLOW Version [<a href=\"" << url_WEB << "/?data_api\">data_api</a>]:</span>" << (!aentry.data_api.empty()?aentry.data_api:"unavailable") << "</li>" << endl;
      if(!aentry.data_source.empty()) 
	oss << "<li><span class=\"description\">AFLOW Version [<a href=\"" << url_WEB << "/?data_source\">data_source</a>]:</span>" << (!aentry.data_source.empty()?aentry.data_source:"unavailable") << "</li>" << endl;
      if(!aentry.data_language.empty()) 
	oss << "<li><span class=\"description\">AFLOW Version [<a href=\"" << url_WEB << "/?data_language\">data_language</a>]:</span>" << (!aentry.data_language.empty()?aentry.data_language:"unavailable") << "</li>" << endl;
      if(!aentry.error_status.empty()) 
	oss << "<li><span class=\"description\">AFLOW Version [<a href=\"" << url_WEB << "/?error_status\">error_status</a>]:</span>" << (!aentry.error_status.empty()?aentry.error_status:"unavailable") << "</li>" << endl;
      if(!aentry.loop.empty()) 
	oss << "<li><span class=\"description\">AFLOW loops [<a href=\"" << url_WEB << "/?loop\">loop</a>]:</span>" << aentry.loop << "</li>" << endl;
      //     oss << "<li><span class=\"description\">AFLOW precision:</span>" << "unavailable" << "</li>" << endl;
      //    oss << "<li><span class=\"description\">AFLOW KPPRA:</span>" << "unavailable" << "</li>" << endl;
      if(!aentry.aflowlib_version.empty()) 
	oss << "<li><span class=\"description\">AFLOWLIB_entry version [<a href=\"" << url_WEB << "/?aflowlib_version\">aflowlib_version</a>]:</span>" << (!aentry.aflowlib_version.empty()?aentry.aflowlib_version:"unavailable") << "</li>" << endl;
      if(!aentry.aflowlib_date.empty()) 
	oss << "<li><span class=\"description\">AFLOWLIB_entry date [<a href=\"" << url_WEB << "/?aflowlib_date\">aflowlib_date</a>]:</span>" << (!aentry.aflowlib_date.empty()?aentry.aflowlib_date:"unavailable") << "</li>" << endl;
      if(!aentry.author.empty()) 
	oss << "<li><span class=\"description\">AFLOW Version [<a href=\"" << url_WEB << "/?author\">author</a>]:</span>" << (!aentry.author.empty()?aentry.author:"unavailable") << "</li>" << endl;
      if(!aentry.corresponding.empty()) 
	oss << "<li><span class=\"description\">AFLOW Version [<a href=\"" << url_WEB << "/?corresponding\">corresponding</a>]:</span>" << (!aentry.corresponding.empty()?aentry.corresponding:"unavailable") << "</li>" << endl;
      if(!aentry.sponsor.empty()) 
	oss << "<li><span class=\"description\">AFLOW Version [<a href=\"" << url_WEB << "/?sponsor\">sponsor</a>]:</span>" << (!aentry.sponsor.empty()?aentry.sponsor:"unavailable") << "</li>" << endl;

      // CORMAC
      if(aentry.energy_cutoff!=AUROSTD_NAN) 
	oss << "<li><span class=\"description\">energy_cutoff [<a href=\"" << url_WEB << "/?energy_cutoff\">energy_cutoff</a>]:</span>" << aentry.energy_cutoff << " eV</li>" << endl;
      //      oss.precision(6);
      if(aentry.delta_electronic_energy_convergence!=AUROSTD_NAN) 
	oss << "<li><span class=\"description\">delta_electronic_energy_convergence [<a href=\"" << url_WEB << "/?delta_electronic_energy_convergence\">delta_electronic_energy_convergence</a>]:</span>" << 1000*aentry.delta_electronic_energy_convergence << " meV</li>" << endl;
      if(aentry.delta_electronic_energy_threshold!=AUROSTD_NAN) 
	oss << "<li><span class=\"description\">delta_electronic_energy_threshold [<a href=\"" << url_WEB << "/?delta_electronic_energy_threshold\">delta_electronic_energy_threshold</a>]:</span>" << 1000*aentry.delta_electronic_energy_threshold << " meV</li>" << endl;
      if(aentry.nkpoints!=0) 
	oss << "<li><span class=\"description\">nkpoints [<a href=\"" << url_WEB << "/?nkpoints\">nkpoints</a>]:</span>" << aentry.nkpoints << " </li>" << endl;
      if(aentry.nkpoints_irreducible!=0) 
	oss << "<li><span class=\"description\">nkpoints_irreducible [<a href=\"" << url_WEB << "/?nkpoints_irreducible\">nkpoints_irreducible</a>]:</span>" << aentry.nkpoints_irreducible << " </li>" << endl;
      if(aentry.kppra!=0) 
	oss << "<li><span class=\"description\">kppra [<a href=\"" << url_WEB << "/?kppra\">kppra</a>]:</span>" << aentry.kppra << " </li>" << endl;
      if(aentry.kpoints.empty()) 
	oss << "<li><span class=\"description\">kpoints [<a href=\"" << url_WEB << "/?kpoints\">kpoints</a>]:</span>" << aentry.kpoints << " </li>" << endl;
      //  oss.precision(3);
	...
	if((vflags.flag("FLAG::EDATA_ORIG") && i==0) || (vflags.flag("FLAG::EDATA_RELAX") && i==1)) {
	  ...
	  if(i==0) oss << "<span class=\"title\"><FONT SIZE=+3> Original Structure</font></span>" << endl;
	  if(i==1) oss << "<span class=\"title\"><FONT SIZE=+3> Relaxed Structure</font></span>" << endl;
	  oss << line_rule << endl;
	  oss << "<ht /> " << endl;
	  oss << "<div class = \"real_space\">" << endl;
	  if(i==0) oss << "<span class=\"title\"> Real Space Lattice</span>" << endl;
	  if(i==1) oss << "<span class=\"title\"> Real Space Lattice</span>" << endl;
	  vector<string> vline_edata;
	  if(i==0 && aurostd::FileExist(directory_RAW+"/edata.orig.out")) aurostd::file2vectorstring(directory_RAW+"/edata.orig.out",vline_edata);
	  if(i==1 && aurostd::FileExist(directory_RAW+"/edata.relax.out")) aurostd::file2vectorstring(directory_RAW+"/edata.relax.out",vline_edata);
	  // 
	  ...
	  oss << "<ul>" << endl;
	  oss << "<li>" << endl;
	  oss << "<span class=\"description\"> Lattice:</span>" << endl;
	  oss << "<div class=\"lattice_table\">" << endl;
	  oss << "<table class=\"lattice\">" << endl;
	  oss << "<tbody>" << endl;
	  oss << "<tr>" << endl << "<td>a=" << abcR.at(0) << "&Aring;</td>" << endl << "<td>b=" << abcR.at(1) << "&Aring;</td>" << endl << "<td>c=" << abcR.at(2) << "&Aring;</td>" << endl << "<td>c/a=" << coveraR << "</td>" << endl << "</tr>" << endl;
	  oss << "<tr>" << endl << "<td>&alpha;=" << abcR.at(3) << "&deg</td>" << endl << "<td>&beta;=" << abcR.at(4) << "&deg</td>" << endl << "<td>&gamma;=" << abcR.at(5) << "&deg</td>" << endl << "<tr>" << endl;
	  oss << "</tbody>" << endl;
	  oss << "</table>" << endl;
	  oss << "</div>" << endl;
	  oss << "</li>" << endl;
	  oss << "<li><span class=\"description\"> Volume:</span>" << volumeR << "&Aring;<sup>3</sup></li>" << endl;
	  oss << "<li><span class=\"description\"> Number of Atoms per Cell:</span>" << aentry.natoms << "</li>" << endl;
	  ...
	  oss << "<li><span class=\"description\"> Density(calc):</span> " << density << " g/cm<sup>3</sup></li>" << endl;
	  oss << "<li><span class=\"description\"> Density(aflowlib):</span> " << aentry.density << " g/cm<sup>3</sup></li>" << endl;
	...
	if (i==1){  //GG 170714 - removed relaxed data from orig
	  if(aurostd::substring2bool(aentry.vfiles_WEB,"CONTCAR.relax.vasp")) {
	    oss << "<li><span class=\"description\"> Relaxed position (aflowlib/VASP):</span>" 
		<< "[<a href=\"" << url_WEB << "/CONTCAR.relax.vasp\"" << html_TAB << ">VASP-POSCAR</a>]" << "</li>" << endl;
	  }
	}

	if (i==1){  //GG 170714 - removed relaxed data from orig
	  if(aurostd::substring2bool(aentry.vfiles_WEB,"CONTCAR.relax.qe")) {
	    oss << "<li><span class=\"description\"> Relaxed position (aflowlib/QE):</span>" 
		<< "[<a href=\"" << url_WEB << "/CONTCAR.relax.qe\"" << html_TAB << ">QE-GEOMETRY</a>]" << "</li>" << endl;
	  }
	}

	if (i==1){  //GG 170714 - removed relaxed data from orig
	  if(aurostd::substring2bool(aentry.vfiles_WEB,"CONTCAR.relax.abinit")) {
	    oss << "<li><span class=\"description\"> Relaxed position (aflowlib/ABINIT):</span>" 
		<< "[<a href=\"" << url_WEB << "/CONTCAR.relax.abinit\"" << html_TAB << ">ABINIT-GEOMETRY</a>]" << "</li>" << endl;
	  }
	}

	if (i==1){  //GG 170714 - removed relaxed data from orig
	  if(aurostd::substring2bool(aentry.vfiles_WEB,"CONTCAR.relax.aims")) {
	    oss << "<li><span class=\"description\"> Relaxed position (aflowlib/AIMS):</span>" 
		<< "[<a href=\"" << url_WEB << "/CONTCAR.relax.aims\"" << html_TAB << ">AIMS-GEOMETRY</a>]" << "</li>" << endl;
	  }
	}

	if (i==1){  //GG 170714 - removed relaxed data from orig
	  if(aurostd::substring2bool(aentry.vfiles_WEB,"INCAR.relax")) {
	    oss << "<li><span class=\"description\"> INCAR for relax calculation (aflowlib/VASP):</span>" 
		<< "[<a href=\"" << url_WEB << "/INCAR.relax\"" << html_TAB << ">INCAR.relax</a>]" << "</li>" << endl;
	  }
	}

	  if(aurostd::substring2bool(aentry.vfiles_WEB,"INCAR.static")) {
	    oss << "<li><span class=\"description\"> INCAR for static calculation (aflowlib/VASP):</span>" 
		<< "[<a href=\"" << url_WEB << "/INCAR.static\"" << html_TAB << ">INCAR.static</a>]" << "</li>" << endl;
	  }

	if (i==1){  //GG 170714 - removed relaxed data from orig
	  if(aurostd::substring2bool(aentry.vfiles_WEB,"INCAR.bands")) {
	    oss << "<li><span class=\"description\"> INCAR for bands calculation (aflowlib/VASP):</span>"
		<< "[<a href=\"" << url_WEB << "/INCAR.bands\"" << html_TAB << ">INCAR.bands</a>]" << "</li>" << endl;
	  }
	}

	if (i==1){  //GG 170714 - removed relaxed data from orig
	  if(aurostd::substring2bool(aentry.vfiles_WEB,"KPOINTS.relax")) {
	    oss << "<li><span class=\"description\"> KPOINTS for relax calculation (aflowlib/VASP):</span>" 
		<< "[<a href=\"" << url_WEB << "/KPOINTS.relax\"" << html_TAB << ">KPOINTS.relax</a>]" << "</li>" << endl;
	  }
	}
	
  if (i==1){  //GG 170714 - removed relaxed data from orig
	  if(aurostd::substring2bool(aentry.vfiles_WEB,"KPOINTS.static")) {
	    oss << "<li><span class=\"description\"> KPOINTS for static calculation (aflowlib/VASP):</span>"
		<< "[<a href=\"" << url_WEB << "/KPOINTS.static\"" << html_TAB << ">KPOINTS.static</a>]" << "</li>" << endl;
	  }
	}
  
  if (i==1){  //GG 170714 - removed relaxed data from orig
	  if(aurostd::substring2bool(aentry.vfiles_WEB,"KPOINTS.bands")) {
	    oss << "<li><span class=\"description\"> KPOINTS for bands calculation (aflowlib/VASP):</span>" 
		<< "[<a href=\"" << url_WEB << "/KPOINTS.bands\"" << html_TAB << ">KPOINTS.bands</a>]" << "</li>" << endl;
	  }
	}
	  if(aurostd::substring2bool(aentry.vfiles_WEB,"edata.orig.out")) {
	    oss << "<li><span class=\"description\"> Extended crystallographic data for original structure:</span>" 
		<< "[<a href=\"" << url_WEB << "/edata.orig.out\"" << html_TAB << ">edata.orig.out</a>]" << "</li>" << endl;
	  }

  if (i==1){  //GG 170714 - removed relaxed data from orig
	  if(aurostd::substring2bool(aentry.vfiles_WEB,"edata.relax.out")) {
	    oss << "<li><span class=\"description\"> Extended crystallographic data for relaxed structure:</span>" 
		<< "[<a href=\"" << url_WEB << "/edata.relax.out\"" << html_TAB << ">edata.relax.out</a>]" << "</li>" << endl;
	  }
	}

  if (i==1){  //GG 170714 - removed relaxed data from orig
	  if(aurostd::substring2bool(aentry.vfiles_WEB,"edata.bands.out")) {
	    oss << "<li><span class=\"description\"> Extended crystallographic data for band-structure:</span>" 
		<< "[<a href=\"" << url_WEB << "/edata.bands.out\"" << html_TAB << ">edata.bands.out</a>]" << "</li>" << endl;
	  }
	}
	  ...
	  oss << "</div>" << endl;
	  oss << "<hr />" << endl;
	  oss << "<div class=\"space_group\">" << endl;
	  oss << "<span class=\"title\"> Bravais Lattice of the Crystal</span>" << endl;
	  oss << "<ul>" << endl;
	  oss << "<li><span class=\"description\"> Space Group Number:</span>" << aentry.sg2 << "</li>" << endl;
	  oss << "<li><span class=\"description\"> Pearson Symbol:</span>" << Crystal_Real_space_Pearson_Symbol << "</li><!br>" << endl;
	  oss << "<li><span class=\"description\"> Bravais Lattice Primitive:</span>" << Crystal_Real_space_Bravais_Lattice_Primitive << "</li><!br>" << endl;
	  oss << "<li><span class=\"description\"> Bravais Lattice Variation:</span>" << Crystal_Real_space_Lattice_Variation << "</li><!br>" << endl;
	  oss << "<li><span class=\"description\"> Bravais Lattice System:</span>" << Crystal_Real_space_Lattice_System << "</li><!br>" << endl;
	  oss << "</ul>" << endl;
	  oss << "</div>" << endl;
	  oss << "<hr />" << endl;
	  oss << "<div class=\"point_group\">" << endl;
	  oss << "<span class=\"title\"> Point Group of the Crystal</span>" << endl;
	  oss << "<ul>" << endl;
	  oss << "<li><span class=\"description\"> Crystal Family:</span>" << Crystal_Real_space_Crystal_Family << "</li><!br>" << endl;
	  oss << "<li><span class=\"description\"> Crystal System:</span>" << Crystal_Real_space_Crystal_System << "</li><!br>" << endl;
	  oss << "<li><span class=\"description\"> Crystal Class:</span>" << Crystal_Real_space_Crystal_Class << "</li><!br>" << endl;
	  //	oss << "<li><span class=\"description\"> Point Group (Hermann Mauguin):</span>" << Crystal_Real_space_Point_Group_Hermann_Mauguin << "</li><!br>" << endl;
	  oss << "<li><span class=\"description\"> Point Group (Herm. Maug.):</span>" << Crystal_Real_space_Point_Group_Hermann_Mauguin << "</li><!br>" << endl;
	  oss << "<li><span class=\"description\"> Point Group (Schoenflies):</span>" << Crystal_Real_space_Point_Group_Schoenflies << "</li><!br>" << endl;
	  oss << "<li><span class=\"description\"> Point Group Orbifold:</span>" << Crystal_Real_space_Point_Group_Orbifold << "</li><!br>" << endl;
	  oss << "<li><span class=\"description\"> Point Group Type:</span>" << Crystal_Real_space_Point_Group_Type << "</li><!br>" << endl;
	  oss << "<li><span class=\"description\"> Point Group Order:</span>" << Crystal_Real_space_Point_Group_Order << "</li><!br>" << endl;
	  oss << "<li><span class=\"description\"> Point Group Structure:</span>" << Crystal_Real_space_Point_Group_Structure << "</li><!br>" << endl;
	  oss << "</ul>" << endl;
	  oss << "</div>" << endl;
	  if(aurostd::substring2bool(aentry.vfiles_WEB,label+"_BZ.png")) {
	    oss << "<div class=\"div_BZ\">" << endl;
	    oss << "<span class=\"description_BZ\"> Brillouin Zone " << art058_link<< "</span>" << endl;
	    oss << "</div>" << endl;
	    oss << "<div class=\"picture_BZ\">" << endl;
	    ...
	    oss << "<img class=\"pic_BZ\" src=\"http://aflowlib.duke.edu/SCIENCE/images/brillouin/" << aurostd::RemoveWhiteSpaces(Lattice_Real_space_Lattice_Variation) << ".PNG\" alt=\"Brillouin Zone of " << label << "\" />" << endl; //CO 170621 - abs path WORKS
	    oss << "</div>" << endl;
	  }
	  oss << "<hr />" << endl;
	  oss << "<div class=\"bravais_lattice\">" << endl;
	  oss << "<span class=\"title\"> Bravais Lattice of the Lattice</span>" << endl;
	  oss << "<ul>" << endl;
	  oss << "<li><span class=\"description\"> Bravais Lattice Primitive</span>" << Lattice_Real_space_Bravais_Lattice_Primitive << "</li><!br>" << endl;
	  oss << "<li><span class=\"description\"> Bravais Lattice Variation:</span>" << Lattice_Real_space_Lattice_Variation << "</li><!br>" << endl;
	  oss << "<li><span class=\"description\"> Bravais Lattice System:</span>" << Lattice_Real_space_Lattice_System << "</li><!br>" << endl;
	  oss << "</ul>" << endl;
	  oss << "</div>" << endl;
	  oss << "<hr />" << endl;
	  oss << "<div class=\"superlattice\">" << endl;
	  oss << "<span class=\"title\"> Superlattice</span>" << endl;
	  oss << "<ul>" << endl;
	  oss << "<li><span class=\"description\"> Superlattice Primitive unit cell:</span>" << Superattice_Real_space_Bravais_Superlattice_Primitive << "</li><!br>" << endl;
	  oss << "<li><span class=\"description\"> Superlattice Variation:</span>" << Superattice_Real_space_Superlattice_Variation << "</li><!br>" << endl;
	  oss << "<li><span class=\"description\"> Superlattice Lattice System :</span>" << Superattice_Real_space_Superlattice_System << "</li><!br>" << endl;
	  oss << "<li><span class=\"description\"> Superlattice Pearson Symbol:</span>" << Superattice_Real_space_Pearson_Symbol_Superlattice << "</li><!br>" << endl;
	  oss << "</ul>" << endl;
	  oss << "</div>" << endl;
	  oss << "<hr />" << endl;
	  oss << "<div class=\"reciprocal\">" << endl;
	  oss << "<span class=\"title\"> Reciprocal Space Lattice </span>" << endl;
	  oss << "<ul>" << endl;
	  oss << "<span class=\"description\"> Reciprocal Lattices:</span>" << endl;
	  oss << "<div class=\"lattice_table\">" << endl;
	  oss << "<table class=\"reciprocal_lattice\">" << endl;
	  oss << "<tbody>" << endl;
	  oss << "<tr>" << endl << "<td>a=" << abcK.at(0) << "&Aring;<sup>-1</sup></td>" << endl << "<td>b=" << abcK.at(1) << "&Aring;<sup>-1</sup></td>" << endl << "<td>c=" << abcK.at(2) << "&Aring;<sup>-1</sup></td>" << endl << "</tr>" << endl;
	  oss << "<tr>" << endl;
	  oss << "<td>&alpha;=" << abcK.at(3) << "&deg;</td>" << endl << "<td>&beta;=" << abcK.at(4) << "&deg;</td>" << endl << "<td>&gamma;=" << abcK.at(5) << "&deg;</td>" << endl;
	  oss << "<tr>" << endl;
	  oss << "</tbody>" << endl;
	  oss << "</table>" << endl;
	  oss << "</div></li>" << endl;
	  oss << "<li><span class=\"description\"> Volume:</span>" << volumeK << " &Aring;<sup>-3</sup></li><!br>" << endl;
	  oss << "<li><span class=\"description\"> Lattice Primitive:</span>" << Reciprocal_lattice_primitive << "</li><!br>" << endl;
	  oss << "<li><span class=\"description\"> Lattice Variation:</span>" << Reciprocal_lattice_variation << "</li><!br>" << endl;
	  oss << "</ul>" << endl;
	  oss << "</div>" << endl; 
	  oss << "<!-- EDATA: END -->" << endl;
	}
    ...
    if(vflags.flag("FLAG::FOUND") && vflags.flag("FLAG::THERMODYNAMICS") && !directory.empty()) {
      ...
      oss << line_rule << endl;
      oss << "<div class=\"Thermodynamics\">" << endl;
      oss << "<span class=\"title\"><FONT SIZE=+3>Thermodynamics Properties</font></span>" << endl;
      //   oss << line_rule << endl;
      //     oss << "<br>" << endl;
      oss << "<span class=\"Thermodynamics_table\">" << endl;
      oss << "<ul>" << endl;
      oss << "<li><span class=\"description\">Formation Enthalpy_cell:</span> " << (aentry.enthalpy_formation_cell!=0.0?aurostd::utype2string(aentry.enthalpy_formation_cell,5)+" (eV) ":"unavailable") << "</li><!br>" << endl;
      oss << "<li><span class=\"description\">Formation Enthalpy_atom:</span> " << (aentry.enthalpy_formation_atom!=0.0?aurostd::utype2string(aentry.enthalpy_formation_atom,5)+" (eV/atom) ":"unavailable") << "</li><!br>" << endl;
      oss << "<li><span class=\"description\"><i>Ab-initio</i> energy_cell:</span> " << (aentry.energy_cell!=0.0?aurostd::utype2string(aentry.energy_cell,5)+" (eV) ":"unavailable") << "</li><!br>" << endl;
      oss << "<li><span class=\"description\"><i>Ab-initio</i> energy_atom:</span> " << (aentry.energy_cell!=0.0?aurostd::utype2string(aentry.energy_atom,5)+" (eV/atom) ":"unavailable") << "</li><!br>" << endl;
      oss << "</ul>" << endl;
      oss << "</div>" << endl;
    ...
    if(vflags.flag("FLAG::FOUND") && vflags.flag("FLAG::AGL") && !directory.empty()) {
      ...
      oss << line_rule << endl;
      oss << "<div class=\"AGL\">" << endl;
      oss << "<span class=\"title\"><FONT SIZE=+3>AGL Properties (Aflow Gibbs Library)</font>" << "<a href=http://materials.duke.edu/AFLOW/README_AFLOW_AGL.TXT>[info]</a>" << "</span>" << endl;
      //   oss << line_rule << endl;
      //     oss << "<br>" << endl;
      oss << "<span class=\"AGL_table\">" << endl;
      oss << "<ul>" << endl;
      if(aurostd::substring2bool(aentry.vfiles_WEB,"aflow.agl.out")) 
	oss << "<li><span class=\"description\"> AGL Output" << art096_link << ":</span>" << "[<a href=\"" << url_WEB << "/aflow.agl.out\"" << html_TAB << ">aflow.agl.out</a>]" << "</li><!br>" << endl;
      if(aurostd::substring2bool(aentry.vfiles_WEB,"AGL.out")) 
	oss << "<li><span class=\"description\"> AGL Complete Output" << art096_link << ":</span>" << "[<a href=\"" << url_WEB << "/AGL.out\"" << html_TAB << ">AGL.out</a>]" << "</li><!br>" << endl;
      if(aurostd::substring2bool(aentry.vfiles_WEB,"AGL_energies_temperature.out")) 
	oss << "<li><span class=\"description\"> AGL Energy versus Temperature" << art096_link << ":</span>" << "[<a href=\"" << url_WEB << "/AGL_energies_temperature.out\"" << html_TAB << ">AGL_energies_temperature.out</a>]" << "</li><!br>" << endl;
      if(aurostd::substring2bool(aentry.vfiles_WEB,"AGL_thermal_properties_temperature.out")) 
	oss << "<li><span class=\"description\"> AGL Thermal Properties versus Temperature" << art096_link << ":</span>" << "[<a href=\"" << url_WEB << "/AGL_thermal_properties_temperature.out\"" << html_TAB << ">AGL_thermal_properties_temperature.out</a>]" << "</li><!br>" << endl;
      if(aentry.agl_thermal_conductivity_300K<AUROSTD_NAN) 
	oss << "<li><span class=\"description\" id=\"agl_thermal_conductivity_300K_td\">AGL Thermal Conductivity at 300K" << art096_link << ":</span> " << aentry.agl_thermal_conductivity_300K << " (W/m*K)</li><!br>" << endl;
      if(aentry.agl_debye<AUROSTD_NAN) 
	oss << "<li><span class=\"description\" id=\"agl_debye_td\">AGL Debye Temperature" << art096_link << ":</span> " << aentry.agl_debye << " (K)</li><!br>" << endl;
      if(aentry.agl_acoustic_debye<AUROSTD_NAN) 
	oss << "<li><span class=\"description\" id=\"agl_acoustic_debye_td\">AGL Debye Acoustic Temperature" << art096_link << ":</span> " << aentry.agl_acoustic_debye << " (K)</li><!br>" << endl;
      if(aentry.agl_gruneisen<AUROSTD_NAN) 
	oss << "<li><span class=\"description\" id=\"agl_gruneisen_td\">AGL Gruneisen parameter" << art096_link << ":</span> " << aentry.agl_gruneisen << "</li><!br>" << endl;
      if(aentry.agl_heat_capacity_Cv_300K<AUROSTD_NAN) 
	oss << "<li><span class=\"description\" id=\"agl_heat_capacity_Cv_300K_td\">AGL Specific Heat Cv at 300K" << art096_link << ":</span> " << aentry.agl_heat_capacity_Cv_300K << " (kB/cell)</li><!br>" << endl;
      if(aentry.agl_heat_capacity_Cp_300K<AUROSTD_NAN) 
	oss << "<li><span class=\"description\" id=\"agl_heat_capacity_Cp_300K_td\">AGL Specific Heat Cp at 300K" << art096_link << ":</span> " << aentry.agl_heat_capacity_Cp_300K << " (kB/cell)</li><!br>" << endl;
      if(aentry.agl_thermal_expansion_300K<AUROSTD_NAN) 
	oss << "<li><span class=\"description\" id=\"agl_thermal_expansion_300K_td\">AGL Thermal Expansion at 300K" << art096_link << ":</span> " << aentry.agl_thermal_expansion_300K << " (1/K)</li><!br>" << endl;
      if(aentry.agl_bulk_modulus_static_300K<AUROSTD_NAN) 
	oss << "<li><span class=\"description\" id=\"agl_bulk_modulus_static_300K_td\">AGL Bulk Modulus Static at 300K" << art096_link << ":</span> " << aentry.agl_bulk_modulus_static_300K << " (GPa)</li><!br>" << endl;
      if(aentry.agl_bulk_modulus_isothermal_300K<AUROSTD_NAN) 
	oss << "<li><span class=\"description\" id=\"agl_bulk_modulus_isothermal_300K_td\">AGL Bulk Modulus Isothermal at 300K" << art096_link << ":</span> " << aentry.agl_bulk_modulus_isothermal_300K << " (GPa)</li><!br>" << endl;
      oss << "</ul>" << endl;
      oss << "</div>" << endl;
    ...
    if(vflags.flag("FLAG::FOUND") && vflags.flag("FLAG::AEL") && !directory.empty()) {
      ...
      oss << line_rule << endl;
      oss << "<div class=\"AEL\">" << endl;
      oss << "<span class=\"title\"><FONT SIZE=+3>AEL Properties (Aflow Elastic Library)</font>" << "<a href=http://materials.duke.edu/AFLOW/README_AFLOW_AEL.TXT>[info]</a>" << "</span>" << endl;
      oss << "<span class=\"AEL_table\">" << endl;
      oss << "<ul>" << endl;
      if(aurostd::substring2bool(aentry.vfiles_WEB,"aflow.ael.out")) 
	oss << "<li><span class=\"description\"> AEL Output" << art100_link << ":</span>" << "[<a href=\"" << url_WEB << "/aflow.ael.out\"" << html_TAB << ">aflow.ael.out</a>]" << "</li><!br>" << endl;
      if(aurostd::substring2bool(aentry.vfiles_WEB,"AEL_Elastic_constants.out")) 
	oss << "<li><span class=\"description\"> AEL Elastic Constants (stiffness tensor)" << art100_link << ":</span>" << "[<a href=\"" << url_WEB << "/AEL_Elastic_constants.out\"" << html_TAB << ">AEL_Elastic_constants.out</a>]" << "</li><!br>" << endl;
      if(aurostd::substring2bool(aentry.vfiles_WEB,"AEL_Compliance_tensor.out")) 
	oss << "<li><span class=\"description\"> AEL Compliance Constants (compliance tensor)" << art100_link << ":</span>" << "[<a href=\"" << url_WEB << "/AEL_Compliance_tensor.out\"" << html_TAB << ">AEL_Compliance_tensor.out</a>]" << "</li><!br>" << endl;
      if(aentry.ael_poisson_ratio<AUROSTD_NAN) 
	oss << "<li><span class=\"description\" id=\"ael_poisson_ratio_td\">AEL Poisson Ratio" << art100_link << ":</span> " << aentry.ael_poisson_ratio << "</li><!br>" << endl;
      if(aentry.ael_bulk_modulus_voigt<AUROSTD_NAN) 
	oss << "<li><span class=\"description\" id=\"ael_bulk_modulus_voigt_td\">AEL Bulk Modulus Voigt" << art100_link << ":</span> " << aentry.ael_bulk_modulus_voigt << " (GPa)</li><!br>" << endl;
      if(aentry.ael_bulk_modulus_reuss<AUROSTD_NAN) 
	oss << "<li><span class=\"description\" id=\"ael_bulk_modulus_reuss_td\">AEL Bulk Modulus Reuss" << art100_link << ":</span> " << aentry.ael_bulk_modulus_reuss << " (GPa)</li><!br>" << endl;
      if(aentry.ael_bulk_modulus_vrh<AUROSTD_NAN) 
	oss << "<li><span class=\"description\" id=\"ael_bulk_modulus_vrh_td\">AEL Bulk Modulus Vrh" << art100_link << ":</span> " << aentry.ael_bulk_modulus_vrh << " (GPa)</li><!br>" << endl;
      if(aentry.ael_shear_modulus_voigt<AUROSTD_NAN) 
	oss << "<li><span class=\"description\" id=\"ael_shear_modulus_voigt_td\">AEL Shear Modulus Voigt" << art100_link << ":</span> " << aentry.ael_shear_modulus_voigt << " (GPa)</li><!br>" << endl;
      if(aentry.ael_shear_modulus_reuss<AUROSTD_NAN) 
	oss << "<li><span class=\"description\" id=\"ael_shear_modulus_reuss_td\">AEL Shear Modulus Reuss" << art100_link << ":</span> " << aentry.ael_shear_modulus_reuss << " (GPa)</li><!br>" << endl;
      if(aentry.ael_shear_modulus_vrh<AUROSTD_NAN) 
	oss << "<li><span class=\"description\" id=\"ael_shear_modulus_vrh_td\">AEL Shear Modulus Vrh" << art100_link << ":</span> " << aentry.ael_shear_modulus_vrh << " (GPa)</li><!br>" << endl;
      if(aentry.ael_elastic_anistropy<AUROSTD_NAN) 
	oss << "<li><span class=\"description\" id=\"ael_elastic_anistropy_td\">AEL Elastic Anisotropy" << art100_link << ":</span> " << aentry.ael_elastic_anistropy << "</li><!br>" << endl;
      oss << "</ul>" << endl;
      oss << "</div>" << endl;
    ...
    if(vflags.flag("FLAG::FOUND") && vflags.flag("FLAG::BADER") && !directory.empty()) {
      ...
      oss << line_rule << endl;
      oss << "<div class=\"Bader\">" << endl;
      oss << "<span class=\"title\"><FONT SIZE=+3>Bader Atoms in Molecules Properties</font>" << "</span>" << endl;
      ...
      oss << "<span class=\"Bader_table\">" << endl;
      oss << "<ul>" << endl;
      string abader_out=aentry.prototype+"_abader.out";
      if(aurostd::substring2bool(aentry.vfiles_WEB,abader_out)) {oss << "<li><span class=\"description\"> Bader Output" << ":</span>" << "[<a href=\"" << url_WEB << "/" << abader_out << "\"" << html_TAB << ">" << abader_out << "</a>]" << "</li><!br>" << endl;}
      //oss << "<li><span class=\"description\"> Bader Output" << art100_link << ":</span>" << "[<a href=\"" << url_WEB << "/" << abader_out << "\"" << html_TAB << ">" << abader_out << "</a>]" << "</li><!br>" << endl;
      string net_charges_wiki_link=" [<a href=http://aflowlib.duke.edu/aflowwiki/doku.php?id=documentation:all_keywords&#bader_net_charges><font color=black><i>info</i></font></a>]";
      string atomic_volumes_wiki_link=" [<a href=http://aflowlib.duke.edu/aflowwiki/doku.php?id=documentation:all_keywords&#bader_atomic_volumes><font color=black><i>info</i></font></a>]";
      if(!aentry.vbader_net_charges.empty()) {
        aurostd::string2tokens(aentry.species,tokens,",");  //tokens=species
        oss << "<li><span class=\"description\" id=\"bader_net_charges_td\">Net Charges" << net_charges_wiki_link << ":</span> ";
        atomCOUNT=0;
        for(uint i=0;i<tokens.size();i++) {
          oss << tokens.at(i) << "={";
        ...
        oss << " (electrons)</li><!br>" << endl;
      }
      if(!aentry.bader_atomic_volumes.empty()) {
        aurostd::string2tokens(aentry.species,tokens,",");  //tokens=species
	oss << "<li><span class=\"description\" id=\"bader_atomic_volumes_td\">Atomic Volumes" << atomic_volumes_wiki_link << ":</span> ";
        atomCOUNT=0;
        for(uint i=0;i<tokens.size();i++) {
          oss << tokens.at(i) << "={";
          for(uint j=0;j<(uint)aentry.vcomposition.at(i);j++) {
        ...
        oss << " (Angst<sup>3</sup>)</li><!br>" << endl;
      }
      oss << "</ul>" << endl;
      oss << "</div>" << endl;
    ...
    if(vflags.flag("FLAG::FOUND") && vflags.flag("FLAG::MAGNETIC") && !directory.empty()) {
      ...
      oss << line_rule << endl;
      oss << "<div class=\"Magnetic\">" << endl;
      oss << "<span class=\"title\"><FONT SIZE=+3>Magnetic Properties </font></span>" << endl;
      oss << "<!span class=\"Magnetic_table\">" << endl;
      oss << "<ul>" << endl;
      oss << "<li><span class=\"description\" id=\"spin_cell_td\">Magnetic Moment:</span> " << aentry.spin_cell << " &mu;<sub>B</sub></li><!br>" << endl;
      oss << "<li><span class=\"description\" id=\"spin_atom_td\">Magnetic Moment/atom:</span> " << aentry.spin_atom << " &mu;<sub>B</sub>/atom</li><!br>" << endl;
      if(aurostd::substring2bool(aentry.loop,"magnetic")) {
	oss << "<li><span class=\"description\" id=\"spinF_td\">Spin Polarization (E<sub>F</sub>):</span> " << aentry.spinF << " <!br></li><!br>" << endl;
	oss << "<br><li><span class=\"description\" id=\"spinD_td\">Spin Decomposition per atoms:</span> {" << aentry.spinD << "} &mu;<sub>B</sub></li><!br>" << endl;
      }
      oss << "</ul>" << endl;
      oss << "</div>" << endl;
    ...
    if(vflags.flag("FLAG::FOUND") && vflags.flag("FLAG::SCINTILLATION") && !directory.empty()) {
      ...
      oss << "<div class=\"scintillation\">" << endl;
      //    oss << "<hr />" << endl;
      oss << "<span class=\"title\"><FONT SIZE=+3>Scintillation Properties</font></span>" << endl;
      //   oss << line_rule << endl;
      oss << "<br>" << endl;
      oss << "<table class=\"scintillation_table\">" << endl;
      oss << "<tbody>" << endl;
      if(aentry.scintillation_attenuation_length>0.0) {
	oss << "<tr>" << endl << "<td class=\"scintillation_table_td\" id=\"attenuation_length\"><span class=\"table_description\">Attenuation Length" << art064_link << ":</span> " << aentry.scintillation_attenuation_length << " cm</td>" << endl << "</tr>" << endl;
      } else {
	oss << "<tr>" << endl << "<td class=\"scintillation_table_td\" id=\"attenuation_length\"><span class=\"table_description\">Attenuation Length" << art064_link << ":</span> " << scintillation_attenuation_length << " cm</td>" << endl << "</tr>" << endl;
      }
      oss << "</tbody>" << endl;
      oss << "</table>" << endl;
      oss << "</div>" << endl;
    ...
    if(vflags.flag("FLAG::FOUND") && vflags.flag("FLAG::ELECTRONIC") && !directory.empty()) {
      ...
      //   oss << "<hr />" << endl;
      oss << "<div class=\"electronic\">" << endl;
      oss << "<span class=\"title\"><FONT SIZE=+3> Electronic Properties </font></span>" << endl;
      oss << "<table class=\"electronic_table\">" << endl;
      oss << "<tbody>" << endl;
      oss << "<tr>" << endl;
      oss << "<td class=\"electronic_table_td\" id=\"band_gap_td\"><span class=\"table_description\">Band Gap:</span> " << aentry.Egap << " eV "
	  << (aentry.Egap_type=="insulator_direct"?"(insulator)":"") << (aentry.Egap_type=="insulator_indirect"?"(insulator)":"") << (aentry.Egap_type=="metal"?"(metal)":"") << "</td>" << endl;
      oss << "<td class=\"electronic_table_td\" id=\"fit_band_gap_td\"><span class=\"table_description\">Fit Band Gap:</span> " << aentry.Egap_fit << " eV</td>" << endl;
      oss << "</tr>" << endl; 
      oss << "<tr>" << endl;
      oss << "<td class=\"electronic_table_td\" id=\"FIX_m_atom_td\"><span class=\"table_description\">Magnetic Moment:</span> " << aentry.spin_cell << " &mu;<sub>B</sub></td>" << endl;
      oss << "<td class=\"electronic_table_td\" id=\"FIX_m_atom_td\"><span class=\"table_description\">Magnetic Moment/atom:</span> " << aentry.spin_atom << " &mu;<sub>B</sub>/atom</td>" << endl;
      oss << "</tr>" << endl;
      oss << "<tr>" << endl;
      oss << "<td class=\"electronic_table_td\" id=\"FIX_e_mass_td\"><span class=\"table_description\">Electron Mass(FIX):</span> XXX (m<sub>0</sub>)</td>" << endl;
      oss << "<td class=\"electronic_table_td\" id=\"FIX_hole_mass_td\"><span class=\"table_description\">Hole Mass(FIX):</span> XXX (m<sub>0</sub>)</td>" << endl;
      oss << "</tr>" << endl;
      oss << "<tr>" << endl;
      if(aentry.Egap_type=="insulator_direct")
	oss << "<td class=\"electronic_table_td\" id=\"FIX_band_gap_type_td\"><span class=\"table_description\">Band Gap Type:</span>  Direct</td>" << endl;
      if(aentry.Egap_type=="insulator_indirect")
	oss << "<td class=\"electronic_table_td\" id=\"FIX_band_gap_type_td\"><span class=\"table_description\">Band Gap Type:</span>  Indirect</td>" << endl;
      oss << "</tr>" << endl;
      oss << "<tr>" << endl;
      oss << "<td class=\"electronic_table_td\"><span class=\"table_description\">Spin Polarization (E<sub>F</sub>):</span> " << aentry.spinF << " </td>" << endl;
      oss << "<td class=\"electronic_table_td\"><span class=\"table_description\">Spin Decomposition per atoms:</span> {" << aentry.spinD << "} &mu;<sub>B</sub> </td>" << endl;
      oss << "</tr>" << endl;
      oss << "</tbody>" << endl;
      oss << "</table>" << endl;
      oss << "<ul>" << endl;
      oss << "<li>" << endl;
      oss << "<span class=\"pic_description_band\">Band Structure:</span>" << endl;

      if(aentry.vfiles_WEB.size()>0) 
	for(uint i=0;i<aentry.vfiles_WEB.size();i++)
	  if(aentry.vfiles_WEB.at(i)==label+".png")
	    oss << "<div class = \"picture_band\" id=\"band_dos_pic\"><a id=\"imgPopup\"><img class=\"pic\" src=\"" << url_WEB << "/" << aentry.vfiles_WEB.at(i) << "\" alt=\"Band Structure of " << label << "\" style='background:white;' /></a></div>" << endl;
      oss << "<div id=\"small_figure\">" << endl;
      if(aentry.vfiles_WEB.size()>0) 
	for(uint i=0;i<aentry.vfiles_WEB.size();i++)
	  if(aurostd::substring2bool(aentry.vfiles_WEB.at(i),"_PEDOS_") && aurostd::substring2bool(aentry.vfiles_WEB.at(i),".png")) 
	    oss << "<img class=\"pic_small\" src=\"" << url_WEB << "/" << aentry.vfiles_WEB.at(i) << "\" alt=\"Band Structure of pdos_" << label << "\" style='background:white;' >" << endl;
      oss << "</div>" << endl;
      oss << "</li></ul>" << endl;
      oss << "</div>" << endl;
