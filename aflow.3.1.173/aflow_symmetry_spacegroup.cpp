...
namespace SYM {
  void orient(xstructure& xstr) {
	...
	if(!(SYM::AtomFPOSMatch(atomic_basis_, obverse1, match_type1, c2f, f2c, skew, _SYM_TOL_) &&
	      ...
	      SYM::AtomFPOSMatch(atomic_basis_, obverse3, match_type3, c2f, f2c, skew, _SYM_TOL_)) &&
	    (match_type1 != match_type2 || match_type2 != match_type3 || match_type1 != match_type3)) {
	  cerr << "SYM::orient: PROBLEM TRANFORMING TO OBVERSE SETTING. QUITING PROGRAM." << endl;
	  exit(0);
	}
      }
...
uint xstructure::GetPrimitiveCell(void) {
  ...
  if(foundbasis == false) {
    ...
    for (uint i = 0; i < atoms_by_type[index_for_smallest_group].size(); i++) {
      diff_vectors.push_back(atoms_by_type[index_for_smallest_group][i].fpos - atoms_by_type[index_for_smallest_group][0].fpos);
    ...
    for (uint d = 1; d < diff_vectors.size(); d++) {  //use d=0 if we do double for loop
      ...
      if(count == atomic_basis_.size()) {
        ...
        // Since the above was a lttice vector, we need to check if the image of this vector is also a lattice vector
        xvector<double> tmp;
        for (int a = -1; a <= 1; a++) {
          for (int b = -1; b <= 1; b++) {
            for (int c = -1; c <= 1; c++) {
              tmp = c2f * (atoms_by_type[index_for_smallest_group][atom_pos[d]].cpos - atoms_by_type[index_for_smallest_group][0].cpos + a * (*this).lattice(1) + b * (*this).lattice(2) + c * (*this).lattice(3));
              bool unique = true;
              for (uint l = 0; l < lattice_vector_candidates.size(); l++) {
                if(identical(tmp, lattice_vector_candidates[l], _ZERO_TOL_)) {
                  unique = false;
                  break;
                }
              }
              if(unique) {
                lattice_vector_candidates.push_back(tmp);
              }
            }
          }
        }
    ...
    if(num_of_candidates == 3) {
      ...
      for (uint i = 0; i < lattice_vector_candidates.size(); i++) {
        for (uint j = 0; j < lattice_vector_candidates.size(); j++) {
          if(i != j) {
            for (uint k = 0; k < lattice_vector_candidates.size(); k++) {
              if(i != k && j != k) {
                xvector<double> a = lattice_vector_candidates[i] * lattice_basis_xmat;
                xvector<double> b = lattice_vector_candidates[j] * lattice_basis_xmat;
                xvector<double> c = lattice_vector_candidates[k] * lattice_basis_xmat;
                double det = aurostd::determinant(SYM::xvec2xmat(a, b, c));
                //In order to find minimum, determinant should be positive,less than the first, and an integer multiple of less than first
                if(det > tol_vol && det < prim_det && aurostd::isinteger(prim_det / det, 0.05)) {
                  //if(det > tol_vol && det<prim_det){
                  if(!SYM::checkAngle(a, b, 2.0 * Pi_r, _SYM_TOL_) &&  //ensure none of the vectors are coplanar
                      !SYM::checkAngle(a, c, 2.0 * Pi_r, _SYM_TOL_) &&
                      !SYM::checkAngle(b, c, 2.0 * Pi_r, _SYM_TOL_)) {
                    if(aurostd::modulus(a) + aurostd::modulus(b) + aurostd::modulus(c) < sum_mods) {
                      sum_mods = aurostd::modulus(a) + aurostd::modulus(b) + aurostd::modulus(c);
                      nonsimp_reduced_basis = SYM::xvec2xmat(a, b, c);
                    }
                  }
                }
              }
      ...
      newbasis = SYM::foldAtomsInCell(expanded_basis, c2f, f2c, skew, same_atom_tol);
      //DEBUG
...
namespace SYM {
  deque<_atom> add_basis(vector<xvector<double> >& expanded_lattice_points, xmatrix<double>& L, xstructure& xstr) {
    ...
    if(xstr.atoms.size() == 0) {
      cerr << "SYM::add_basis: atoms is empty vector!" << endl;
      exit(1);
    }
...
namespace SYM {
  xstructure ConventionalCell(xstructure& xstr, int& IT, int& cell_choice, bool& last_orientation, string& crystalsystem_prev,
	...
	if(SYM::MapAtom(primitive_atomic_basis[i], primitive_atomic_basis[j], FALSE, c2f, f2c, skew, _SYM_TOL_)) {
	  if(LDEBUG) { cerr << "SYM::ConventionalCell: WARNING: Atoms occupying the same space (given tolerance " << _SYM_TOL_ << "). Check structure." << endl; }
    ...
    if(LDEBUG){
      cerr << "SYM::ConventionalCell: Number of mirror operations: " << mirror_ops_vec.size() << endl;
      for(uint m=0;m<mirror_ops_vec.size();m++){
        cerr << "SYM::ConventionalCell: mirror #" << m+1 << " : " << mirror_ops_vec[m] << endl;
      }
      cerr << "SYM::ConventionalCell: Number of twofold operations: " << twofold_ops_vec.size() << endl;
      for(uint m=0;m<twofold_ops_vec.size();m++){
        cerr << "SYM::ConventionalCell: twofold operator #" << m+1 << " : " << twofold_ops_vec[m] << endl;
      }
      cerr << "SYM::ConventionalCell: Number of n-fold (n>2) operations: " << rot_ops_vec.size() << endl;
	...
	if((mcount == 9 && fourcount == 3) || crystalsystem == "CUBIC") {
	  if(LDEBUG) { cerr << "SYM::ConventionalCell: CUBIC" << endl; }
	  symmetryfound = true;
	  crystalsystem = "CUBIC";
	...
	// =============== HEXAGONAL =============== //
	  ...
	  if(LDEBUG) { cerr << "SYM::ConventionalCell: HEX" << endl; }
	...
	// =============== TETRAGONAL =============== //
	  ...
	  if(LDEBUG) { cerr << "SYM::ConventionalCell: TET" << endl; }
	...
	// =============== ORTHORHOMBIC/RHOMBOHEDRAL =============== //
	  ...
	  symmetryfound = true;
	  if(LDEBUG) { cerr << "SYM::ConventionalCell: ORTHO/RHOMB" << endl; }
	...
	//  if(continue_ortho == true) {
	    if(LDEBUG) { cerr << "SYM::ConventionalCell: ORTHO true: " << endl; }
	...
	//  else {
	  ...
	  symmetryfound = true;
	    ...
	    if(LDEBUG) { cerr << "SYM::ConventionalCell: RHOMB" << endl; }
	...
	// =============== MONOCLINIC =============== //
	  ...
	  if(LDEBUG) { cerr << "SYM::ConventionalCell: MONO" << endl; }
	...
	// =============== TRICLINIC =============== //
	  ...
	  if(LDEBUG) { cerr << "SYM::ConventionalCell: TRI" << endl; }
	...
	xmatrix<double> CL = candidate_lattice_vectors[i];
        if(LDEBUG){
	  cerr << "SYM::ConventionalCell: Possible conventional lattice vectors (choice " << (i+1) << " of " << candidate_lattice_vectors.size() << "):" << endl;
          cerr << CL << endl;
        }
	f2c = trasp(CL);
	...
	if(consistent_ratio == false) {
	  if(LDEBUG) { cerr << "SYM::ConventionalCell: WARNING: Conventional cell atomic basis ratio is inconsistent with the primitive cell! Recalculating." << endl; }
	  symmetryfound = false;
	  continue;
	}
	...
	if(LDEBUG) { cerr << "SYM::ConventionalCell: Pearson: " << pearson_symbol << endl; }
	...
	if(checkops.latticesystem == "REDO") {
	  if(LDEBUG) { cerr << "SYM::ConventionalCell: Symmetry check failed..." << endl; }
	}
...
namespace SYM {
  symfolder check_ccell(xstructure& xstr) {
	...
	if(LDEBUG) { cerr << "SYM::check_ccell: WARNING: Point group mapping failed (number of symmetry elements does not match with a given point group)." << endl; }
	SOps.latticesystem = "REDO";
	SOps.commensurate = false;
	return SOps;
      } else {
    ...
    if(centeringops.size() != centeringgroups) {
      ...
      if(LDEBUG) { cerr << "SYM::check_ccell: WARNING: Centering operations incommensurate..." << endl; }
      SOps.latticesystem = "REDO";
      SOps.commensurate = false;
      return SOps;
    }
...
string xstructure::aflow2sg(void) {
  ...
  return GetSpaceGroupName(sgroup) + " #" + aurostd::utype2string(sgroup);
}
...
uint xstructure::SpaceGroup_ITC(double& use_tol, const int& manual_it, bool& no_scan) {
  if(use_tol < _ZERO_TOL_) {
    cerr << "SYM::SpaceGroup_ITC: ERROR: Tolerance cannot be zero (i.e. less than 1e-10)" << endl;
    return 0;
  }
  ...
  if(use_tol > min_dist) {
    cerr << "SYM::SpaceGroup_ITC: ERROR: The tolerance cannot be larger than the minimum interatomic distance (" << min_dist << " Angstroms). Exiting." << endl;
    exit(1);
  }
  ...
  if(num_species_label != xstr.atoms.size() && num_species_label != 0) {
    cerr << "SYM::SpaceGroup_ITC: POSCAR ERROR (Some atoms labeled, others are not ... please label all atoms)" << endl;
    exit(0);
  }
  ...
  if(generatefakenames == true) {
    if(LDEBUG) { cerr << "SYM::SpaceGroup_ITC: Atoms in POSCAR not labeled ... assigning fake names" << endl; }
...
//  bool first_wyckoff = true;
      ...
      if(SYM::invec<int>(sg_search, sgindex[j])) {
          ...
          if(match_count == generators[j].size() * (centeringgroups)) {
            ...
            for (uint ix = 0; ix < CG.size(); ix++) {
              ...
              if(SYM::solve_overdetermined_system(LHS, RHS, OriginShift, CCell.lattice, CCell.dist_nn_min)) {
                if(LDEBUG){
                  cerr << "SYM::SpaceGroup_ITC: Generators and origin shift of space group #" << sgindex[j] << " matched to ITC. Testing Wyckoff positions" << endl;
                }
        ...
        if(foundspacegroup == true) {
		  ...
		  for (uint w = 0; w < vmin_multiplicity.size(); w++) {
			...
			ss_double.clear();
			tmp_shift[coord] = atof(tmp_num.c_str());
			coord++;
		      }
		    }
		    //DX 9/11/17 -START 
  ...
  if(foundspacegroup == false) {
    cerr << "SYM::SpaceGroup_ITC: ERROR: Failed to find WYCKOFF POSITIONS, ORIGIN SHIFT, or inconsistent number of GENERATORS." << endl;
    exit(1);
  }

  if(LDEBUG) { cerr << "SYM::SpaceGroup_ITC: Tolerance used to obtain this space group: " << _SYM_TOL_ << endl; }
  ...
  wss << CCell.title << "| SG: " << GetSpaceGroupName(spacegroup) << " " << spacegroup << " PG: " << pointgroup << " BL: " << CCell.bravais_label_ITC << " | sym_eps: " << (*this).sym_eps << endl;
