...
namespace estructure {
  string PEDOS_GENERATE_GNUPLOTSCRIPT(const string& plot_pdosname, const string& PEDOStitle, const double& DOS_Emin, const double& DOS_Emax, const double& DOSMAX, const double& DOSSCALE, const int& Natom_pdos, const vector<vector<vector<double> > >& PDOS, const string& pdosdatafile) {
    ...
    ss_gnuplot << "#Generated by AFLOW (Kesong Yang [kesong.yang@gmail.com], 2011, Duke University)" << endl;
...
namespace estructure {
  void PLOT_DOS(string options) {
    ...
    cerr << "estructure::PLOT_DOS: working in directory=" << directory << endl;

    ICSDName=KBIN::ExtractSystemName(directory); //Get SystemName

    //READ OUTCAR, get the number of IONS, Fermi Level  
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // [OBSOLETE]  aflowlib::vaspfile2stringstream(directory, "OUTCAR", ss_outcar);
    // [OBSOLETE]  // [OBSOLETE]  xOUTCAR pedos_outcar; pedos_outcar.GetProperties(ss_outcar.str());
    // [OBSOLETE]  xOUTCAR pedos_outcar; pedos_outcar.GetProperties(ss_outcar);
    ...
    if(aurostd::FileExist(ICSDName+"_DOS.eps"))  aurostd::execute(XHOST.command("bzip2")+" -9f " + ICSDName + "_DOS.eps"); // [CORMAC]
...
namespace estructure {
  void PLOT_DOSWEB(string options) {
    ...
    cerr << "estructure::PLOT_DOSWEB: working in directory=" << directory << endl;

    //READ DOSCAR.static
    bool found=FALSE;

    // [OBSOLETE]  string doscarfile_tmp=aurostd::TmpFileCreate("DOSCAR");
    // [OBSOLETE]  
    // [OBSOLETE]  if(!found && aurostd::FileExist(directory+"/DOSCAR.static.bz2")) {
    // [OBSOLETE] 	aurostd::execute(XHOST.command("bzcat")+" "+directory+"/DOSCAR.static.bz2 > " + doscarfile_tmp);
    // [OBSOLETE] 	found=TRUE;
    // [OBSOLETE]  }
    // [OBSOLETE]  if(!found && aurostd::FileExist(directory+"/DOSCAR.static.gz")) {
    // [OBSOLETE] 	aurostd::execute("gunzip "+directory+"/DOSCAR.static.gz");
    // [OBSOLETE] 	DOSCARfile = directory + "/DOSCAR.static";
    // [OBSOLETE] 	aurostd::execute("cat "+ DOSCARfile + " > " + doscarfile_tmp);
    // [OBSOLETE] 	aurostd::RemoveFile(DOSCARfile);
    // [OBSOLETE] 	found=TRUE;
    // [OBSOLETE]  }
    // [OBSOLETE]  if(!found && aurostd::FileExist(directory+"/DOSCAR.bz2")) {
    // [OBSOLETE] 	aurostd::execute(XHOST.command("bzcat")+" "+directory+"/DOSCAR.bz2 > " + doscarfile_tmp);
    // [OBSOLETE] 	found=TRUE;
    // [OBSOLETE]  }
    // [OBSOLETE]  if(!found && aurostd::FileExist(directory+"/DOSCAR.gz")) {
    // [OBSOLETE] 	aurostd::execute("gunzip "+directory+"/DOSCAR.gz");
    // [OBSOLETE] 	DOSCARfile = directory + "/DOSCAR";
    // [OBSOLETE] 	aurostd::execute("cat "+ DOSCARfile + " > " + doscarfile_tmp);
    // [OBSOLETE] 	aurostd::RemoveFile(DOSCARfile);
    // [OBSOLETE] 	found=TRUE;
    // [OBSOLETE]  }
    // [OBSOLETE]  if(!found) {
    // [OBSOLETE] 	cerr<<"DOSCAR or DOSCAR.static or DOSCAR.static.bz2 not found in the directory, aborting."<< endl;
    // [OBSOLETE] 	exit(0);
    // [OBSOLETE]  }
    // [OBSOLETE] 
    // [OBSOLETE]  dosfile.clear(); dosfile.str(std::string());
    // [OBSOLETE]  DOSCARfile = doscarfile_tmp;
    // [OBSOLETE]  aurostd::file2stringstream(DOSCARfile, dosfile);
    // [OBSOLETE]  if(aurostd::FileExist(doscarfile_tmp)) aurostd::RemoveFile(doscarfile_tmp);

    if(!found&&aurostd::FileExist(directory+"/DOSCAR.static.bz2")) found=aurostd::efile2stringstream(directory+"/DOSCAR.static.bz2",dosfile);
    if(!found&&aurostd::FileExist(directory+"/DOSCAR.static.gz")) found=aurostd::efile2stringstream(directory+"/DOSCAR.static.gz",dosfile);
    if(!found&&aurostd::FileExist(directory+"/DOSCAR.static")) found=aurostd::efile2stringstream(directory+"/DOSCAR.static",dosfile);
    if(!found&&aurostd::FileExist(directory+"/DOSCAR.bz2")) found=aurostd::efile2stringstream(directory+"/DOSCAR.bz2",dosfile);
    if(!found&&aurostd::FileExist(directory+"/DOSCAR.gz")) found=aurostd::efile2stringstream(directory+"/DOSCAR.gz",dosfile);
    if(!found&&aurostd::FileExist(directory+"/DOSCAR")) found=aurostd::efile2stringstream(directory+"/DOSCAR",dosfile);
    
    if(!found) {
      cerr<<"ERROR: estructure::PLOT_DOSWEB: DOSCAR[.static][.bz2|.gz| ] not found in the directory, aborting."<< endl;
      exit(0);
    }

    // [OBSOLETE]    int TotalNumberDOSCAR = aurostd::GetNLinesFile(DOSCARfile);
...
namespace estructure {
  void PLOT_PEDOSALL_AFLOWLIB(string options, _aflags& aflags) {
    ...
    cerr << "estructure::PLOT_PEDOSALL_AFLOWLIB: working in directory=" << directory << endl;
    ...
    bool LSCOUPLING = pedos_outcar.LSCOUPLING; 
    if(LSCOUPLING) {   
      cerr << "ERROR - estructure::PLOT_PEDOSALL_AFLOWLIB:" << endl;
      cerr << "Not working on LSCOUPLING now!"; exit(0);} 
	...
	+ AtomicName[Natom-1] + " atom!";
      cerr << "estructure::PLOT_PEDOSALL_AFLOWLIB: working on " << str_out << endl;
	...
	if(aurostd::FileExist(plot_pdosname+".eps"))  aurostd::execute(XHOST.command("bzip2")+" -9f " + plot_pdosname + ".eps"); // [CORMAC]
	//Postprocess
	if(!XHOST.vflag_control.flag("KEEP::GPL")) { 
    ...
    //  if(LDEBUG) 
    cerr << "estructure::PLOT_PEDOSALL_AFLOWLIB: END" << endl;
  } // function
}
...
namespace estructure {
  void PLOT_PEDOSALL(string options) {
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // [OBSOLETE] aflowlib::vaspfile2stringstream(directory,"OUTCAR",ss_outcar);
    // [OBSOLETE] xOUTCAR pedos_outcar; pedos_outcar.GetProperties(ss_outcar.str());
    // [OBSOLETE] xOUTCAR pedos_outcar; pedos_outcar.GetProperties(ss_outcar);
...
namespace estructure {
  void PLOT_PEDOS(string options) {
    ...
    bool LSCOUPLING = pedos_outcar.LSCOUPLING; if(LSCOUPLING) {
      cerr << "ERROR - estructure::PLOT_PEDOS: Not working on LSCOUPLING now!"; exit(0);} 
    NIONS = pedos_outcar.NIONS;
    //Efermi = pedos_outcar.Efermi;
...
namespace estructure {
  void PLOT_BAND(string options) {
    ...
    cerr << "estructure::PLOT_BAND: working in directory=" << directory << endl;
    ...
    //if(aurostd::FileExist(ICSDName+"_BZ.eps")) aurostd::execute(XHOST.command("bzip2")+" -9f " + ICSDName + "_BZ.eps");
    ...
    if(aurostd::FileExist(ICSDName+".eps"))  aurostd::execute(XHOST.command("bzip2")+" -9f " + ICSDName + ".eps"); // [CORMAC]
  }
}
...
namespace estructure {
  void PLOT_BAND_SPINSPLIT(string options) {
    ...
    cerr << "estructure::PLOT_BAND_SPINSPLIT: working in directory=" << directory << endl;

    ICSDName=KBIN::ExtractSystemName(directory); //Get SystemName

    // READ OUTCAR, get the number of IONS, Fermi Level  
    // [OBSOLETE]   string outcarfile_tmp=aurostd::TmpFileCreate("OUTCAR");
    // [OBSOLETE]    if(aurostd::FileExist(directory+"/OUTCAR"))         OUTCARfile = directory + "/OUTCAR";
    // [OBSOLETE]     else if(aurostd::FileExist(directory+"/OUTCAR.bands")) OUTCARfile = directory + "/OUTCAR.bands";
    // [OBSOLETE]     else if(aurostd::FileExist(directory+"/OUTCAR.bands.bz2")) {
    // [OBSOLETE]       aurostd::execute(XHOST.command("bzcat")+" "+directory+"/OUTCAR.bands.bz2 > " + outcarfile_tmp);
    // [OBSOLETE]       OUTCARfile = outcarfile_tmp;
    // [OBSOLETE]     } else {
    // [OBSOLETE]       cerr<<"OUTCAR or OUTCAR.bands or OUTCAR.bands.bz2 not found in the directory, aborting."<< endl;
    // [OBSOLETE]       exit(0);
    // [OBSOLETE]     }
    // [OBSOLETE]     straus.clear(); straus.str(std::string());
    // [OBSOLETE]     aurostd::file2stringstream(OUTCARfile, straus);  //Converting OUTCARfile to a stringstream
    // [OBSOLETE]    if(aurostd::FileExist(outcarfile_tmp)) aurostd::RemoveFile(outcarfile_tmp);

    found=FALSE;
    if(!found&&aurostd::FileExist(directory+"/OUTCAR.bands.bz2")) found=aurostd::efile2stringstream(directory+"/OUTCAR.bands.bz2",straus);
    if(!found&&aurostd::FileExist(directory+"/OUTCAR.bands.gz")) found=aurostd::efile2stringstream(directory+"/OUTCAR.bands.gz",straus);
    if(!found&&aurostd::FileExist(directory+"/OUTCAR.bands")) found=aurostd::efile2stringstream(directory+"/OUTCAR.bands",straus);
    if(!found&&aurostd::FileExist(directory+"/OUTCAR.bz2")) found=aurostd::efile2stringstream(directory+"/OUTCAR.bz2",straus);
    if(!found&&aurostd::FileExist(directory+"/OUTCAR.gz")) found=aurostd::efile2stringstream(directory+"/OUTCAR.gz",straus);
    if(!found&&aurostd::FileExist(directory+"/OUTCAR")) found=aurostd::efile2stringstream(directory+"/OUTCAR",straus);
    if(!found) {cerr<<"ERROR: estructure::PLOT_BAND_SPINSPLI: OUTCAR[.bands][.bz2|.gz| ] not found in the directory, aborting."<< endl;exit(0);}
    ...
    //***********************************DEALING WITH BANDSDATA**************************************************************
    //READ EIGENVAL
    // [OBSOLETE]    string eigenvalfile_tmp=aurostd::TmpFileCreate("EIGENVAL");
    // [OBSOLETE]     if(aurostd::FileExist(directory+"/EIGENVAL")) 	 	EIGENVALfile = directory + "/EIGENVAL";
    // [OBSOLETE]     else if(aurostd::FileExist(directory+"/EIGENVAL.bands"))	EIGENVALfile = directory + "/EIGENVAL.bands";
    // [OBSOLETE]     else if(aurostd::FileExist(directory+"/EIGENVAL.bands.bz2")) {
    // [OBSOLETE]       aurostd::execute(XHOST.command("bzcat")+" "+directory+"/EIGENVAL.bands.bz2 > " + eigenvalfile_tmp);
    // [OBSOLETE]       EIGENVALfile = eigenvalfile_tmp;
    // [OBSOLETE]     } else {
    // [OBSOLETE]       cerr<<"EIGENVAL or EIGENVAL.bands or EIGENVAL.bands.bz2 not found in the directory, aborting."<< endl;
    // [OBSOLETE]       exit(0);
    // [OBSOLETE]    }
    // [OBSOLETE]     straus.clear(); straus.str(std::string());
    // [OBSOLETE]     aurostd::file2stringstream(EIGENVALfile, straus);  //Converting EIGENVALfile to a stringstream
    // [OBSOLETE]     if(aurostd::FileExist(eigenvalfile_tmp))  aurostd::RemoveFile(eigenvalfile_tmp);
    
    found=FALSE;
    if(!found&&aurostd::FileExist(directory+"/EIGENVAL.bands.bz2")) found=aurostd::efile2stringstream(directory+"/EIGENVAL.bands.bz2",straus);
    if(!found&&aurostd::FileExist(directory+"/EIGENVAL.bands.gz")) found=aurostd::efile2stringstream(directory+"/EIGENVAL.bands.gz",straus);
    if(!found&&aurostd::FileExist(directory+"/EIGENVAL.bands")) found=aurostd::efile2stringstream(directory+"/EIGENVAL.bands",straus);
    if(!found&&aurostd::FileExist(directory+"/EIGENVAL.bz2")) found=aurostd::efile2stringstream(directory+"/EIGENVAL.bz2",straus);
    if(!found&&aurostd::FileExist(directory+"/EIGENVAL.gz")) found=aurostd::efile2stringstream(directory+"/EIGENVAL.gz",straus);
    if(!found&&aurostd::FileExist(directory+"/EIGENVAL")) found=aurostd::efile2stringstream(directory+"/EIGENVAL",straus);
    if(!found) {cerr<<"ERROR: estructure::PLOT_BAND_SPINSPLI: EIGENVAL[.bands][.bz2|.gz| ] not found in the directory, aborting."<< endl;exit(0);}
    ...
    //READ KPOINTS	
    // [OBSOLETE]    string kpointsfile_tmp=aurostd::TmpFileCreate("KPOINTS");
    // [OBSOLETE]    if(aurostd::FileExist(directory+"/KPOINTS")) 	 	KPOINTSfile = directory + "/KPOINTS";
    // [OBSOLETE]    else if(aurostd::FileExist(directory+"/KPOINTS.bands"))	KPOINTSfile = directory + "/KPOINTS.bands";
    // [OBSOLETE]    else if(aurostd::FileExist(directory+"/KPOINTS.bands.bz2")) {
    // [OBSOLETE]      aurostd::execute(XHOST.command("bzcat")+" "+directory+"/KPOINTS.bands.bz2 > " + kpointsfile_tmp);
    // [OBSOLETE]      KPOINTSfile = kpointsfile_tmp;
    // [OBSOLETE]    } else {
    // [OBSOLETE]      cerr<<"KPOINTS or KPOINTS.bands or KPOINTS.bands.bz2 not found in the directory, aborting."<< endl;
    // [OBSOLETE]      exit(0);
    // [OBSOLETE]    }
    // [OBSOLETE]    straus.clear(); straus.str(std::string());
    // [OBSOLETE]    aurostd::file2stringstream(KPOINTSfile, straus);  //Converting KPOINTSfile to a stringstream
    // [OBSOLETE]    if(aurostd::FileExist(kpointsfile_tmp)) aurostd::RemoveFile(kpointsfile_tmp);

    found=FALSE;
    if(!found&&aurostd::FileExist(directory+"/KPOINTS.bands.bz2")) found=aurostd::efile2stringstream(directory+"/KPOINTS.bands.bz2",straus);
    if(!found&&aurostd::FileExist(directory+"/KPOINTS.bands.gz")) found=aurostd::efile2stringstream(directory+"/KPOINTS.bands.gz",straus);
    if(!found&&aurostd::FileExist(directory+"/KPOINTS.bands")) found=aurostd::efile2stringstream(directory+"/KPOINTS.bands",straus);
    if(!found&&aurostd::FileExist(directory+"/KPOINTS.bz2")) found=aurostd::efile2stringstream(directory+"/KPOINTS.bz2",straus);
    if(!found&&aurostd::FileExist(directory+"/KPOINTS.gz")) found=aurostd::efile2stringstream(directory+"/KPOINTS.gz",straus);
    if(!found&&aurostd::FileExist(directory+"/KPOINTS")) found=aurostd::efile2stringstream(directory+"/KPOINTS",straus);    
    if(!found) {cerr<<"ERROR: estructure::PLOT_BAND_SPINSPLI: KPOINTS[.bands][.bz2|.gz| ] not found in the directory, aborting."<< endl;exit(0);}
...
namespace estructure {
  bool GET_DOS_DATA(stringstream& ss_dosfile, stringstream& ss_outfile, double& Efermi, vector<vector<double> >& TDOS, vector<vector<double> >& TOTALPDOS, vector<vector<vector<double> > >& PDOS) {
	...
	ss_line >> strtmp >> strtmp >> strtmp >> strtmp >> strtmp >> strtmp >> strtmp >> strtmp >> strtmp >> strtmp >> strtmp >> NIONS;
      ...
      if(line.find(anchor_word_LSORBIT) !=string::npos) LSCOUPLING = true;
    }
...
namespace estructure{
    ...
    bool DOSDATA_JSON(aurostd::xoption& vpflow, string directory, stringstream& json,bool wrapping_brackets){ //CO 171025
        ...
        if(!found) {
          cerr<<"ERROR: estructure::DOSDATA_JSON: DOSCAR[.static][.bz2|.gz| ] not found in the directory, aborting."<< endl;
          return FALSE;
          //exit(0);
        }
        ...
        if(POSCAR_file.empty()&&aurostd::FileExist(directory+"/POSCAR.static.bz2")) POSCAR_file=directory+"/POSCAR.static.bz2";
        if(POSCAR_file.empty()&&aurostd::FileExist(directory+"/POSCAR.static.gz")) POSCAR_file=directory+"/POSCAR.static.gz";
        if(POSCAR_file.empty()&&aurostd::FileExist(directory+"/POSCAR.static")) POSCAR_file=directory+"/POSCAR.static";
        if(POSCAR_file.empty()&&aurostd::FileExist(directory+"/POSCAR.bz2")) POSCAR_file=directory+"/POSCAR.bz2";
        if(POSCAR_file.empty()&&aurostd::FileExist(directory+"/POSCAR.gz")) POSCAR_file=directory+"/POSCAR.gz";
        if(POSCAR_file.empty()&&aurostd::FileExist(directory+"/POSCAR")) POSCAR_file=directory+"/POSCAR";
        
        if(POSCAR_file.empty()) {
          cerr<<"ERROR: estructure::DOSDATA_JSON: POSCAR[.static][.bz2|.gz| ] not found in the directory, aborting."<< endl;
          return FALSE;
          //exit(0);
        }
        ...
        //[OBSOLETE]  if(!iatomsfound&&aurostd::FileExist(directory+"/aflow.iatoms.out.bz2")) iatomsfound=aurostd::efile2stringstream(directory+"/aflow.iatoms.out.bz2",iatomsfile);
        //[OBSOLETE]  if(!iatomsfound) {
        //[OBSOLETE]    cerr<<"ERROR: estructure::DOSDATA_JSON: aflow.iatoms.out.bz2 not found in the directory, aborting."<< endl;
