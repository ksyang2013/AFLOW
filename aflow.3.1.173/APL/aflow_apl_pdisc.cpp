...
void PhononDispersionCalculator::initPath(const string& USER_DC_INITPATH,
                                          int USER_DC_NPOINTS) {
  // Use PLATON to get space group number if user did not get it...
  string path = USER_DC_INITPATH;
  if (path.empty()) {
    xstructure a(_pc.getPrimitiveCellStructure());
    //CO - START
    if (a.bravais_lattice_variation_type == "") {
      if (a.spacegroup == "") {
        if(a.space_group_ITC<1 || a.space_group_ITC>230){ //if (a.space_group_ITC == 0) { //CO 180214 - if not set then it could be 32767
      ...
      //	path = LATTICE_Lattice_Variation_SpaceGroup(spacegroupNumber,_pc.getPrimitiveCellStructure());
      //path = LATTICE::SpaceGroup2LatticeVariation(spacegroupNumber,_pc.getPrimitiveCellStructure());
      path = LATTICE::SpaceGroup2LatticeVariation(a.space_group_ITC, a);
    } else {
      path = a.bravais_lattice_variation_type;
    }
    _logger << "The phonon dispersion curves will be generated for lattice variation " << path << "." << apl::endl;
  }
  //CO - END

  // cerr << "PATH=" << path << std::endl;
  // Suck point definition from the electronic structure part of AFLOW...
  _pb.takeAflowElectronicPath(path,
                              _pc.getPrimitiveCellStructure(),
                              _pc.getSuperCellStructure());
  _pb.setDensity(USER_DC_NPOINTS);

  // Get points...
  _qpoints = _pb.getPath();
}
