...
eos_calculate::eos_calculate(Supercell& sc, StrPairs& strpair, _xinput& xinput, //_xvasp& xvasp,
                             ...
                             string& AflowIn,
                             Logger& l) : PhononCalculator(sc, strpair, xinput, aflags, kflags, xflags, AflowIn, l) { //xvasp, aflags, kflags, vflags, l) {
  GRUNEISEN = false;
  EOS = false;
  GP_dir_names.clear();
  PH_dir_names.clear();
...
void eos_calculate::GP_RUN() {
  ...
  for (double i = -GP_VOL_DISTORTION; i <= GP_VOL_DISTORTION; i += GP_VOL_DISTORTION) {
    ...
    double scale = 0.00;
    GPxRuns[idxRun].setXStr(PhononCalculator::_supercell.getPrimitiveStructure());
    xmatrix<double> m(3, 3, 1, 1);
    scale = GPxRuns[idxRun].getXStr().scale;
    m = GPxRuns[idxRun].getXStr().lattice;
    GPxRuns[idxRun].getXStr().atoms = PhononCalculator::_supercell.getPrimitiveStructure().atoms;
    ...
    for (uint j = 0; j < GPxRuns[idxRun].getXStr().atoms.size(); j++)
      GPxRuns[idxRun].getXStr().atoms[j] = PhononCalculator::_supercell.getPrimitiveStructure().atoms[j];
    m.clear();
...
void eos_calculate::writeGPOUTPUT(const _xinput& xinput, const bool _isGP) {
  if(xinput.AFLOW_MODE_VASP){return createGPAFLOWIN(xinput.xvasp,_isGP);}
  if(xinput.AFLOW_MODE_AIMS){return createGPAIMSOUTPUT(xinput.xaims);}
  else{
    throw APLRuntimeError("apl::eos_calculate::writeGPOUTPUT(); Input -> aflow.in conversion unknown.");
  }
}

void eos_calculate::createGPAIMSOUTPUT(const _xaims& xaims_input) {
  ...
  if (!aurostd::FileExist(filename))
    throw apl::APLRuntimeError("apl::eos_calculate::createGPAIMSOUTPUT(); Cannot create [" + AFLOWRC_DEFAULT_AIMS_EXTERNAL_GEOM + "] file.");
...
// ***************************************************************************************
//create aflow.in for GRUNEISEN calculation
void eos_calculate::createGPAFLOWIN(const _xvasp& xvasp_input, const bool _isGP) {
  //const xstructure& pc = _supercell.getPrimitiveStructure();
  ...
  //CO - START
  //if( !outfile.is_open() )
  //  throw apl::APLRuntimeError("apl::PhononCalculator::createGPAFLOWIN; Cannot create [aflow.in] file.");
  // CHMOD a+rw aflow.in
  //aurostd::ChmodFile("a+rw",filename);
  //CO - END
...
void eos_calculate::EOS_RUN(string& AflowIn,const double sd, const double ed, const double dm) {
  ...
  for (double i = sd; i <= ed; i += dm) {
    ...
    // new scale factor calulation
    double scale;
    EOSxRuns[idxRun].setXStr(PhononCalculator::_supercell.getPrimitiveStructure());
    if (!PH) PHxRuns[idPHVaspRun].setXStr(PhononCalculator::_supercell.getPrimitiveStructure());
    xmatrix<double> m(3, 3, 1, 1);
    ...
    for (uint j = 0; j < EOSxRuns[idxRun].getXStr().atoms.size(); j++) {
      EOSxRuns[idxRun].getXStr().atoms[j] = PhononCalculator::_supercell.getPrimitiveStructure().atoms[j];
      if (!PH) PHxRuns[idPHVaspRun].getXStr().atoms[j] = PhononCalculator::_supercell.getPrimitiveStructure().atoms[j];
    }
    EOS_Volumes.push_back(newscale * newscale * newscale * det);
    m.clear();
    // new scale factor calulation end

    //if directories does't exist create directories
...
void eos_calculate::createEOSAFLOWIN(const _xvasp& xvasp_input) {
  ...
  if (vflags.KBIN_VASP_FORCE_OPTION_AUTO_PSEUDOPOTENTIALS.isentry) aflowin << "[VASP_FORCE_OPTION]AUTO_PSEUDOPOTENTIALS=" << vflags.KBIN_VASP_FORCE_OPTION_AUTO_PSEUDOPOTENTIALS.scheme << std::endl;
  if (vflags.KBIN_VASP_FORCE_OPTION_ABMIX.isentry) aflowin << "[VASP_FORCE_OPTION]ABMIX=" << vflags.KBIN_VASP_FORCE_OPTION_ABMIX.scheme << std::endl;
  if (vflags.KBIN_VASP_FORCE_OPTION_TYPE.isentry) aflowin << "[VASP_FORCE_OPTION]TYPE=" << vflags.KBIN_VASP_FORCE_OPTION_TYPE.scheme << std::endl;
  ...
  // TYPE WRITING
  if (xvasp.AVASP_flag_TYPE.scheme.at(0) == 'M')
  ...
  if (xvasp.AVASP_flag_TYPE.scheme.at(0) == 'I')
  ...
  if (xvasp.AVASP_flag_TYPE.scheme.at(0) == 'S')
