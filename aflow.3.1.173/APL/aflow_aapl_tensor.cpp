...
void PhononCalculator::VaspTensor() {
  ...
  double cells = _supercell.getStructure().atoms.size() / _supercell.getPrimitiveStructure().atoms.size();
      ...
      for (uint i = 0; i < 6; i++) {
        for (uint j = 0; j < 6; j++) {
          if (_strpair.ipairs[ipa].dist_in[i][j]) {
            ...
            //CO - START
            xInputsT[idxRun].setXStr(_supercell.getStructureLight());  
            //CO - END
            ...
            system = system + "@" + runname;
              ...
              xInputsT[idxRun].xvasp.INCAR << "ENCUT= 500   # For finer forces" << std::endl;
              xInputsT[idxRun].xvasp.INCAR << "EDIFF = 1E-6   # For finer forces" << std::endl;
...
void PhononCalculator::StoreForcesTensor() {
  ...
  for (uint idxRun = 0; idxRun < xInputsT.size() && !found_outfile_anywhere; idxRun++) {
    ...
    string tarfilename = xInputsT[idxRun].getDirectory() + ".tar.bz2";
  ...
  for (uint idxRun = 0; idxRun < xInputsT.size(); idxRun++) {
    ...
    if (DOtar)
      if (!aurostd::FileExist(tarfilename)) aurostd::execute(string("tar -cjf ") + tarfilename + " " + xInputsT[idxRun].getDirectory() + "/");
    if (DOtar)
      if (aurostd::FileExist(tarfilename)) aurostd::execute(string("rm -rf ") + xInputsT[idxRun].getDirectory() + "/");
  }
  ...
  for (uint i = 0; i < _supercell.getPrimitiveStructure().atoms.size(); i++) {
    vector<vector<vector<xvector<double> > > > pForces;
    for (uint j = 0; j < _supercell.getStructure().atoms.size(); j++) {
      vector<vector<xvector<double> > > atomsForces;
      for (uint k = 0; k < _supercell.getStructure().atoms.size(); k++) {
        ...
        for (uint j = 0; j < 6; j++) {
          if (_strpair.ipairs[ipa].dist_in[i][j]) {
            ...
            if ((uint)_strpair.ipairs[ipa].dist[i][j] == (i * 6) + j) {
              ...
              int SymU = _strpair.getU(_supercell.getStructure(), ipa, at1sc, at2, i, j);
              for (int at3 = 0; at3 < _supercell.getNumberOfAtoms(); at3++) {
                int at4 = _strpair.getAtom(_supercell.getStructure(), at3, SymU);
                bigTensor[at1][at2][at3][(i * 6) + j] = _supercell.getStructure().fgroup[SymU].Uc * bigTensor[at1][at2][at4][_strpair.ipairs[ipa].dist[i][j]];
              }
            }
          }
        }
...
void PhononCalculator::PrintTensorBTE(string filename) {
  ...
  int half = _supercell.getStructure().atoms.size() / _supercell.getPrimitiveStructure().atoms.size();
  xvector<double> vec1, vec2, at1, at2, at3;

  xmatrix<double> lattice = _supercell.getStructure().lattice;

  int counter = 1;
  for (int i = 0; i < int(_supercell.getPrimitiveStructure().atoms.size()); i++) {
    uint at1sc = _supercell.pc2scMap(i);
    at1 = _supercell.getStructure().atoms[at1sc].cpos;
    for (int j = 0; j < half; j++) {
      vec1 = _supercell.getStructure().atoms[j].fpos;
      for (int r = 1; r < 4; r++) {
        if (vec1[r] > 0.5000000) {
          vec1[r] = vec1[r] - 1;
        }
      }
      vec1 = F2C(_supercell.getStructure().lattice, vec1);
      for (int j2 = 0; j2 < int(_supercell.getPrimitiveStructure().atoms.size()); j2++) {
        at2 = _supercell.getStructure().atoms[j + j2 * half].cpos;
        double dist1 = _strpair.getMinDist(lattice, at1, at2);
        if (dist1 < _strpair.cutoff) {
          for (int k = 0; k < half; k++) {
            vec2 = _supercell.getStructure().atoms[k].fpos;
            for (int r = 1; r < 4; r++) {
              if (vec2[r] > 0.5000000) {
                vec2[r] = vec2[r] - 1;
              }
            }
            vec2 = F2C(_supercell.getStructure().lattice, vec2);
            for (int k2 = 0; k2 < int(_supercell.getPrimitiveStructure().atoms.size()); k2++) {
              at3 = _supercell.getStructure().atoms[k + k2 * half].cpos;
  ...
  for (uint at = 0; at < _supercell.getPrimitiveStructure().atoms.size(); at++) {
    xvector<double> kk;
    kk = _supercell.getPrimitiveStructure().atoms[at].fpos;
    for (uint zz = 0; zz < 4; zz++) {
      kk[3] = (_supercell.getPrimitiveStructure().atoms[at].fpos[3] / 4.0) + double(zz) / 4.0;
      for (uint yy = 0; yy < 4; yy++) {
        kk[2] = (_supercell.getPrimitiveStructure().atoms[at].fpos[2] / 4.0) + double(yy) / 4.0;
        for (uint xx = 0; xx < 4; xx++) {
          kk[1] = (_supercell.getPrimitiveStructure().atoms[at].fpos[1] / 4.0) + double(xx) / 4.0;
  ...
  for (uint at = 0; at < _supercell.getStructure().atoms.size(); at++) {
    for (uint at2 = 0; at2 < _supercell.getStructure().atoms.size(); at2++) {
      if (aurostd::modulus(_supercell.getStructure().atoms[at2].fpos - list_vec[at]) < _AFLOW_APL_EPS_) {
        list_index[at] = at2;
        at2 = _supercell.getStructure().atoms.size();
      }
    }
  }
  ...
  outfile2 << std::fixed << std::showpoint << std::setprecision(0) << _supercell.getStructure().atoms.size() << std::endl;
...
xmatrix<xcomplex<double> > PhononCalculator::getNonanalyticalTermWangAAPL(const xvector<double> _q, vector<xmatrix<xcomplex<double> > >& kk) {
  const xstructure& sc = _supercell.getStructure();
  const xstructure& pc = _supercell.getPrimitiveStructure();
...
xmatrix<xcomplex<double> > PhononCalculator::getDynamicMatrixAAPL2(const xvector<double> kpoint, vector<xmatrix<xcomplex<double> > >& dDynM) {
  xstructure pc = _supercell.getPrimitiveStructure();
  uint sc_atoms = _supercell.getStructure().atoms.size();
  uint pcAtomsSize = pc.atoms.size();
  uint nBranches = 3 * pcAtomsSize;
  double volume = det(pc.lattice);
  ...
  xmatrix<double> lattvec = _supercell.getPrimitiveStructure().lattice;
  xmatrix<double> lattvecSc = _supercell.getStructure().lattice;
      ...
      for (int ix1 = 0; ix1 < scell(1); ix1++) {
        for (int iy1 = 0; iy1 < scell(2); iy1++) {
          for (int iz1 = 0; iz1 < scell(3); iz1++) {
            ...
            for (int tatoms = 0; tatoms < int(_supercell.getStructure().atoms.size()); tatoms++) {
              xvector<double> fdiff = isatom2_vec - _supercell.getStructure().atoms[tatoms].fpos;
              SYM::PBC(fdiff);
              if (aurostd::modulus(fdiff) < 1E-04) {
                isatom2 = tatoms;
                tatoms = _supercell.getStructure().atoms.size();
              }
            }
