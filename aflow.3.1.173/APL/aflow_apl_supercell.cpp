...
Supercell::Supercell(const xstructure& xstr, Logger& l) : _logger(l) {
  ...
  // Copy
  _pcStructure = xstr;
  _pcStructure.ReScale(1.0);
  _pcStructure.ShifOriginToAtom(0);
  _pcStructure.BringInCell();  //clean up

  if(LDEBUG){
    cerr << "APL::Supercell::Supercell: this is the structure to be analyzed" << std::endl;
    cerr << _pcStructure << std::endl;
  }

  //COREY, DO NOT MODIFY THE STRUCTURE BELOW HERE, THIS INCLUDES RESCALE(), BRINGINCELL(), SHIFORIGINATOM(), etc.
  _logger << "Estimating the symmetry of structure." << apl::endl; //primitive cell." << apl::endl; //CO 180216 - we do NOT primitivize unless requested via [VASP_FORCE_OPTION]CONVERT_UNIT_CELL
  calculateWholeSymmetry(_pcStructure);

  //store some mutual properties
  //need this for getPrimitiveStructureLight()
  //#if CENTER_PRIM
  _pcStructure_original = _pcStructure;
  LightCopy(_pcStructure, _pcStructure_light);
  //_pcStructure_atoms_original = _pcStructure.atoms;
  //#endif
  _skew = SYM::isLatticeSkewed(_pcStructure.lattice, _pcStructure.dist_nn_min, _pcStructure.sym_eps);
  _sym_eps = _pcStructure.sym_eps;
  clear();
}
#else
Supercell::Supercell(const xstructure& xstr, Logger& l) : _logger(l) {
  ...
  _pcStructure.ReScale(1.0);
  _pcStructure.ShifOriginToAtom(0);
  _pcStructure.BringInCell();

  //
  _logger << "Reducing the input structure into a standard primitive form." << apl::endl;
  //_pcStructure = GetPrimitive(xstr);
  _pcStructure.Standard_Primitive_UnitCellForm();
  _pcStructure.ReScale(1.0);
  _pcStructure.ShifOriginToAtom(0);
  _pcStructure.BringInCell();
...
Supercell& Supercell::operator=(const Supercell& that) {
  if (this != &that) {
    ...
    _pcStructure = that._pcStructure;
    _pcStructure_original = that._pcStructure_original;  //CO
    _scStructure = that._scStructure;
...
void Supercell::build(int nx, int ny, int nz, bool VERBOSE) {
  ...
  if (VERBOSE) {
    _logger << "The supercell is going to build as " << nx << " x " << ny << " x " << nz
            << " (" << (uint)(nx * ny * nz * _pcStructure.atoms.size()) << " atoms)." << apl::endl;
  }
  ...
  // Get supercell
  //_scStructure = GetSuperCell(_pcStructure, scale, _sc2pcMap, _pc2scMap, TRUE, _derivative_structure);  //now gets symmetries too! no need for full_basis (just a check)
  _scStructure = GetSuperCell(_pcStructure, scale, _sc2pcMap, _pc2scMap, TRUE, get_full_sym);  //now gets symmetries too! no need for full_basis (just a check)
  ...
  //}
  //exit(0);  //COREY REMOVE
  //_scStructure = GetSuperCell(_pcStructure, scale, _sc2pcMap, _pc2scMap, TRUE, GETFULLSYMBASIS);  //now gets symmetries too! no need for full_basis (just a check)
...
void Supercell::build(int nx, int ny, int nz, bool VERBOSE) {
  ...
  scale(2, 2) = ny;
  scale(3, 3) = nz;
  _scStructure.lattice = scale * _pcStructure.lattice;
  _scStructure.FixLattices();

  // Print info
  if (VERBOSE) _logger << "The supercell is going to build as " << nx << " x " << ny << " x " << nz
                       << " (" << (uint)(nx * ny * nz * _pcStructure.atoms.size()) << " atoms). ";

  //
  _atom atom;
  xvector<double> cshift(3);
  for (uint ia = 0; ia < _pcStructure.iatoms.size(); ia++) {
    for (uint iia = 0; iia < _pcStructure.iatoms[ia].size(); iia++) {
      ...
      for (_AFLOW_APL_REGISTER_ int i = 0; i < nx; i++)
        for (_AFLOW_APL_REGISTER_ int j = 0; j < ny; j++)
          for (_AFLOW_APL_REGISTER_ int k = 0; k < nz; k++) {
            ...
            atom = _pcStructure.atoms[_pcStructure.iatoms[ia][iia]];
            cshift = (((double)i) * _pcStructure.lattice(1) +
                      ((double)j) * _pcStructure.lattice(2) +
                      ((double)k) * _pcStructure.lattice(3));
            ...
            _scStructure.agroup.push_back(_pcStructure.agroup[_pcStructure.iatoms[ia][iia]]);

            // Update our mapping arrays...
            _sc2pcMap.push_back(_pcStructure.iatoms[ia][iia]);
  ...
  for (_AFLOW_APL_REGISTER_ uint l = 0; l < _pcStructure.fgroup.size(); l++) {
    for (_AFLOW_APL_REGISTER_ int i = 0; i < nx; i++)
      for (_AFLOW_APL_REGISTER_ int j = 0; j < ny; j++)
        for (_AFLOW_APL_REGISTER_ int k = 0; k < nz; k++) {
          // Create position of new atoms...
          ...
          _sym_op symOp = _pcStructure.fgroup[l];
  ...
  /*
    ...
    ( (double)j ) * _pcStructure.lattice(2) +
    ( (double)k ) * _pcStructure.lattice(3) );
    _scStructure.fgroup.push_back(_pcStructure.fgroup[l]);
    ...
    for (_AFLOW_APL_REGISTER_ int j = start; j < end - 1; j++) {
      ...
      for (_AFLOW_APL_REGISTER_ int k = j + 1; k < end; k++) {
        ...
        if (_scStructure.atoms[k].fpos(1) < _scStructure.atoms[j].fpos(1)) {
          ...
          for (_AFLOW_APL_REGISTER_ int l = 0; l < (int)_pcStructure.atoms.size(); l++) {
            if (_pc2scMap[l] == j)
              _pc2scMap[l] = k;
...
int Supercell::buildSuitableForShell(int MIN_NN_SHELLS, bool shouldBeFullShell, bool VERBOSE) {
  ...
  for (uint i = 0; i < _pcStructure.iatoms.size(); i++) {
    ShellHandle s;
    sh.push_back(s);
    sh.back().init(_pcStructure, _pcStructure.iatoms[i][0], MIN_NN_SHELLS);
    try {
  ...
  while (true) {
    _scStructure = GetSuperCell(_pcStructure, i, j, k);
    //build(i,j,k);

    uint ia = 0;
    for (; ia < _pcStructure.iatoms.size(); ia++) {
      // Get ID of origin atom from pc in the sc
      uint l = 0;
      for (; l < _scStructure.atoms.size(); l++) {
        if (aurostd::modulus(_scStructure.atoms[l].cpos -
                             _pcStructure.atoms[_pcStructure.iatoms[ia][0]].cpos) < _AFLOW_APL_EPS_)
          break;
      }
    ...
    if (ia != _pcStructure.iatoms.size()) {
      i++;
      j = k = i;
      /*
	  if( i == j && j == k )
  ...
  return i * j * k * _pcStructure.atoms.size();
}
...
void Supercell::setupShellRestrictions(int MAX_NN_SHELLS) {
  ...
  for (uint i = 0; i < _pcStructure.iatoms.size(); i++) {
    ShellHandle s;
    sh.push_back(s);
    sh.back().init(_pcStructure, _pcStructure.iatoms[i][0], MAX_NN_SHELLS);
  ...
  for (uint i = 0; i < _pcStructure.iatoms.size(); i++) {
    ...
    for (uint j = 0; j < _pcStructure.iatoms[i].size(); j++) {
...
const xstructure& Supercell::getStructure() {
  return _scStructure;
}
...
xstructure Supercell::getStructureLight() {
...
const xstructure& Supercell::getPrimitiveStructure() {
  return _pcStructure;
}

// ///////////////////////////////////////////////////////////////////////////

xstructure Supercell::getPrimitiveStructureLight() {
  return _pcStructure_light;
  //xstructure a;
  //stringstream POSCAR;
  //POSCAR.str("");
  //POSCAR << _pcStructure;
  ...
  //enable inequivalent flag to work
  //for(uint i=0;i<a.atoms.size();i++){
  //  a.atoms[i].equivalent=_pcStructure.atoms[i].equivalent;
  //  a.atoms[i].is_inequivalent=_pcStructure.atoms[i].is_inequivalent;
  //  a.atoms[i].num_equivalents=_pcStructure.atoms[i].num_equivalents;
  //}
  //enable inequivalent flag to work
  //a.write_inequivalent_flag = _pcStructure.write_inequivalent_flag;
  //a.info = _pcStructure.info;
  //return a;
}
...
#if CENTER_PRIM
  //_pcStructure.ShifOriginToAtom(_sc2pcMap[i]);
  //_pcStructure.BringInCell();
  origin = _pcStructure_original.atoms[_sc2pcMap[i]].cpos;
  frigin = _pcStructure_original.atoms[_sc2pcMap[i]].fpos;
  _pcStructure.origin = origin;
  for (uint i = 0; i < _pcStructure.atoms.size(); i++) {
    _pcStructure.atoms[i].cpos = _pcStructure_original.atoms[i].cpos - origin;
    _pcStructure.atoms[i].fpos = _pcStructure_original.atoms[i].fpos - frigin;
    _pcStructure.atoms[i] = BringInCell(_pcStructure.atoms[i], _pcStructure.lattice);
  }
#endif
...
#if CENTER_PRIM
  _pcStructure.origin = _pcStructure_original.origin;
  for (uint i = 0; i < _pcStructure.atoms.size(); i++) {
    _pcStructure.atoms[i].fpos = _pcStructure_original.atoms[i].fpos;
    _pcStructure.atoms[i].cpos = _pcStructure_original.atoms[i].cpos;
  }
#endif
...
void Supercell::calcRl() {
  double cells = _scStructure.atoms.size() / _pcStructure.atoms.size();
  ...
  for (uint i = 0; i < (uint)cells; i++) {
    ...
    for (uint j = 0; j < _pcStructure.atoms.size(); j++) {
