  ...
  if (_isPolarMaterial) {
    try {
      ...
      if (aurostd::FileExist(tarfilename)) {
        ...
        if(_kbinFlags.AFLOW_MODE_VASP){command+="/OUTCAR*";}
        if(_kbinFlags.AFLOW_MODE_AIMS){command+="/aims.out*";}
...
void PhononCalculator::runVASPCalculationsBE(void) {
  ...
  // Generate POSCAR
  xInput.setXStr(_supercell.getPrimitiveStructure());
...
void LinearResponsePC::runVASPCalculationsFF() {
  ...
  // Generate POSCAR
  xInput.setXStr(_supercell.getStructureLight());
...
void PhononCalculator::readBornEffectiveChargesFromOUTCAR(void) {
  ...
  for (uint i = 0; i < _supercell.getPrimitiveStructure().atoms.size(); i++) {
    ...
    //if( id != _supercell.getPrimitiveStructure().iatoms[i][0] ) continue;
    //tokens.clear();
...
void PhononCalculator::symmetrizeBornEffectiveChargeTensors(void) {
  ...
  for (uint i = 0; i < _bornEffectiveChargeTensor.size(); i++) {
    int id = i;
    _logger << "Atom [" << sf("%03d") << id << "] (" << sf("%f")
            << sw(2) << _supercell.getPrimitiveStructure().atoms[id].cleanname
...
// Find
  ...
  // Symmetrize
  /*
      const vector<_sym_op>& pgroup = _supercell.getStructure().pgroup;
      xmatrix<double> sum(3,3);
