...
namespace aflowlib {
  uint PrototypeLibrariesSpeciesNumber(const string& label) {
	...
	for(uint i=0;i<tokens.size()-1;i++)
	  _label+=tokens.at(i)+(i<tokens.size()-2?".":"");
  ...
  xstructure PrototypeLibraries(ostream &oss,string label,string parameters,deque<string> &vatomX,deque<double> &vvolumeX,double volume_in,int mode,bool flip_option) { // COMPLETE ONE
    ...
    if(mode==LIBRARY_MODE_ICSD_AFLOWLIB || mode==LIBRARY_MODE_HTQC_ICSD_AFLOWLIB) {
      if(!XHOST.vflag_control.flag("AFLOWLIB_SERVER") && !XHOST.vflag_control.flag("AFLOWLIB_SERVER"))
        XHOST.vflag_control.addattachedscheme("AFLOWLIB_SERVER",AFLOWLIB_SERVER_DEFAULT,TRUE); // some default
	...
	*voss << "ERROR - aflowlib::PrototypeLibraries: Structure not present in the HTQC database " << _HTQC_STRING_ << "  (" << label << ")" << endl;
      if(mode==LIBRARY_MODE_ICSD_AFLOWLIB || mode==LIBRARY_MODE_HTQC_ICSD_AFLOWLIB)
	...
	*voss << "WARNING - aflowlib::PrototypeLibraries: Switching to " << XHOST.vflag_control.getattachedscheme("AFLOWLIB_SERVER") << " download" << endl;
    ...
    if(mode_load!=STRUCTURE_MODE_ICSD) {
      ...
      str.title+=" - "+str.prototype+" "+_HTQC_STRING_+" ";
    }
    if(mode_load==STRUCTURE_MODE_ICSD) {
      str.title=label_library+" - "+"("+label+")";
	...
	*voss << "ERROR: aflow_xproto.cpp  in the SWAP_AB MODE" << endl; exit(0);
	...
	if(flip_option==FALSE) {
      ...
      str.spacegroup=GetSpaceGroupName(str.spacegroupnumber);
	...
	label_library="";
	vatomX.at(0)+vatomX.at(1)+"/"+label_library;
      }
...
namespace aflowlib {
  string CALCULATED(string options) {
    ...
    aurostd::string2tokens(options,tokens,",");
    if(tokens.size()>1) {
      init::ErrorOption(cout,options,"aflowlib::CALCULATED","aflow --calculated[[=]all|icsd|lib1|lib2|lib3|lib4|lib5|lib6|lib7|lib8|lib9|auro]");
      exit(0);
    }
...
namespace aflowlib {
  xstructure _____PrototypeLibraries(ostream &oss,_PROTO_PARAMS *PARAMS) { // COMPLETE ONE
	...
	*voss << "ERROR - aflowlib::PrototypeLibraries:  Structure not present in the HTQC database " << _HTQC_STRING_ << "  (" << PARAMS->label << ")" << endl;
