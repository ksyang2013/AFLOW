...
#include "aflow_contrib_junkai_basic.h"
...
uint ApennsyARGs(vector<string> &argv,vector<string> &cmds,aurostd::xoption &vflag) {
  ...
  vflag.flag("APENNSY::LATEX_SNAPSHOT",aurostd::args2flag(argv,cmds,"--snapshot"));

  vflag.flag("APENNSY::LATEX_OUTPUT",!aurostd::args2flag(argv,cmds,"--NOLATEX|--nolatex"));
  vflag.flag("APENNSY::LATEX_CITE",aurostd::args2flag(argv,cmds,"--cite"));
  ...
  if(!vflag.flag("APENNSY::ENTHALPY_TOT") && 
    ...
    vflag.addscheme("APENNSY::ENTHALPY_TOT");
  ...
  if(vflag.flag("APENNSY::UPDATE")) {
    vflag.addscheme("APENNSY::ENTHALPY_LIST");
    vflag.addscheme("APENNSY::CONVEX_HULL");
    vflag.addscheme("APENNSY::GNUPLOT");
  }
  ...
  if(vflag.flag("APENNSY::LATEX_SNAPSHOT")) {
    vflag.addscheme("APENNSY::LATEX_OUTPUT");
    vflag.addscheme("APENNSY::LATEX_CITE");
    XHOST.vflag_control.flag("PRINT_MODE::HYPERLINKS");
    vflag.addscheme("APENNSY::ENTHALPY_LIST");
    vflag.addscheme("APENNSY::CONVEX_HULL");
    vflag.addscheme("APENNSY::GNUPLOT");
  } 

  if(LDEBUG) cout << "ApennsyARGs: scheme=" << vflag.scheme << endl;
  if(LDEBUG) cout << "ApennsyARGs: vscheme.size()=" << vflag.vscheme.size() << endl;
  if(LDEBUG) cout << "ApennsyARGs: argv.size()=" << argv.size() << endl;
  
  return vflag.vscheme.size();
}
...
int Apennsymain(vector<string> &argv,vector<string> &cmds) {
  ...
  if(aflags.vflag.vscheme.size()==0 || argv.size()==1) {cout << init::InitGlobalObject("README_AFLOW_APENNSY_TXT");cout << aflow::Banner("BANNER_BIG");exit(1);}
  ...
  // PRIORITIES
  if(aflags.vflag.flag("APENNSY::WEB")) XHOST.vflag_control.addscheme("PRINT_MODE::PNG");
  ...
  // ---------------------------------------------------------------
  // Priorities
  // aflags.vflag.addscheme("APENNSY::ENTHALPY_LIST");
  ...
  if(aflags.vflag.flag("APENNSY::CONVEX_HULL") && !aflags.vflag.flag("APENNSY::MATLAB") && !aflags.vflag.flag("APENNSY::GNUPLOT"))
    // aflags.vflag.addscheme("APENNSY::MATLAB");
    aflags.vflag.addscheme("APENNSY::GNUPLOT");  // RHT
  // ---------------------------------------------------------------
  // Introductions
  // cerr << aflow::Banner("BANNER_BIG");
  // ---------------------------------------------------------------
  ...
  if(aflags.vflag.flag("APENNSY::LATEX_SNAPSHOT"))          cerr << "APENNSY::LATEX_SNAPSHOT" << endl;
  if(aflags.vflag.flag("APENNSY::LATEX_OUTPUT"))            cerr << "APENNSY::LATEX_OUTPUT" << endl;
  if(aflags.vflag.flag("APENNSY::LATEX_CITE"))              cerr << "APENNSY::LATEX_CITE" << endl;
  ...
  if(aflags.vflag.flag("APENNSY::RULES")) aflags.vflag.addscheme("APENNSY::LOAD_LIB2"); // save some typing
  // if(aflags.vflag.flag("APENNSY::ORDER")) aflags.vflag.addscheme("APENNSY::LOAD_LIB2"); // sove some typing
  if(aflags.vflag.flag("APENNSY::ORDER")) aflags.vflag.addscheme("APENNSY::LOAD_LIB2U"); // sove some typing
  // *****************************************************************************
  // cerr << argv.size() << endl;
  ...
  if(!aflags.vflag.flag("APENNSY::LOAD_LIB2X") && !aflags.vflag.flag("APENNSY::APOOL_PUBLIC") && !aflags.vflag.flag("APENNSY::APOOL_PRIVATE") && !aflags.vflag.flag("APENNSY::APOOL_TEST")) {
    ...
    if(!XHOST.vflag_control.flag("OSS::COUT") && aflags.vflag.flag("APENNSY::ENTHALPY_LIST")) {
      ...
      if(aflags.vflag.flag("APENNSY::LATEX_CITE")) aus << XHOST.command("latex") << " " << namefile << ".tex | grep -v Underfull" << endl;
      if(aflags.vflag.flag("APENNSY::LATEX_CITE")) aus << XHOST.command("latex") << " " << namefile << ".tex | grep -v Underfull" << endl;
      ...
      aus << "rm -f ";
      if(!(XHOST.vflag_control.flag("flag_KEEP::TEX") || aflags.vflag.flag("APENNSY::LATEX_SNAPSHOT"))) aus << namefile << ".tex ";
  ...
  if(aflags.vflag.flag("APENNSY::LOAD_LIB2X") && !aflags.vflag.flag("APENNSY::APOOL_PUBLIC") && !aflags.vflag.flag("APENNSY::APOOL_PRIVATE") && !aflags.vflag.flag("APENNSY::APOOL_TEST")) {
    ...
    if(!aflags.vflag.flag("APENNSY::MATLAB") && !aflags.vflag.flag("APENNSY::GNUPLOT")) aflags.vflag.addscheme("APENNSY::GNUPLOT"); //default output mode
    // if(aflags.vflag.flag("APENNSY::LOAD_ALLOY") && aflags.vflag.flag("APENNSY::CONVEX_HULL") && aflags.vflag.flag("APENNSY::GNUPLOT")) { params.APENNSY_ConvexHull(TRUE,aflags,_GNUPLOT_OUTPUT_MODE_); }
...
// CLASSES PRINT STUFF
// **********************************************************************************************
string Verbatim(_aflags aflags,bool VERBATIM) {
  if(VERBATIM==TRUE) {
    if(aflags.vflag.flag("APENNSY::LATEX_OUTPUT")) return "\\verb|";
    if(XHOST.vflag_control.flag("PRINT_MODE::HTML")) return "<tt>";
  }
  if(VERBATIM==FALSE) {
    if(aflags.vflag.flag("APENNSY::LATEX_OUTPUT")) return "|";
    if(XHOST.vflag_control.flag("PRINT_MODE::HTML")) return "</tt>";
  }
  return "";
}

string StrWithLink(_aflags aflags,string alloy, string structure_name) {
  stringstream oss;
  string _structure_name=structure_name;
  aurostd::StringSubst(_structure_name,"_","\\_");

  if(aflags.vflag.flag("APENNSY::LATEX_OUTPUT") && !XHOST.vflag_control.flag("PRINT_MODE::HYPERLINKS")) 
    oss << "" << aurostd::PaddedPRE(CleanNameStructure(structure_name),APENNSY_STR_DEPTH); 		
  if(aflags.vflag.flag("APENNSY::LATEX_OUTPUT") && XHOST.vflag_control.flag("PRINT_MODE::HYPERLINKS")) 
    oss << Verbatim(aflags,FALSE) << "{\\href{http://" << AFLOW_MATERIALS_SERVER_DEFAULT << "/AFLOWDATA/LIB2_RAW/" << alloy << "/" << structure_name << "/" << _XENTRY_ << "}{" 
	<< CleanNameStructure(_structure_name) << "}}" << Verbatim(aflags,TRUE) << aurostd::PaddedPOST("",APENNSY_STR_DEPTH-1-CleanNameStructure(structure_name).length());
  //	<< aurostd::PaddedPRE(CleanNameStructure(structure_name),APENNSY_STR_DEPTH-1) << " }}" << Verbatim(aflags,TRUE);
...
string APENNSY_Parameters::APENNSY_EnergyList(bool _verbose,_aflags &aflags) {
  ...
  string BANNER=Verbatim(aflags,TRUE)+"************************************************************************************************************** "+Verbatim(aflags,FALSE);
  if(XHOST.vflag_control.flag("PRINT_MODE::HTML")) BANNER+="<br>";
  if(aflags.vflag.flag("APENNSY::LATEX_OUTPUT")) BANNER+="\\\\";
  ...
  if(aflags.vflag.flag("APENNSY::LATEX_SNAPSHOT")) {
  ...
  if(_verbose) cerr << "**********************************************************" << endl;
  if(aflags.vflag.flag("APENNSY::LATEX_OUTPUT")) {
    if(!aflags.vflag.flag("APENNSY::LATEX_CITE")) oss << "\\documentclass" << APENNSY_LATEX_DOC_TYPE_TWOCOLUMS << " %LATEX" << endl;
    if(aflags.vflag.flag("APENNSY::LATEX_CITE"))  oss << "\\documentclass[aps,amssymb,twocolumn]{article} %LATEX" << endl;
    oss << "\\usepackage{epsfig} %LATEX" << endl;
    oss << "\\voffset -40mm %LATEX" << endl;
    if(!aflags.vflag.flag("APENNSY::LATEX_CITE")) oss << "\\hoffset -40mm %LATEX" << endl;
    if(aflags.vflag.flag("APENNSY::LATEX_CITE")) oss << "\\hoffset -15mm %LATEX" << endl;
    if(!aflags.vflag.flag("APENNSY::LATEX_CITE")) oss << LATEX_TEXT_HEIGHT << endl; // oss << "\\textheight 288mm %LATEX" << endl;
    if(aflags.vflag.flag("APENNSY::LATEX_CITE")) oss << "\\textheight 266mm %LATEX" << endl;
    oss << "\\textwidth 190mm %LATEX" << endl;
    oss << "\\usepackage{fancyvrb}     % HYPERLINKS" << endl;
    
    if(aflags.vflag.flag("APENNSY::LATEX_CITE")) {
    ...
    if(aflags.vflag.flag("APENNSY::LATEX_SNAPSHOT")) {
      ...
      oss << "\\title{\\ \\\\ \\ \\\\ AFLOWLIB.ORG: Snapshot of 3d-4d-5d binary intermetallics\\\\ (" << TODAY ")} %LATEX %SNAPSHOT" << endl;
 ...
 for(uint k=0;k<alloys.size();k++)  // if(Alloy2MiscibilityHT.at(k)!=MISCIBILITY_SYSTEM_MISCIBLE)
  {    
    if(aflags.vflag.flag("APENNSY::LATEX_OUTPUT")) {
	if(!aflags.vflag.flag("APENNSY::LATEX_CITE")) oss << "{\\LARGE " << Verbatim(aflags,TRUE) << speciesAB.at(k) << Verbatim(aflags,FALSE) << "}\\ \\\\" << endl;
	//	if(aflags.vflag.flag("APENNSY::LATEX_CITE")) oss << "\\section{" << speciesAB.at(k) << "\\ \\cite{eqweqweq}}" << endl << "\\vskip -3mm" << endl;
	if(aflags.vflag.flag("APENNSY::LATEX_CITE")) {
	...
	//	if(aflags.vflag.flag("APENNSY::LATEX_CITE")) oss << "\\cite{eqweqweq}" << endl;
	...
	oss << BANNER; // " *********************************************************************************************
	oss << BANNER; // " *********************************************************************************************
	oss << Verbatim(aflags,TRUE) 
	    << alloys.at(k)
	    << Verbatim(aflags,FALSE) << "<br>";
	...
	if(ZConcentrations.at(k).at(j)>-CEPSILON && ZConcentrations.at(k).at(j)<1+0+CEPSILON) {  // [0,1]
	  ...
	  oss << Verbatim(aflags,FALSE);
	  if(XHOST.vflag_control.flag("PRINT_MODE::HTML")) oss << "<br>";
	  if(aflags.vflag.flag("APENNSY::LATEX_OUTPUT")) oss << "\\\\";
	  uint i1=0;
 	  for(uint i=0;i<RankLib.at(k).at(j).size();i++) {
		...
		if(i<=_APENNSY_MAX_STRUCTURES_NUMBER_) {
		  ...
		  oss << "str=" << StrWithLink(aflags,alloys.at(k),ZLibrary.at(k).at(ii).structure_name);
		  ...
		  if(aflags.vflag.flag("APENNSY::LATEX_OUTPUT")) oss << Verbatim(aflags,FALSE) << " $|$" << Verbatim(aflags,TRUE);
		  ...
		  oss << Verbatim(aflags,FALSE) << endl;
		  // if(XHOST.vflag_control.flag("PRINT_MODE::HTML")) oss << "<br>";
		  if(aflags.vflag.flag("APENNSY::LATEX_OUTPUT")) oss << "\\\\";
		}	
		if(0) { // OSZICAR1 > OSZICAR2
		  if((ZLibrary.at(k).at(ii).energy_atom_relax1-ZLibrary.at(k).at(ii).energy_atom)<-0.0) { // OLD STYLE
			...
			<< ZLibrary.at(k).at(ii).energy_atom_relax1-ZLibrary.at(k).at(ii).energy_atom << Verbatim(aflags,FALSE) << endl;
      ...
      oss << BANNER; // " *********************************************************************************************
      oss << BANNER; // " *********************************************************************************************
      if(aflags.vflag.flag("APENNSY::LATEX_OUTPUT")) oss << Verbatim(aflags,TRUE);
      //      if(XHOST.vflag_control.flag("PRINT_MODE::HTML")) oss << "<br>" << endl;
      oss << "RELAXATION STRUCTURE TEST STEFANO                                         " << alloys.at(k) << "  ";
      if(aflags.vflag.flag("APENNSY::LATEX_OUTPUT")) oss << Verbatim(aflags,FALSE);
      if(aflags.vflag.flag("APENNSY::LATEX_OUTPUT")) oss << "\\\\";
      oss << endl;
      if(aflags.vflag.flag("APENNSY::LATEX_OUTPUT")) oss << Verbatim(aflags,TRUE);
	...
	for(uint i2=i1+1;i2<this->ZLibrary.at(k).size();i2++) {
	  if(AlloyStructureIdentity.at(k).at(i1).at(i2)==TRUE) {
	    ...
	    {
	      oss << "  " << StrWithLink(aflags,alloys.at(k),ZLibrary.at(k).at(i1).structure_name) // aurostd::PaddedPRE(CleanNameStructure(ZLibrary.at(k).at(i1).structure_name),APENNSY_STR_DEPTH)
		  << "  " << StrWithLink(aflags,alloys.at(k),ZLibrary.at(k).at(i2).structure_name) // aurostd::PaddedPRE(CleanNameStructure(ZLibrary.at(k).at(i2).structure_name),APENNSY_STR_DEPTH)
	...
	if(ZConcentrations.at(k).at(j)>CEPSILON && ZConcentrations.at(k).at(j)<1.0-CEPSILON) {  // ]0,1[
	  oss << Verbatim(aflags,TRUE);
	  if(XHOST.vflag_control.flag("PRINT_MODE::HTML")) oss << "<font color=#228b22>";
	  oss << "2-CONCENTRATION=" << ZConcentrations.at(k).at(j) << Verbatim(aflags,FALSE) << endl;
	  if(XHOST.vflag_control.flag("PRINT_MODE::HTML")) oss << "</font>";
	  // if(XHOST.vflag_control.flag("PRINT_MODE::HTML")) oss << "<br>";
	  if(aflags.vflag.flag("APENNSY::LATEX_OUTPUT")) oss << "\\\\";
	  oss << Verbatim(aflags,TRUE);
  ...
  if(aflags.vflag.flag("APENNSY::LATEX_OUTPUT") && aflags.vflag.flag("APENNSY::LATEX_CITE")) {
    vector<_outreach> voutreach;
    voutreach_load(voutreach,"PUBLICATIONS");
    //    oss << "\\newpage  %LATEX" << endl;
    oss << "\\twocolumn" << endl;
    ...
    oss << " {\\it AFLOWLIB.ORG: a distributed materials properties repository from high-throughput {\\it ab initio} calculations}, % ART75 - aflow 30767 %LATEX" << endl;
