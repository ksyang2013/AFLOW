...
void CalculateNewStateAFLOW(string & SL_name) {
  ...
  if( pID < 0 ) {
    ...
    int fout = open("out.dat", O_WRONLY | O_CREAT);
    int ferr = open("err.dat", O_WRONLY | O_CREAT);
    ...
    aurostd::execute("cd "+SL_name+" &&  "+DEFAULT_CHMOD_BIN+" "+ _MODE + " *");
...
double GetResultFromAFLOW() {
  ...
  aurostd::execute(XHOST.command("bunzip2")+" " + AFLOW_RESULT_FILE + ".bz2");
  ...
  aurostd::execute(XHOST.command("bzip2")+" " + AFLOW_RESULT_FILE);
...
void MoveFiles(int count) {
  ...
  aurostd::execute(XHOST.command("gzip")+" -r " + postfix);
}
