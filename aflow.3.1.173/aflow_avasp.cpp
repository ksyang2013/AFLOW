...
#define AVASP_DEFAULT_POTENTIAL  "potpaw_PBE"
...
#define SPECIE_RAW_LIB2U "Ag,Au,Cd,Co,Cr_pv,Cu_pv,Fe_pv,Hf_pv,Hg,Ir,La,Mn_pv,Mo_pv,Nb_sv,Ni_pv,Os_pv,Pd_pv,Pt,Re_pv,Rh_pv,Ru_pv,Sc_sv,Ta_pv,Tc_pv,Ti_sv,V_sv,W_pv,Y_sv,Zn,Zr_sv"

#define SPECIE_RAW_LIB2 "Ag,Al,As,Au,B_h,Ba_sv,Be_sv,Bi_d,Br,Ca_sv,Cd,Cl,Co,Cr_pv,Cu_pv,Fe_pv,Ga_h,Ge_h,Hf_pv,Hg,In_d,Ir,K_sv,La,Li_sv,Mg_pv,Mn_pv,Mo_pv,Na_pv,Nb_sv,Ni_pv,Os_pv,P,Pb_d,Pd_pv,Pt,Re_pv,Rh_pv,Ru_pv,Sb,Sc_sv,Se,Si,Sn,Sr_sv,Ta_pv,Tc_pv,Te,Ti_sv,Tl_d,V_sv,W_pv,Y_sv,Zn,Zr_sv"

//#define SPECIE_RAW_LIB3 "Ag,Au,Cd,Co,Cr_pv,Cu_pv,Fe_pv,Hf_pv,Hg,Ir,La,Mn_pv,Mo_pv,Nb_sv,Ni_pv,Os_pv,Pd_pv,Pt,Re_pv,Rh_pv,Ru_pv,Sc_sv,Ta_pv,Tc_pv,Ti_sv,V_sv,W_pv,Y_sv,Zn,Zr_sv"

#define SPECIE_RAW_LIB3 "Ag,Al,As,Au,B_h,Ba_sv,Be_sv,Bi_d,Br,Ca_sv,Cd,Cl,Co,Cr_pv,Cu_pv,Fe_pv,Ga_h,Ge_h,Hf_pv,Hg,In_d,Ir,K_sv,La,Li_sv,Mg_pv,Mn_pv,Mo_pv,Na_sv,Nb_sv,Ni_pv,Os_pv,P,Pb_d,Pd_pv,Pt,Re_pv,Rh_pv,Ru_pv,Sb,Sc_sv,Se,Si,Sn,Sr_sv,Ta_pv,Tc_pv,Te,Ti_sv,Tl_d,V_sv,W_pv,Y_sv,Zn,Zr_sv"
//#define SPECIE_RAW_LIB3 "Ag,Al,As,Au,B_h,Bi_d,Cd,Co,Cr_pv,Cu_pv,Fe_pv,Ga_h,Ge_h,Hf_pv,Hg,In_d,Ir,La,Mg_pv,Mn_pv,Mo_pv,Nb_sv,Ni_pv,Os_pv,P,Pb_d,Pd_pv,Pt,Re_pv,Rh_pv,Ru_pv,Sb,Sc_sv,Se,Si,Sn,Ta_pv,Te,Tc_pv,Ti_sv,V_sv,W_pv,Y_sv,Zn,Zr_sv"
...
// ***************************************************************************
string AVASP_Get_PseudoPotential_PAWPBE(string species) {
  string error;
  ...
  if(ALLOW_ACTINIDIES==FALSE) {
    if(species=="Ra" || species=="Ac" || species=="Th" || species=="Pa" || species=="U" ||
      ...
      error="WARNING AVASP_Get_PseudoPotential_PAWPBE: not producing Actinides";
      aurostd::execute("echo "+error+" >> "+_AFLOW_ERVASP_FILE_);
      cerr << error << endl;
      exit(0);
    }
    if(species=="D" || species=="T") {
      error="WARNING AVASP_Get_PseudoPotential_PAWPBE: not producing heavy hydrogen";
      aurostd::execute("echo "+error+" >> "+_AFLOW_ERVASP_FILE_);
      cerr << error << endl;
      exit(0);
    }
  ...
  if(species=="Ac") return "Ac"; // UNTESTED STEFANO of Ac,Ac_s take the hardest one [Wed Nov 23 EST 2011]
  if(species=="Ag") return "Ag"; // unique choice...
  if(species=="Al") return "Al"; // STEFANO Al_h has ghost states
  if(species=="Ar") return "Ar"; // unique choice...
  if(species=="As") return "As"; // unique choice...
  if(species=="Au") return "Au"; // unique choice...
  if(species=="B")  return "B_h"; // STEFANO
  //if(species=="B")  return "B";//vasp recommends
  if(species=="Ba") return "Ba_sv"; // unique choice...
  if(species=="Be") return "Be_sv"; // STEFANO
  if(species=="Bi") return "Bi_d"; //STEFANO,WAHYU
  if(species=="Br") return "Br";// unique choice...
  if(species=="C")  return "C"; // STEFANO, vasp recommends
  if(species=="Br") return "Br"; //WAHYU
  if(species=="Ca") return "Ca_sv"; // STEFANO
  if(species=="Cd") return "Cd"; // unique choice..
  if(species=="Ce") return "Ce"; //WAHYU
  if(species=="Cl") return "Cl"; //WAHYU
  if(species=="Co") return "Co"; // unique choice..
  if(species=="Cr") return "Cr_pv"; // STEFANO
  if(species=="Cs") return "Cs_sv"; // unique choice..
  if(species=="Cu") return "Cu_pv"; // STEFANO
  if(species=="Dy") return "Dy_3";// unique choice...
  if(species=="Er") return "Er_3"; // WAHYU, teragrid project
  //  if(species=="Eu") return "Eu_3"; // WAHYU, frozen f
  if(species=="Eu") return "Eu"; // WAHYU, with f-states, teragrid project
  if(species=="F")  return "F"; // WAHYU
  if(species=="Fe") return "Fe_pv"; // STEFANO
  if(species=="Ga") return "Ga_h"; // STEFANO
  if(species=="Ge") return "Ge_h"; // STEFANO
  // if(species=="Ge") return "Ge_d"; // WAHYU
  if(species=="Gd") return "Gd"; // WAHYU // LDAU
  if(species=="H")  return "H"; // WAHYU
  if(species=="He") return "He"; // unique choice...
  if(species=="Hf") return "Hf_pv"; // STEFANO
  if(species=="Hg") return "Hg"; // unique choice...
  if(species=="Ho") return "Ho_3";// unique choice...
  if(species=="I")  return "I"; // unique choice...
  if(species=="In") return "In_d"; // STEFANO
  if(species=="Ir") return "Ir"; // unique choice...
  if(species=="K")  return "K_sv"; // STEFANO
  if(species=="Kr") return "Kr"; // unique choice...
  if(species=="La") return "La";// STEFANO,WAHYU // LDAU
  if(species=="Li") return "Li_sv"; // STEFANO
  //  if(species=="Lu") return "Lu_3"; // WAHYU, frozen f
  if(species=="Lu") return "Lu";//WAHYU, with f-states,  LDAU
  if(species=="Mg") return "Mg_pv"; // STEFANO
  if(species=="Mn") return "Mn_pv"; // STEFANO
  if(species=="Mo") return "Mo_pv"; // STEFANO
  if(species=="Na") return "Na_sv"; // was pv "; // STEFANO,WAHYU
  if(species=="N")  return "N"; // vasp recommends
  if(species=="Nb") return "Nb_sv"; // STEFANO
  //  if(species=="Nd") return "Nd_3"; // WAHYU, frozen f
  if(species=="Nd") return "Nd"; // WAHYU, with f-states, teragrid project
  if(species=="Ne") return "Ne"; // unique choice...
  if(species=="Ni") return "Ni_pv"; // STEFANO
  if(species=="Np") return "Np_s"; // STEFANO NEVER USED JUST FOR COMPLETENESS
  if(species=="O")  return "O";//WAHYU
  if(species=="Os") return "Os_pv"; // STEFANO
  if(species=="P")  return "P"; // WAHYU, teragrid project
  if(species=="Pa") return "Pa"; // UNTESTED STEFANO of Pa,Pa_s take the hardest one [Wed Nov 23 EST 2011]
  if(species=="Pb") return "Pb_d";//STEFANO,WAHYU
  if(species=="Pd") return "Pd_pv"; // STEFANO
  //  if(species=="Pm") return "Pm_3"; // WAHYU, frozen f
  if(species=="Pm") return "Pm"; // WAHYU, with f-states, teragrid project
  if(species=="Pt") return "Pt"; // STEFANO
  //  if(species=="Pr") return "Pr_3"; // WAHYU, frozen f
  if(species=="Pr") return "Pr";  // WAHYU, with f-states, teragrid project
  if(species=="Pu") return "Pu_s"; // STEFANO NEVER USED JUST FOR COMPLETENESS
  if(species=="Rb") return "Rb_sv"; // vasp recommends
  if(species=="Re") return "Re_pv"; // STEFANO
  if(species=="Rh") return "Rh_pv"; // STEFANO
  if(species=="Ru") return "Ru_pv"; // STEFANO
  if(species=="S")  return "S"; // WAHYU, teragrid project
  if(species=="Sb") return "Sb";// unique choice...
  if(species=="Sc") return "Sc_sv"; // unique choice...
  if(species=="Se") return "Se";// unique choice...
  if(species=="Si") return "Si"; // was _h";//STEFANO,WAHYU
  //  if(species=="Sm") return "Sm_3"; // WAHYU, frozen f
  if(species=="Sm") return "Sm"; // WAHYU, with f-states, teragrid project
  if(species=="Sn") return "Sn";//STEFANO
  if(species=="Sr") return "Sr_sv";// unique choice...
  if(species=="Ta") return "Ta_pv"; // STEFANO
  if(species=="Tb") return "Tb_3"; //  unique choice...
  if(species=="Tc") return "Tc_pv"; // STEFANO
  if(species=="Te") return "Te"; // unique choice...
  if(species=="Th") return "Th_s"; // STEFANO NEVER USED JUST FOR COMPLETENESS
  if(species=="Ti") return "Ti_sv"; // STEFANO
  if(species=="Tl") return "Tl_d"; // STEFANO
  //  if(species=="Tm") return "Tm_3"; // WAHYU, frozen f
  if(species=="Tm") return "Tm";  // WAHYU, with f-states, teragrid project
  if(species=="U") return "U"; // UNTESTED STEFANO of U,U_s take the hardest one [Wed Nov 23 EST 2011]
  if(species=="V")  return "V_sv"; // STEFANO
  if(species=="W")  return "W_pv"; // STEFANO
  if(species=="Xe") return "Xe"; // unique choice...
  if(species=="Y")  return "Y_sv"; // unique choice...
  //  if(species=="Yb") return "Yb_2"; // WAHYU, frozen f
  if(species=="Yb") return "Yb";  // WAHYU, with f-states, teragrid project
  if(species=="Zn") return "Zn"; // unique choice...
  if(species=="Zr") return "Zr_sv"; // STEFANO

  if(species=="Po") {
    error="ERROR AVASP_Get_PseudoPotential_PAWPBE: No pseudopotential available: "+species;
    aurostd::execute("echo "+error+" >> "+_AFLOW_ERVASP_FILE_);
    cerr << error << endl;
    exit(0);
  }
  // If not found then UNKNOWN
  error="ERROR AVASP_Get_PseudoPotential_PAWPBE: Potential Not found: "+species;
  aurostd::execute("echo "+error+" >> "+_AFLOW_ERVASP_FILE_);
  cerr << error << endl;
...
// ***************************************************************************
string AVASP_Get_PseudoPotential_PAWGGA(string species) {
  ...
  return  AVASP_Get_PseudoPotential_PAWPBE(species);
}

// ***************************************************************************
string AVASP_Get_PseudoPotential_PAWLDA(string species) {
  // not defined... let`s hope the PAWGGA is good naming
  return  AVASP_Get_PseudoPotential_PAWGGA(species);
}
...
string AVASP_Get_PseudoPotential_GGA(string species) {
  ...
  if(species=="B")  return "B"; // vasp recommends
  if(species=="Ba") return "Ba_pv"; // unique choice...
  if(species=="Be") return "Be"; //  unique choice...
  if(species=="Bi") return "Bi"; //  unique choice...

  if(species=="C")  return "C"; // stefano
  if(species=="Ca") return "Ca_pv"; // stefano
  if(species=="Cl") return "Cl"; // unique choice..
  if(species=="Cr") return "Cr"; // unique choice..
  if(species=="Cs") return "Cs_pv"; // unique choice..
  if(species=="Cu") return "Cu"; // unique choice..

  if(species=="Fe") return "Fe"; // unique choice..
  if(species=="Ga") return "Ga_d"; // stefano
  if(species=="Ge") return "Ge"; // wahyu
  if(species=="H")  return "H_soft"; // wahyu
  if(species=="Hf") return "Hf"; // unique choice...
  if(species=="Hg") return "Hg"; // unique choice...

  if(species=="K")  return "K_pv"; // stefano
  if(species=="Kr") return "Kr"; // unique choice...

  if(species=="Li") return "Li_pv"; // stefano
  if(species=="Mg") return "Mg_pv"; // stefano
  if(species=="Mn") return "Mn"; // unique choice...

  if(species=="N")  return "N"; // vasp recommends
  if(species=="Na") return "Na_pv"; // there is no _sv in potGGA
  if(species=="Nb") return "Nb_pv"; // stefano
  if(species=="Ne") return "Ne"; // unique choice...
  if(species=="Ni") return "Ni"; // unique choice...
  if(species=="O")  return "O";//wahyu
  if(species=="Os") return "Os"; // unique choice...

  if(species=="Pd") return "Pd"; // stefano
  if(species=="Pt") return "Pt"; // stefano
  if(species=="Rb") return "Rb_pv"; // vasp recommends
  if(species=="Re") return "Re"; // stefano
  if(species=="Rh") return "Rh"; // stefano
  if(species=="Ru") return "Ru"; // stefano
  if(species=="Sc") return "Sc_pv"; // unique choice...
  if(species=="Sm") return "Sm_3";
  if(species=="Sr") return "Sr_pv";// unique choice...
  if(species=="Ta") return "Ta"; // stefano
  if(species=="Tc") return "Tc"; // stefano
  if(species=="Ti") return "Ti_pv"; // stefano
  if(species=="V")  return "V_pv"; // stefano
  if(species=="W")  return "W"; // stefano
  if(species=="Y")  return "Y_pv"; // unique choice...
  if(species=="Zr") return "Zr_pv"; // stefano
...
bool AVASP_MakeSingleAFLOWIN(_xvasp& xvasp_in,stringstream &_aflowin,bool flag_WRITE,int pthread) {
  //  if(flag_WRITE==FALSE) DEBUG=TRUE;
  bool LDEBUG=(FALSE || XHOST.DEBUG);

  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: " << "[0]" << endl;
  if(LDEBUG && xvasp_in.AVASP_prototype_mode==LIBRARY_MODE_HTQC)       cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp_in.AVASP_prototype_mode==LIBRARY_MODE_HTQC" << endl;
  if(LDEBUG && xvasp_in.AVASP_prototype_mode==LIBRARY_MODE_ICSD)       cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp_in.AVASP_prototype_mode==LIBRARY_MODE_ICSD" << endl;
  if(LDEBUG && xvasp_in.AVASP_prototype_mode==LIBRARY_MODE_HTQC_ICSD)  cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp_in.AVASP_prototype_mode==LIBRARY_MODE_HTQC_ICSD" << endl;
  if(LDEBUG && xvasp_in.AVASP_prototype_mode==LIBRARY_MODE_LIB3)       cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp_in.AVASP_prototype_mode==LIBRARY_MODE_LIB3" << endl;
  if(LDEBUG && xvasp_in.AVASP_prototype_mode==LIBRARY_MODE_PROTOTYPE)  cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp_in.AVASP_prototype_mode==LIBRARY_MODE_PROTOTYPE" << endl;
  if(LDEBUG && xvasp_in.AVASP_prototype_mode==LIBRARY_MODE_XSTRUCTURE) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp_in.AVASP_prototype_mode==LIBRARY_MODE_XSTRUCTURE" << endl;

  _xvasp xvasp=xvasp_in; // copy

  // xvasp.aopts.flag("AFLOWIN_FLAG::PSTRESS",TRUE);
  //  if(xvasp.aopts.flag("AFLOWIN_FLAG::PSTRESS",TRUE)) xvasp.aopts.addattachedscheme("AFLOWIN_FLAG::PSTRESS","7.65",TRUE);
  // xvasp.AVASP_value_PSTRESS=7.65;

  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp.aopts.flag(\"FLAG::AVASP_FORCE_NOLDAU\")=" << xvasp.aopts.flag("FLAG::AVASP_FORCE_NOLDAU") << endl;
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp.aopts.flag(\"FLAG::AVASP_FORCE_NOLDAU\")=" << xvasp.aopts.flag("FLAG::AVASP_FORCE_NOLDAU") << endl;
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp.aopts.flag(\"AFLOWIN_FLAG::PSTRESS\")=" << xvasp.aopts.flag("AFLOWIN_FLAG::PSTRESS") << endl;
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp.aopts.getattachedscheme(\"AFLOWIN_FLAG::PSTRESS\")=" << xvasp.aopts.getattachedscheme("AFLOWIN_FLAG::PSTRESS") << endl;
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp.aopts.flag(\"AFLOWIN_FLAG::MODULE\")=" << xvasp.aopts.flag("AFLOWIN_FLAG::MODULE") << endl;  //CO 180214
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp.aopts.getattachedscheme(\"AFLOWIN_FLAG::MODULE\")=" << xvasp.aopts.getattachedscheme("AFLOWIN_FLAG::MODULE") << endl;  //CO 180214
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp.aopts.flag(\"AFLOWIN_FLAG::APL_SUPERCELL\")=" << xvasp.aopts.flag("AFLOWIN_FLAG::APL_SUPERCELL") << endl;  //CO 180214
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp.aopts.getattachedscheme(\"AFLOWIN_FLAG::APL_SUPERCELL\")=" << xvasp.aopts.getattachedscheme("AFLOWIN_FLAG::APL_SUPERCELL") << endl;  //CO 180214
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp.aopts.flag(\"AFLOWIN_FLAG::EDIFFG\")=" << xvasp.aopts.flag("AFLOWIN_FLAG::EDIFFG") << endl;
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp.aopts.getattachedscheme(\"AFLOWIN_FLAG::EDIFFG\")=" << xvasp.aopts.getattachedscheme("AFLOWIN_FLAG::EDIFFG") << endl;
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp.aopts.flag(\"AFLOWIN_FLAG::POTIM\")=" << xvasp.aopts.flag("AFLOWIN_FLAG::POTIM") << endl;
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp.aopts.getattachedscheme(\"AFLOWIN_FLAG::POTIM\")=" << xvasp.aopts.getattachedscheme("AFLOWIN_FLAG::POTIM") << endl;
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp.aopts.flag(\"AFLOWIN_FLAG::PRECISION\")=" << xvasp.aopts.flag("AFLOWIN_FLAG::PRECISION") << endl;
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp.aopts.getattachedscheme(\"AFLOWIN_FLAG::PRECISION\")=" << xvasp.aopts.getattachedscheme("AFLOWIN_FLAG::PRECISION") << endl;
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp.aopts.flag(\"AFLOWIN_FLAG::ALGORITHM\")=" << xvasp.aopts.flag("AFLOWIN_FLAG::ALGORITHM") << endl;
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp.aopts.getattachedscheme(\"AFLOWIN_FLAG::ALGORITHM\")=" << xvasp.aopts.getattachedscheme("AFLOWIN_FLAG::ALGORITHM") << endl;
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp.aopts.flag(\"AFLOWIN_FLAG::METAGGA\")=" << xvasp.aopts.flag("AFLOWIN_FLAG::METAGGA") << endl;
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp.aopts.getattachedscheme(\"AFLOWIN_FLAG::METAGGA\")=" << xvasp.aopts.getattachedscheme("AFLOWIN_FLAG::METAGGA") << endl;
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp.aopts.flag(\"AFLOWIN_FLAG::IVDW\")=" << xvasp.aopts.flag("AFLOWIN_FLAG::IVDW") << endl;
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp.aopts.getattachedscheme(\"AFLOWIN_FLAG::IVDW\")=" << xvasp.aopts.getattachedscheme("AFLOWIN_FLAG::IVDW") << endl;
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp.aopts.flag(\"AFLOWIN_FLAG::RELAX_TYPE\")=" << xvasp.aopts.flag("AFLOWIN_FLAG::RELAX_TYPE") << endl;  //CO 180214
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp.aopts.getattachedscheme(\"AFLOWIN_FLAG::RELAX_TYPE\")=" << xvasp.aopts.getattachedscheme("AFLOWIN_FLAG::RELAX_TYPE") << endl;  //CO 180214
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp.aopts.flag(\"AFLOWIN_FLAG::RELAX_MODE\")=" << xvasp.aopts.flag("AFLOWIN_FLAG::RELAX_MODE") << endl;
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp.aopts.getattachedscheme(\"AFLOWIN_FLAG::RELAX_MODE\")=" << xvasp.aopts.getattachedscheme("AFLOWIN_FLAG::RELAX_MODE") << endl;
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp.aopts.flag(\"AFLOWIN_FLAG::TYPE\")=" << xvasp.aopts.flag("AFLOWIN_FLAG::TYPE") << endl;
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp.aopts.getattachedscheme(\"AFLOWIN_FLAG::TYPE\")=" << xvasp.aopts.getattachedscheme("AFLOWIN_FLAG::TYPE") << endl;
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp.aopts.flag(\"AFLOWIN_FLAG::CONVERT_UNIT_CELL\")=" << xvasp.aopts.flag("AFLOWIN_FLAG::CONVERT_UNIT_CELL") << endl;
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp.aopts.getattachedscheme(\"AFLOWIN_FLAG::CONVERT_UNIT_CELL\")=" << xvasp.aopts.getattachedscheme("AFLOWIN_FLAG::CONVERT_UNIT_CELL") << endl;
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp.aopts.flag(\"AFLOWIN_FLAG::VOLUME_PLUS_EQUAL\")=" << xvasp.aopts.flag("AFLOWIN_FLAG::VOLUME_PLUS_EQUAL") << endl;
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp.aopts.getattachedscheme(\"AFLOWIN_FLAG::VOLUME_PLUS_EQUAL\")=" << xvasp.aopts.getattachedscheme("AFLOWIN_FLAG::VOLUME_PLUS_EQUAL") << endl;
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp.aopts.flag(\"AFLOWIN_FLAG::VOLUME_MULTIPLY_EQUAL\")=" << xvasp.aopts.flag("AFLOWIN_FLAG::VOLUME_MULTIPLY_EQUAL") << endl;
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp.aopts.getattachedscheme(\"AFLOWIN_FLAG::VOLUME_MULTIPLY_EQUAL\")=" << xvasp.aopts.getattachedscheme("AFLOWIN_FLAG::VOLUME_MULTIPLY_EQUAL") << endl;
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp.aopts.flag(\"AFLOWIN_FLAG::NO_VOLUME_ADJUSTMENT\")=" << xvasp.aopts.flag("AFLOWIN_FLAG::NO_VOLUME_ADJUSTMENT") << endl;  //CO 180214
 
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp.aopts.flag(\"AFLOWIN_FLAG::KPPRA\")=" << xvasp.aopts.flag("AFLOWIN_FLAG::KPPRA") << endl;
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp.aopts.getattachedscheme(\"AFLOWIN_FLAG::KPPRA\")=" << xvasp.aopts.getattachedscheme("AFLOWIN_FLAG::KPPRA") << endl;
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp.aopts.flag(\"AFLOWIN_FLAG::KPPRA_STATIC\")=" << xvasp.aopts.flag("AFLOWIN_FLAG::KPPRA_STATIC") << endl;
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp.aopts.getattachedscheme(\"AFLOWIN_FLAG::KPPRA_STATIC\")=" << xvasp.aopts.getattachedscheme("AFLOWIN_FLAG::KPPRA_STATIC") << endl;
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp.aopts.flag(\"AFLOWIN_FLAG::BANDS_GRID\")=" << xvasp.aopts.flag("AFLOWIN_FLAG::BANDS_GRID") << endl;
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp.aopts.getattachedscheme(\"AFLOWIN_FLAG::BANDS_GRID\")=" << xvasp.aopts.getattachedscheme("AFLOWIN_FLAG::BANDS_GRID") << endl;

  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp.aopts.flag(\"AFLOWIN_FLAG::ENMAX_MULTYPLY\")=" << xvasp.aopts.flag("AFLOWIN_FLAG::ENMAX_MULTYPLY") << endl;
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp.aopts.getattachedscheme(\"AFLOWIN_FLAG::ENMAX_MULTYPLY\")=" << xvasp.aopts.getattachedscheme("AFLOWIN_FLAG::ENMAX_MULTYPLY") << endl;
  // if(LDEBUG) exit(0);
  ...
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: " << "[1]" << endl;
  //clean up name
  ...
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: " << "[2]" << endl;
  ...
  if(xvasp.str.num_each_type.size()!=xvasp.str.species.size()) {
    if(LDEBUG) cerr << "AVASP_MakeSingleAFLOWIN: WARNING 1 (fixing)" << endl;
  ...
  if(xvasp.str.species_pp.size()!=xvasp.str.species.size()) {
    if(LDEBUG) cerr <<"AVASP_MakeSingleAFLOWIN: WARNING 2 (fixing)" << endl;
  ...
  if(xvasp.str.species_volume.size()!=xvasp.str.species.size()) {
    if(LDEBUG) cerr <<"AVASP_MakeSingleAFLOWIN: WARNING 3 (fixing)" << endl;
  ...
  if(xvasp.str.species_mass.size()!=xvasp.str.species.size()) {
    if(LDEBUG) cerr <<"AVASP_MakeSingleAFLOWIN: WARNING 4 (fixing)" << endl;
  ...
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: " << "[3]" << endl;
  ...
  if(xvasp.aopts.flag("FLAG::AVASP_AUTO_PSEUDOPOTENTIALS")) {
	...
	cout << "AVASP_MakeSingleAFLOWIN: Pseudopotential exception: " << xvasp.str.species_pp.at(0) << xvasp.str.species_pp.at(1) << " -> potpaw_GGA " << endl;
	...
	for(uint isp=0;isp<xvasp.str.species.size();isp++)
	  xvasp.str.species_pp.at(isp)=AVASP_Get_PseudoPotential_PAWGGA(KBIN::VASP_PseudoPotential_CleanName(xvasp.str.species.at(isp)));
	if(xvasp.str.species.size()==2) {
	  // check for EXCEPTION GaMg
	  if(KBIN::VASP_PseudoPotential_CleanName(xvasp.str.species.at(0))=="Ga" && KBIN::VASP_PseudoPotential_CleanName(xvasp.str.species.at(1))=="Mg") {
	    cout << "AVASP_MakeSingleAFLOWIN: Pseudopotential exception: " << xvasp.str.species_pp.at(0) << xvasp.str.species_pp.at(1) << " -> ";
      ...
      if(aurostd::substring2bool(xvasp.AVASP_potential,"potpaw_PBE")) {
	for(uint isp=0;isp<xvasp.str.species.size();isp++)
	  xvasp.str.species_pp.at(isp)=AVASP_Get_PseudoPotential_PAWPBE(KBIN::VASP_PseudoPotential_CleanName(xvasp.str.species.at(isp)));
	if(xvasp.str.species.size()==2) {
	  ...
	  if(KBIN::VASP_PseudoPotential_CleanName(xvasp.str.species.at(0))=="B" && KBIN::VASP_PseudoPotential_CleanName(xvasp.str.species.at(1))=="Sm") {
	    ...
	    cout << "AVASP_MakeSingleAFLOWIN: Pseudopotential exception: \"potpaw_PBE\"=>\"potpaw_GGA\"" << endl;
	    xvasp.aopts.flag("FLAG::AVASP_AUTO_PSEUDOPOTENTIALS",FALSE);
	    cout << "AVASP_MakeSingleAFLOWIN: turning OFF pseudopotential" << endl;	    
	  }
	}
      }
      for(uint isp=0;isp<xvasp.str.species.size();isp++)  {
	//	if(aurostd::substring2bool(xvasp.AVASP_potential,"potpaw_GGA"))
	//	  xvasp.str.species_pp.at(isp)=AVASP_Get_PseudoPotential_PAWGGA(KBIN::VASP_PseudoPotential_CleanName(xvasp.str.species.at(isp)));
	if(aurostd::substring2bool(xvasp.AVASP_potential,"potpaw_LDA"))
	  xvasp.str.species_pp.at(isp)=AVASP_Get_PseudoPotential_PAWLDA(KBIN::VASP_PseudoPotential_CleanName(xvasp.str.species.at(isp)));
  ...
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: " << "[4]" << endl;
  
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: [5] xvasp.str.species.size()=" << xvasp.str.species.size() << endl;
  ...
  if((xvasp.AVASP_prototype_mode==LIBRARY_MODE_HTQC || xvasp.AVASP_prototype_mode==LIBRARY_MODE_HTQC_ICSD || xvasp.AVASP_prototype_mode==LIBRARY_MODE_LIB3)) {
    ...
    if(xvasp.AVASP_prototype_from_library_) {
      if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: [5a.1]" << endl;
	...
	if(xvasp.POTCAR_TYPE_DATE_PRINT_flag) { // add potential type and date
	  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: [5a.2]" << endl;
   	  string FilePotcar;
	  if(!KBIN::VASP_Find_POTCAR(xvasp.AVASP_potential+"/"+xvasp.str.species_pp.at(i),FilePotcar)) {
	    ...
	    if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN:  POTCAR [" << xvasp.AVASP_potential+"/"+xvasp.str.species_pp.at(i) << "] = " << FilePotcar << endl;
      ...
      if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: [5a.3]" << endl;
      directory+="/"+xvasp.AVASP_label;
      //  cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: xvasp.AVASP_prototype_from_library_ directory(1)=" << directory << endl;    
      ...
      if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: [5a.4]" << endl;
      if(XHOST.hostname==XHOST.AFLOW_MATERIALS_SERVER || XHOST.hostname==XHOST.AFLOW_WEB_SERVER) {
	if(xvasp.AVASP_libbase.size()==0) {
	  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: [5a.5]" << endl;
	  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: [5a.6] XHOST_LIBRARY_LIB2=" << XHOST_LIBRARY_LIB2 << endl;
	  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: [5a.7] vAFLOW_PROJECTS_DIRECTORIES.size()=" << vAFLOW_PROJECTS_DIRECTORIES.size() << endl;
	  ...
	  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: [5a.6] label_AFLOWLIB_LIB2=" << label_AFLOWLIB_LIB2 << endl;
	  ...
	  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: [5d.02] else CHECK THESE BY HAND" << endl;
	  ...
	  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: [5d.02] label_AFLOWLIB_LIB1=" << label_AFLOWLIB_LIB1 << endl;
	  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: [5d.02] label_AFLOWLIB_LIB2=" << label_AFLOWLIB_LIB2 << endl;
	  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: [5d.02] label_AFLOWLIB_LIB3=" << label_AFLOWLIB_LIB3 << endl;	
	  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: [5d.02] label_AFLOWLIB_LIB4=" << label_AFLOWLIB_LIB4 << endl;
	  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: [5d.02] label_AFLOWLIB_LIB5=" << label_AFLOWLIB_LIB5 << endl;
	  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: [5d.02] label_AFLOWLIB_LIB6=" << label_AFLOWLIB_LIB6 << endl;
	  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: [5d.02] label_AFLOWLIB_LIB7=" << label_AFLOWLIB_LIB7 << endl;
	  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: [5d.02] label_AFLOWLIB_LIB8=" << label_AFLOWLIB_LIB8 << endl;
	  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: [5d.02] label_AFLOWLIB_LIB9=" << label_AFLOWLIB_LIB9 << endl;
	}
	if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: [5d.1]" << endl;
	...
	if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: [5d.03] no check for MIX in non aflowlib servers " << endl;
      }	
      //    exit(0);
      ...
      if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: [6] xvasp.str.species.size()=" << xvasp.str.species.size() << endl;

      if(xvasp.str.species.size()==1) { // because specie 1 is part of the binary stuff
	if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: [6a] xvasp.str.species.size()=" << xvasp.str.species.size() << endl;	
	...
	// xvasp.str=PrototypeBinary(oaus,xvasp.AVASP_label,xvasp.str.species_pp.at(0),xvasp.str.species_volume.at(0),xvasp.str.species_pp.at(0),xvasp.str.species_volume.at(0),xvasp.AVASP_volume_in); // OLD WAY
 	xvasp.str.species_mass=vmass;
      ...
      if(xvasp.str.species.size()>=2) {
	if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: [7]  xvasp.str.species.size()>=2" << endl;
	for(uint i=0;i<xvasp.str.species_pp.size();i++) if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: [7.0a]: xvasp.str.species_pp.at(i) " << xvasp.str.species_pp.at(i) << endl;
	for(uint i=0;i<xvasp.str.species_pp_type.size();i++) if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: [7.0b]: xvasp.str.species_pp_type.at(i) " << xvasp.str.species_pp_type.at(i) << endl;
	for(uint i=0;i<xvasp.str.species_pp_version.size();i++) if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: [7.0c]: xvasp.str.species_pp_version.at(i) " << xvasp.str.species_pp_version.at(i) << endl;
	for(uint i=0;i<xvasp.str.species_pp_ZVAL.size();i++) if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: [7.0c]: xvasp.str.species_pp_ZVAL.at(i) " << xvasp.str.species_pp_ZVAL.at(i) << endl;
	for(uint i=0;i<xvasp.str.species_pp_vLDAU.size();i++) if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: [7.0c]: xvasp.str.species_pp_vLDAU.at(i) " << xvasp.str.species_pp_vLDAU.at(i).size() << endl;
	for(uint i=0;i<xvasp.str.species_volume.size();i++) if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: [7.0d]: xvasp.str.species_volume.at(i) " << xvasp.str.species_volume.at(i) << endl;
	for(uint i=0;i<xvasp.str.species_mass.size();i++) if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: [7.0e]: xvasp.str.species_mass.at(i) " << xvasp.str.species_mass.at(i) << endl;
	if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: [7.1a1]: xvasp.AVASP_volume_in=" << xvasp.AVASP_volume_in << endl;
	if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: [7.1a2]: xvasp.AVASP_label=" << xvasp.AVASP_label << endl;
	if(LDEBUG) {cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: [7.1a3]: xvasp.str.species_pp.size()=" << xvasp.str.species_pp.size() << ": "; for(uint i=0;i<xvasp.str.species_pp.size();i++) cerr << " " << xvasp.str.species_pp.at(i); cerr << endl;}
	if(LDEBUG) {cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: [7.1a3]: xvasp.str.species_volume.size()=" << xvasp.str.species_volume.size() << ": "; for(uint i=0;i<xvasp.str.species_volume.size();i++) cerr << " " << xvasp.str.species_volume.at(i); cerr << endl;}
	xstructure a;
	...
	if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: [7.1b]  xvasp.str.species.size()>=2" << endl;
       	if(LDEBUG) cerr << xvasp.str << endl;
	if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: [7.1c]  xvasp.str.species.size()>=2" << endl;
	// xvasp.str=PrototypeBinary(oaus,xvasp.AVASP_label,xvasp.str.species_pp.at(0),xvasp.str.species_volume.at(0),xvasp.str.species_pp.at(1),xvasp.str.species_volume.at(1),xvasp.AVASP_volume_in); // OLD WAY
      ...
      if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: [8] xvasp.str.species.size()=" << xvasp.str.species.size() << endl;
      ...
      // cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: directory(2)=" << directory << endl;
      system=aurostd::vectorstring2string(xvasp.str.species_pp)+"."+xvasp.AVASP_label;
      label_MIX=xvasp.AVASP_libbase+"/"+aurostd::vectorstring2string(xvasp.str.species_pp)+"/"+xvasp.AVASP_label+"/"+_AFLOWIN_;
    }
    if(xvasp.AVASP_directory_from_library_==FALSE) {
      directory=xvasp.AVASP_dirbase+"/"+xvasp.AVASP_label;
      //   cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: directory(3)=" << directory << endl;
      system=xvasp.AVASP_label;
    }
    if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: [9]" << endl;
  ...
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: " << "[10]" << endl;
  ...
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: " << "[11]" << endl;
  ...
  // check for PSTRESS/PRESSURE   //  if(xvasp.aopts.flag("AFLOWIN_FLAG::PSTRESS",TRUE)) xvasp.aopts.addattachedscheme("AFLOWIN_FLAG::PSTRESS","7.65",TRUE);
  if(xvasp.aopts.flag("AFLOWIN_FLAG::PSTRESS")==TRUE) {
    ...
    if(abs(PSTRESS)>0.00001) {
      if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: with pressure " << "[12p]" << endl;
      string pressure_add_on=":P="+aurostd::utype2string(PSTRESS,_AVASP_DOUBLE2STRING_PRECISION_)+"kB";
      if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: with pressure " << "[12p2] pressure_add_on="  << pressure_add_on << endl;
  ...
  if(0 && xvasp.aopts.flag("FLAG::AVASP_LDAU1") && !aurostd::substring2bool(system,"_ICSD")) {
    if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: with LDAU1 " << "[12p3]" << endl;
  ...
  if((xvasp.aopts.flag("FLAG::AVASP_LDAU1") || xvasp.aopts.flag("FLAG::AVASP_LDAU2") ) && !aurostd::substring2bool(system,"_ICSD")) {
    if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: with LDAU2 " << "[12p4]" << endl;
    string ldau2_add_on=":LDAU2";
    system+=ldau2_add_on;directory+=ldau2_add_on;xvasp.AVASP_label+=ldau2_add_on;
  }

  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: " << "[12.2]" << endl;
  ...
  aflowin << AFLOWIN_SEPARATION_LINE << endl; // [AFLOW] **************************************************
  aflowin << "[AFLOW_MODE_ZIP=bzip2] " << endl;
  ...
  if(!xvasp.aopts.flag("FLAGS::AVASP_QHA=OFF")) {
    ...
    aflowin << aurostd::PaddedPOST("[AFLOW_QHA]GP_VOL_DISTORTION_PERCENTAGE=0.03",_aflowinpad_) << "// README_AFLOW_APL.TXT" << endl;
    aflowin << aurostd::PaddedPOST("[AFLOW_QHA]DISPLACEMENTS=y",_aflowinpad_) << "// README_AFLOW_APL.TXT" << endl;
    aflowin << aurostd::PaddedPOST("[AFLOW_QHA]PROJECTION_DIR=1:1:1",_aflowinpad_) << "// README_AFLOW_APL.TXT" << endl;
    aflowin << aurostd::PaddedPOST("[AFLOW_QHA]EOS=n",_aflowinpad_) << "// README_AFLOW_APL.TXT" << endl;
    aflowin << aurostd::PaddedPOST("[AFLOW_QHA]EOS_VOLRANGE_DIST=-2:4:0.5",_aflowinpad_) << "// README_AFLOW_APL.TXT" << endl;
    aflowin << aurostd::PaddedPOST("[AFLOW_QHA]EOS_KPOINTS_MODE=32768:10000:20:100000",_aflowinpad_) << "// README_AFLOW_APL.TXT" << endl;
    aflowin << AFLOWIN_SEPARATION_LINE << endl; // [AFLOW] **************************************************
  }
  ...
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: " << "[12.3]" << endl;
  ...
  if(xvasp.aopts.flag("FLAG::AVASP_AUTO_PSEUDOPOTENTIALS")) {
    aflowin << aurostd::PaddedPOST("[VASP_FORCE_OPTION]AUTO_PSEUDOPOTENTIALS="+xvasp.AVASP_potential,_aflowinpad_) << "// pot_LDA | pot_GGA | potpaw_LDA | potpaw_GGA | potpaw_PBE " << endl;
  } else {
    //   cerr << "AVASP_MakeSingleAFLOWIN: AUTO_PSEUDOPOTENTIALS=FALSE" << endl;
    aflowin << aurostd::PaddedPOST("#[VASP_FORCE_OPTION]AUTO_PSEUDOPOTENTIALS="+xvasp.AVASP_potential,_aflowinpad_) << "// pot_LDA | pot_GGA | potpaw_LDA | potpaw_GGA | potpaw_PBE " << endl;
  }
  ...
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: " << "[12.4]" << endl;
  ...
  // TYPE WRITING
  string TYPE=xvasp.AVASP_flag_TYPE.scheme;
  ...
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: " << "[12.5]" << endl;
  ...
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: " << "[12.6]" << endl;
  ...
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: " << "[12.7]" << endl;
  if(AFLOWIN_NEW) {
    vector<string> vstr;string tstr;
    vstr.clear();tstr="";
  ...
  if(xvasp.aopts.flag("AFLOWIN_FLAG::VOLUME_PLUS_EQUAL") && !xvasp.aopts.flag("AFLOWIN_FLAG::NO_VOLUME_ADJUSTMENT")) {
    ...
    xvasp.aopts.purgeattachedscheme("AFLOWIN_FLAG::VOLUME_MULTIPLY_EQUAL");
  } else {
  ...
  if(xvasp.aopts.flag("AFLOWIN_FLAG::VOLUME_MULTIPLY_EQUAL") && !xvasp.aopts.flag("AFLOWIN_FLAG::NO_VOLUME_ADJUSTMENT")) {
    ...
    xvasp.aopts.purgeattachedscheme("AFLOWIN_FLAG::VOLUME_PLUS_EQUAL");
  } else {
  ...
  aflowin << aurostd::PaddedPOST("#EDIFFG="+aurostd::utype2string(DEFAULT_VASP_PREC_EDIFFG)+"    # For relaxed forces ",_aflowinpad_) << "# for hand modification" << endl;
  aflowin << aurostd::PaddedPOST("#POTIM="+aurostd::utype2string(DEFAULT_VASP_PREC_EDIFFG)+"       # default ",_aflowinpad_) << "# for hand modification" << endl;
  ...
  for(uint i=0;i<xvasp.str.species.size();i++) {
	...
	if(LDEBUG) cerr << "AVASP_MakeSingleAFLOWIN: AUTO_PSEUDOPOTENTIALS=FALSE - " << xvasp.AVASP_potential << "/" << xvasp.str.species_pp.at(i) << endl;
      }
    } else {
      if(xvasp.str.species.at(i)!="" && xvasp.str.species.at(i)!="X") // need because some times we have the "" around
  ...
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: " << "[14]" << endl;
  ...
  if(LDEBUG) cerr << "DEBUG - AVASP_MakeSingleAFLOWIN: " << "[16]" << endl;
...
bool AVASP_MakeSingleAFLOWIN(_xvasp& xvasp_in,bool flag_WRITE,int pthread) {
  stringstream _aflowin;
  return AVASP_MakeSingleAFLOWIN(xvasp_in,_aflowin,flag_WRITE,pthread);
}

bool AVASP_MakeSingleAFLOWIN(_xvasp& xvasp_in,int pthread) {
  stringstream _aflowin;
  return AVASP_MakeSingleAFLOWIN(xvasp_in,_aflowin,TRUE,pthread);
}
...
bool AVASP_MakePrototype_AFLOWIN(_AVASP_PROTO *PARAMS) {
  bool LDEBUG=(FALSE || XHOST.DEBUG);
  if(LDEBUG) cerr << " AVASP_MakePrototype_AFLOWIN: BEGIN" << endl; 
  ...
  if(PARAMS->vparams.flag("AFLOWIN_FLAG::MODULE")) {xvasp.aopts.flag("AFLOWIN_FLAG::MODULE",TRUE);xvasp.aopts.addattachedscheme("AFLOWIN_FLAG::MODULE",PARAMS->vparams.getattachedscheme("AFLOWIN_FLAG::MODULE"),TRUE);}  //CO 180214
  if(PARAMS->vparams.flag("AFLOWIN_FLAG::APL_SUPERCELL")) {xvasp.aopts.flag("AFLOWIN_FLAG::APL_SUPERCELL",TRUE);xvasp.aopts.addattachedscheme("AFLOWIN_FLAG::APL_SUPERCELL",PARAMS->vparams.getattachedscheme("AFLOWIN_FLAG::APL_SUPERCELL"),TRUE);}  //CO 180214
  if(PARAMS->vparams.flag("AFLOWIN_FLAG::POTIM")) {xvasp.aopts.flag("AFLOWIN_FLAG::POTIM",TRUE);xvasp.aopts.addattachedscheme("AFLOWIN_FLAG::POTIM",PARAMS->vparams.getattachedscheme("AFLOWIN_FLAG::POTIM"),TRUE);}
  if(PARAMS->vparams.flag("AFLOWIN_FLAG::PRECISION")) {xvasp.aopts.flag("AFLOWIN_FLAG::PRECISION",TRUE);xvasp.aopts.addattachedscheme("AFLOWIN_FLAG::PRECISION",PARAMS->vparams.getattachedscheme("AFLOWIN_FLAG::PRECISION"),TRUE);}
  if(PARAMS->vparams.flag("AFLOWIN_FLAG::ALGORITHM")) {xvasp.aopts.flag("AFLOWIN_FLAG::ALGORITHM",TRUE);xvasp.aopts.addattachedscheme("AFLOWIN_FLAG::ALGORITHM",PARAMS->vparams.getattachedscheme("AFLOWIN_FLAG::ALGORITHM"),TRUE);}
  if(PARAMS->vparams.flag("AFLOWIN_FLAG::METAGGA")) {xvasp.aopts.flag("AFLOWIN_FLAG::METAGGA",TRUE);xvasp.aopts.addattachedscheme("AFLOWIN_FLAG::METAGGA",PARAMS->vparams.getattachedscheme("AFLOWIN_FLAG::METAGGA"),TRUE);}  
  if(PARAMS->vparams.flag("AFLOWIN_FLAG::IVDW")) {xvasp.aopts.flag("AFLOWIN_FLAG::IVDW",TRUE);xvasp.aopts.addattachedscheme("AFLOWIN_FLAG::IVDW",PARAMS->vparams.getattachedscheme("AFLOWIN_FLAG::IVDW"),TRUE);}  
  if(PARAMS->vparams.flag("AFLOWIN_FLAG::RELAX_TYPE")) {xvasp.aopts.flag("AFLOWIN_FLAG::RELAX_TYPE",TRUE);xvasp.aopts.addattachedscheme("AFLOWIN_FLAG::RELAX_TYPE",PARAMS->vparams.getattachedscheme("AFLOWIN_FLAG::RELAX_TYPE"),TRUE);}  //CO 180214
  if(PARAMS->vparams.flag("AFLOWIN_FLAG::RELAX_MODE")) {xvasp.aopts.flag("AFLOWIN_FLAG::RELAX_MODE",TRUE);xvasp.aopts.addattachedscheme("AFLOWIN_FLAG::RELAX_MODE",PARAMS->vparams.getattachedscheme("AFLOWIN_FLAG::RELAX_MODE"),TRUE);}
  if(PARAMS->vparams.flag("AFLOWIN_FLAG::TYPE")) {xvasp.aopts.flag("AFLOWIN_FLAG::TYPE",TRUE);xvasp.aopts.addattachedscheme("AFLOWIN_FLAG::TYPE",PARAMS->vparams.getattachedscheme("AFLOWIN_FLAG::TYPE"),TRUE);}
  if(PARAMS->vparams.flag("AFLOWIN_FLAG::CONVERT_UNIT_CELL")) {xvasp.aopts.flag("AFLOWIN_FLAG::CONVERT_UNIT_CELL",TRUE);xvasp.aopts.addattachedscheme("AFLOWIN_FLAG::CONVERT_UNIT_CELL",PARAMS->vparams.getattachedscheme("AFLOWIN_FLAG::CONVERT_UNIT_CELL"),TRUE);}
  if(PARAMS->vparams.flag("AFLOWIN_FLAG::VOLUME_PLUS_EQUAL")) {xvasp.aopts.flag("AFLOWIN_FLAG::VOLUME_PLUS_EQUAL",TRUE);xvasp.aopts.addattachedscheme("AFLOWIN_FLAG::VOLUME_PLUS_EQUAL",PARAMS->vparams.getattachedscheme("AFLOWIN_FLAG::VOLUME_PLUS_EQUAL"),TRUE);}
  if(PARAMS->vparams.flag("AFLOWIN_FLAG::VOLUME_MULTIPLY_EQUAL")) {xvasp.aopts.flag("AFLOWIN_FLAG::VOLUME_MULTIPLY_EQUAL",TRUE);xvasp.aopts.addattachedscheme("AFLOWIN_FLAG::VOLUME_MULTIPLY_EQUAL",PARAMS->vparams.getattachedscheme("AFLOWIN_FLAG::VOLUME_MULTIPLY_EQUAL"),TRUE);}
  ...
  if(PARAMS->vparams.flag("AFLOWIN_FLAG::EDIFFG")) {xvasp.aopts.flag("AFLOWIN_FLAG::EDIFFG",TRUE);xvasp.aopts.addattachedscheme("AFLOWIN_FLAG::EDIFFG",PARAMS->vparams.getattachedscheme("AFLOWIN_FLAG::EDIFFG"),TRUE);}
  if(PARAMS->vparams.flag("AFLOWIN_FLAG::KPPRA")) {xvasp.aopts.flag("AFLOWIN_FLAG::KPPRA",TRUE);xvasp.aopts.addattachedscheme("AFLOWIN_FLAG::KPPRA",PARAMS->vparams.getattachedscheme("AFLOWIN_FLAG::KPPRA"),TRUE);}
  if(PARAMS->vparams.flag("AFLOWIN_FLAG::KPPRA_STATIC")) {xvasp.aopts.flag("AFLOWIN_FLAG::KPPRA_STATIC",TRUE);xvasp.aopts.addattachedscheme("AFLOWIN_FLAG::KPPRA_STATIC",PARAMS->vparams.getattachedscheme("AFLOWIN_FLAG::KPPRA_STATIC"),TRUE);}
  if(PARAMS->vparams.flag("AFLOWIN_FLAG::BANDS_GRID")) {xvasp.aopts.flag("AFLOWIN_FLAG::BANDS_GRID",TRUE);xvasp.aopts.addattachedscheme("AFLOWIN_FLAG::BANDS_GRID",PARAMS->vparams.getattachedscheme("AFLOWIN_FLAG::BANDS_GRID"),TRUE);}
  if(PARAMS->vparams.flag("AFLOWIN_FLAG::ENMAX_MULTIPLY")) {xvasp.aopts.flag("AFLOWIN_FLAG::ENMAX_MULTIPLY",TRUE);xvasp.aopts.addattachedscheme("AFLOWIN_FLAG::ENMAX_MULTIPLY",PARAMS->vparams.getattachedscheme("AFLOWIN_FLAG::ENMAX_MULTIPLY"),TRUE);}
  ...
  if(LDEBUG) cerr << "DEBUG - AVASP_MakePrototype_AFLOWIN: labels=" << PARAMS->ucell.at(0) << endl;
  if(LDEBUG) cerr << "DEBUG - AVASP_MakePrototype_AFLOWIN: nspecies=" << nspecies << endl;
  if(LDEBUG) cerr << "DEBUG - AVASP_MakePrototype_AFLOWIN: nspeciesHTQC=" << nspeciesHTQC << endl;
  if(LDEBUG) cerr << "DEBUG - AVASP_MakePrototype_AFLOWIN: alphabetic=" << alphabetic << endl;
  if(LDEBUG) cerr << "DEBUG - AVASP_MakePrototype_AFLOWIN: PARAMS->ucell.at(0)=" << PARAMS->ucell.at(0) << endl;
  if(LDEBUG) cerr << "DEBUG - AVASP_MakePrototype_AFLOWIN: PARAMS->ucell.size()=" << PARAMS->ucell.size() << endl;
  for(uint i=0;i<PARAMS->ucell.size();i++)
    if(LDEBUG) cerr << "DEBUG - AVASP_MakePrototype_AFLOWIN: PARAMS->ucell.at(" << i << ")=" << PARAMS->ucell.at(i) << endl;
  if(LDEBUG) cerr << "DEBUG - AVASP_MakePrototype_AFLOWIN: PARAMS->vparams.flag(\"AFLOWIN_FLAG::HTQC_ICSD\")=" << PARAMS->vparams.flag("AFLOWIN_FLAG::HTQC_ICSD") << endl;
  if(LDEBUG) cerr << "DEBUG - AVASP_MakePrototype_AFLOWIN: 1+nspecies=" << 1+nspecies << endl;
  if(LDEBUG) cerr << "DEBUG - AVASP_MakePrototype_AFLOWIN: 1+nspecies+1=" << 1+nspecies+1 << endl;
  if(LDEBUG) cerr << "DEBUG - AVASP_MakePrototype_AFLOWIN: 2+nspecies=" << 2+nspecies << endl;
  if(LDEBUG) cerr << "DEBUG - AVASP_MakePrototype_AFLOWIN: xvasp.AVASP_potential=" << xvasp.AVASP_potential << endl;
  if(LDEBUG) {cerr << "DEBUG - AVASP_MakePrototype_AFLOWIN: ";for(uint i=0;i<PARAMS->ucell.size();i++) cerr << PARAMS->ucell.at(i) << " ";cerr << endl;}
  ...
  if(PARAMS->ucell.size()!=1+nspecies && PARAMS->ucell.size()!=1+nspecies+1 && PARAMS->ucell.size()!=1+nspecies*2) {
    cerr << "AVASP_MakePrototype_AFLOWIN: need to specify more arguments... exiting" << endl;
  ...
  if(LDEBUG) cerr << "DEBUG - AVASP_MakePrototype_AFLOWIN: [x1] " << endl;
  ...
  if(LDEBUG) cerr << "DEBUG - AVASP_MakePrototype_AFLOWIN: [x2] " << endl;
  ...
  if(PARAMS->ucell.size()==1+nspecies)
    {
      if(LDEBUG) cerr << "DEBUG - AVASP_MakePrototype_AFLOWIN: [x2a] " << endl;
  ...
  if(LDEBUG) cerr << "DEBUG - AVASP_MakePrototype_AFLOWIN: [x3] " << endl;
  ...
  if(PARAMS->ucell.size()==1+nspecies) {  // USE AUTOMATIC VOLUMES
    if(LDEBUG) cerr << "DEBUG - AVASP_MakePrototype_AFLOWIN: [x3b] - USE AUTOMATIC VOLUMES" << endl;
    for(uint ispecies=0;ispecies<nspecies;ispecies++)
  ...
  if(PARAMS->ucell.size()==1+nspecies+1) {  // USE ONE VOLUME FITS ALL
    if(LDEBUG) cerr << "DEBUG - AVASP_MakePrototype_AFLOWIN: [x3d] - USE ONE VOLUME FITS ALL" << endl;
    for(uint ispecies=0;ispecies<nspecies;ispecies++) {
  ...
  if(PARAMS->ucell.size()==1+nspecies*2) {
    if(LDEBUG) cerr << "DEBUG - AVASP_MakePrototype_AFLOWIN: [x3e] - EXTRACT VOLUMES FROM STRING" << endl;
    for(uint ispecies=0;ispecies<nspecies;ispecies++) {
      vector<string> tokens;tokens.clear();
  ...
  aurostd::StringSubst(string_POTENTIAL,  // TEMPORARY PATCH Sun Apr 14 22:05:01 EDT 2013
		       ...
		       _AVASP_PSEUDOPOTENTIAL_POTPAW_PBE_+_AVASP_PSEUDOPOTENTIAL_DELIMITER_+_AVASP_PSEUDOPOTENTIAL_POTENTIAL_COMPLETE_); // TEMPORARY PATCH Sun Apr 14 22:05:01 EDT 2013
  xvasp.AVASP_potential=_AVASP_PSEUDOPOTENTIAL_POTPAW_PBE_;
  if(aurostd::substring2bool(string_POTENTIAL,_AVASP_PSEUDOPOTENTIAL_AUTO_)) xvasp.AVASP_potential=_AVASP_PSEUDOPOTENTIAL_POTPAW_PBE_;
  if((!aurostd::substring2bool(string_POTENTIAL,_AVASP_PSEUDOPOTENTIAL_POTPAW_PBE_) && !aurostd::substring2bool(string_POTENTIAL,_AVASP_PSEUDOPOTENTIAL_AUTO_)) ||
  ...
  // cerr << "xvasp.AVASP_potential=" << xvasp.AVASP_potential << endl;
  // for(uint i=0;i<PARAMS->ucell.size();i++) cerr << PARAMS->ucell.at(i) << endl;
  // exit(0);

  if(!aurostd::substring2bool(xvasp.AVASP_potential,_AVASP_PSEUDOPOTENTIAL_POT_LDA_) &&
     !aurostd::substring2bool(xvasp.AVASP_potential,_AVASP_PSEUDOPOTENTIAL_POT_GGA_) &&
     !aurostd::substring2bool(xvasp.AVASP_potential,_AVASP_PSEUDOPOTENTIAL_POT_PBE_) &&
     !aurostd::substring2bool(xvasp.AVASP_potential,_AVASP_PSEUDOPOTENTIAL_POTPAW_LDA_) &&
     !aurostd::substring2bool(xvasp.AVASP_potential,_AVASP_PSEUDOPOTENTIAL_POTPAW_GGA_) &&
     !aurostd::substring2bool(xvasp.AVASP_potential,_AVASP_PSEUDOPOTENTIAL_POTPAW_PBE_)) {
    // bad potential
  ...
  if(LDEBUG) cerr << "DEBUG - AVASP_MakePrototype_AFLOWIN: [x4] " << endl;
  ...
  // 1 SPECIES
  if(nspecies==1) {
    if(LDEBUG) cerr <<"DEBUG - AVASP_MakePrototype_AFLOWIN: [5] 1species" << endl;
    uint ii[nspecies];
	...
	for(uint ip=0;ip<PARAMS->vpressure.size();ip++) {
	  ...
	  xaus.aopts.addattachedscheme("AFLOWIN_FLAG::PSTRESS",aurostd::utype2string(PARAMS->vpressure.at(ip)),TRUE);
	  if(PARAMS->vpressure.size()==1 && abs(aurostd::string2utype<double>(xaus.aopts.getattachedscheme("AFLOWIN_FLAG::PSTRESS")))<0.00001) {
	    xaus.aopts.flag("AFLOWIN_FLAG::PSTRESS",FALSE);xaus.aopts.addattachedscheme("AFLOWIN_FLAG::PSTRESS","0.0",TRUE);};
  ...
  // 2 SPECIES
  if(nspecies==2) {
    if(LDEBUG) cerr <<"DEBUG - AVASP_MakePrototype_AFLOWIN: [5] 2species" << endl;
    uint ii[nspecies];
    for(ii[0]=0;ii[0]<specieX.at(0).size();ii[0]++) {
      if(LDEBUG) cerr <<"DEBUG - AVASP_MakePrototype_AFLOWIN: [5a] specieX.at(0).size()=" << specieX.at(0).size() << "  -  specieX.at(0).at(ii[0])=" << specieX.at(0).at(ii[0]) << endl;
      for(ii[1]=0;ii[1]<specieX.at(1).size();ii[1]++) {
	if(LDEBUG) cerr <<"DEBUG - AVASP_MakePrototype_AFLOWIN: [5b] specieX.at(1).size()=" << specieX.at(1).size() << "  -  specieX.at(1).at(ii[1])=" << specieX.at(1).at(ii[1]) << endl;
	if(specieX.at(0).at(ii[0])<specieX.at(1).at(ii[1])) {
	  ...
	  if(flag_LIB2U==FALSE && flag_LIB2==FALSE && flag_LIB3==FALSE) {
		...
		cerr << "AVASP_MakePrototype_AFLOWIN: system ";
  ...
  if(nspecies==3) {
    //   LDEBUG=TRUE;
    if(LDEBUG) cerr <<"DEBUG - AVASP_MakePrototype_AFLOWIN: [5] 3species" << endl;
    uint ii[nspecies];
    for(ii[0]=0;ii[0]<specieX.at(0).size();ii[0]++) {
      for(ii[1]=0;ii[1]<specieX.at(1).size();ii[1]++) {
	for(ii[2]=0;ii[2]<specieX.at(2).size();ii[2]++) {
	  if(LDEBUG) cerr <<"DEBUG - AVASP_MakePrototype_AFLOWIN: [5.0.3] 3species specieX.at(2).at(ii[2])=" << specieX.at(2).at(ii[2]) <<  endl;
	  if(LDEBUG) { 
	    cerr << "DEBUG - AVASP_MakePrototype_AFLOWIN: [5.0.3] 3species = " << specieX.at(0).at(ii[0]) << " " << specieX.at(1).at(ii[1]) << " " << specieX.at(2).at(ii[2]);
	  ...
	  if(specieX.at(0).at(ii[0])<specieX.at(1).at(ii[1]) && specieX.at(1).at(ii[1])<specieX.at(2).at(ii[2])) { // check if identical is done later
	    if(LDEBUG) cerr <<"DEBUG - AVASP_MakePrototype_AFLOWIN: [5.0.4] 3species " << specieX.at(0).at(ii[0]) << specieX.at(1).at(ii[1]) << specieX.at(2).at(ii[2]) <<  endl;
	    ...
	    for(uint k=0;k<label.size();k++) {
	      if(LDEBUG) cerr <<"DEBUG - AVASP_MakePrototype_AFLOWIN: [5.1] 3species label.at(" << k << ")=" << label.at(k) << endl;
	      for(uint ip=0;ip<PARAMS->vpressure.size();ip++) {
		if(LDEBUG) cerr <<"DEBUG - AVASP_MakePrototype_AFLOWIN: [5.1] 3species PARAMS->vpressure.at(" << ip << ")=" << PARAMS->vpressure.at(ip) << endl;
		_xvasp xaus(xvasp);
		xaus.AVASP_label=label.at(k);
		xaus.str.species.clear();xaus.str.species_pp.clear();xaus.str.species_pp_type.clear();xaus.str.species_pp_version.clear();xaus.str.species_pp_ZVAL.clear();xaus.str.species_pp_vLDAU.clear();xaus.str.species_volume.clear();xaus.str.species_mass.clear();
		for(uint i=0;i<nspecies;i++) {xaus.str.species.push_back(specieX.at(i).at(ii[i]));xaus.str.species_volume.push_back(volumeX.at(i).at(ii[i]));xaus.str.species_mass.push_back(massX.at(i).at(ii[i]));}
  ...
  // 4 SPECIES
  if(nspecies==4) {
    if(LDEBUG) cerr <<"DEBUG - AVASP_MakePrototype_AFLOWIN: [5] 4species" << endl;
    uint ii[nspecies];
    for(ii[0]=0;ii[0]<specieX.at(0).size();ii[0]++) {
      for(ii[1]=0;ii[1]<specieX.at(1).size();ii[1]++) {
	for(ii[2]=0;ii[2]<specieX.at(2).size();ii[2]++) {
	  for(ii[3]=0;ii[3]<specieX.at(3).size();ii[3]++) {
		...
		for(uint ip=0;ip<PARAMS->vpressure.size();ip++) {
		  ...
		  if(1) { // workaround elpasolites
		    ...
		    // xaus.AVASP_flag_TYPE.scheme.at(0)=='M'=FALSE;
		    // xaus.AVASP_flag_TYPE.scheme.at(0)=='I'=TRUE;
...
bool AVASP_DefaultValuesICSD_AFLOWIN(_xvasp &xvasp) {        
  ...
  // [OBSOLETE] xvasp.aopts.flag("FLAG::IVDW_SET",FALSE);
  // [OBSOLETE] xvasp.AVASP_flag_IVDW_scheme=DEFAULT_VASP_FORCE_OPTION_IVDW_SCHEME;
  //  xvasp.AVASP_flag_TYPE.scheme.at(0)=='I'=TRUE; wrong
...
bool AVASP_MakePrototypeICSD_AFLOWIN(_AVASP_PROTO *PARAMS,bool flag_AFLOW_IN_ONLY_IF_MISSING) {
  ...
  bool LDEBUG=(FALSE || XHOST.DEBUG);
  if(LDEBUG) cerr << "AVASP_MakePrototypeICSD_AFLOWIN: 0a" << endl;
  _xvasp xvasp;
  ...
  // recompile and ahave fun
  // cerr << "AVASP_MakePrototypeICSD_AFLOWIN: 0b" << endl;
  ...
  if(LDEBUG) cerr << "AVASP_MakePrototypeICSD_AFLOWIN: 0c" << endl;
  ...
  if(LDEBUG) cerr << "AVASP_MakePrototypeICSD_AFLOWIN: 0d" << endl;
  ...
  if(LDEBUG) cerr << "AVASP_MakePrototypeICSD_AFLOWIN: 0d-1" << endl;
  xvasp.str=aflowlib::PrototypeLibraries(cerr,label,parameters,LIBRARY_MODE_ICSD);
  if(LDEBUG) cerr << "AVASP_MakePrototypeICSD_AFLOWIN: 0d-2" << endl;
  if(LDEBUG) cerr << "AVASP_MakePrototypeICSD_AFLOWIN: DEBUG: xvasp.str.bravais_lattice_type=" << xvasp.str.bravais_lattice_type << endl;
  if(LDEBUG) cerr << "AVASP_MakePrototypeICSD_AFLOWIN: DEBUG: xvasp.str.bravais_lattice_variation_type=" << xvasp.str.bravais_lattice_variation_type << endl;
  if(LDEBUG) cerr << "AVASP_MakePrototypeICSD_AFLOWIN: 0d-3" << endl;

  if(LATTICE::lattice_is_working(xvasp.str.bravais_lattice_type)==FALSE) exit(0); // DO NOT PRODUCE THE ONES NOT READY TO BRILLOUIN
  if(LDEBUG) cerr << "AVASP_MakePrototypeICSD_AFLOWIN: 0d-4" << endl;
  if(xvasp.str.error_flag==TRUE) {         // flag TRUE is error
    cerr << "ERROR AVASP_MakePrototypeICSD_AFLOWIN: " << endl << xvasp.str.error_string << endl;
    exit(0);
  }
  if(LDEBUG) cerr << "AVASP_MakePrototypeICSD_AFLOWIN: 0d-5" << endl;

  // get the species
  bool potpaw_PBE=TRUE,potpaw_GGA=FALSE,pot_GGA=FALSE;
  if(LDEBUG) cerr << "AVASP_MakePrototypeICSD_AFLOWIN: DEBUG: label=" << label << endl;
  //  if(aurostd::substring2bool(label,"Cs")) {potpaw_PBE=FALSE;potpaw_GGA=FALSE;pot_GGA=TRUE;}
  if(aurostd::substring2bool(label,"Cs")) {potpaw_PBE=TRUE;potpaw_GGA=FALSE;pot_GGA=FALSE;}
  //  if(aurostd::substring2bool(label,"Sm")) {potpaw_PBE=FALSE;potpaw_GGA=TRUE;pot_GGA=FALSE;}
  ...
  if(LDEBUG) cerr << "AVASP_MakePrototypeICSD_AFLOWIN: DEBUG: potential=" << xvasp.AVASP_potential << endl;
  ...
  // POTENTIALS
  for(uint i=0;i<xvasp.str.species.size();i++) {
    if(potpaw_PBE==TRUE) xvasp.str.species.at(i)=AVASP_Get_PseudoPotential_PAWPBE(aurostd::CleanStringASCII(xvasp.str.species.at(i)));
    if(potpaw_GGA==TRUE) xvasp.str.species.at(i)=AVASP_Get_PseudoPotential_PAWGGA(aurostd::CleanStringASCII(xvasp.str.species.at(i)));
    if(pot_GGA==TRUE) xvasp.str.species.at(i)=AVASP_Get_PseudoPotential_GGA(aurostd::CleanStringASCII(xvasp.str.species.at(i)));
  }
  
  // Parameters
  if(LDEBUG) cerr << "AVASP_MakePrototypeICSD_AFLOWIN: 1a" << endl;
  ...
  // xvasp.AVASP_flag_TYPE.scheme.at(0)=='M'=FALSE;
  // xvasp.AVASP_flag_TYPE.scheme.at(0)=='I'=TRUE;
  ...
  if(LDEBUG) cerr << "AVASP_MakePrototypeICSD_AFLOWIN: 2a" << endl;
  if(LDEBUG) cerr << xvasp.str << endl;
  if(LDEBUG) cerr << "AVASP_MakePrototypeICSD_AFLOWIN: Running AVASP_MakeSingleAFLOWIN" << endl;
  AVASP_MakeSingleAFLOWIN(xvasp,!PARAMS->vparams.flag("AFLOWIN_FLAG::STDOUT"));
  if(LDEBUG) cerr << "AVASP_MakePrototypeICSD_AFLOWIN: 3a" << endl;
...
string AVASP_Shortcuts_for_Binaries(string &label) {
  ...
  // SEARCH FOR LABELS
  if(label=="LIBRARY_PROTOTYPES" || label=="LIBRARY_PROTOS" || label=="PROTOS" || label=="ALL"|| label=="all") label=LABEL_RAW_LIBRARY_PROTOTYPES;
