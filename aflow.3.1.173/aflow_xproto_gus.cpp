...
namespace aflowlib {
  string PrototypeBinaryGUS_Cache_LibraryS_Extract(ostream &FileMESSAGE,const string& auslat,const string& labelclean) {
	...
	structure_line=aurostd::execute2string(XHOST.command("zcat")+" "+FileLibrary+" | grep gus"+auslat+" | grep \""+auslat+labelclean+" \"");
      } else  {
	if(aurostd::substring2bool(FileLibrary,".bz2")) {
	  structure_line=aurostd::execute2string(XHOST.command("bzcat")+" "+FileLibrary+" | grep gus"+auslat+" | grep \""+auslat+labelclean+" \"");
	} else {
	  structure_line=aurostd::execute2string(XHOST.command("cat")+" "+FileLibrary+" | grep gus"+auslat+" | grep \""+auslat+labelclean+" \"");
	}
      }
    }
...
namespace aflowlib {
  xstructure PrototypeBinaryGUS(ostream &FileMESSAGE,string label,
    ...
    int z1, z2, z3, ic,iD;
    gusstd::matrix_inverse(HNF,Sinv); //**
    gusstd::matrix_inverse(sLV,sLVinv);
	...
	for( z2=(b*z1)/a; z2<=c+(b*z1)/a - 1; z2++) {
	  for( z3 = z1*(d-(e*b)/c)/a+(e*z2)/c; z3<= f+z1*(d-(e*b)/c)/a+(e*z2)/c - 1; z3++) {
	    ...
	    //** Move this to outside the loop: gusstd::matrix_inverse(HNF,Sinv);
    ...
    // FIX title
    str.scale=1.0;
    str.title=title+" - "+label+" - ("+title_database+","+bulksurf+") "+_GUS_STRING_;
