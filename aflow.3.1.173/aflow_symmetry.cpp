...
namespace SYM { 
  double defaultTolerance(xstructure& xstr){
    ...
    if(xstr.dist_nn_min == AUROSTD_NAN){xstr.MinDist();}
    min_dist = xstr.dist_nn_min; 
  ...
  bool change_tolerance(xstructure& xstr, double& tolerance, double& min_dist, bool& no_scan){
	...
	if(tolerance < _ZERO_TOL_){
	  ...
	  tolerance = std::pow(10.0,(std::log10(orig_tolerance)+(sign*range)));
        ...
        if(LDEBUG){
	  cerr << "SYM::change_tolerance: Inconsistent symmetry at this tolerance " << incomming_tolerance << ", changing tolerance to: " << tolerance << endl;
        } 
	  return TRUE; 
      }
      else{
	cerr << "SYM::change_tolerance WARNING: Inconsistent symmetry, tolerance range (" << std::pow(10.0,(std::log10(orig_tolerance)-(max_range))) << " to " << std::pow(10.0,(std::log10(orig_tolerance)+(max_range))) << ") tested [dir=" << xstr.directory << "]." << endl;
	//DX 9/1/17 exit(0);
          //count -=1;
	no_scan = true;
...
namespace SYM {
  bool minimizeCartesianDistance(const xvector<double>& coord1, const xvector<double>& coord2, xvector<double>& out, 
                                 xmatrix<double>& c2f, xmatrix<double>& f2c, double& tol){
    xvector<int> ijk;
...
namespace SYM {
  bool minimizeCartesianDistance(const xvector<double>& coord1, const xvector<double>& coord2, xvector<double>& out, 
                                 xmatrix<double>& c2f, xmatrix<double>& f2c, xvector<int>& ijk, bool& restriction, double& tol){
...
double minimumCartesianDistance(const xvector<double>& coord1, const xvector<double>& coord2, xmatrix<double>& lattice, 
    ...
    for(int a=-dims[1];a<=dims[1];a++){
      for(int b=-dims[2];b<=dims[2];b++){
        for(int c=-dims[3];c<=dims[3];c++){
          tmp = coord1-coord2+a*lattice(1)+b*lattice(2)+c*lattice(3);
...
namespace SYM {
  bool ComparePointGroupAndSpaceGroupString(xstructure& xstr, int& multiplicity_of_primitive, bool& derivative_structure){
    ...
    if(multiplicity_of_primitive<0){
      cerr << "SYM::ComparePointGroupAndSpaceGroupString: ERROR: Multiplicity of original to primitive cell is negative." << endl;
      //exit(1);
    }
    ...
    if(xstr.space_group_ITC != 0){ 
      ...
      if(LDEBUG){
        cerr << "SYM::ComparePointGroupAndSpaceGroupString: WARNING: The space group was not found; cannot compare space group string and the point group of the crystal." << endl;
      }
      return false;
    }
...
namespace SYM {
  ...
  bool ApplyAtomValidate(const _atom &atom_in,_atom& atom_out,
                         const _sym_op &symop,xstructure& a){
    bool _incell_=false;
  ...
  bool ApplyAtomValidate(const _atom &atom_in,_atom& atom_out,
                         const _sym_op &symop,xstructure& a,bool _incell_,bool roff){
    ...
    double min_dist = AUROSTD_NAN;
    if(a.dist_nn_min == AUROSTD_NAN){a.MinDist();}
      min_dist = a.dist_nn_min;
  ...
  bool ApplyAtomValidate(const _atom &atom_in,_atom& atom_out,
                         const _sym_op &symop,xstructure& a,bool skew, bool _incell_,bool roff, double _eps_){
    return ApplyAtomValidate(atom_in,atom_out,symop,a.lattice,a.c2f,a.f2c,skew,_incell_,roff,_eps_);
  }
  bool ApplyAtomValidate(const _atom &atom_in,_atom& atom_out,
                         const _sym_op &symop,xmatrix<double>& lattice,xmatrix<double>& c2f, xmatrix<double>& f2c,
...
namespace SYM {
  ...
  _atom ApplyAtom(const _atom &atom_in,const _sym_op &symop,xmatrix<double>& lattice,xmatrix<double>& c2f, xmatrix<double>& f2c,bool skew, bool _incell_,bool roff, bool validatePosition, double tolerance) {
    return ApplyAtom_20161115(atom_in,symop,lattice,c2f,f2c,skew,_incell_,roff,validatePosition,tolerance);
  }

  _atom ApplyAtom_20161115(const _atom &atom_in,const _sym_op &symop,xmatrix<double>& lattice,xmatrix<double>& c2f, xmatrix<double>& f2c,bool skew, bool _incell_,bool roff,bool validatePosition, double _eps_) {
    _atom atom;
...
//DX 9/16/17 [OBSOLETE]    //   cerr << pgname << endl;
    ...
    if(pgname=="-43m") {
      ...
      a.point_group_type="-";a.point_group_order="24";a.point_group_structure="symmetric";
    }
    if(pgname=="432") {
      pg_found=TRUE;
    ...
    if(pgname=="-6m2" || pgname=="-62m") {
      ...
      a.point_group_type="-";a.point_group_order="12";a.point_group_structure="dihedral";
    }
    if(pgname=="6mm") {
      pg_found=TRUE;
    ...
    if(pgname=="-42m" || pgname=="-4m2") {
      ...
      a.point_group_type="-";a.point_group_order="8";a.point_group_structure="dihedral";
    }
    if(pgname=="4mm") {
      pg_found=TRUE;
    ...
    if(pgname=="-6") {
      ...
      a.point_group_type="-";a.point_group_order="6";a.point_group_structure="cyclic";
    }
    if(pgname=="6") {
      pg_found=TRUE;
    ...
    if(pgname=="-4") {
      ...
      a.point_group_type="-";a.point_group_order="4";a.point_group_structure="cyclic";
    }
    if(pgname=="4") {
      pg_found=TRUE;
...
namespace SYM {
  ...
  bool getFullSymBasis(xstructure& a, _sym_op& symOp,bool map_types,double& tolerance,vector<int>& basis_atoms_map,vector<int>& basis_types_map){
    if(a.dist_nn_min == AUROSTD_NAN){a.MinDist();}
    bool skew = isLatticeSkewed(a.lattice,a.dist_nn_min,tolerance);
    ...
    for(uint iiat=0;iiat<a.iatoms.size();iiat++) {
      ...
      for(uint ieat=1;ieat<a.iatoms[iiat].size();ieat++){
        ...
        if(found_fgroup){
          ...
          //if(!found_fgroup){
          if(DEBUG_SYMMETRY){
            cerr << "SYM::CalculateSitePointGroup_Equivalent warning[1a] - Cannot find fgroup mapping between atom " << eat << " and atom " << iat << " [dir=" << a.directory << "]" << endl;
          }
          return FALSE;
...
string SymmetryToJson(vector<_sym_op>& group, char& mode){
  ...
  for(uint i=0;i<group.size();i++){
    ...
    if(group[i].generator.lrows){
      sscontent_json << "\"generator_coefficients\":[" << aurostd::joinWDelimiter(xvecDouble2vecString(group[i].generator_coefficients,5),",") << "]" << eendl; 
    } else{
    ...
    if(group[i].axis.lrows){
      sscontent_json << "\"axis\":[" << aurostd::joinWDelimiter(xvecDouble2vecString(group[i].axis,5),",") << "]" << eendl;
    } else{
...
bool KBIN_SymmetryWrite(ofstream &FileMESSAGE,xstructure &a,_aflags &aflags,char mode,const bool& osswrite,ostream& oss,const string& format) { //DX 8/2/17
  ...
  if(mode==_PGROUP_) FileNameOUTPUT=aflags.Directory+"/"+_AFLOW_PGROUP_FILE_;
  if(mode==_PGROUPK_) FileNameOUTPUT=aflags.Directory+"/"+_AFLOW_PGROUPK_FILE_;
  if(mode==_FGROUP_) FileNameOUTPUT=aflags.Directory+"/"+_AFLOW_FGROUP_FILE_;
  if(mode==_SGROUP_) FileNameOUTPUT=aflags.Directory+"/"+_AFLOW_SGROUP_FILE_;
  if(mode==_AGROUP_) FileNameOUTPUT=aflags.Directory+"/"+_AFLOW_AGROUP_FILE_;
  if(mode==_IATOMS_) FileNameOUTPUT=aflags.Directory+"/"+_AFLOW_IATOMS_FILE_;
  if(mode==_PGROUP_XTAL_) FileNameOUTPUT=aflags.Directory+"/"+_AFLOW_PGROUP_XTAL_FILE_;
  if(mode==_PGROUPK_XTAL_) FileNameOUTPUT=aflags.Directory+"/"+_AFLOW_PGROUPK_XTAL_FILE_; //DX 12/5/17 - Added pgroupk_xtal
  }
  if(format=="json" || format=="Json" || format=="JSON"){
    if(mode==_PGROUP_) FileNameOUTPUT=aflags.Directory+"/"+_AFLOW_PGROUP_JSON_;
    if(mode==_PGROUPK_) FileNameOUTPUT=aflags.Directory+"/"+_AFLOW_PGROUPK_JSON_;
    if(mode==_FGROUP_) FileNameOUTPUT=aflags.Directory+"/"+_AFLOW_FGROUP_JSON_;
    if(mode==_SGROUP_) FileNameOUTPUT=aflags.Directory+"/"+_AFLOW_SGROUP_JSON_;
    if(mode==_AGROUP_) FileNameOUTPUT=aflags.Directory+"/"+_AFLOW_AGROUP_JSON_;
    if(mode==_IATOMS_) FileNameOUTPUT=aflags.Directory+"/"+_AFLOW_IATOMS_JSON_;
    if(mode==_PGROUP_XTAL_) FileNameOUTPUT=aflags.Directory+"/"+_AFLOW_PGROUP_XTAL_JSON_;
    if(mode==_PGROUPK_XTAL_) FileNameOUTPUT=aflags.Directory+"/"+_AFLOW_PGROUPK_XTAL_JSON_; //DX 12/5/17 - Added pgroupk_xtal
  }
  string _lines_="------------------------------------------------------------------------------------------------";
