...
namespace KBIN {
  bool AIMS_Produce_CONTROL(_xaims& xaims,string AflowIn,ofstream &FileMESSAGE,_aflags &aflags,_kflags &kflags,_aimsflags &aimsflags) { // AFLOW_FUNCTION_IMPLEMENTATION
    ...
    bool KBIN_AIMS_CONTROL_MODE_EMPTY=!aimsflags.KBIN_AIMS_CONTROL_MODE.isscheme("IMPLICIT") && !aimsflags.KBIN_AIMS_CONTROL_MODE.isscheme("EXPLICIT") && !aimsflags.KBIN_AIMS_CONTROL_MODE.isscheme("EXTERNAL");

    // IMPLICIT or EXPLICIT or EXTERNAL for CONTROL
    Krun=(Krun && (aimsflags.KBIN_AIMS_CONTROL_MODE.isscheme("IMPLICIT") ||
          aimsflags.KBIN_AIMS_CONTROL_MODE.isscheme("EXPLICIT") ||
          aimsflags.KBIN_AIMS_CONTROL_MODE.isscheme("EXTERNAL") || KBIN_AIMS_CONTROL_MODE_EMPTY));
    if(!Krun) {
    ...
    // IMPLICIT ************************************************** CONTROL
    if(Krun && aimsflags.KBIN_AIMS_CONTROL_MODE.isscheme("IMPLICIT")) {  // [AIMS_CONTROL_MODE_IMPLICIT] construction
      aus << "00000  MESSAGE CONTROL   generation IMPLICIT file from " << _AFLOWIN_ << " " << Message(aflags,"user,host,time") << endl;
      aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
      if(aimsflags.KBIN_AIMS_CONTROL_FILE.isscheme("SYSTEM_AUTO")) {
        xaims.CONTROL << "#AFLOW CONTROL automatically generated" << endl;
      }
    }
    // EXPLICIT ************************************************** CONTROL
    if(Krun && aimsflags.KBIN_AIMS_CONTROL_MODE.isscheme("EXPLICIT")) {  // [AIMS_CONTROL_MODE_EXPLICIT] construction
      if(aimsflags.KBIN_AIMS_CONTROL_FILE.isscheme("KEYWORD") && !aimsflags.KBIN_AIMS_CONTROL_MODE.isscheme("EXPLICIT_START_STOP")) {
      ...
      } else if(!aimsflags.KBIN_AIMS_CONTROL_FILE.isscheme("KEYWORD") && aimsflags.KBIN_AIMS_CONTROL_MODE.isscheme("EXPLICIT_START_STOP")) {
    ...
    // EXTERNAL **************************************************
    if(Krun && aimsflags.KBIN_AIMS_CONTROL_MODE.isscheme("EXTERNAL")) {  // [AIMS_CONTROL_MODE_EXTERNAL] construction
      string file;
      aus << "00000  MESSAGE CONTROL   generation EXTERNAL file from " << _AFLOWIN_ << " " << Message(aflags,"user,host,time") << endl;
      aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
      if(aimsflags.KBIN_AIMS_CONTROL_FILE.isscheme("COMMAND") && aimsflags.KBIN_AIMS_CONTROL_FILE.isscheme("FILE")) {
      ...
      if(!aimsflags.KBIN_AIMS_CONTROL_FILE.isscheme("COMMAND") && (aimsflags.KBIN_AIMS_CONTROL_FILE.isscheme("FILE") || !aimsflags.KBIN_AIMS_CONTROL_FILE.isscheme("FILE"))) {
        if(aimsflags.KBIN_AIMS_CONTROL_FILE.isscheme("FILE")) {
...
namespace KBIN {
  bool AIMS_Produce_GEOM(_xaims& xaims,string AflowIn,ofstream &FileMESSAGE,_aflags &aflags,_kflags &kflags,_aimsflags &aimsflags) {        // AFLOW_FUNCTION_IMPLEMENTATION
    ...
    Krun=(Krun && (aimsflags.KBIN_AIMS_GEOM_MODE.isscheme("IMPLICIT") ||
          aimsflags.KBIN_AIMS_GEOM_MODE.isscheme("EXPLICIT") ||
          aimsflags.KBIN_AIMS_GEOM_MODE.isscheme("EXTERNAL")));
    if(!Krun) {
      aurostd::StringstreamClean(aus);
    ...
    // IMPLICIT **************************************************
    if(Krun && aimsflags.KBIN_AIMS_GEOM_MODE.isscheme("IMPLICIT")) {  // [AIMS_GEOM_MODE_IMPLICIT] construction
      aus << "00000  MESSAGE GEOM  generation IMPLICIT file from " << _AFLOWIN_ << " " << Message(aflags,"user,host,time") << endl;
      aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
      if(!aimsflags.KBIN_AIMS_GEOM_FILE.isscheme("PROTOTYPE")) {
    ...
    // EXPLICIT **************************************************
    if(Krun && aimsflags.KBIN_AIMS_GEOM_MODE.isscheme("EXPLICIT")) {  // [AIMS_GEOM_MODE_EXPLICIT] construction
      if(aimsflags.KBIN_AIMS_GEOM_FILE.isscheme("KEYWORD") && !aimsflags.KBIN_AIMS_GEOM_MODE.isscheme("EXPLICIT_START_STOP") && !aimsflags.KBIN_AIMS_GEOM_MODE.isscheme("EXPLICIT_START_STOP_POINT")) {
        ...
        xaims.GEOM << xaims.str;
      } else if(!aimsflags.KBIN_AIMS_GEOM_FILE.isscheme("KEYWORD") && (aimsflags.KBIN_AIMS_GEOM_MODE.isscheme("EXPLICIT_START_STOP") || aimsflags.KBIN_AIMS_GEOM_MODE.isscheme("EXPLICIT_START_STOP_POINT"))) {
        ...
        if(aimsflags.KBIN_AIMS_GEOM_MODE.isscheme("EXPLICIT_START_STOP")) {
        ...
        // get ONE of MANY
        if(aimsflags.KBIN_AIMS_GEOM_MODE.isscheme("EXPLICIT_START_STOP_POINT")) {
    ...
    // EXTERNAL **************************************************
    if(Krun && aimsflags.KBIN_AIMS_GEOM_MODE.isscheme("EXTERNAL")) {  // [AIMS_GEOM_MODE_EXTERNAL] construction
      string file;
      aus << "00000  MESSAGE GEOM  generation EXTERNAL file from " << _AFLOWIN_ << " " << Message(aflags,"user,host,time") << endl;
      aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
      if(aimsflags.KBIN_AIMS_GEOM_FILE.isscheme("COMMAND") && aimsflags.KBIN_AIMS_GEOM_FILE.isscheme("FILE")) {
        aus << "EEEEE   [AIMS_GEOM_MODE]FILE=  and  [AIMS_GEOM_MODE]COMMAND=  can not be used together " << Message(aflags,"user,host,time") << endl;
        aurostd::PrintErrorStream(FileMESSAGE,aus,XHOST.QUIET);
        Krun=FALSE;
        return Krun;
      }
      if(!aimsflags.KBIN_AIMS_GEOM_FILE.isscheme("COMMAND") && (aimsflags.KBIN_AIMS_GEOM_FILE.isscheme("FILE") || !aimsflags.KBIN_AIMS_GEOM_FILE.isscheme("FILE"))) {
        if(aimsflags.KBIN_AIMS_GEOM_FILE.isscheme("FILE")) {
...
namespace KBIN {
  bool AIMS_Modify_GEOM(_xaims& xaims,string AflowIn,ofstream &FileMESSAGE,_aflags &aflags,_aimsflags &aimsflags) {        // AFLOW_FUNCTION_IMPLEMENTATION
    ...
    if(LDEBUG) cerr << "aimsflags.KBIN_AIMS_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme(\"PRESERVE\")=" << aimsflags.KBIN_AIMS_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("PRESERVE") << endl;
    // GEOM must be modified before doing the KPOINTS
    if(Krun && aimsflags.KBIN_AIMS_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("PRESERVE")) {  // [AIMS_FORCE_OPTION]CONVERT_UNIT_CELL_PRESERVE construction /*************** GEOM **************/
      aus << "00000  MESSAGE GEOM  PRESERVE Unit Cell " << Message(aflags,"user,host,time") << endl;
      aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
    }

    if(LDEBUG) cerr << "aimsflags.KBIN_AIMS_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme(\"STANDARD_PRIMITIVE\")=" << aimsflags.KBIN_AIMS_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("STANDARD_PRIMITIVE") << endl;
    // GEOM must be modified before doing the KPOINTS
    if(Krun && aimsflags.KBIN_AIMS_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("STANDARD_PRIMITIVE")) {  // [AIMS_FORCE_OPTION]CONVERT_UNIT_CELL_STANDARD_PRIMITIVE construction /*************** GEOM **************/
    ...
    if(LDEBUG) cerr << "aimsflags.KBIN_AIMS_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme(\"STANDARD_CONVENTIONAL\")=" << aimsflags.KBIN_AIMS_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("STANDARD_CONVENTIONAL") << endl;
    // GEOM must be modified before doing the KPOINTS
    if(Krun && aimsflags.KBIN_AIMS_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("STANDARD_CONVENTIONAL")) {  // [AIMS_FORCE_OPTION]CONVERT_UNIT_CELL_STANDARD_CONVENTIONAL construction /*************** GEOM **************/
    ...
    if(LDEBUG) cerr << "aimsflags.KBIN_AIMS_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme(\"NIGGLI\")=" << aimsflags.KBIN_AIMS_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("NIGGLI") << endl;
    // GEOM must be modified before doing the KPOINTS
    if(Krun && aimsflags.KBIN_AIMS_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("NIGGLI")) {  // [AIMS_FORCE_OPTION]CONVERT_UNIT_CELL_NIGGLI construction                     /*************** GEOM **************/
    ...
    if(LDEBUG) cerr << "aimsflags.KBIN_AIMS_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme(MINKOWSKI)=" << aimsflags.KBIN_AIMS_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("MINKOWSKI") << endl;
    if(Krun && aimsflags.KBIN_AIMS_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("MINKOWSKI")) {  // [AIMS_FORCE_OPTION]CONVERT_UNIT_CELL_MINKOWSKI construction               /*************** GEOM **************/
    ...
    if(LDEBUG) cerr << "aimsflags.KBIN_AIMS_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme(\"INCELL\")=" << aimsflags.KBIN_AIMS_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("INCELL") << endl;
    if(Krun && aimsflags.KBIN_AIMS_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("INCELL")) {  // [AIMS_FORCE_OPTION]CONVERT_UNIT_CELL_INCELL construction                     /*************** GEOM **************/
    ...
    if(LDEBUG) cerr << "aimsflags.KBIN_AIMS_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme(\"COMPACT\")=" << aimsflags.KBIN_AIMS_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("COMPACT") << endl;
    if(Krun && aimsflags.KBIN_AIMS_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("COMPACT")) {  // [AIMS_FORCE_OPTION]CONVERT_UNIT_CELL_COMPACT construction                   /*************** GEOM **************/
    ...
    if(LDEBUG) cerr << "aimsflags.KBIN_AIMS_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme(\"WIGNERSEITZ\")=" << aimsflags.KBIN_AIMS_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("WIGNERSEITZ") << endl;
    if(Krun && aimsflags.KBIN_AIMS_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("WIGNERSEITZ")) {  // [AIMS_FORCE_OPTION]CONVERT_UNIT_CELL_WIGNERSEITZ construction           /*************** GEOM **************/
    ...
    if(LDEBUG) cerr << "aimsflags.KBIN_AIMS_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme(\"CARTESIAN\")=" << aimsflags.KBIN_AIMS_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("CARTESIAN") << endl;
    if(Krun && aimsflags.KBIN_AIMS_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("CARTESIAN")) {  // [AIMS_FORCE_OPTION]CONVERT_UNIT_CELL_CARTESIAN construction               /*************** GEOM **************/
    ...
    if(LDEBUG) cerr << "aimsflags.KBIN_AIMS_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme(\"FRACTIONAL\")=" << aimsflags.KBIN_AIMS_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("FRACTIONAL") << endl;
    if(Krun && aimsflags.KBIN_AIMS_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("FRACTIONAL")) {  // [AIMS_FORCE_OPTION]CONVERT_UNIT_CELL_FRACTIONAL construction            /*************** GEOM **************/
    ...
    if(LDEBUG) cerr << "aimsflags.KBIN_AIMS_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme(\"DIRECT\")=" << aimsflags.KBIN_AIMS_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("DIRECT") << endl;
    if(Krun && aimsflags.KBIN_AIMS_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("DIRECT")) {  // [AIMS_FORCE_OPTION]CONVERT_UNIT_CELL_DIRECT construction                    /*************** GEOM **************/
    ...
    if(LDEBUG) cerr << "aimsflags.KBIN_AIMS_GEOM_FILE_VOLUME.isscheme(\"EQUAL_EQUAL\")=" << aimsflags.KBIN_AIMS_GEOM_FILE_VOLUME.isscheme("EQUAL_EQUAL") << endl;
    if(Krun && aimsflags.KBIN_AIMS_GEOM_MODE.isscheme("IMPLICIT") && aimsflags.KBIN_AIMS_GEOM_FILE_VOLUME.isscheme("EQUAL_EQUAL")) {  // [AIMS_GEOM_FILE]VOLUME=                       /*************** GEOM **************/
    ...
    if(LDEBUG) cerr << "aimsflags.KBIN_AIMS_FORCE_OPTION_VOLUME.isscheme(\"EQUAL_EQUAL\")=" << aimsflags.KBIN_AIMS_FORCE_OPTION_VOLUME.isscheme("EQUAL_EQUAL") << endl;
    if(Krun && aimsflags.KBIN_AIMS_FORCE_OPTION_VOLUME.isscheme("EQUAL_EQUAL")) {  // [AIMS_FORCE_OPTION]VOLUME=                                                    /*************** GEOM **************/
    ...
    if(LDEBUG) cerr << "aimsflags.KBIN_AIMS_GEOM_FILE_VOLUME.isscheme(\"MULTIPLY_EQUAL\")=" << aimsflags.KBIN_AIMS_GEOM_FILE_VOLUME.isscheme("MULTIPLY_EQUAL") << endl;
    if(Krun && aimsflags.KBIN_AIMS_GEOM_MODE.isscheme("IMPLICIT") && aimsflags.KBIN_AIMS_GEOM_FILE_VOLUME.isscheme("MULTIPLY_EQUAL")) {  // [AIMS_GEOM_FILE]VOLUME*=             /*************** GEOM **************/
      double factor=aurostd::substring2utype<double>(AflowIn,"[AIMS_GEOM_FILE]VOLUME*=",FALSE);
    ...
    if(LDEBUG) cerr << "aimsflags.KBIN_AIMS_FORCE_OPTION_VOLUME.isscheme(\"MULTIPLY_EQUAL\")=" << aimsflags.KBIN_AIMS_FORCE_OPTION_VOLUME.isscheme("MULTIPLY_EQUAL") << endl;
    if(Krun && aimsflags.KBIN_AIMS_FORCE_OPTION_VOLUME.isscheme("MULTIPLY_EQUAL")) {  // [AIMS_FORCE_OPTION]VOLUME*=                                                    /*************** GEOM **************/
      double factor1=aurostd::substring2utype<double>(AflowIn,"[AIMS_FORCE_OPTION]VOLUME*=",FALSE);
    ...
    if(LDEBUG) cerr << "aimsflags.KBIN_AIMS_GEOM_FILE_VOLUME.isscheme(PLUS_EQUAL)=" << aimsflags.KBIN_AIMS_GEOM_FILE_VOLUME.isscheme("PLUS_EQUAL") << endl;
    if(Krun && aimsflags.KBIN_AIMS_GEOM_MODE.isscheme("IMPLICIT") && aimsflags.KBIN_AIMS_GEOM_FILE_VOLUME.isscheme("PLUS_EQUAL")) {  // [AIMS_GEOM_FILE]VOLUME+=               /*************** GEOM **************/
      double factor=aurostd::substring2utype<double>(AflowIn,"[AIMS_GEOM_FILE]VOLUME+=",FALSE);
      aus << "00000  MESSAGE GEOM  IMPLICIT Volume += " << factor << " " << Message(aflags,"user,host,time") << endl;
      aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
      aus << "00000  MESSAGE GEOM  IMPLITIC Old Volume= " << xaims.str.Volume() << " " << Message(aflags,"user,host,time") << endl;
    ...
    if(LDEBUG) cerr << "aimsflags.KBIN_AIMS_FORCE_OPTION_VOLUME.isscheme(PLUS_EQUAL)=" << aimsflags.KBIN_AIMS_FORCE_OPTION_VOLUME.isscheme("PLUS_EQUAL") << endl;
    if(Krun && aimsflags.KBIN_AIMS_FORCE_OPTION_VOLUME.isscheme("PLUS_EQUAL")) {  // [AIMS_FORCE_OPTION]VOLUME+=                                                    /*************** GEOM **************/
      double factor1=aurostd::substring2utype<double>(AflowIn,"[AIMS_FORCE_OPTION]VOLUME+=",FALSE);
