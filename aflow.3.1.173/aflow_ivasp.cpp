...
// INPUT
namespace KBIN {
  bool VASP_Produce_INPUT(_xvasp& xvasp,string AflowIn,ofstream &FileMESSAGE,_aflags &aflags,_kflags &kflags,_vflags &vflags) {
...
namespace KBIN {
  string VASP_PseudoPotential_CleanName(const string& speciesIN) {
    ...
    //  for(i=1;i<=imax;i++) species=aurostd::RemoveSubStringFirst(species,"1");
    //COREY - START
    for(i=1;i<=imax;i++) species=aurostd::RemoveSubStringFirst(species,"pot_GGA/");
    for(i=1;i<=imax;i++) species=aurostd::RemoveSubStringFirst(species,"pot_LDA/");
    for(i=1;i<=imax;i++) species=aurostd::RemoveSubStringFirst(species,"potpaw_GGA/");
    for(i=1;i<=imax;i++) species=aurostd::RemoveSubStringFirst(species,"potpaw_LDA/");
    for(i=1;i<=imax;i++) species=aurostd::RemoveSubStringFirst(species,"potpaw_PBE/");
    //COREY - END
    return species;
  }
}
...
// INCAR
namespace KBIN {
  bool VASP_Produce_INCAR(_xvasp& xvasp,string AflowIn,ofstream &FileMESSAGE,_aflags &aflags,_kflags &kflags,_vflags &vflags) { // AFLOW_FUNCTION_IMPLEMENTATION
    ...
    bool KBIN_VASP_INCAR_MODE_EMPTY=!vflags.KBIN_VASP_INCAR_MODE.isscheme("IMPLICIT") && !vflags.KBIN_VASP_INCAR_MODE.isscheme("EXPLICIT") && !vflags.KBIN_VASP_INCAR_MODE.isscheme("EXTERNAL");

    // IMPLICIT or EXPLICIT or EXTERNAL for INCAR
    Krun=(Krun && (vflags.KBIN_VASP_INCAR_MODE.isscheme("IMPLICIT") ||
          vflags.KBIN_VASP_INCAR_MODE.isscheme("EXPLICIT") ||
          vflags.KBIN_VASP_INCAR_MODE.isscheme("EXTERNAL") || KBIN_VASP_INCAR_MODE_EMPTY));
    if(!Krun) {
    ...
    // IMPLICIT ************************************************** INCAR
    if(Krun && vflags.KBIN_VASP_INCAR_MODE.isscheme("IMPLICIT")) {  // [VASP_INCAR_MODE_IMPLICIT] construction
      aus << "00000  MESSAGE INCAR   generation IMPLICIT file from " << _AFLOWIN_ << " " << Message(aflags,"user,host,time") << endl;
      aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
      if(vflags.KBIN_VASP_INCAR_FILE.isscheme("SYSTEM_AUTO")) {
        xvasp.INCAR << "#AFLOW INCAR automatically generated" << endl;
    ...
    // EXPLICIT ************************************************** INCAR
    if(Krun && vflags.KBIN_VASP_INCAR_MODE.isscheme("EXPLICIT")) {  // [VASP_INCAR_MODE_EXPLICIT] construction
      if(vflags.KBIN_VASP_INCAR_FILE.isscheme("KEYWORD") && !vflags.KBIN_VASP_INCAR_MODE.isscheme("EXPLICIT_START_STOP")) {
      ...
      } else if(!vflags.KBIN_VASP_INCAR_FILE.isscheme("KEYWORD") && vflags.KBIN_VASP_INCAR_MODE.isscheme("EXPLICIT_START_STOP")) {
    ...
    // EXTERNAL **************************************************
    if(Krun && vflags.KBIN_VASP_INCAR_MODE.isscheme("EXTERNAL")) {  // [VASP_INCAR_MODE_EXTERNAL] construction
      string file;
      aus << "00000  MESSAGE INCAR   generation EXTERNAL file from " << _AFLOWIN_ << " " << Message(aflags,"user,host,time") << endl;
      aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
      if(vflags.KBIN_VASP_INCAR_FILE.isscheme("COMMAND") && vflags.KBIN_VASP_INCAR_FILE.isscheme("FILE")) {
        aus << "EEEEE   [VASP_INCAR_MODE]FILE=  and  [VASP_INCAR_MODE]COMMAND=  can not be used together " << Message(aflags,"user,host,time") << endl;
        aurostd::PrintErrorStream(FileMESSAGE,aus,XHOST.QUIET);
        Krun=FALSE;
        return Krun;
      }
      if(!vflags.KBIN_VASP_INCAR_FILE.isscheme("COMMAND") && (vflags.KBIN_VASP_INCAR_FILE.isscheme("FILE") || !vflags.KBIN_VASP_INCAR_FILE.isscheme("FILE"))) {
        if(vflags.KBIN_VASP_INCAR_FILE.isscheme("FILE")) {
...
namespace KBIN {
  bool VASP_Modify_INCAR(_xvasp& xvasp,ofstream &FileMESSAGE,_aflags &aflags,_kflags &kflags,_vflags &vflags) {        // AFLOW_FUNCTION_IMPLEMENTATION
    ...
    // if(Krun && !vflags.KBIN_VASP_RUN.isscheme("STATIC") && !vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.isscheme("STATIC") && ! vflags.KBIN_VASP_RUN_RELAX_STATIC_PATCH_STATIC) //o change for relax if STATIC !!!
    if(Krun && vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.isentry && 
        (vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.isscheme("ALL")               || vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.isscheme("IONS") ||
         ...
         vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.isscheme("IONS_CELL_VOLUME"))) {      /*************** INCAR **************/
      if(vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.isscheme("ALL"))                 aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]RELAX_ALL - " << Message(aflags,"user,host,time") << endl;
      if(vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.isscheme("IONS"))                aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]RELAX_IONS - " << Message(aflags,"user,host,time") << endl;
      if(vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.isscheme("CELL_SHAPE"))          aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]RELAX_CELL_SHAPE - " << Message(aflags,"user,host,time") << endl;
      if(vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.isscheme("CELL_VOLUME"))         aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]RELAX_CELL_VOLUME - " << Message(aflags,"user,host,time") << endl;
      if(vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.isscheme("IONS_CELL_VOLUME"))    aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]RELAX_IONS_CELL_VOLUME - " << Message(aflags,"user,host,time") << endl;
      if(vflags.KBIN_VASP_FORCE_OPTION_RELAX_MODE.isentry) {
        if(vflags.KBIN_VASP_FORCE_OPTION_RELAX_MODE.scheme=="ENERGY")         aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]RELAX_MODE=ENERGY (default) - " << Message(aflags,"user,host,time") << endl;
        if(vflags.KBIN_VASP_FORCE_OPTION_RELAX_MODE.scheme=="FORCES")         aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]RELAX_MODE=FORCES - " << Message(aflags,"user,host,time") << endl;
        if(vflags.KBIN_VASP_FORCE_OPTION_RELAX_MODE.scheme=="ENERGY_FORCES")  aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]RELAX_MODE=ENERGY_FORCES - " << Message(aflags,"user,host,time") << endl;
        if(vflags.KBIN_VASP_FORCE_OPTION_RELAX_MODE.scheme=="FORCES_ENERGY")  aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]RELAX_MODE=FORCES_ENERGY - " << Message(aflags,"user,host,time") << endl;
      } else {
    ...
    if(Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry==FALSE) {                                                     /*************** INCAR **************/
      if(vflags.KBIN_VASP_FORCE_OPTION_PREC.isentry==TRUE) {                                                         /*************** INCAR **************/
        aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]PREC=" << vflags.KBIN_VASP_FORCE_OPTION_PREC.scheme << " - " << Message(aflags,"user,host,time") << endl;
    ...
    if(Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry==FALSE) {                                                    /*************** INCAR **************/
      if(vflags.KBIN_VASP_FORCE_OPTION_ALGO.isentry) {                                                              /*************** INCAR **************/
        aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]ALGO=" << vflags.KBIN_VASP_FORCE_OPTION_ALGO.scheme << " - " << Message(aflags,"user,host,time") << endl;
    ...
    if(Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry==FALSE) {                                                    /*************** INCAR **************/
      if(vflags.KBIN_VASP_FORCE_OPTION_METAGGA.isentry) {                                                              /*************** INCAR **************/
        aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]METAGGA=" << vflags.KBIN_VASP_FORCE_OPTION_METAGGA.scheme << " - " << Message(aflags,"user,host,time") << endl;;
      } else {
    ...
    if(Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry==FALSE) {                                                    /*************** INCAR **************/
      if(vflags.KBIN_VASP_FORCE_OPTION_IVDW.isentry) {                                                              /*************** INCAR **************/
        aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]IVDW=" << vflags.KBIN_VASP_FORCE_OPTION_IVDW.scheme << " - " << Message(aflags,"user,host,time") << endl;;
      } else {
    ...
    if(Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry==FALSE) {                                                    /*************** INCAR **************/
      if(vflags.KBIN_VASP_FORCE_OPTION_ABMIX.isentry) {                                                              /*************** INCAR **************/
        aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]ABMIX=" << vflags.KBIN_VASP_FORCE_OPTION_ABMIX.scheme << " - " << Message(aflags,"user,host,time") << endl;
    ...
    if(Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry==FALSE) {                                                    /*************** INCAR **************/
      if(vflags.KBIN_VASP_FORCE_OPTION_TYPE.isentry) {                                                              /*************** INCAR **************/
        if(vflags.KBIN_VASP_FORCE_OPTION_TYPE.scheme.at(0)=='D') aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]TYPE=" << vflags.KBIN_VASP_FORCE_OPTION_TYPE.scheme << "  - " << Message(aflags,"user,host,time") << endl;
        if(vflags.KBIN_VASP_FORCE_OPTION_TYPE.scheme.at(0)=='M') aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]TYPE=" << vflags.KBIN_VASP_FORCE_OPTION_TYPE.scheme << "  - " << Message(aflags,"user,host,time") << endl;
        if(vflags.KBIN_VASP_FORCE_OPTION_TYPE.scheme.at(0)=='S') aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]TYPE=" << vflags.KBIN_VASP_FORCE_OPTION_TYPE.scheme << "  - " << Message(aflags,"user,host,time") << endl;
        if(vflags.KBIN_VASP_FORCE_OPTION_TYPE.scheme.at(0)=='I') aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]TYPE=" << vflags.KBIN_VASP_FORCE_OPTION_TYPE.scheme << "  - " << Message(aflags,"user,host,time") << endl;
      } else {
        aus << "00000  MESSAGE-DEFAULT TYPE=" << DEFAULT_VASP_FORCE_OPTION_TYPE_SCHEME << " - " << Message(aflags,"user,host,time") << endl;
      }
      aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
      KBIN::XVASP_INCAR_PREPARE_GENERIC("TYPE",xvasp,vflags,vflags.KBIN_VASP_FORCE_OPTION_TYPE.scheme,0,0.0,FALSE);
      xvasp.aopts.flag("FLAG::XVASP_INCAR_changed",TRUE);
    }
    ...
    if(Krun && vflags.KBIN_VASP_FORCE_OPTION_IGNORE_AFIX.isentry) {                                                    /*************** INCAR **************/
      ...
      for(uint i=0;i<vflags.KBIN_VASP_FORCE_OPTION_IGNORE_AFIX.vscheme.size();i++) 
        aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]IGNORE_AFIX=" << vflags.KBIN_VASP_FORCE_OPTION_IGNORE_AFIX.vscheme.at(i) << "  - " << Message(aflags,"user,host,time") << endl;
      aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
    }
    ...
    if(Krun && vflags.KBIN_VASP_RUN.isscheme("STATIC")) {                                                                      /*************** INCAR **************/
      aus << "00000  MESSAGE-OPTION  [VASP_RUN_STATIC] - " << Message(aflags,"user,host,time") << endl;
      aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
      KBIN::XVASP_INCAR_Static_ON(xvasp,vflags);
      xvasp.aopts.flag("FLAG::XVASP_INCAR_changed",TRUE);
    }
    if(Krun && vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.isscheme("STATIC")) {                                                             /*************** INCAR **************/
...
// POSCAR
namespace KBIN {
  bool VASP_Produce_POSCAR(_xvasp& xvasp,string AflowIn,ofstream &FileMESSAGE,_aflags &aflags,_kflags &kflags,_vflags &vflags) {        // AFLOW_FUNCTION_IMPLEMENTATION
    ...
    Krun=(Krun && (vflags.KBIN_VASP_POSCAR_MODE.isscheme("IMPLICIT") ||
          vflags.KBIN_VASP_POSCAR_MODE.isscheme("EXPLICIT") ||
          vflags.KBIN_VASP_POSCAR_MODE.isscheme("EXTERNAL")));
    if(!Krun) {
      aurostd::StringstreamClean(aus);
    ...
    // IMPLICIT **************************************************
    if(Krun && vflags.KBIN_VASP_POSCAR_MODE.isscheme("IMPLICIT")) {  // [VASP_POSCAR_MODE_IMPLICIT] construction
      aus << "00000  MESSAGE POSCAR  generation IMPLICIT file from " << _AFLOWIN_ << " " << Message(aflags,"user,host,time") << endl;
      aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
      if(!vflags.KBIN_VASP_POSCAR_FILE.isscheme("PROTOTYPE")) {
    ...
    // EXPLICIT **************************************************
    if(Krun && vflags.KBIN_VASP_POSCAR_MODE.isscheme("EXPLICIT")) {  // [VASP_POSCAR_MODE_EXPLICIT] construction
      if(vflags.KBIN_VASP_POSCAR_FILE.isscheme("KEYWORD") && !vflags.KBIN_VASP_POSCAR_MODE.isscheme("EXPLICIT_START_STOP") && !vflags.KBIN_VASP_POSCAR_MODE.isscheme("EXPLICIT_START_STOP_POINT")) {
        ...
        xvasp.POSCAR << xvasp.str;
      } else if(!vflags.KBIN_VASP_POSCAR_FILE.isscheme("KEYWORD") && (vflags.KBIN_VASP_POSCAR_MODE.isscheme("EXPLICIT_START_STOP") || vflags.KBIN_VASP_POSCAR_MODE.isscheme("EXPLICIT_START_STOP_POINT"))) {
        ...
        if(vflags.KBIN_VASP_POSCAR_MODE.isscheme("EXPLICIT_START_STOP")) {
    ...
    // EXTERNAL **************************************************
    if(Krun && vflags.KBIN_VASP_POSCAR_MODE.isscheme("EXTERNAL")) {  // [VASP_POSCAR_MODE_EXTERNAL] construction
      string file;
      aus << "00000  MESSAGE POSCAR  generation EXTERNAL file from " << _AFLOWIN_ << " " << Message(aflags,"user,host,time") << endl;
      aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
      if(vflags.KBIN_VASP_POSCAR_FILE.isscheme("COMMAND") && vflags.KBIN_VASP_POSCAR_FILE.isscheme("FILE")) {
        aus << "EEEEE   [VASP_POSCAR_MODE]FILE=  and  [VASP_POSCAR_MODE]COMMAND=  can not be used together " << Message(aflags,"user,host,time") << endl;
        aurostd::PrintErrorStream(FileMESSAGE,aus,XHOST.QUIET);
        Krun=FALSE;
        return Krun;
      }
      if(!vflags.KBIN_VASP_POSCAR_FILE.isscheme("COMMAND") && (vflags.KBIN_VASP_POSCAR_FILE.isscheme("FILE") || !vflags.KBIN_VASP_POSCAR_FILE.isscheme("FILE"))) {
        if(vflags.KBIN_VASP_POSCAR_FILE.isscheme("FILE")) {
...
namespace KBIN {
  bool VASP_Modify_POSCAR(_xvasp& xvasp,string AflowIn,ofstream &FileMESSAGE,_aflags &aflags,_vflags &vflags) {        // AFLOW_FUNCTION_IMPLEMENTATION
    bool LDEBUG=(FALSE || XHOST.DEBUG);
    ostringstream aus;
    bool Krun=TRUE;
    ...
    if(LDEBUG) cerr << "vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme(\"PRESERVE\")=" << vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("PRESERVE") << endl;
    // POSCAR must be modified before doing the KPOINTS
    if(Krun && vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("PRESERVE")) {  // [VASP_FORCE_OPTION]CONVERT_UNIT_CELL_PRESERVE construction /*************** POSCAR **************/
      aus << "00000  MESSAGE POSCAR  PRESERVE Unit Cell " << Message(aflags,"user,host,time") << endl;
      aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
    }

    if(LDEBUG) cerr << "vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme(\"STANDARD_PRIMITIVE\")=" << vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("STANDARD_PRIMITIVE") << endl;
    // POSCAR must be modified before doing the KPOINTS
    if(Krun && vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("STANDARD_PRIMITIVE")) {  // [VASP_FORCE_OPTION]CONVERT_UNIT_CELL_STANDARD_PRIMITIVE construction /*************** POSCAR **************/
    ...
    if(LDEBUG) cerr << "vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme(\"STANDARD_CONVENTIONAL\")=" << vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("STANDARD_CONVENTIONAL") << endl;
    // POSCAR must be modified before doing the KPOINTS
    if(Krun && vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("STANDARD_CONVENTIONAL")) {  // [VASP_FORCE_OPTION]CONVERT_UNIT_CELL_STANDARD_CONVENTIONAL construction /*************** POSCAR **************/
    ...
    if(LDEBUG) cerr << "vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme(\"NIGGLI\")=" << vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("NIGGLI") << endl;
    // POSCAR must be modified before doing the KPOINTS
    if(Krun && vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("NIGGLI")) {  // [VASP_FORCE_OPTION]CONVERT_UNIT_CELL_NIGGLI construction                     /*************** POSCAR **************/
    ...
    if(LDEBUG) cerr << "vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme(MINKOWSKI)=" << vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("MINKOWSKI") << endl;
    if(Krun && vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("MINKOWSKI")) {  // [VASP_FORCE_OPTION]CONVERT_UNIT_CELL_MINKOWSKI construction               /*************** POSCAR **************/
    ...
    if(LDEBUG) cerr << "vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme(\"INCELL\")=" << vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("INCELL") << endl;
    if(Krun && vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("INCELL")) {  // [VASP_FORCE_OPTION]CONVERT_UNIT_CELL_INCELL construction                     /*************** POSCAR **************/
    ...
    if(LDEBUG) cerr << "vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme(\"COMPACT\")=" << vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("COMPACT") << endl;
    if(Krun && vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("COMPACT")) {  // [VASP_FORCE_OPTION]CONVERT_UNIT_CELL_COMPACT construction                   /*************** POSCAR **************/
    ...
    if(LDEBUG) cerr << "vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme(\"WIGNERSEITZ\")=" << vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("WIGNERSEITZ") << endl;
    if(Krun && vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("WIGNERSEITZ")) {  // [VASP_FORCE_OPTION]CONVERT_UNIT_CELL_WIGNERSEITZ construction           /*************** POSCAR **************/
    ...
    if(LDEBUG) cerr << "vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme(\"CARTESIAN\")=" << vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("CARTESIAN") << endl;
    if(Krun && vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("CARTESIAN")) {  // [VASP_FORCE_OPTION]CONVERT_UNIT_CELL_CARTESIAN construction               /*************** POSCAR **************/
    ...
    if(LDEBUG) cerr << "vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme(\"FRACTIONAL\")=" << vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("FRACTIONAL") << endl;
    if(Krun && vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("FRACTIONAL")) {  // [VASP_FORCE_OPTION]CONVERT_UNIT_CELL_FRACTIONAL construction            /*************** POSCAR **************/
    ...
    if(LDEBUG) cerr << "vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme(\"DIRECT\")=" << vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("DIRECT") << endl;
    if(Krun && vflags.KBIN_VASP_FORCE_OPTION_CONVERT_UNIT_CELL.isscheme("DIRECT")) {  // [VASP_FORCE_OPTION]CONVERT_UNIT_CELL_DIRECT construction                    /*************** POSCAR **************/
    ...
    if(LDEBUG) cerr << "vflags.KBIN_VASP_POSCAR_FILE_VOLUME.isscheme(\"EQUAL_EQUAL\")=" << vflags.KBIN_VASP_POSCAR_FILE_VOLUME.isscheme("EQUAL_EQUAL") << endl;
    if(Krun && vflags.KBIN_VASP_POSCAR_MODE.isscheme("IMPLICIT") && vflags.KBIN_VASP_POSCAR_FILE_VOLUME.isscheme("EQUAL_EQUAL")) {  // [VASP_POSCAR_FILE]VOLUME=                       /*************** POSCAR **************/
    ...
    if(LDEBUG) cerr << "vflags.KBIN_VASP_FORCE_OPTION_VOLUME.isscheme(\"EQUAL_EQUAL\")=" << vflags.KBIN_VASP_FORCE_OPTION_VOLUME.isscheme("EQUAL_EQUAL") << endl;
    if(Krun && vflags.KBIN_VASP_FORCE_OPTION_VOLUME.isscheme("EQUAL_EQUAL")) {  // [VASP_FORCE_OPTION]VOLUME=                                                    /*************** POSCAR **************/
    ...
    if(LDEBUG) cerr << "vflags.KBIN_VASP_POSCAR_FILE_VOLUME.isscheme(\"MULTIPLY_EQUAL\")=" << vflags.KBIN_VASP_POSCAR_FILE_VOLUME.isscheme("MULTIPLY_EQUAL") << endl;
    if(Krun && vflags.KBIN_VASP_POSCAR_MODE.isscheme("IMPLICIT") && vflags.KBIN_VASP_POSCAR_FILE_VOLUME.isscheme("MULTIPLY_EQUAL")) {  // [VASP_POSCAR_FILE]VOLUME*=             /*************** POSCAR **************/
      double factor=aurostd::substring2utype<double>(AflowIn,"[VASP_POSCAR_FILE]VOLUME*=",FALSE);
    ...
    if(LDEBUG) cerr << "vflags.KBIN_VASP_FORCE_OPTION_VOLUME.isscheme(\"MULTIPLY_EQUAL\")=" << vflags.KBIN_VASP_FORCE_OPTION_VOLUME.isscheme("MULTIPLY_EQUAL") << endl;
    if(Krun && vflags.KBIN_VASP_FORCE_OPTION_VOLUME.isscheme("MULTIPLY_EQUAL")) {  // [VASP_FORCE_OPTION]VOLUME*=                                                    /*************** POSCAR **************/
      double factor1=aurostd::substring2utype<double>(AflowIn,"[VASP_FORCE_OPTION]VOLUME*=",FALSE);
    ...
    // [OBSOLETE]    if(LDEBUG) cerr << "vflags.KBIN_VASP_POSCAR_FILE_VOLUME.isscheme(DIVIDE_EQUAL)=" << vflags.KBIN_VASP_POSCAR_FILE_VOLUME.isscheme("DIVIDE_EQUAL") << endl;
    // [OBSOLETE]    if(Krun && vflags.KBIN_VASP_POSCAR_MODE.isscheme("IMPLICIT") && vflags.KBIN_VASP_POSCAR_FILE_VOLUME.isscheme("DIVIDE_EQUAL")) {  // [VASP_POSCAR_FILE]VOLUME/=              /*************** POSCAR **************/
    // [OBSOLETE]      double factor=aurostd::substring2utype<double>(AflowIn,"[VASP_POSCAR_FILE]VOLUME/=",FALSE);
    // [OBSOLETE]      aus << "00000  MESSAGE POSCAR  IMPLICIT Volume /= " << factor << " " << Message(aflags,"user,host,time") << endl;
    // [OBSOLETE]      aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
    // [OBSOLETE]      if(factor<=0.0) {
    // [OBSOLETE]	aus << "EEEEE  KBIN::VASP_Modify_POSCAR Volume can not be <=0 (factor=" << factor << ")" << Message(aflags,"user,host,time") << endl;
    // [OBSOLETE]	aurostd::PrintErrorStream(FileMESSAGE,aus,XHOST.QUIET);
    // [OBSOLETE]	Krun=FALSE;
    // [OBSOLETE]	return Krun;
    // [OBSOLETE]      }
    // [OBSOLETE]      aus << "00000  MESSAGE POSCAR  IMPLITIC Old Volume= " << xvasp.str.Volume() << " " << Message(aflags,"user,host,time") << endl;
    // [OBSOLETE]      aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
    // [OBSOLETE]      xvasp.str.SetVolume(xvasp.str.Volume()/factor);
    // [OBSOLETE]      aus << "00000  MESSAGE POSCAR  IMPLITIC New Volume= " << xvasp.str.Volume() << " " << Message(aflags,"user,host,time") << endl;
    // [OBSOLETE]      aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
    // [OBSOLETE]      xvasp.str.title=xvasp.str.title+" [Forced Volume /=]";
    // [OBSOLETE]      xvasp.POSCAR.str(std::string());xvasp.POSCAR.clear();
    // [OBSOLETE]      xvasp.POSCAR << xvasp.str;
    // [OBSOLETE]      xvasp.aopts.flag("FLAG::XVASP_POSCAR_generated",TRUE);
    // [OBSOLETE]      xvasp.aopts.flag("FLAG::XVASP_POSCAR_changed",TRUE);
    // [OBSOLETE]    }
    // [OBSOLETE]
    // [OBSOLETE]    if(LDEBUG) cerr << "vflags.KBIN_VASP_FORCE_OPTION_VOLUME.isscheme(DIVIDE_EQUAL)=" << vflags.KBIN_VASP_FORCE_OPTION_VOLUME.isscheme("DIVIDE_EQUAL") << endl;
    // [OBSOLETE]    if(Krun && vflags.KBIN_VASP_FORCE_OPTION_VOLUME.isscheme("DIVIDE_EQUAL")) {  // [VASP_FORCE_OPTION]VOLUME/=                                                    /*************** POSCAR **************/
    // [OBSOLETE]      double factor1=aurostd::substring2utype<double>(AflowIn,"[VASP_FORCE_OPTION]VOLUME/=",FALSE);
    // [OBSOLETE]      double factor=vflags.KBIN_VASP_FORCE_OPTION_VOLUME.getattachedutype<double>("DIVIDE_EQUAL");
    // [OBSOLETE]      aus << "00000  MESSAGE POSCAR  FORCE Volume /= " << factor << " (factor1=" << factor1 << ")  " << Message(aflags,"user,host,time") << endl;
    // [OBSOLETE]      aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
    // [OBSOLETE]      if(factor<=0.0) {
    // [OBSOLETE]	aus << "EEEEE  KBIN::VASP_Modify_POSCAR Volume can not be <=0 (factor=" << factor << ")" << Message(aflags,"user,host,time") << endl;
    // [OBSOLETE]	aurostd::PrintErrorStream(FileMESSAGE,aus,XHOST.QUIET);
    ...
    // [OBSOLETE]	return Krun;
    // [OBSOLETE]      }
    // [OBSOLETE]      aus << "00000  MESSAGE POSCAR  FORCE Old Volume= " << xvasp.str.Volume() << " " << Message(aflags,"user,host,time") << endl;
    // [OBSOLETE]      aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
    // [OBSOLETE]      xvasp.str.SetVolume(xvasp.str.Volume()/factor);
    // [OBSOLETE]      aus << "00000  MESSAGE POSCAR  FORCE New Volume= " << xvasp.str.Volume() << " " << Message(aflags,"user,host,time") << endl;
    // [OBSOLETE]      aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
    // [OBSOLETE]      xvasp.str.title=xvasp.str.title+" [Forced Volume /=]";
    // [OBSOLETE]      xvasp.POSCAR.str(std::string());xvasp.POSCAR.clear();
    // [OBSOLETE]      xvasp.POSCAR << xvasp.str;
    ...
    if(LDEBUG) cerr << "vflags.KBIN_VASP_POSCAR_FILE_VOLUME.isscheme(PLUS_EQUAL)=" << vflags.KBIN_VASP_POSCAR_FILE_VOLUME.isscheme("PLUS_EQUAL") << endl;
    if(Krun && vflags.KBIN_VASP_POSCAR_MODE.isscheme("IMPLICIT") && vflags.KBIN_VASP_POSCAR_FILE_VOLUME.isscheme("PLUS_EQUAL")) {  // [VASP_POSCAR_FILE]VOLUME+=               /*************** POSCAR **************/
      double factor=aurostd::substring2utype<double>(AflowIn,"[VASP_POSCAR_FILE]VOLUME+=",FALSE);
      aus << "00000  MESSAGE POSCAR  IMPLICIT Volume += " << factor << " " << Message(aflags,"user,host,time") << endl;
      aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
      aus << "00000  MESSAGE POSCAR  IMPLITIC Old Volume= " << xvasp.str.Volume() << " " << Message(aflags,"user,host,time") << endl;
    ...
    if(LDEBUG) cerr << "vflags.KBIN_VASP_FORCE_OPTION_VOLUME.isscheme(PLUS_EQUAL)=" << vflags.KBIN_VASP_FORCE_OPTION_VOLUME.isscheme("PLUS_EQUAL") << endl;
    if(Krun && vflags.KBIN_VASP_FORCE_OPTION_VOLUME.isscheme("PLUS_EQUAL")) {  // [VASP_FORCE_OPTION]VOLUME+=                                                    /*************** POSCAR **************/
      double factor1=aurostd::substring2utype<double>(AflowIn,"[VASP_FORCE_OPTION]VOLUME+=",FALSE);
    ...
    // [OBSOLETE]   if(LDEBUG) cerr << "vflags.KBIN_VASP_POSCAR_FILE_VOLUME.isscheme(MINUS_EQUAL)=" << vflags.KBIN_VASP_POSCAR_FILE_VOLUME.isscheme("MINUS_EQUAL") << endl;
    // [OBSOLETE]    if(Krun && vflags.KBIN_VASP_POSCAR_MODE.isscheme("IMPLICIT") && vflags.KBIN_VASP_POSCAR_FILE_VOLUME.isscheme("MINUS_EQUAL")) {  // [VASP_POSCAR_FILE]VOLUME-=              /*************** POSCAR **************/
    // [OBSOLETE]      double factor=aurostd::substring2utype<double>(AflowIn,"[VASP_POSCAR_FILE]VOLUME-=",FALSE);
    // [OBSOLETE]      aus << "00000  MESSAGE POSCAR  IMPLICIT Volume -= " << factor << " " << Message(aflags,"user,host,time") << endl;
    // [OBSOLETE]      aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
    // [OBSOLETE]      aus << "00000  MESSAGE POSCAR  IMPLITIC Old Volume= " << xvasp.str.Volume() << " " << Message(aflags,"user,host,time") << endl;
    ...
    // [OBSOLETE]    }
    // [OBSOLETE]
    // [OBSOLETE]    if(LDEBUG) cerr << "vflags.KBIN_VASP_FORCE_OPTION_VOLUME.isscheme(MINUS_EQUAL)=" << vflags.KBIN_VASP_FORCE_OPTION_VOLUME.isscheme("MINUS_EQUAL") << endl;
    // [OBSOLETE]    if(Krun && vflags.KBIN_VASP_FORCE_OPTION_VOLUME.isscheme("MINUS_EQUAL")) {  // [VASP_FORCE_OPTION]VOLUME-=                                                    /*************** POSCAR **************/
    // [OBSOLETE]      double factor1=aurostd::substring2utype<double>(AflowIn,"[VASP_FORCE_OPTION]VOLUME-=",FALSE);
    // [OBSOLETE]      double factor=vflags.KBIN_VASP_FORCE_OPTION_VOLUME.getattachedutype<double>("MINUS_EQUAL");
    // [OBSOLETE]      aus << "00000  MESSAGE POSCAR  FORCE Volume -= " << factor << " (factor1=" << factor1 << ")  " << Message(aflags,"user,host,time") << endl;
    // [OBSOLETE]      aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
    // [OBSOLETE]      aus << "00000  MESSAGE POSCAR  FORCE Old Volume= " << xvasp.str.Volume() << " " << Message(aflags,"user,host,time") << endl;
    // [OBSOLETE]      aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
...
// KPOINTS
namespace KBIN {
  bool VASP_Produce_KPOINTS(_xvasp& xvasp,string AflowIn,ofstream &FileMESSAGE,_aflags &aflags,_kflags &kflags,_vflags &vflags) {        // AFLOW_FUNCTION_IMPLEMENTATION
    ...
    Krun=(Krun && (vflags.KBIN_VASP_KPOINTS_MODE.isscheme("IMPLICIT") ||
          vflags.KBIN_VASP_KPOINTS_MODE.isscheme("EXPLICIT") ||
          vflags.KBIN_VASP_KPOINTS_MODE.isscheme("EXTERNAL")));
    if(!Krun) {
      aurostd::StringstreamClean(aus);
    ...
    if(Krun && vflags.KBIN_VASP_KPOINTS_MODE.isscheme("EXPLICIT")) {  // [VASP_KPOINTS_MODE_EXPLICIT] construction
      IMPLICIT=FALSE;
      if(vflags.KBIN_VASP_KPOINTS_FILE.isscheme("KEYWORD") && !vflags.KBIN_VASP_KPOINTS_MODE.isscheme("EXPLICIT_START_STOP")) {
      ...
      } else if(!vflags.KBIN_VASP_KPOINTS_FILE.isscheme("KEYWORD") && vflags.KBIN_VASP_KPOINTS_MODE.isscheme("EXPLICIT_START_STOP")) {
    ...
    if(Krun && vflags.KBIN_VASP_KPOINTS_MODE.isscheme("IMPLICIT")) { // IT MIGHT NOT CONTAIN OPTION SO IT IS ALL DEFAULT && vflags.KBIN_VASP_KPOINTS_FILE) {  // [VASP_KPOINTS_MODE_IMPLICIT] construction
      IMPLICIT=TRUE;
      ...
      if(vflags.KBIN_VASP_RUN.isscheme("STATIC")) { // do the switching
    ...
    // EXTERNAL **************************************************
    if(Krun && vflags.KBIN_VASP_KPOINTS_MODE.isscheme("EXTERNAL")) {  // [VASP_KPOINTS_MODE_EXTERNAL] construction
      string file;
      aus << "00000  MESSAGE KPOINTS  generation EXTERNAL file from " << _AFLOWIN_ << " " << Message(aflags,"user,host,time") << endl;
      aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
      if(vflags.KBIN_VASP_KPOINTS_FILE.isscheme("COMMAND") && vflags.KBIN_VASP_KPOINTS_FILE.isscheme("FILE")) {
      ...
      if(!vflags.KBIN_VASP_KPOINTS_FILE.isscheme("COMMAND") && (vflags.KBIN_VASP_KPOINTS_FILE.isscheme("FILE") || !vflags.KBIN_VASP_KPOINTS_FILE.isscheme("FILE"))) {
        if(vflags.KBIN_VASP_KPOINTS_FILE.isscheme("FILE")) {
  ...
  namespace KBIN {
    bool VASP_Modify_KPOINTS(_xvasp& xvasp,ofstream &FileMESSAGE,_aflags &aflags,_vflags &vflags) {        // AFLOW_FUNCTION_IMPLEMENTATION
      ...
      if(Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry==FALSE && vflags.KBIN_VASP_FORCE_OPTION_KPOINTS.isscheme("KEEPK")==FALSE && 0) {             /*************** KPOINTS **************/
      ...
      if(Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry==FALSE && vflags.KBIN_VASP_FORCE_OPTION_KPOINTS.isscheme("KEEPK")) {                        /*************** KPOINTS **************/
      ...
      if(Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry==FALSE && vflags.KBIN_VASP_FORCE_OPTION_KPOINTS.isscheme("EVEN")) {                        /*************** KPOINTS **************/
      ...
      if(Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry==FALSE && vflags.KBIN_VASP_FORCE_OPTION_KPOINTS.isscheme("ODD")) {                         /*************** KPOINTS **************/
      ...
      if(Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry==FALSE && vflags.KBIN_VASP_FORCE_OPTION_KPOINTS.isscheme("KSHIFT_GAMMA_EVEN")) {           /*************** KPOINTS **************/
      ...
      if(Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry==FALSE && vflags.KBIN_VASP_FORCE_OPTION_KPOINTS.isscheme("KSHIFT_GAMMA_ODD")) {            /*************** KPOINTS **************/
      ...
      if(Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry==FALSE && vflags.KBIN_VASP_FORCE_OPTION_KPOINTS.isscheme("KSCHEME_MONKHORST_PACK")) {      /*************** KPOINTS **************/
      ...
      if(Krun && vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry==FALSE && vflags.KBIN_VASP_FORCE_OPTION_KPOINTS.isscheme("KSCHEME_GAMMA")) {               /*************** KPOINTS **************/
      ...
      if(Krun && (vflags.KBIN_VASP_FORCE_OPTION_NOTUNE.isentry==FALSE && vflags.KBIN_VASP_FORCE_OPTION_KPOINTS.isscheme("KSCHEME_AUTO"))) {              /*************** KPOINTS **************/
      ...
      if(vflags.KBIN_VASP_FORCE_OPTION_KPOINTS.isscheme("GAMMA")) {
        vflags.KBIN_VASP_WRITE_KPOINTS=TRUE;
        xvasp.str.kpoints_k1=1;xvasp.str.kpoints_k2=1;xvasp.str.kpoints_k3=1;
        xvasp.str.kpoints_kmax=max(xvasp.str.kpoints_k1,xvasp.str.kpoints_k2,xvasp.str.kpoints_k3);
        xvasp.str.kpoints_kppra=xvasp.str.kpoints_k1*xvasp.str.kpoints_k2*xvasp.str.kpoints_k3*xvasp.str.atoms.size();
      ...
      if(vflags.KBIN_VASP_FORCE_OPTION_KPOINTS.isscheme("IBZKPT")) {
  ...
  // POTCAR
  namespace KBIN {
    bool VASP_Find_POTCAR(const string& species_pp,string &FilePotcar) {
      bool LDEBUG=(FALSE || XHOST.DEBUG);
      string pseudopotential="nothing";
      if(aurostd::substring2bool(species_pp,"pot_LDA")) pseudopotential="pot_LDA";
      if(aurostd::substring2bool(species_pp,"pot_GGA")) pseudopotential="pot_GGA";
      if(aurostd::substring2bool(species_pp,"pot_PBE")) pseudopotential="pot_PBE";
      if(aurostd::substring2bool(species_pp,"potpaw_LDA")) pseudopotential="potpaw_LDA";
      if(aurostd::substring2bool(species_pp,"potpaw_GGA")) pseudopotential="potpaw_GGA";
      if(aurostd::substring2bool(species_pp,"potpaw_PBE")) pseudopotential="potpaw_PBE";
  ...
  namespace KBIN {
    bool VASP_Produce_POTCAR(_xvasp& xvasp,string AflowIn,ofstream &FileMESSAGE,_aflags &aflags,_kflags &kflags,_vflags &vflags) {        // AFLOW_FUNCTION_IMPLEMENTATION
      bool LDEBUG=(FALSE || XHOST.DEBUG);
      ...
      Krun=(Krun && (vflags.KBIN_VASP_POTCAR_MODE.isscheme("IMPLICIT") || vflags.KBIN_VASP_POTCAR_MODE.isscheme("EXPLICIT") || vflags.KBIN_VASP_POTCAR_MODE.isscheme("EXTERNAL")));
      if(!Krun) {
        aurostd::StringstreamClean(aus);
      ...
      // IMPLICIT **************************************************
      if(Krun && vflags.KBIN_VASP_POTCAR_MODE.isscheme("IMPLICIT") && vflags.KBIN_VASP_POTCAR_FILE.isscheme("KEYWORD")) { // [VASP_POTCAR_MODE_IMPLICIT] construction
        // Prepare POTCAR
        ...
        if(vflags.KBIN_VASP_FORCE_OPTION_AUTO_PSEUDOPOTENTIALS.isentry) {         /*************** POTCAR **************/
          ...
          aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
          aus << "00000  MESSAGE-OPTION  [VASP_FORCE_OPTION]AUTO_PSEUDOPOTENTIALS=\"" << vflags.KBIN_VASP_FORCE_OPTION_AUTO_PSEUDOPOTENTIALS.scheme << "\" - " << Message(aflags,"user,host,time") << endl;
          aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
        }
        // Prepare POTCAR
        ifstream FileINPUT;
        ...
        string FilePotcar;
        if(!vflags.KBIN_VASP_POTCAR_FILE.isscheme("SYSTEM_AUTO")) {
        ...
        if(vflags.KBIN_VASP_POTCAR_FILE.isscheme("SYSTEM_AUTO")) {
          for(uint i=0;i<xvasp.str.species.size();i++) {
            if(xvasp.str.species.at(i)!="") {
              subS="";
              if(!vflags.KBIN_VASP_POTCAR_FILE.isscheme("PREFIX")) {
              ...
              if(!vflags.KBIN_VASP_POTCAR_FILE.isscheme("SUFFIX"))
        ...
        if(vflags.KBIN_VASP_FORCE_OPTION_AUTO_PSEUDOPOTENTIALS.isentry) {
          for(uint i=0;i<FilePotcars.size();i++) {
            ...
            if(aurostd::substring2bool(vflags.KBIN_VASP_FORCE_OPTION_AUTO_PSEUDOPOTENTIALS.scheme,"potpaw_PBE"))
              FilePotcars.at(i)=vflags.KBIN_VASP_FORCE_OPTION_AUTO_PSEUDOPOTENTIALS.scheme+"/"+AVASP_Get_PseudoPotential_PAWPBE(cleanname);
            if(aurostd::substring2bool(vflags.KBIN_VASP_FORCE_OPTION_AUTO_PSEUDOPOTENTIALS.scheme,"potpaw_GGA"))
              FilePotcars.at(i)=vflags.KBIN_VASP_FORCE_OPTION_AUTO_PSEUDOPOTENTIALS.scheme+"/"+AVASP_Get_PseudoPotential_PAWGGA(cleanname);
            if(aurostd::substring2bool(vflags.KBIN_VASP_FORCE_OPTION_AUTO_PSEUDOPOTENTIALS.scheme,"potpaw_LDA"))
              FilePotcars.at(i)=vflags.KBIN_VASP_FORCE_OPTION_AUTO_PSEUDOPOTENTIALS.scheme+"/"+AVASP_Get_PseudoPotential_PAWLDA(cleanname);
            if(aurostd::substring2bool(vflags.KBIN_VASP_FORCE_OPTION_AUTO_PSEUDOPOTENTIALS.scheme,"pot_GGA"))
              FilePotcars.at(i)=vflags.KBIN_VASP_FORCE_OPTION_AUTO_PSEUDOPOTENTIALS.scheme+"/"+AVASP_Get_PseudoPotential_GGA(cleanname);
            if(aurostd::substring2bool(vflags.KBIN_VASP_FORCE_OPTION_AUTO_PSEUDOPOTENTIALS.scheme,"pot_LDA"))
              FilePotcars.at(i)=vflags.KBIN_VASP_FORCE_OPTION_AUTO_PSEUDOPOTENTIALS.scheme+"/"+AVASP_Get_PseudoPotential_LDA(cleanname);
          }
        }
        ...
        for(uint i=0;i<FilePotcars.size();i++) {                        // cycle through all POTCARS
          ...
          FilePotcarNaked=aurostd::RemoveSubStringFirst(FilePotcarNaked,"/POTCAR.gz");
          FilePotcarNaked=aurostd::RemoveSubStringFirst(FilePotcarNaked,"/POTCAR.bz2");
          FilePotcarNaked=aurostd::RemoveSubStringFirst(FilePotcarNaked,"/POTCAR");
          aurostd::string2tokens(FilePotcarNaked,tokens,"/");
          xvasp.POTCAR_POTENTIALS << tokens.at(tokens.size()-1);
          // now build it
          bool found=FALSE;
          //      cerr << "i=" << i << " " << FilePotcars.at(i) << endl;
          found=KBIN::VASP_Find_POTCAR(FilePotcars.at(i),FilePotcar);

          if(found) {
            aus << "00000  MESSAGE POTCAR  Found potcar file = [" << FilePotcar << "]" << endl;
            aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
          } else {
          ...
          char c;
          while (FileINPUT.get(c)) xvasp.POTCAR.put(c);
          FileINPUT.clear();FileINPUT.close();
        }
      }
      // EXPLICIT **************************************************
      if(Krun && vflags.KBIN_VASP_POTCAR_MODE.isscheme("EXPLICIT")) {   // [VASP_POTCAR_MODE_EXPLICIT] construction
        // Prepare POTCAR
      ...
      // EXTERNAL **************************************************
      if(Krun && vflags.KBIN_VASP_POTCAR_MODE.isscheme("EXTERNAL")) {  // [VASP_POTCAR_MODE_EXTERNAL] construction
        string file;
        aus << "00000  MESSAGE POTCAR  generation EXTERNAL file from " << _AFLOWIN_ << " " << Message(aflags,"user,host,time") << endl;
        aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
        if(vflags.KBIN_VASP_POTCAR_FILE.isscheme("COMMAND") && vflags.KBIN_VASP_POTCAR_FILE.isscheme("FILE")) {
          aus << "EEEEE   [VASP_POTCAR_MODE]FILE=  and  [VASP_POTCAR_MODE]COMMAND=  can not be used together " << Message(aflags,"user,host,time") << endl;
          aurostd::PrintErrorStream(FileMESSAGE,aus,XHOST.QUIET);
          Krun=FALSE;
          return Krun;
        }
        if(!vflags.KBIN_VASP_POTCAR_FILE.isscheme("COMMAND") && (vflags.KBIN_VASP_POTCAR_FILE.isscheme("FILE") || !vflags.KBIN_VASP_POTCAR_FILE.isscheme("FILE"))) {
          if(vflags.KBIN_VASP_POTCAR_FILE.isscheme("FILE")) {
      ...
      // [OBSOLETE] // get xvasp.POTCAR_ENMAX **************************************************
      // [OBSOLETE] string _strline;
      // [OBSOLETE] string _strstream=xvasp.POTCAR.str();
      // [OBSOLETE] 
      // [OBSOLETE] xvasp.POTCAR_ENMAX=0.0;
      // [OBSOLETE] double _enmax_;
      // [OBSOLETE]    string::size_type idx0;
      // [OBSOLETE]   bool boolout=FALSE;
      // [OBSOLETE]   while(!boolout) {
      // [OBSOLETE]     idx0=_strstream.find("ENMAX");
      // [OBSOLETE]     if(idx0==string::npos) boolout=TRUE; // NO COMMENT "#
      // [OBSOLETE]     if(idx0!=string::npos) {
      // [OBSOLETE]   	boolout=FALSE; // COMMENT AFTER
      // [OBSOLETE]   	_strstream=_strstream.substr(_strstream.find("ENMAX")+1);
      // [OBSOLETE]   	_strline=_strstream.substr(_strstream.find("=")+1,20);
      // [OBSOLETE]   	//  _strline= aurostd::RemoveCharacter(_strline,';');
      // [OBSOLETE]   	_enmax_=strtod(_strline.c_str(),NULL);
      // [OBSOLETE]   	aus << "00000  MESSAGE POTCAR  ENMAX found = " << _enmax_ << Message(aflags,"user,host,time") << endl;
      // [OBSOLETE]   	aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
      // [OBSOLETE]   	if(_enmax_>xvasp.POTCAR_ENMAX) xvasp.POTCAR_ENMAX=_enmax_;
      // [OBSOLETE]     }
      // [OBSOLETE]   }
      ...
      // [OBSOLETE] // get xvasp.POTCAR_PAW **************************************************
      // [OBSOLETE] xvasp.POTCAR_PAW=FALSE;
      // [OBSOLETE] if(_strstream.find("PAW")!=string::npos) xvasp.POTCAR_PAW=TRUE;
      // [OBSOLETE] aus << "00000  MESSAGE POTCAR  PAW = " << (xvasp.POTCAR_PAW ? "TRUE":"FALSE") << " - " << Message(aflags,"user,host,time") << endl;
      // [OBSOLETE] aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
      // [OBSOLETE] // POTCAR DONE **************************************************
      // [OBSOLETE] xvasp.POTCAR_orig << xvasp.POTCAR.str();
      // [OBSOLETE] xvasp.aopts.flag("FLAG::XVASP_POTCAR_generated",TRUE);
      // [OBSOLETE] // CHECK THE TYPE
      // [OBSOLETE] 
      // [OBSOLETE] vector<string> vPOTCAR;
      // [OBSOLETE] aurostd::string2vectorstring(xvasp.POTCAR.str(),vPOTCAR);
      // [OBSOLETE] 
      // [OBSOLETE] // cerr << vPOTCAR.size() << endl;exit(0);
      // [OBSOLETE] // CHECK FOR US => LDA/GGA
      // [OBSOLETE] bool is_US=FALSE,is_LDA=FALSE;
      // [OBSOLETE] xvasp.POTCAR_TYPE="";
      // [OBSOLETE] for(uint i=0;i<vPOTCAR.size()&&xvasp.POTCAR_TYPE.empty();i++) {
      // [OBSOLETE]   //  cerr << vPOTCAR.at(i) << endl;
      // [OBSOLETE]   if(aurostd::substring2bool(vPOTCAR.at(i),"TITEL") && aurostd::substring2bool(vPOTCAR.at(i),"US")) is_US=TRUE;
      // [OBSOLETE]   if(aurostd::substring2bool(vPOTCAR.at(i),"GGA ")) is_LDA=TRUE;
      // [OBSOLETE] }
      // [OBSOLETE] if(is_US && is_LDA) xvasp.POTCAR_TYPE="LDA";
      // [OBSOLETE] if(is_US && !is_LDA) xvasp.POTCAR_TYPE="GGA";
      // [OBSOLETE] for(uint i=0;i<vPOTCAR.size()&&xvasp.POTCAR_TYPE.empty();i++) {
      // [OBSOLETE]   if(aurostd::substring2bool(vPOTCAR.at(i),"TITEL")) {
      // [OBSOLETE] 	vector<string> tokens;
      // [OBSOLETE] 	aurostd::string2tokens(vPOTCAR.at(i),tokens,"=");vPOTCAR.at(i)=tokens.at(1);aurostd::string2tokens(vPOTCAR.at(i),tokens," ");
      // [OBSOLETE] 	if(tokens.at(0)=="PAW") xvasp.POTCAR_TYPE="PAW_LDA";
      // [OBSOLETE] 	if(tokens.at(0)=="PAW_GGA") xvasp.POTCAR_TYPE="PAW_GGA";
      // [OBSOLETE] 	if(tokens.at(0)=="PAW_PBE") xvasp.POTCAR_TYPE="PAW_PBE";
      // [OBSOLETE] 	if(tokens.at(0)=="PAW_RPBE") xvasp.POTCAR_TYPE="PAW_RPBE";
      // [OBSOLETE]   }
      // [OBSOLETE] }    
      // [OBSOLETE] aus << "00000  MESSAGE POTCAR  POTCAR_TYPE=" <<  xvasp.POTCAR_TYPE << " - " << Message(aflags,"user,host,time") << endl;
      // [OBSOLETE] aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
  ...
  namespace KBIN {
    void XVASP_INCAR_Relax_ON(_xvasp& xvasp,_vflags& vflags,int number) {        // AFLOW_FUNCTION_IMPLEMENTATION
      string FileContent,strline;
      int imax,isif=3;
      if(vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.isscheme("IONS_CELL_VOLUME")==FALSE) {  // whatever is the number
        if(vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.isscheme("IONS")==TRUE  && vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.isscheme("CELL_SHAPE")==FALSE && vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.isscheme("CELL_VOLUME")==FALSE) isif=2; // 0,1,2 FIX
        if(vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.isscheme("IONS")==TRUE  && vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.isscheme("CELL_SHAPE")==TRUE  && vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.isscheme("CELL_VOLUME")==TRUE)  isif=3;
        if(vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.isscheme("IONS")==TRUE  && vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.isscheme("CELL_SHAPE")==TRUE  && vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.isscheme("CELL_VOLUME")==FALSE) isif=4;
        if(vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.isscheme("IONS")==FALSE && vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.isscheme("CELL_SHAPE")==TRUE  && vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.isscheme("CELL_VOLUME")==FALSE) isif=5;
        if(vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.isscheme("IONS")==FALSE && vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.isscheme("CELL_SHAPE")==TRUE  && vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.isscheme("CELL_VOLUME")==TRUE)  isif=6;
        if(vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.isscheme("IONS")==FALSE && vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.isscheme("CELL_SHAPE")==FALSE && vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.isscheme("CELL_VOLUME")==TRUE)  isif=7;
        if(vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.isscheme("ALL")) isif=3;
      }
      if(vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.isscheme("IONS_CELL_VOLUME")) {  // whatever is the number
      ...
      if(vflags.KBIN_VASP_FORCE_OPTION_RELAX_MODE.scheme=="ENERGY") {
        for(int i=1;i<=imax;i++) {
          strline=aurostd::GetLineString(FileContent,i);
      ...
      if(vflags.KBIN_VASP_FORCE_OPTION_RELAX_MODE.scheme=="FORCES") {
        for(int i=1;i<=imax;i++) {
          strline=aurostd::GetLineString(FileContent,i);
          if(aurostd::substring2bool(strline,"IBRION",TRUE) || aurostd::substring2bool(strline,"#IBRION",TRUE) ||
  ...
  namespace KBIN {
    void XVASP_INCAR_Relax_ON(_xvasp& xvasp,bool VERBOSE) {        // AFLOW_FUNCTION_IMPLEMENTATION
      ...
      vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.addscheme("ALL");
      vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.addscheme("IONS");
      vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.addscheme("CELL_SHAPE");
      vflags.KBIN_VASP_FORCE_OPTION_RELAX_TYPE.addscheme("CELL_VOLUME");
  ...
  namespace KBIN {
    void XVASP_INCAR_Precision(_xvasp& xvasp,_vflags& vflags) {        // AFLOW_FUNCTION_IMPLEMENTATION
      ...
      for(int i=1;i<=imax;i++) {
        strline=aurostd::GetLineString(FileContent,i);
        if(vflags.KBIN_VASP_FORCE_OPTION_PREC.scheme=="LOW" || vflags.KBIN_VASP_FORCE_OPTION_PREC.scheme=="MEDIUM" || vflags.KBIN_VASP_FORCE_OPTION_PREC.scheme=="NORMAL") {
          if((aurostd::substring2bool(strline,"PREC",TRUE) && !aurostd::substring2bool(strline,"SYMPREC",TRUE)) || aurostd::substring2bool(strline,"#PREC",TRUE) || //CO 171003 - don't confuse PREC and SYMPREC
            ...
            if(vflags.KBIN_VASP_INCAR_VERBOSE) {
              if(vflags.KBIN_VASP_FORCE_OPTION_PREC.scheme=="LOW") xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR PREC=LOW) " << endl;
              if(vflags.KBIN_VASP_FORCE_OPTION_PREC.scheme=="MEDIUM") xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR PREC=MEDIUM) " << endl;
              if(vflags.KBIN_VASP_FORCE_OPTION_PREC.scheme=="NORMAL") xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR PREC=NORMAL) " << endl;
              if(vflags.KBIN_VASP_FORCE_OPTION_PREC.scheme=="HIGH") xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR PREC=HIGH) " << endl;
              if(vflags.KBIN_VASP_FORCE_OPTION_PREC.scheme=="ACCURATE") xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR PREC=ACCURATE) " << endl;
            }
          } else {
            if(!vflags.KBIN_VASP_INCAR_VERBOSE && strline.length()) xvasp.INCAR << strline << endl;
            if(vflags.KBIN_VASP_INCAR_VERBOSE) xvasp.INCAR << strline << endl;
        ...
        if(vflags.KBIN_VASP_FORCE_OPTION_PREC.scheme=="ACCURATE" || vflags.KBIN_VASP_FORCE_OPTION_PREC.scheme=="HIGH") {
          if(((aurostd::substring2bool(strline,"PREC",TRUE) && !aurostd::substring2bool(strline,"SYMPREC",TRUE)) || aurostd::substring2bool(strline,"#PREC",TRUE) ||  //CO 171003 - don't confuse PREC and SYMPREC
                aurostd::substring2bool(strline,"ENMAX",TRUE) || aurostd::substring2bool(strline,"#ENMAX",TRUE) ||
        ...
        //BEGIN JJPR
        if(vflags.KBIN_VASP_FORCE_OPTION_PREC.scheme=="PHONONS") {
          if(((aurostd::substring2bool(strline,"PREC",TRUE) && !aurostd::substring2bool(strline,"SYMPREC",TRUE)) || aurostd::substring2bool(strline,"#PREC",TRUE) ||  //CO 171003 - don't confuse PREC and SYMPREC
                aurostd::substring2bool(strline,"ENMAX",TRUE) || aurostd::substring2bool(strline,"#ENMAX",TRUE) ||
                aurostd::substring2bool(strline,"LREAL",TRUE) || aurostd::substring2bool(strline,"#LREAL",TRUE) ||
                aurostd::substring2bool(strline,"EDIFF",TRUE) || aurostd::substring2bool(strline,"#EDIFF",TRUE) ||
              ...
              (!aurostd::substring2bool(strline,"EDIFFG",TRUE)  && !aurostd::substring2bool(strline,"#EDIFFG",TRUE))) {
            if(vflags.KBIN_VASP_INCAR_VERBOSE) {
              if(vflags.KBIN_VASP_FORCE_OPTION_PREC.scheme=="PHONONS") xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR PREC=PHONONS) " << endl;
            }
          } else {
            if(!vflags.KBIN_VASP_INCAR_VERBOSE && strline.length()) xvasp.INCAR << strline << endl;
            if(vflags.KBIN_VASP_INCAR_VERBOSE) xvasp.INCAR << strline << endl;
      ...
      // xvasp.INCAR << endl;
      if(vflags.KBIN_VASP_INCAR_VERBOSE) xvasp.INCAR << "# Performing Precision PREC=" << vflags.KBIN_VASP_FORCE_OPTION_PREC.scheme << " [AFLOW] begin " << endl;

      if(vflags.KBIN_VASP_FORCE_OPTION_PREC.scheme=="LOW") {
      ...
      if(vflags.KBIN_VASP_FORCE_OPTION_PREC.scheme=="MEDIUM") {
      ...
      if(vflags.KBIN_VASP_FORCE_OPTION_PREC.scheme=="NORMAL") {
      ...
      if(vflags.KBIN_VASP_FORCE_OPTION_PREC.scheme=="HIGH") {
      ...
      if(vflags.KBIN_VASP_FORCE_OPTION_PREC.scheme=="PHONONS") { 
        ...
        xvasp.INCAR << aurostd::PaddedPOST("ALGO=NORMAL",_incarpad_) << "# fast determination of ground state " << endl;
      };
      // END   JJPR
      if(vflags.KBIN_VASP_INCAR_VERBOSE) xvasp.INCAR << "# Performing Precision PREC=" << vflags.KBIN_VASP_FORCE_OPTION_PREC.scheme << " [AFLOW] end " << endl;
      ...
      void XVASP_INCAR_Metagga(_xvasp& xvasp,_vflags& vflags) {        // AFLOW_FUNCTION_IMPLEMENTATION   TPSS | RTPSS | M06L | MBJL | SCAN | MS0 | MS1 | MS2 | NONE
        ...
        for(int i=1;i<=imax;i++) {
          ...
          if(aurostd::substring2bool(strline,"METAGGA",TRUE) || aurostd::substring2bool(strline,"#METAGGA",TRUE)) {
            if(vflags.KBIN_VASP_INCAR_VERBOSE) {
              if(vflags.KBIN_VASP_FORCE_OPTION_METAGGA.scheme=="TPSS") xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR METAGGA=TPSS) " << endl;
              if(vflags.KBIN_VASP_FORCE_OPTION_METAGGA.scheme=="RTPSS") xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR METAGGA=RTPSS) " << endl;
              if(vflags.KBIN_VASP_FORCE_OPTION_METAGGA.scheme=="M06L") xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR METAGGA=M06L) " << endl;
              if(vflags.KBIN_VASP_FORCE_OPTION_METAGGA.scheme=="MBJL") xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR METAGGA=MBJL) " << endl;
              if(vflags.KBIN_VASP_FORCE_OPTION_METAGGA.scheme=="SCAN") xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR METAGGA=SCAN) " << endl;
              if(vflags.KBIN_VASP_FORCE_OPTION_METAGGA.scheme=="MS0") xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR METAGGA=MS0) " << endl;
              if(vflags.KBIN_VASP_FORCE_OPTION_METAGGA.scheme=="MS1") xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR METAGGA=MS1) " << endl;
              if(vflags.KBIN_VASP_FORCE_OPTION_METAGGA.scheme=="MS2") xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR METAGGA=MS2) " << endl;
              if(vflags.KBIN_VASP_FORCE_OPTION_METAGGA.scheme=="NONE") xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR METAGGA=NONE) " << endl;
            }
          } else {
            if(!vflags.KBIN_VASP_INCAR_VERBOSE && strline.length()) xvasp.INCAR << strline << endl;
            if(vflags.KBIN_VASP_INCAR_VERBOSE) xvasp.INCAR << strline << endl;
        ...
        // xvasp.INCAR << endl;
        if(vflags.KBIN_VASP_FORCE_OPTION_METAGGA.scheme!="NONE") {
          if(vflags.KBIN_VASP_INCAR_VERBOSE) xvasp.INCAR << "# Performing Metagga METAGGA=" << vflags.KBIN_VASP_FORCE_OPTION_METAGGA.scheme << " [AFLOW] begin " << endl;
          if(vflags.KBIN_VASP_FORCE_OPTION_METAGGA.scheme=="TPSS") xvasp.INCAR << aurostd::PaddedPOST("METAGGA=TPSS",_incarpad_) << "# METAGGA = TPSS  " << endl;
          if(vflags.KBIN_VASP_FORCE_OPTION_METAGGA.scheme=="RTPSS") xvasp.INCAR << aurostd::PaddedPOST("METAGGA=RTPSS",_incarpad_) << "# METAGGA = RTPSS  " << endl;
          if(vflags.KBIN_VASP_FORCE_OPTION_METAGGA.scheme=="M06L") xvasp.INCAR << aurostd::PaddedPOST("METAGGA=M06L",_incarpad_) << "# METAGGA = M06L  " << endl;
          if(vflags.KBIN_VASP_FORCE_OPTION_METAGGA.scheme=="MBJL") xvasp.INCAR << aurostd::PaddedPOST("METAGGA=MBJL",_incarpad_) << "# METAGGA = MBJL  " << endl;
          if(vflags.KBIN_VASP_FORCE_OPTION_METAGGA.scheme=="SCAN") xvasp.INCAR << aurostd::PaddedPOST("METAGGA=SCAN",_incarpad_) << "# METAGGA = SCAN  " << endl;
          if(vflags.KBIN_VASP_FORCE_OPTION_METAGGA.scheme=="MS0") xvasp.INCAR << aurostd::PaddedPOST("METAGGA=MS0",_incarpad_) << "# METAGGA = MS0  " << endl;
          if(vflags.KBIN_VASP_FORCE_OPTION_METAGGA.scheme=="MS1") xvasp.INCAR << aurostd::PaddedPOST("METAGGA=MS1",_incarpad_) << "# METAGGA = MS1  " << endl;
          if(vflags.KBIN_VASP_FORCE_OPTION_METAGGA.scheme=="MS2") xvasp.INCAR << aurostd::PaddedPOST("METAGGA=MS2",_incarpad_) << "# METAGGA = MS2  " << endl;
          if(vflags.KBIN_VASP_FORCE_OPTION_METAGGA.scheme=="NONE") xvasp.INCAR << aurostd::PaddedPOST("METAGGA=NONE",_incarpad_) << "# METAGGA = NONE  " << endl;
          if(vflags.KBIN_VASP_INCAR_VERBOSE) xvasp.INCAR << "# Performing Metagga METAGGA=" << vflags.KBIN_VASP_FORCE_OPTION_METAGGA.scheme << " [AFLOW] end " << endl;
        }
      }
    }
    ...
    namespace KBIN {
      void XVASP_INCAR_Ivdw(_xvasp& xvasp,_vflags& vflags) {        // AFLOW_FUNCTION_IMPLEMENTATION   number_for_VASP_see_manual_for_IVDW | 0
        ...
        // xvasp.INCAR << endl;
        if(vflags.KBIN_VASP_FORCE_OPTION_IVDW.scheme!="0") {
          if(vflags.KBIN_VASP_INCAR_VERBOSE) xvasp.INCAR << "# Performing Ivdw IVDW=" << vflags.KBIN_VASP_FORCE_OPTION_IVDW.scheme << " [AFLOW] begin " << endl;
          if(vflags.KBIN_VASP_FORCE_OPTION_IVDW.scheme=="0" || vflags.KBIN_VASP_FORCE_OPTION_IVDW.scheme=="OFF" ||
              vflags.KBIN_VASP_FORCE_OPTION_IVDW.scheme=="NONE" || vflags.KBIN_VASP_FORCE_OPTION_IVDW.scheme=="") {
            xvasp.INCAR << aurostd::PaddedPOST("#IVDW=0",_incarpad_) << "# IVDW = 0  " << endl;
          } else {	
            xvasp.INCAR << aurostd::PaddedPOST("IVDW="+vflags.KBIN_VASP_FORCE_OPTION_IVDW.scheme,_incarpad_) << "# IVDW = " << vflags.KBIN_VASP_FORCE_OPTION_IVDW.scheme << "  " << endl;
          }
          if(vflags.KBIN_VASP_INCAR_VERBOSE) xvasp.INCAR << "# Performing Ivdw IVDW=" << vflags.KBIN_VASP_FORCE_OPTION_IVDW.scheme << " [AFLOW] end " << endl;
        }
      }
    }
    ...
    namespace KBIN {
      void XVASP_INCAR_ABMIX(_xvasp& xvasp,_vflags& vflags) {        // AFLOW_FUNCTION_IMPLEMENTATION
        ...
        for(int i=1;i<=imax;i++) {
          strline=aurostd::GetLineString(FileContent,i);
          if(aurostd::substring2bool(strline,"AMAX",TRUE) || aurostd::substring2bool(strline,"#AMAX",TRUE) ||
              aurostd::substring2bool(strline,"BMAX",TRUE) || aurostd::substring2bool(strline,"#BMAX",TRUE)) {
        ...
        if(vflags.KBIN_VASP_FORCE_OPTION_ABMIX.scheme.at(0)=='A') { // AUTO
          // needs to check PPs
        ...
        if(vflags.KBIN_VASP_FORCE_OPTION_ABMIX.scheme=="US" || isUS) { // US
        ...
        if(vflags.KBIN_VASP_FORCE_OPTION_ABMIX.scheme=="PAW" || isPAW) { // PAW
    ...
    namespace KBIN {
      bool XVASP_INCAR_PREPARE_GENERIC(string command,_xvasp& xvasp,_vflags& vflags,string svalue,int ivalue,double dvalue,bool OPTION) {
        ...
        if(command=="ALGO") {
          for(int i=1;i<=imax;i++) {
            ...
            if(aurostd::substring2bool(strline,"ALGO",TRUE) || aurostd::substring2bool(strline,"#ALGO",TRUE) ||
                aurostd::substring2bool(strline,"IALGO",TRUE) || aurostd::substring2bool(strline,"#IALGO",TRUE)) {
              if(vflags.KBIN_VASP_INCAR_VERBOSE) {
                if(vflags.KBIN_VASP_FORCE_OPTION_ALGO.scheme=="NORMAL") xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR ALGO=NORMAL) " << endl;
                if(vflags.KBIN_VASP_FORCE_OPTION_ALGO.scheme=="VERYFAST") xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR ALGO=VERYFAST) " << endl;
                if(vflags.KBIN_VASP_FORCE_OPTION_ALGO.scheme=="FAST") xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR ALGO=FAST) " << endl;
                if(vflags.KBIN_VASP_FORCE_OPTION_ALGO.scheme=="ALL") xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR ALGO=ALL) " << endl;
                if(vflags.KBIN_VASP_FORCE_OPTION_ALGO.scheme=="DAMPED") xvasp.INCAR << "# " << strline << " # AFLOW REMOVED (KBIN::XVASP_INCAR ALGO=DAMPED) " << endl;
              }
            } else {
              if(!vflags.KBIN_VASP_INCAR_VERBOSE && strline.length()) xvasp.INCAR << strline << endl;
              if(vflags.KBIN_VASP_INCAR_VERBOSE) xvasp.INCAR << strline << endl;
          ...
          // xvasp.INCAR << endl;
          if(vflags.KBIN_VASP_INCAR_VERBOSE) xvasp.INCAR << "# Performing Algo ALGO=" << vflags.KBIN_VASP_FORCE_OPTION_ALGO.scheme << " [AFLOW] begin " << endl;

          if(vflags.KBIN_VASP_FORCE_OPTION_ALGO.scheme=="NORMAL") xvasp.INCAR << aurostd::PaddedPOST("ALGO=Normal",_incarpad_) << "# ALGO = Normal: will select, IALGO=38 (blocked Davidson block iteration scheme)" << endl;
          if(vflags.KBIN_VASP_FORCE_OPTION_ALGO.scheme=="VERYFAST") xvasp.INCAR << aurostd::PaddedPOST("ALGO=Veryfast",_incarpad_) << "# ALGO = Very_Fast: will select IALGO=48 (RMM-DIIS)" << endl;
          if(vflags.KBIN_VASP_FORCE_OPTION_ALGO.scheme=="FAST") xvasp.INCAR << aurostd::PaddedPOST("ALGO=Fast",_incarpad_) << "# ALGO = Fast: IALGO=38 is used for the initial phase, and then VASP switches to IALGO=48" << endl;
          if(vflags.KBIN_VASP_FORCE_OPTION_ALGO.scheme=="ALL") xvasp.INCAR << aurostd::PaddedPOST("ALGO=All",_incarpad_) << "# ALGO = All: The all band simultaneous update of wavefunctions can be selected using (IALGO=58)" << endl;
          if(vflags.KBIN_VASP_FORCE_OPTION_ALGO.scheme=="DAMPED") xvasp.INCAR << aurostd::PaddedPOST("ALGO=Damped",_incarpad_) << "# ALGO = Damped: A damped velocity friction algorithm is selected (IALGO=53)" << endl;
          if(vflags.KBIN_VASP_INCAR_VERBOSE) xvasp.INCAR << "# Performing Algo ALGO=" << vflags.KBIN_VASP_FORCE_OPTION_ALGO.scheme << " [AFLOW] end " << endl;
          DONE=TRUE;
        }
        ...
        if(command=="ENMAX_MULTIPLY") {
          ...
          // xvasp.INCAR << endl;     
          if(vflags.KBIN_VASP_INCAR_VERBOSE) xvasp.INCAR << "# Performing ENMAX_MULTIPLY=" << vflags.KBIN_VASP_FORCE_OPTION_PREC.scheme << " [AFLOW] begin " << endl;
          ...
          if(vflags.KBIN_VASP_INCAR_VERBOSE) xvasp.INCAR << "# Performing ENMAX_MULTIPLY=" << vflags.KBIN_VASP_FORCE_OPTION_PREC.scheme << " [AFLOW] end " << endl;
          //  cerr << xvasp.INCAR.str() << endl; exit(0);
          DONE=TRUE;
        }
      ...
      namespace KBIN {
        void XVASP_KPOINTS_IBZKPT_UPDATE(_xvasp& xvasp,_aflags &aflags,_vflags& vflags,int step,ofstream &FileMESSAGE) {        // AFLOW_FUNCTION_IMPLEMENTATION
          ...
          // aus << "00000  MESSAGE IBZKPT.relax1 - vflags.KBIN_VASP_FORCE_OPTION_KPOINTS.isscheme("IBZKPT")=" << vflags.KBIN_VASP_FORCE_OPTION_KPOINTS.isscheme("IBZKPT") << " " << Message(aflags,"user,host,time") << endl;
          // aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
          if(step>=1 && vflags.KBIN_VASP_FORCE_OPTION_KPOINTS.isscheme("IBZKPT")) {
      ...
      namespace KBIN {
        void XVASP_INCAR_KPOINTS_Dielectric_SET(_xvasp& xvasp,_kflags &kflags,_vflags& vflags,string svalue) {        // AFLOW_FUNCTION_IMPLEMENTATION
          ...
          if(svalue=="DYNAMIC" || svalue=="dynamic") {
            ...
            // [OBSOLETE] aurostd::string2tokensAdd(aurostd::execute2string("cat "+xvasp.Directory+"/OUTCAR.dielectric_static | grep NBANDS="),nbands_tokens,"=");
      ...
      namespace KBIN {
        void XVASP_Afix_NBANDS(_xvasp& xvasp,int& nbands,bool VERBOSE) {
          ...
          aurostd::execute(aus_exec);
          // [OBSOLETE] aus_exec << "cd " << xvasp.Directory << endl;
          // [OBSOLETE] aus_exec << " cat OUTCAR | grep NBANDS | grep number > aflow.tmp " << endl;
          // [OBSOLETE] aurostd::execute(aus_exec);
          // [OBSOLETE] FileNameAUS=xvasp.Directory+"/aflow.tmp";
          // [OBSOLETE] FileAUS.open(FileNameAUS.c_str(),std::ios::in);
          // [OBSOLETE] FileAUS.getline(buffer,BUFFER_MAXLEN,'\n');
          // [OBSOLETE] FileAUS.clear();FileAUS.close();
          // [OBSOLETE] word=buffer;
          nbands=0;
          xOUTCAR OUTCAR_NBANDS(xvasp.Directory+"/OUTCAR");
          nbands=OUTCAR_NBANDS.NBANDS;
          // [OBSOLETE] if(aurostd::substring2bool(word,"NBANDS")) {  // GET BANDS FROM OUTCAR
          // [OBSOLETE]  nbands=atoi(word.substr(word.find("NBANDS")+7).c_str());
          // [OBSOLETE] } else {                                     // GET BANDS FROM INCAR
        ...
        namespace KBIN {
          double XVASP_Afix_GENERIC(string mode,_xvasp& xvasp,_kflags& kflags,_vflags& vflags,double param_double,int param_int) {
            ...
            if(mode=="BRMIX") {
              ...
              reload_incar=TRUE;
              vflags.KBIN_VASP_FORCE_OPTION_ALGO.scheme="VERYFAST";
              KBIN::XVASP_INCAR_PREPARE_GENERIC("ALGO",xvasp,vflags,"",0,0.0,FALSE);
            ...
            if(mode=="CSLOSHING") {
              ...
              reload_incar=TRUE;
              vflags.KBIN_VASP_FORCE_OPTION_ALGO.scheme="VERYFAST";
              vflags.KBIN_VASP_FORCE_OPTION_ALGO.scheme="NORMAL";
              KBIN::XVASP_INCAR_PREPARE_GENERIC("ALGO",xvasp,vflags,"",0,0.0,FALSE);
              // [OBSOLETE] KBIN::XVASP_INCAR_ALGO(xvasp,vflags);
              aurostd::stringstream2file(xvasp.INCAR,string(xvasp.Directory+"/INCAR"));
              // fix aflowlin
        ...
        namespace KBIN {
          vector<string> ExtractAtomicSpecies(string directory) {
            ...
            ///////////////////////////////////////////////////////////////////////////////////////////////
            //READ POSCAR.orig
            string poscarfile_tmp=aurostd::TmpFileCreate("POSCAR.tmp");
            string outcarfile_tmp=aurostd::TmpFileCreate("OUTCAR.tmp");

            //GET its lattice vectors and reciprocal lattice vectors
            if(aurostd::FileExist(directory+"/POSCAR.bands")) POSCARfile = directory + "/POSCAR.bands";
            else if(aurostd::FileExist(directory+"/POSCAR.static")) POSCARfile = directory + "/POSCAR.static";
            else if(aurostd::FileExist(directory+"/POSCAR.relax1")) POSCARfile = directory + "/POSCAR.relax1";
            else if(aurostd::FileExist(directory+"/POSCAR")) POSCARfile = directory + "/POSCAR";
            else if(aurostd::FileExist(directory+"/POSCAR.bands.bz2")) {
              aurostd::execute(XHOST.command("bzcat")+" "+directory+"/POSCAR.bands.bz2 > " + poscarfile_tmp);
              POSCARfile = poscarfile_tmp;
            }
            else if(aurostd::FileExist(directory+"/POSCAR.static.bz2")) {
              aurostd::execute(XHOST.command("bzcat")+" "+directory+"/POSCAR.static.bz2 > " + poscarfile_tmp);
              POSCARfile = poscarfile_tmp;
            }
            else if(aurostd::FileExist(directory+"/POSCAR.relax1.bz2")) {
              aurostd::execute(XHOST.command("bzcat")+" "+directory+"/POSCAR.relax1.bz2 > " + poscarfile_tmp);
              POSCARfile = poscarfile_tmp;
            }
            else {
              cerr <<"Any types of POSCAR are not found in the directory, aborting." << endl;
              exit(0);
            }
            ...
            for(unsigned int i=0; i<xstr_name.species.size(); i++) {
              if(xstr_name.species.at(i).compare("")!=0) {
                ...
                if(aurostd::FileExist(directory+"/OUTCAR")) OUTCARfile = directory + "/OUTCAR";
                else if(aurostd::FileExist(directory+"/OUTCAR.static")) OUTCARfile = directory + "/OUTCAR.static";
                else if(aurostd::FileExist(directory+"/OUTCAR.static.bz2")) {
                  aurostd::execute(XHOST.command("bzcat")+" "+directory+"/OUTCAR.static.bz2 > " + outcarfile_tmp);
                  OUTCARfile = outcarfile_tmp;
                }
                else {
                  cerr <<"OUTCAR or OUTCAR.static or OUTCAR.static.bz2 are not found in the directory, aborting." << endl;
                  exit(0);
                }
            ...
            if(aurostd::FileExist(poscarfile_tmp)) aurostd::RemoveFile(poscarfile_tmp);
            if(aurostd::FileExist(outcarfile_tmp)) aurostd::RemoveFile(outcarfile_tmp);
        ...
        namespace KBIN {
          string ExtractSystemName(string _directory) {
            ...
            if(!found && aurostd::FileExist(directory+"/DOSCAR")) {
              DOSCARfile = directory + "/DOSCAR"; found=TRUE;}
            if(!found && aurostd::FileExist(directory+"/DOSCAR.static")) {
              DOSCARfile = directory + "/DOSCAR.static"; found=TRUE;}
            if(!found && aurostd::FileExist(directory+"/DOSCAR.static.bz2")) {
              aurostd::execute(XHOST.command("bzcat")+" "+directory+"/DOSCAR.static.bz2 > " + doscarfile_tmp); DOSCARfile = doscarfile_tmp;found=TRUE;} 
            ...
            if(found) {
              ...
              if(FLAG_MAGNETIC) return SystemName;
              else {
                vector<string> data;
                ...
                if((SystemName.find("ICSD")==string::npos)) {
                  if(datasize >=2) {
                    if(FLAG_LAST_ENTRY_BANDS) {
                      if(FLAG_SYSTEMNAME_UNKNOWN && FLAG_DIRECTORY_ICSD) SystemName = data.at(datasize-2);
                      else SystemName = data.at(datasize-3)+"."+data.at(datasize-2);

                    }
                    else {
                      if(FLAG_SYSTEMNAME_UNKNOWN && FLAG_DIRECTORY_ICSD) SystemName = Last_Entry;
                      else SystemName=data.at(datasize-2)+"."+data.back();
                    }
                  }
                  else SystemName=data.back();
                }
                return SystemName;
              }
            }
            ...
            // TRY OUTCAR.relax1
            if(!found && aurostd::FileExist(directory+"/OUTCAR.relax1.bz2")) {
              found=TRUE;
              xOUTCAR outcar(directory+"/OUTCAR.relax1.bz2");
              SystemName=outcar.SYSTEM;
              //     cerr << outcar.SYSTEM << endl; exit(0);
              return SystemName;
            }

            // NOTHING FOUND !!!
            if(!found) {
              cerr <<"KBIN::ExtractSystemName: DOSCAR[ | .static | .static.bz2], OUTCAR.relax1.bz2 are not found in the directory, aborting." << endl;
              exit(0);
            }
            return "";
          }
