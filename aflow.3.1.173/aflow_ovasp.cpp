...
xOUTCAR::xOUTCAR() {
  ...
  LSCOUPLING=FALSE;             // for aflowlib_libraries.cpp
...
bool xOUTCAR::GetProperties(const stringstream& stringstreamIN,bool QUIET) {
  ...
  while(getline(sss,line)) {
    ...
    if(line.find(anchor_word_LSORBIT) !=string::npos) {
      ...
      if(LSlabel == "T") LSCOUPLING=TRUE;
      if(LSlabel == "F") LSCOUPLING=FALSE;
    }
  ...
  if(LVERBOSE) cerr << "xOUTCAR::GetProperties: LSCOUPLING=" << LSCOUPLING << endl; //exit(0);
...
bool xOUTCAR::identicalKPoints(xvector<double>& kpoint1,xvector<double>& kpoint2,double tol){
  if(identical(kpoint1,kpoint2,tol)){return true;}  //we can be dam sure at this tolerance
  return false;
}
...
bool xOUTCAR::GetBandGap(double EFERMI,double efermi_tol,double energy_tol,double occ_tol) { //CO 171004 - redoing camilo's function more robustly
  ...
  double max_VBT,min_CBB;
  double dist,dist_min;
  uint imax_VBT,imin_CBB; //absolutes, then our final picks
...
bool is_equal_position_kEn_str(const kEn_st & k1, const kEn_st & k2) {
  const double _ENER_EPS=1.0e-4;
  return isequal(k1.kpoint, k2.kpoint, _ENER_EPS);
}
...
bool PrintBandGap(string& WorkDir, ostream &oss) {
  ...
  char LastChar = *WorkDir.rbegin();
  if(LastChar == '/') WorkDir.erase(WorkDir.size()-1);

  // OUTCAR_bands
  if(!aurostd::EFileExist(WorkDir + "/OUTCAR.bands",path_outcar_bands)){
    oss << "WARNING - OUTCAR.bands not found here: " << WorkDir << endl;
    oss << endl;
    return FALSE;
  }
  aurostd::efile2stringstream(path_outcar_bands, ss_outcar_bands);
  ...
  if(!xoutcar_bands.GetProperties(ss_outcar_bands)){
    oss << "WARNING - " << xoutcar_bands.ERROR << endl << "   filename=[" << path_outcar_bands << "]" << endl;
    oss << endl;
    return FALSE;
  }
  //try to grab xstr from OUTCAR
  if(!xoutcar_bands.GetXStructure()){
    //try grabbing POSCAR first - START
    if(!aurostd::EFileExist(WorkDir + "/POSCAR.bands",Path_Poscar)) {
      oss << "WARNING - " << xoutcar_bands.ERROR << endl << "   filename=[" << path_outcar_bands << "]" << endl;
      oss << endl;
    return FALSE;
  }
  ...
  if(!aurostd::EFileExist(WorkDir + "/OUTCAR.static",path_outcar_static)){
    oss << "WARNING - OUTCAR.static not found here: " << WorkDir << endl;
    oss << "WARNING - Defaulting E-Fermi to that found in OUTCAR.bands" << endl;
    oss << endl;
    //return FALSE;
  }else{
    aurostd::efile2stringstream(path_outcar_static, ss_outcar_static);
    if(!xoutcar_static.GetProperties(ss_outcar_static)){
      oss << "WARNING - " << xoutcar_static.ERROR << endl << "   filename=[" << path_outcar_static << "]" << endl;
      oss << "WARNING - Defaulting E-Fermi to that found in OUTCAR.bands" << endl;
      oss << endl;
      //return FALSE;
    }else{
...
bool PrintEffectiveMass(string& WorkDir, ostream &oss) {
  ...
  bool SPIN_UP = FALSE, SPIN_DN = FALSE;
  bool EM_TAG;
  string Path_Outcar, Path_Doscar, Path_Eigenval, Path_Poscar;
  ...
  if(LastChar == '/') WorkDir.erase(WorkDir.size()-1);
  // OUTCAR
  if(aurostd::FileExist(WorkDir + "/OUTCAR.static.bz2")) {
    Path_Outcar = WorkDir + "/OUTCAR.static.bz2"; // bz2
    aurostd::efile2stringstream(Path_Outcar, File_Outcar);
  }   
  else if(aurostd::FileExist(WorkDir + "/OUTCAR.static")) {
    Path_Outcar = WorkDir + "/OUTCAR.static"; // plain text
    aurostd::file2stringstream(Path_Outcar, File_Outcar);
  }
  else return FALSE;
  // DOSCAR
  if(aurostd::FileExist(WorkDir + "/DOSCAR.static.bz2")) {
    Path_Doscar = WorkDir + "/DOSCAR.static.bz2"; // bz2
    aurostd::efile2stringstream(Path_Doscar, File_Doscar);
  }
  else if(aurostd::FileExist(WorkDir + "/DOSCAR.static")) {
    Path_Doscar = WorkDir + "/DOSCAR.static"; // plain text
    aurostd::file2stringstream(Path_Doscar, File_Doscar);
  }
  else return FALSE;
  // EIGENVAL
  if(aurostd::FileExist(WorkDir   + "/EIGENVAL.static.bz2")) {
    Path_Eigenval = WorkDir + "/EIGENVAL.static.bz2"; // bz2
    aurostd::efile2stringstream(Path_Eigenval, File_Eigenval);
  }
  else if(aurostd::FileExist(WorkDir + "/EIGENVAL.static")) {
    Path_Eigenval = WorkDir + "/EIGENVAL.static"; // plain text
    aurostd::file2stringstream(Path_Eigenval, File_Eigenval);
  }
  else return FALSE;
  // POSCAR
  if(aurostd::FileExist(WorkDir + "/POSCAR.bands.bz2")) {
    Path_Poscar = WorkDir + "/POSCAR.bands.bz2"; // bz2
  }
  else if(aurostd::FileExist(WorkDir + "/POSCAR.bands")) {
    Path_Poscar = WorkDir + "/POSCAR.bands"; // plain text
  }
  else return FALSE;
  // GET THE DATA
...
bool PrintEigCurv(string& WorkDir, ostream &oss) {
  ...
  string Path_Eigenval, Path_Kpoints, Path_Outcar, Path_Poscar;
  double _ZERO=0.0 ;
  int GRIDS ;
  xEIGENVAL xeigenval ;
  xOUTCAR   xoutcar ;
  ...
  if(LastChar == '/') WorkDir.erase(WorkDir.size()-1);
  //EIGENVAL
  if(aurostd::FileExist(WorkDir   + "/EIGENVAL.bands.bz2")) {
    Path_Eigenval = WorkDir + "/EIGENVAL.bands.bz2"; // bz2
    aurostd::efile2stringstream(Path_Eigenval, File_Eigenval);
  } else if(aurostd::FileExist(WorkDir + "/EIGENVAL.bands")) {
    Path_Eigenval = WorkDir + "/EIGENVAL.bands"; // plain text
    aurostd::file2stringstream(Path_Eigenval, File_Eigenval);
  } else {
    return FALSE ;
  }
  // KPOINTS
  if(aurostd::FileExist(WorkDir   + "/KPOINTS.bands.bz2")) {
    Path_Kpoints  = WorkDir + "/KPOINTS.bands.bz2"; // bz2
    aurostd::efile2stringstream(Path_Kpoints, File_Kpoints);
  } else if(aurostd::FileExist(WorkDir + "/KPOINTS.bands")) {
    Path_Kpoints = WorkDir + "/KPOINTS.bands"; // plain text
    aurostd::file2stringstream(Path_Kpoints, File_Kpoints);
  } else {
    return FALSE ;
  }
  // OUTCAR
  if(aurostd::FileExist(WorkDir + "/OUTCAR.bands.bz2")) {
    Path_Outcar = WorkDir + "/OUTCAR.bands.bz2"; // bz2
    aurostd::efile2stringstream(Path_Outcar, File_Outcar);
  } else if(aurostd::FileExist(WorkDir + "/OUTCAR.bands")) {  
    Path_Outcar = WorkDir + "/OUTCAR.bands"; // plain text
    aurostd::file2stringstream(Path_Outcar, File_Outcar);
  } else {
    return FALSE ;
  }
...
bool xCHGCAR::GetProperties(const stringstream& stringstreamIN,bool QUIET) {
  ...
  xtensor3<double> tvalues_aus(grid(1),grid(2),grid(3));
  int iii=0;
  for(int i3=1;i3<=grid(3);i3++) {  //CO - x is fastest, z is slowest
	...
	iii++;
	tvalues_aus(i1,i2,i3)=vvalues(iii);
      }
    }
  }
  
  tvalues=tvalues_aus;
  if(LVERBOSE) cerr << "xCHGCAR::GetProperties: tvalues.index[1]=" << tvalues.index[1] << endl;
  if(LVERBOSE) cerr << "xCHGCAR::GetProperties: tvalues.index[2]=" << tvalues.index[2] << endl;
  if(LVERBOSE) cerr << "xCHGCAR::GetProperties: tvalues.index[3]=" << tvalues.index[3] << endl;
  // ----------------------------------------------------------------------   
  // ----------------------------------------------------------------------
  // DONE NOW RETURN  
