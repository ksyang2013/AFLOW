...
/*
   ...
   bool EnthalpyReference(string pp,string type,deque<double> LDAU,string& gs,double& enthalpy_atom,double& volume_atom,double& spin_atom, ostream& oss);
...
namespace aflowlib {
  uint GREP_Species_ALL(vector<string> vspecies,                         // IN  [0,nspecies[ the species Ag,Cd,...   nspecies=number of these items    nspecies=naries
    ...
    if(vspecies_pp.size()==0 || vspecies.size()!=vspecies_pp.size())  { // generate some default ones
      vspecies_pp.clear(); for(uint i=0;i<nspecies;i++)vspecies_pp.push_back(AVASP_Get_PseudoPotential_PAWPBE(vspecies.at(i)));
    }
    for(uint i=0;i<nspecies;i++)
      if(vspecies_pp.at(i).empty()) vspecies_pp.at(i)=AVASP_Get_PseudoPotential_PAWPBE(vspecies.at(i)); // in the event that I pass an empty string
  ...
  namespace aflowlib {
    ...
    namespace aflowlib {
      bool LIBS_EFormation(string options) {
        ...
        for(uint i=0;i<vspecies.size();i++) {
          ...
          vspecies_pp.push_back("");//AVASP_Get_PseudoPotential_PAWPBE(vspecies.at(i)));
          ...
          species_pp=AVASP_Get_PseudoPotential_PAWPBE(species);
    ...
    namespace aflowlib {
      bool LIB2RAW_ALL(string options,bool flag_FORCE) {
        ...
        int multish_value=XHOST.CPU_Cores;
        if(XHOST.vflag_control.flag("XPLUG_NUM_THREADS")) multish_value=aurostd::string2utype<int>(XHOST.vflag_control.getattachedscheme("XPLUG_NUM_THREADS"));

        cerr << "multish_value=" << multish_value << endl;
        //    exit(0);
        deque<string> dcmds;
	   ...
	   PROJECT_LIBRARY==vAFLOW_PROJECTS_DIRECTORIES.at(XHOST_LIBRARY_LIB9)) {
          ...
          if(multish_value==0 || multish_value==1) {
            ...
            // DO MULTI
            aurostd::multithread_execute(dcmds,multish_value,FALSE);
          }
          return TRUE;
        }
    ...
    namespace aflowlib {
      uint GetSpeciesDirectory(string directory,vector<string>& vspecies) {
        ...
        // check _AFLOWIN_ first
        if(!vspecies.size() && aurostd::FileExist(directory+"/"+_AFLOWIN_)) {
          aurostd::string2vectorstring(aurostd::execute2string("cat "+directory+"/" + _AFLOWIN_ + " | grep VASP_POTCAR_FILE"),vspecies);
        ...
        // check _AFLOWIN_.bz2
        if(!vspecies.size() && aurostd::FileExist(directory+"/" + _AFLOWIN_ + ".bz2")) {
          aurostd::string2vectorstring(aurostd::execute2string(XHOST.command("bzcat")+" "+directory+"/" + _AFLOWIN_ + ".bz2 | grep VASP_POTCAR_FILE"),vspecies);
          for(uint i=0;i<vspecies.size();i++) aurostd::StringSubst(vspecies.at(i),"[VASP_POTCAR_FILE]","");
          for(uint i=0;i<vspecies.size();i++) KBIN::VASP_PseudoPotential_CleanName(vspecies.at(i));
        }
        //  // check POSCAR.bands first
        //  if(!vspecies.size() && aurostd::FileExist(directory+"/POSCAR.bands")) {
        //   oss.str(aurostd::execute2string("cat "+directory+"/POSCAR.bands"));
        ...
        //  }
        // check POSCAR.orig first
        if(!vspecies.size() && aurostd::FileExist(directory+"/POSCAR.orig")) {
          oss.str(aurostd::execute2string("cat "+directory+"/POSCAR.orig"));
          xstructure xstr(oss,IOVASP_POSCAR);
          if(xstr.species.size()>0) if(xstr.species.at(0)!="") for(uint i=0;i<xstr.species.size();i++) vspecies.push_back(xstr.species.at(i)); // dont change order
        }
        // check POSCAR.relax1 first
        if(!vspecies.size() && aurostd::FileExist(directory+"/POSCAR.relax1")) {
          oss.str(aurostd::execute2string("cat "+directory+"/POSCAR.relax1"));
          xstructure xstr(oss,IOVASP_POSCAR);
          if(xstr.species.size()>0) if(xstr.species.at(0)!="") for(uint i=0;i<xstr.species.size();i++) vspecies.push_back(xstr.species.at(i)); // dont change order
        }
        // check POSCAR.bands.bz2 first
        if(!vspecies.size() && aurostd::FileExist(directory+"/POSCAR.bands.bz2")) {
          oss.str(aurostd::execute2string(XHOST.command("bzcat")+" "+directory+"/POSCAR.bands.bz2"));
          xstructure xstr(oss,IOVASP_POSCAR);
          if(xstr.species.size()>0) if(xstr.species.at(0)!="") for(uint i=0;i<xstr.species.size();i++) vspecies.push_back(xstr.species.at(i)); // dont change order
        }
        // check POSCAR.orig.bz2 first
        if(!vspecies.size() && aurostd::FileExist(directory+"/POSCAR.orig.bz2")) {
          oss.str(aurostd::execute2string(XHOST.command("bzcat")+" "+directory+"/POSCAR.orig.bz2"));
          xstructure xstr(oss,IOVASP_POSCAR);
          if(xstr.species.size()>0) if(xstr.species.at(0)!="") for(uint i=0;i<xstr.species.size();i++) vspecies.push_back(xstr.species.at(i)); // dont change order
        }
        // check POSCAR.relax1.bz2 first
        if(!vspecies.size() && aurostd::FileExist(directory+"/POSCAR.relax1.bz2")) {
          oss.str(aurostd::execute2string(XHOST.command("bzcat")+" "+directory+"/POSCAR.relax1.bz2"));
          xstructure xstr(oss,IOVASP_POSCAR);
          if(xstr.species.size()>0) if(xstr.species.at(0)!="") for(uint i=0;i<xstr.species.size();i++) vspecies.push_back(xstr.species.at(i)); // dont change order
        }
        // check OUTCAR.relax1 first
        if(!vspecies.size() && aurostd::FileExist(directory+"/OUTCAR.relax1")) {
          aurostd::string2vectorstring(aurostd::execute2string("cat "+directory+"/OUTCAR.relax1 | grep TITEL"),vs);
        ...
        // check OUTCAR.static first
        if(!vspecies.size() && aurostd::FileExist(directory+"/OUTCAR.static")) {
          aurostd::string2vectorstring(aurostd::execute2string("cat "+directory+"/OUTCAR.static | grep TITEL"),vs);
          for(uint i=0;i<vs.size();i++) {aurostd::string2tokens(vs.at(i),tokens," ");vspecies.push_back(KBIN::VASP_PseudoPotential_CleanName(tokens.at(3)));}
        }
        // check OUTCAR.relax1.bz2 first
        if(!vspecies.size() && aurostd::FileExist(directory+"/OUTCAR.relax1.bz2")) {
          aurostd::string2vectorstring(aurostd::execute2string(XHOST.command("bzcat")+" "+directory+"/OUTCAR.relax1.bz2 | grep TITEL"),vs);
          for(uint i=0;i<vs.size();i++) {aurostd::string2tokens(vs.at(i),tokens," ");vspecies.push_back(KBIN::VASP_PseudoPotential_CleanName(tokens.at(3)));}
        }
        // check OUTCAR.static.bz2 first
        if(!vspecies.size() && aurostd::FileExist(directory+"/OUTCAR.static.bz2")) {
          aurostd::string2vectorstring(aurostd::execute2string(XHOST.command("bzcat")+" "+directory+"/OUTCAR.static.bz2 | grep TITEL"),vs);
          for(uint i=0;i<vs.size();i++) {aurostd::string2tokens(vs.at(i),tokens," ");vspecies.push_back(KBIN::VASP_PseudoPotential_CleanName(tokens.at(3)));}
        }
      ...
      bool LIB2RAW(string options,bool flag_FORCE,bool LOCAL) {
        ...
        if(tokens.size()>=1) {
          if(tokens.at(0)=="all") {
            ...
            XHOST.vflag_pflow.flag("MULTISH",FALSE);
        ...
        if(LOCAL){
          ...
          directory_RAW=directory_LIB+"/RAW";
          directory_WEB=directory_LIB+"/WEB";
	     ...
	     PROJECT_LIBRARY==vAFLOW_PROJECTS_DIRECTORIES.at(XHOST_LIBRARY_LIB2)) {
            directory_LIB=directory;directory_WEB="";
          }
          if(PROJECT_LIBRARY==vAFLOW_PROJECTS_DIRECTORIES.at(XHOST_LIBRARY_LIB3) ||
	     PROJECT_LIBRARY==vAFLOW_PROJECTS_DIRECTORIES.at(XHOST_LIBRARY_LIB4) ||
	     PROJECT_LIBRARY==vAFLOW_PROJECTS_DIRECTORIES.at(XHOST_LIBRARY_LIB5) ||
	     PROJECT_LIBRARY==vAFLOW_PROJECTS_DIRECTORIES.at(XHOST_LIBRARY_LIB6) ||
	     PROJECT_LIBRARY==vAFLOW_PROJECTS_DIRECTORIES.at(XHOST_LIBRARY_LIB7) ||
	     PROJECT_LIBRARY==vAFLOW_PROJECTS_DIRECTORIES.at(XHOST_LIBRARY_LIB8) ||
	     PROJECT_LIBRARY==vAFLOW_PROJECTS_DIRECTORIES.at(XHOST_LIBRARY_LIB9)) {
            directory_LIB=directory;directory_WEB=directory;    
          }
          ...
          directory_RAW=aurostd::CleanFileName(PROJECT_LIBRARY+"/RAW/"+directory_RAW);
        ...
        if(aurostd::FileExist(directory_LIB+"/OUTCAR.relax1.bz2") ||
            aurostd::FileExist(directory_LIB+"/OUTCAR.relax2.bz2") ||
            aurostd::FileExist(directory_LIB+"/OUTCAR.relax3.bz2")) perform_THERMODYNAMICS=TRUE;
        if(aurostd::FileExist(directory_LIB+"/OUTCAR.static.bz2")) perform_MAGNETIC=TRUE;
        if(aurostd::FileExist(directory_LIB+"/OUTCAR.bands.bz2")) perform_BANDS=TRUE;
        if(aurostd::FileExist(directory_LIB+"/AECCAR0.static.bz2") && aurostd::FileExist(directory_LIB+"/AECCAR2.static.bz2")) perform_BADER=TRUE;
        if(aurostd::FileExist(directory_LIB+"/aflow.agl.out.bz2")) perform_AGL=TRUE;
        if(aurostd::FileExist(directory_LIB+"/aflow.ael.out.bz2")) perform_AEL=TRUE;
        ...
        if(flag_FORCE==FALSE) {
          if(aurostd::FileExist(directory_RAW+"/"+_FILE_AFLOWLIB_ENTRY_OUT_)) {  // directory_RAW+"/"+_AFLOWIN_)
            cout << "aflowlib::LIB2RAW: ALREADY CALCULATED = " << directory_RAW << endl;
            return FALSE;
          }
          if(perform_BANDS) {
            if(aurostd::FileExist(directory_RAW+"/EIGENVAL.bands") ||
                aurostd::FileExist(directory_RAW+"/EIGENVAL.bands.bz2") ||
                aurostd::FileExist(directory_RAW+"/EIGENVAL.bands.gz")) {
              // return FALSE;
        ...
        if((perform_THERMODYNAMICS || perform_BANDS ||  perform_MAGNETIC)) {
          ...
          vector<string> vfiles;   // the needed files
          ...
          if(perform_THERMODYNAMICS) {
            ...
            aflowlib::LIB2RAW_Loop_Thermodynamics(directory_LIB,directory_RAW,vfiles,aflowlib_data,"aflowlib::LIB2RAW (thermodynamics):",LOCAL);	// identifier inside
          }
          // ---------------------------------------------------------------------------------------------------------------------------------  
          // do the BANDS
          if(perform_BANDS) {
            cout << "aflowlib::LIB2RAW: BANDS LOOP ---------------------------------------------------------------------------------" << endl;
            aflowlib::LIB2RAW_Loop_Bands(directory_LIB,directory_RAW,vfiles,aflowlib_data,"aflowlib::LIB2RAW (bands):");
            // MOVE/LINK PICS data
          }
          // ---------------------------------------------------------------------------------------------------------------------------------  
          // do the MAGNETIC
          if((perform_MAGNETIC || perform_BANDS)) { // JUNKAI
            ...
            aflowlib::LIB2RAW_Loop_Magnetic(directory_LIB,directory_RAW,vfiles,aflowlib_data,"aflowlib::LIB2RAW (magnetic):");	
          }
          // ---------------------------------------------------------------------------------------------------------------------------------  
          // do the BADER
          if(perform_BADER) {
            cout << "aflowlib::LIB2RAW: BADER LOOP ---------------------------------------------------------------------------------" << endl;
            aflowlib::LIB2RAW_Loop_Bader(directory_LIB,directory_RAW,vfiles,aflowlib_data,"aflowlib::LIB2RAW (bader):");
            // MOVE/LINK PICS data
            if(flag_WEB) {
          ...
          if(perform_AGL) {
            ...
            aflowlib::LIB2RAW_Loop_AGL(directory_LIB,directory_RAW,vfiles,aflowlib_data,"aflowlib::LIB2RAW (agl):");
            if(flag_WEB) {
          ...
          if(perform_AEL) {
            ...
            aflowlib::LIB2RAW_Loop_AEL(directory_LIB,directory_RAW,vfiles,aflowlib_data,"aflowlib::LIB2RAW (ael):");
            if(flag_WEB) {
          ...
          if(perform_LOCK) {
            ...
            aflowlib::LIB2RAW_Loop_LOCK(directory_LIB,directory_RAW,vfiles,aflowlib_data,"aflowlib::LIB2RAW (LOCK):");	
          }
          // ---------------------------------------------------------------------------------------------------------------------------------  
          // write DOS + BANDS JSON //CO 171025
          string system_name=KBIN::ExtractSystemName(directory_LIB);
          if(aurostd::EFileExist(directory_LIB+"/DOSCAR.static") && aurostd::EFileExist(directory_LIB+"/POSCAR.static")){
            ...
            if(estructure::DOSDATA_JSON(vpflow,directory_LIB,_json_file,true)){
              ...
              aurostd::stringstream2bz2file(json_file,directory_RAW+"/"+system_name+"_dosdata.json"); json_file.str("");
              if(aurostd::EFileExist(directory_LIB+"/EIGENVAL.bands") && aurostd::EFileExist(directory_LIB+"/KPOINTS.bands")){
                if(estructure::BANDSDATA_JSON(vpflow,directory_LIB,_json_file,true)){
                  json_file << _json_file.str() << endl; _json_file.str("");
                  aurostd::stringstream2bz2file(json_file,directory_RAW+"/"+system_name+"_bandsdata.json"); json_file.str("");
                }
              }
            }
          }
          ...
          aurostd::RemoveFile(directory_RAW+"/*eps.bz2");
          //  aurostd::RemoveFile(directory_RAW+"/core*");
          for(uint i=0;i<vfiles.size();i++) {
            // cout << vfiles.at(i) << endl;
            if(!aurostd::substring2bool(vfiles.at(i),".bz2") && aurostd::FileExist(directory_RAW+"/"+vfiles.at(i))) {
              // [OBSOLETE] if(vfiles.at(i)=="DOSCAR.static") {aurostd::BzipFile(directory_RAW+"/"+vfiles.at(i));} // seems to work, although I do not like if(SC-0512)
              if(LOCAL){ //CO 171025
                if(aurostd::EFileExist(directory_LIB+"/"+vfiles.at(i))){aurostd::RemoveFile(directory_RAW+"/"+vfiles.at(i));} //it's all sitting in the directory above, wasteful
                if(vfiles[i]=="KPOINTS.relax"){aurostd::RemoveFile(directory_RAW+"/"+vfiles.at(i));}
                if(vfiles[i]=="EIGENVAL.bands.old"){aurostd::RemoveFile(directory_RAW+"/"+vfiles.at(i));}
                if(vfiles[i]=="OUTCAR.relax"){aurostd::RemoveFile(directory_RAW+"/"+vfiles.at(i));}
              }else{
                if(vfiles.at(i)=="DOSCAR.static") {
                  aurostd::RemoveFile(directory_RAW+"/"+vfiles.at(i));
                  //if(LOCAL){ //CO 171025
                  //  aurostd::execute("cd "+aurostd::CleanFileName(directory_RAW)+"; ln -sf "+aurostd::CleanFileName("../"+vfiles.at(i)+".bz2")+" ./"); // also link LIB to RAW
                  //}else{
                  aurostd::execute("ln -sf "+aurostd::CleanFileName(directory_LIB+"/"+vfiles.at(i)+".bz2")+" "+directory_RAW); // also link LIB to RAW
                  //}
                }
                // [OBSOLETE] if(vfiles.at(i)=="OUTCAR.static") {aurostd::BzipFile(directory_RAW+"/"+vfiles.at(i));}  // seems to work, although I do not like if(SC-0512)
                if(vfiles.at(i)=="OUTCAR.static") {
                  aurostd::RemoveFile(directory_RAW+"/"+vfiles.at(i));
                  //if(LOCAL){ //CO 171025
                  //  aurostd::execute("cd "+aurostd::CleanFileName(directory_RAW)+"; ln -sf "+aurostd::CleanFileName("../"+vfiles.at(i)+".bz2")+" ./"); // also link LIB to RAW
                  //}else{
                ...
                // [NO bzip2] if(vfiles.at(i)=="KPOINTS.relax") aurostd::BzipFile(directory_RAW+"/"+vfiles.at(i)); // seems to work, although I do not like if(SC-0914)
                // [NO bzip2] if(vfiles.at(i)=="KPOINTS.static") aurostd::BzipFile(directory_RAW+"/"+vfiles.at(i)); // seems to work, although I do not like if(SC-0914)
                // [NO bzip2] if(vfiles.at(i)=="KPOINTS.bands") aurostd::BzipFile(directory_RAW+"/"+vfiles.at(i)); // seems to work, although I do not like if(SC-0914)
                // [NO bzip2] if(vfiles.at(i)=="INCAR.relax") aurostd::BzipFile(directory_RAW+"/"+vfiles.at(i)); // seems to work, although I do not like if(SC-0914)
                // [NO bzip2] if(vfiles.at(i)=="INCAR.static") aurostd::BzipFile(directory_RAW+"/"+vfiles.at(i)); // seems to work, although I do not like if(SC-0914)
                // [NO bzip2] if(vfiles.at(i)=="INCAR.bands") aurostd::BzipFile(directory_RAW+"/"+vfiles.at(i)); // seems to work, although I do not like if(SC-0914)
                if(vfiles.at(i)=="KPOINTS.bands.old") aurostd::RemoveFile(directory_RAW+"/"+vfiles.at(i)); // seems to work, although I do not like if(SC-0512)
                if(vfiles.at(i)=="EIGENVAL.bands") aurostd::BzipFile(directory_RAW+"/"+vfiles.at(i)); // seems to work, although I do not like if(SC-0512)
                if(vfiles.at(i)=="EIGENVAL.bands.old") aurostd::RemoveFile(directory_RAW+"/"+vfiles.at(i)); // seems to work, although I do not like if(SC-0512)
                if(vfiles.at(i)=="OSZICAR.bands") aurostd::BzipFile(directory_RAW+"/"+vfiles.at(i)); // seems to work, although I do not like if(SC-0512)
                if(vfiles.at(i)=="OUTCAR.relax") aurostd::RemoveFile(directory_RAW+"/"+vfiles.at(i));  //aurostd::BzipFile(directory_RAW+"/"+vfiles.at(i));  // seems to work, although I do not like if(SC-0512)
                if(vfiles.at(i)=="OUTCAR.relax1") aurostd::RemoveFile(directory_RAW+"/"+vfiles.at(i));  //aurostd::BzipFile(directory_RAW+"/"+vfiles.at(i));  // seems to work, although I do not like if(SC-0512)
                //  if(vfiles.at(i)=="OUTCAR.relax") aurostd::BzipFile(directory_RAW+"/"+vfiles.at(i));  // seems to work, although I do not like if(SC-0512)
                if(vfiles.at(i)=="OUTCAR.bands") aurostd::RemoveFile(directory_RAW+"/"+vfiles.at(i));  //aurostd::BzipFile(directory_RAW+"/"+vfiles.at(i));  // seems to work, although I do not like if(SC-0512)
                if(vfiles.at(i)=="AECCAR0.static") aurostd::BzipFile(directory_RAW+"/"+vfiles.at(i)); // seems to work, although I do not like if(SC-0512)
                if(vfiles.at(i)=="AECCAR2.static") aurostd::BzipFile(directory_RAW+"/"+vfiles.at(i)); // seems to work, although I do not like if(SC-0512)
                if(vfiles.at(i)=="CHGCAR.static") aurostd::BzipFile(directory_RAW+"/"+vfiles.at(i)); // seems to work, although I do not like if(SC-0512)
              }
            }
          }
          ...
          if(perform_BANDS || 1) {
            ...
            if(flag_WEB) {
              ...
              aurostd::execute("ln -sf "+aurostd::CleanFileName(directory_RAW+"/"+_FILE_AFLOWLIB_ENTRY_OUT_)+" "+directory_WEB);    // LINK
              aurostd::execute("ln -sf "+aurostd::CleanFileName(directory_RAW+"/"+_FILE_AFLOWLIB_ENTRY_JSON_)+" "+directory_WEB);    // LINK
              aurostd::execute("ln -sf "+aurostd::CleanFileName(directory_RAW+"/edata.*")+" "+directory_WEB);         // LINK [FORNARI]
              aurostd::execute("ln -sf "+aurostd::CleanFileName(directory_RAW+"/*structure_relax*.json")+" "+directory_WEB);  //CO 171024
              aurostd::execute("ln -sf "+aurostd::CleanFileName(directory_RAW+"/aflow.pgroup.*")+" "+directory_WEB);          //CO 171024
              aurostd::execute("ln -sf "+aurostd::CleanFileName(directory_RAW+"/aflow.pgroupk.*")+" "+directory_WEB);         //CO 171024
              aurostd::execute("ln -sf "+aurostd::CleanFileName(directory_RAW+"/aflow.fgroup.*")+" "+directory_WEB);          //CO 171024
              aurostd::execute("ln -sf "+aurostd::CleanFileName(directory_RAW+"/aflow.pgroup_xtal.*")+" "+directory_WEB);     //CO 171024
              aurostd::execute("ln -sf "+aurostd::CleanFileName(directory_RAW+"/aflow.iatoms.*")+" "+directory_WEB);          //CO 171024
              aurostd::execute("ln -sf "+aurostd::CleanFileName(directory_RAW+"/aflow.agroup.*")+" "+directory_WEB);          //CO 171024

              //CO 171025 - START
              if(aurostd::FileExist(directory_RAW+"/"+system_name+"_dosdata.json") ||
                  aurostd::FileExist(directory_RAW+"/"+system_name+"_dosdata.json.bz2") ){
                aurostd::execute("ln -sf "+aurostd::CleanFileName(directory_RAW+"/*_dosdata.json*")+" "+directory_WEB);          //CO 171024
              }
              if(aurostd::FileExist(directory_RAW+"/"+system_name+"_bandsdata.json") ||
                  aurostd::FileExist(directory_RAW+"/"+system_name+"_bandsdata.json.bz2") ){
                aurostd::execute("ln -sf "+aurostd::CleanFileName(directory_RAW+"/*_bandsdata.json*")+" "+directory_WEB);          //CO 171024
              }
              //CO 171025 - END

              if(aurostd::FileExist(directory_RAW+"/EIGENVAL.bands.bz2") || aurostd::FileExist(directory_RAW+"/EIGENVAL.bands"))
                aurostd::execute("ln -sf "+aurostd::CleanFileName(directory_RAW+"/EIGENVAL.bands*")+" "+directory_WEB);         // LINK

              if(aurostd::FileExist(directory_RAW+"/DOSCAR.static.bz2") || aurostd::FileExist(directory_RAW+"/DOSCAR.static"))
                aurostd::execute("ln -sf "+aurostd::CleanFileName(directory_RAW+"/DOSCAR.static*")+" "+directory_WEB);         // LINK
              if(aurostd::FileExist(directory_RAW+"/OUTCAR.static.bz2") || aurostd::FileExist(directory_RAW+"/OUTCAR.static"))
                aurostd::execute("ln -sf "+aurostd::CleanFileName(directory_RAW+"/OUTCAR.static*")+" "+directory_WEB);         // LINK
              if(aurostd::FileExist(directory_RAW+"/CONTCAR.relax.bz2") || aurostd::FileExist(directory_RAW+"/CONTCAR.relax"))
                aurostd::execute("ln -sf "+aurostd::CleanFileName(directory_RAW+"/CONTCAR.relax*")+" "+directory_WEB);         // LINK

              if(aurostd::FileExist(directory_RAW+"/POSCAR.bands.bz2") || aurostd::FileExist(directory_RAW+"/POSCAR.bands"))
                aurostd::execute("ln -sf "+aurostd::CleanFileName(directory_RAW+"/POSCAR.bands*")+" "+directory_WEB);         // LINK
              // aurostd::execute("ln -sf "+aurostd::CleanFileName(directory_RAW+"/CONTCAR.relax.vasp")+" "+directory_WEB);    // LINK [FORNARI]
              // aurostd::execute("ln -sf "+aurostd::CleanFileName(directory_RAW+"/CONTCAR.relax.qe")+" "+directory_WEB);    // LINK [FORNARI]
              // aurostd::execute("ln -sf "+aurostd::CleanFileName(directory_RAW+"/CONTCAR.relax.abinit")+" "+directory_WEB);    // LINK [FORNARI]
              // aurostd::execute("ln -sf "+aurostd::CleanFileName(directory_RAW+"/CONTCAR.relax.aims")+" "+directory_WEB);    // LINK [FORNARI]

              if(aurostd::FileExist(directory_RAW+"/KPOINTS.relax.bz2") || aurostd::FileExist(directory_RAW+"/KPOINTS.relax"))
                aurostd::execute("ln -sf "+aurostd::CleanFileName(directory_RAW+"/KPOINTS.relax*")+" "+directory_WEB);         // LINK
              if(aurostd::FileExist(directory_RAW+"/KPOINTS.static.bz2") || aurostd::FileExist(directory_RAW+"/KPOINTS.static"))
                aurostd::execute("ln -sf "+aurostd::CleanFileName(directory_RAW+"/KPOINTS.static*")+" "+directory_WEB);         // LINK
              if(aurostd::FileExist(directory_RAW+"/KPOINTS.bands.bz2") || aurostd::FileExist(directory_RAW+"/KPOINTS.bands"))
                aurostd::execute("ln -sf "+aurostd::CleanFileName(directory_RAW+"/KPOINTS.bands*")+" "+directory_WEB);         // LINK
              if(aurostd::FileExist(directory_RAW+"/INCAR.bands.bz2") || aurostd::FileExist(directory_RAW+"/INCAR.bands"))
                aurostd::execute("ln -sf "+aurostd::CleanFileName(directory_RAW+"/INCAR.bands*")+" "+directory_WEB);         // LINK

              // maybe adding something for bader
              if(aurostd::FileExist(directory_RAW+"/AECCAR0.static.bz2") || aurostd::FileExist(directory_RAW+"/AECCAR0.static"))
                aurostd::execute("ln -sf "+aurostd::CleanFileName(directory_RAW+"/AECCAR0.static*")+" "+directory_WEB);         // LINK
              if(aurostd::FileExist(directory_RAW+"/AECCAR2.static.bz2") || aurostd::FileExist(directory_RAW+"/AECCAR2.static"))
                aurostd::execute("ln -sf "+aurostd::CleanFileName(directory_RAW+"/AECCAR2.static*")+" "+directory_WEB);         // LINK
              if(aurostd::FileExist(directory_RAW+"/CHGCAR.static.bz2") || aurostd::FileExist(directory_RAW+"/CHGCAR.static"))
                aurostd::execute("ln -sf "+aurostd::CleanFileName(directory_RAW+"/CHGCAR.static*")+" "+directory_WEB);         // LINK
            }
          }
          // DONE
          ...
          //     cout << _FILE_AFLOWLIB_ENTRY_OUT_ << ": " << aflowlib_data.aflowlib2file(directory_RAW+"/"+_FILE_AFLOWLIB_ENTRY_OUT_);
          ...
          // write aflowlib.out
          cout << _FILE_AFLOWLIB_ENTRY_OUT_ << ": " << aflowlib_data.aflowlib2file(directory_RAW+"/"+_FILE_AFLOWLIB_ENTRY_OUT_,"out");
          //      cout << _FILE_AFLOWLIB_ENTRY_OUT_ << ": " << aflowlib_data.aflowlib2file(directory_WEB+"/"+_FILE_AFLOWLIB_ENTRY_OUT_);
          // write aflowlib.json
          cout << _FILE_AFLOWLIB_ENTRY_JSON_ << ": " << aflowlib_data.aflowlib2file(directory_RAW+"/"+_FILE_AFLOWLIB_ENTRY_JSON_,"json");
          //      cout << _FILE_AFLOWLIB_ENTRY_JSON_ << ": " << aflowlib_data.aflowlib2file(directory_WEB+"/"+_FILE_AFLOWLIB_ENTRY_JSON_);
          if(flag_WEB){
            if(!LOCAL){ //CO 171025
          ...
          // DONE
          cout << "aflowlib::LIB2RAW: dir=" << directory_LIB << "   END_DATE =" << Message(aflags,"time") << endl;
        ...
        // COMPRESS
        /*
        //  aurostd::BzipFile(directory_RAW+"/POSCAR.bands");
        //  aurostd::BzipFile(directory_RAW+"/plotbz.sh");
        */
        // DELETE STUFF
        ...
        aurostd::execute("find "+directory_LIB+" -type d -exec chmod 755 {} \\;");
        aurostd::execute("find "+directory_LIB+" -type f -exec chmod 644 {} \\;");
        aurostd::execute("find "+directory_RAW+" -type d -exec chmod 755 {} \\;");
        aurostd::execute("find "+directory_RAW+" -type f -exec chmod 644 {} \\;");
        if(CHMODWEB) {
          aurostd::execute("find "+directory_WEB+" -type d -exec chmod 755 {} \\;");
          aurostd::execute("find "+directory_WEB+" -type f -exec chmod 644 {} \\;");
        }
    ...
    namespace aflowlib {
      bool LIB2RAW_FileNeeded(string directory_LIB,string fileLIB,string directory_RAW,string fileRAW,vector<string> &vfiles,string MESSAGE) {
        ...
        string file_LIB_nocompress=directory_LIB+"/"+fileLIB;aurostd::StringSubst(file_LIB_nocompress,".bz2","");aurostd::StringSubst(file_LIB_nocompress,".gz","");
        string file_RAW_nocompress=directory_RAW+"/"+fileRAW;aurostd::StringSubst(file_RAW_nocompress,".bz2","");aurostd::StringSubst(file_RAW_nocompress,".gz","");
        string file_LIB_compress=file_LIB_nocompress+".bz2";
        string file_RAW_compress=file_RAW_nocompress+".bz2";
        ...
        if(!aurostd::FileExist(file_LIB) && !aurostd::FileExist(file_LIB_nocompress) && !aurostd::FileExist(file_LIB_compress)) {
          if(!aurostd::FileExist(file_LIB)) {cout << MESSAGE << " ERROR - file not found " << file_LIB << endl;exit(0);}
          if(!aurostd::FileExist(file_LIB_nocompress)) {cout << MESSAGE << " ERROR - file not found " << file_LIB_nocompress << endl;exit(0);}
          if(!aurostd::FileExist(file_LIB_compress)) {cout << MESSAGE << " ERROR - file not found " << file_LIB_compress << endl;exit(0);}
        }
        ...
        if(aurostd::FileExist(file_LIB_compress)) aurostd::CopyFile(file_LIB_compress,file_RAW_compress);
        if(aurostd::FileExist(file_RAW) && aurostd::substring2bool(file_RAW,".bz2")) aurostd::BunzipFile(file_RAW); // only if bz2
        if(aurostd::FileExist(file_RAW_compress) && aurostd::substring2bool(file_RAW_compress,".bz2")) aurostd::BunzipFile(file_RAW_compress); // only if bz2
        // aurostd::GunzipFile(file_RAW_compress); // only if gz
        string file2add=fileRAW;aurostd::StringSubst(file2add,".bz2","");aurostd::StringSubst(file2add,".gz","");
        vfiles.push_back(file2add);
        return TRUE;
      }
    }
    ...
    // ***************************************************************************
    namespace aflowlib {
      bool LIB2RAW_Loop_Bands(string& directory_LIB,string& directory_RAW,vector<string> &vfiles,aflowlib::_aflowlib_entry& data,string MESSAGE) {
        bool LDEBUG=(FALSE || XHOST.DEBUG);
        // LDEBUG=TRUE;
        ...
        // copy _AFLOWIN_ LOCK DOSCAR.static.bz2 EIGENVAL.bands.bz2 KPOINTS.bands.bz2 POSCAR.bands.bz2
        ...
        aflowlib::LIB2RAW_FileNeeded(directory_LIB,"OSZICAR.static",directory_RAW,"OSZICAR.static",vfiles,MESSAGE);  // OSZICAR.static
        aflowlib::LIB2RAW_FileNeeded(directory_LIB,"OSZICAR.bands",directory_RAW,"OSZICAR.bands",vfiles,MESSAGE);  // OSZICAR.bands
        aflowlib::LIB2RAW_FileNeeded(directory_LIB,"EIGENVAL.bands",directory_RAW,"EIGENVAL.bands",vfiles,MESSAGE);  // EIGENVAL.bands
        aflowlib::LIB2RAW_FileNeeded(directory_LIB,"KPOINTS.static",directory_RAW,"KPOINTS.static",vfiles,MESSAGE);  // KPOINTS.static  // not needed but good for show off SC 0914
        aflowlib::LIB2RAW_FileNeeded(directory_LIB,"KPOINTS.bands",directory_RAW,"KPOINTS.bands",vfiles,MESSAGE);  // KPOINTS.bands 
        //  aflowlib::LIB2RAW_FileNeeded(directory_LIB,"INCAR.static",directory_RAW,"INCAR.static",vfiles,MESSAGE);  // INCAR.static  // not needed but good for show off SC 0914
        aflowlib::LIB2RAW_FileNeeded(directory_LIB,"INCAR.bands",directory_RAW,"INCAR.bands",vfiles,MESSAGE);  // INCAR.bands  // not needed but good for show off SC 0914
        //  aflowlib::LIB2RAW_FileNeeded(directory_LIB,"POSCAR.relax1",directory_RAW,"POSCAR.relax1",vfiles,MESSAGE);  // POSCAR.relax1, Get Atomic Species Name


        if(TRUE || flag_DATA_BANDS_) { // POSCAR.bands.bz2
          aflowlib::LIB2RAW_FileNeeded(directory_LIB,"POSCAR.bands",directory_RAW,"POSCAR.bands",vfiles,MESSAGE);  // POSCAR.bands
        }
        ...
        // bzip2 -dv
        // aus_exec << xhost.bzip2 << " -dfq " << directory_RAW << "/*.bz2" << endl;
        // aurostd::execute(aus_exec);
        //  cout << MESSAGE << " done: " << directory_LIB << endl;
        ...
        if(flag_use_MATLAB) { // MATLAB STUFF  OLD WAHYU-STEFANO
          ...
          command << "mv KPOINTS.bands KPOINTS.bands.old" << endl; vfiles.push_back("KPOINTS.bands.old"); // so it is compressed
          command << "mv EIGENVAL.bands EIGENVAL.bands.old" << endl; vfiles.push_back("EIGENVAL.bands.old"); // so it is compressed
    ...
    // ***************************************************************************
    // namespace aflowlib {
    //   bool LIB2RAW_Loop_DATA(string& directory_LIB,string& directory_RAW,vector<string> &vfiles,aflowlib::_aflowlib_entry& data,string MESSAGE) {
    //     bool LDEBUG=(FALSE || XHOST.DEBUG);
    //     // LDEBUG=TRUE;
    ...
    //     if(flag_DATA_ORIG_ || flag_EDATA_ORIG_) {  // POSCAR.orig.bz2
    //       // if(flag_ORIG==FALSE) {
    //       bool found=FALSE;  
    //       file_LIB=directory_LIB+"/POSCAR.orig.bz2"; 
    //       if(!found && (found=aurostd::FileExist(file_LIB))) { 
    // 	cout << MESSAGE << " aflowlib::LIB2RAW_Loop_DATAs: building POSCAR.orig from POSCAR.orig.bz2" << endl;
    // 	aflowlib::LIB2RAW_FileNeeded(directory_LIB,"POSCAR.orig",directory_RAW,"POSCAR.orig",vfiles,MESSAGE);  // POSCAR.orig    
    //       }
    //       file_LIB=directory_LIB+"/POSCAR.relax1.bz2"; 
    //       if(!found && (found=aurostd::FileExist(file_LIB))) { 
    // 	cout << MESSAGE << " aflowlib::LIB2RAW_Loop_DATAs: building POSCAR.orig from POSCAR.relax1.bz2" << endl;
    ...
    //     if(flag_DATA_RELAX_ || flag_EDATA_RELAX_) {  // CONTCAR.relax.bz2
    //       for(uint i=1;i<=relax_max;i++) {
    // 	file_LIB=directory_LIB+"/CONTCAR.relax"+aurostd::utype2string(i)+".bz2";file_RAW=directory_RAW+"/CONTCAR.relax.bz2";
    // 	if(aurostd::FileExist(file_LIB)) {aurostd::CopyFile(file_LIB,file_RAW);aurostd::BunzipFile(file_RAW);vfiles.push_back("CONTCAR.relax");}
    //       }
    //       if(aurostd::FileExist(file_RAW)) {cout << MESSAGE << ": ERROR - aflowlib::LIB2RAW_Loop_DATAs:[1] - file not prepared " << file_LIB << endl;exit(0);}
    //     }

    //     if(!flag_DATA_BANDS_) {
    //       file_LIB=directory_LIB+"/POSCAR.bands.bz2"; 
    //       if(aurostd::FileExist(file_LIB)) {aurostd::CopyFile(file_LIB,file_RAW);aurostd::BunzipFile(file_RAW);vfiles.push_back("POSCAR.bands");}
    //     }
    ...
    //       if(!aurostd::FileExist(directory_RAW+"/"+_FILE_EDATA_ORIG_OUT_+"") && aurostd::FileExist(directory_RAW+"/POSCAR.orig")) {
    ...
    // 	aurostd::stringstream2file(sss,directory_RAW+"/"+_FILE_EDATA_ORIG_OUT_);
    // 	vcif.clear();vcif.push_back(str);vcif.push_back(str_sp);vcif.push_back(str_sc);
    //       }
    //     }
    //     if(flag_EDATA_RELAX_) { // RELAX
    //       if(!aurostd::FileExist(directory_RAW+"/"+_FILE_EDATA_RELAX_OUT_+"") && aurostd::FileExist(directory_RAW+"/CONTCAR.relax")) {
    ...
    // 	aurostd::stringstream2file(sss,directory_RAW+"/"+_FILE_EDATA_RELAX_OUT_);
    // 	vcif.clear();vcif.push_back(str);vcif.push_back(str_sp);vcif.push_back(str_sc);
    //       }
    //     }
    ...
    //       if(!aurostd::FileExist(directory_RAW+"/"+_FILE_EDATA_BANDS_OUT_+"") && aurostd::FileExist(directory_RAW+"/POSCAR.bands")) {
    ...
    // 	stringstream sss; sss << aflow::Banner("BANNER_TINY") << endl; 
    // 	pflow::PrintData(str,str_sp,str_sc,sss,"EDATA"); // EDATA
    // 	aurostd::stringstream2file(sss,directory_RAW+"/"+_FILE_EDATA_BANDS_OUT_);
    // 	vcif.clear();vcif.push_back(str);vcif.push_back(str_sp);vcif.push_back(str_sc);
    //       }
    //     } 
    ...
    //       if(!aurostd::FileExist(directory_RAW+"/"+_FILE_DATA_ORIG_OUT_+"") && aurostd::FileExist(directory_RAW+"/POSCAR.orig")) {
    // 	cout << MESSAGE << " DATA doing orig (POSCAR.orig): " << directory_RAW << endl;
    // 	str=xstructure(directory_RAW+"/POSCAR.orig",IOAFLOW_AUTO);str_sp.Clear();str_sc.Clear();
    // 	stringstream sss; sss << aflow::Banner("BANNER_TINY") << endl; 
    // 	pflow::PrintData(str,str_sp,str_sc,sss,"DATA"); // DATA
    // 	aurostd::stringstream2file(sss,directory_RAW+"/"+_FILE_DATA_ORIG_OUT_);
    //       }
    //     }
    //     if(flag_DATA_RELAX_) { // RELAX
    //       if(!aurostd::FileExist(directory_RAW+"/"+_FILE_DATA_RELAX_OUT_+"") && aurostd::FileExist(directory_RAW+"/CONTCAR.relax")) {
    // 	cout << MESSAGE << " DATA doing relax (CONTCAR.relax): " << directory_RAW << endl;
    // 	str=xstructure(directory_RAW+"/CONTCAR.relax",IOAFLOW_AUTO);str_sp.Clear();str_sc.Clear();
    // 	stringstream sss; sss << aflow::Banner("BANNER_TINY") << endl; 
    // 	pflow::PrintData(str,str_sp,str_sc,sss,"DATA"); // DATA
    // 	aurostd::stringstream2file(sss,directory_RAW+"/"+_FILE_DATA_RELAX_OUT_);
    //       }
    //     }
    //     if(flag_DATA_BANDS_) { // BANDS
    //       if(!aurostd::FileExist(directory_RAW+"/"+_FILE_DATA_BANDS_OUT_+"") && aurostd::FileExist(directory_RAW+"/POSCAR.bands")) {
    // 	cout << MESSAGE << " DATA doing bands (POSCAR.bands): " << directory_RAW << endl;
    // 	str=xstructure(directory_RAW+"/POSCAR.bands",IOAFLOW_AUTO);str_sp.Clear();str_sc.Clear();
    // 	stringstream sss; sss << aflow::Banner("BANNER_TINY") << endl; 
    // 	pflow::PrintData(str,str_sp,str_sc,sss,"DATA"); // DATA
    // 	aurostd::stringstream2file(sss,directory_RAW+"/"+_FILE_DATA_BANDS_OUT_);
    //       }
    //     }

    //     // NOW DO THE CIFS    
    //     aflowlib::LIB2RAW_FileNeeded(directory_LIB,"CONTCAR.relax2",directory_RAW,"CONTCAR.relax2",vfiles,MESSAGE);  // POSCAR.bands

    //     //      cout << MESSAGE << " CIF bands: " << directory_LIB << endl;
    ...
    //       if(j==0) cout << MESSAGE << " CIF bands: data.spacegroup_relax=" << data.spacegroup_relax << endl;
    //       if(j==0) cout << MESSAGE << " CIF bands: " << directory_LIB << " doing normal" << endl;
    //       if(j==1) cout << MESSAGE << " CIF bands: " << directory_LIB << " doing sprim" << endl;
    //       if(j==2) cout << MESSAGE << " CIF bands: " << directory_LIB << " doing sconv" << endl;
    //       stringstream oss;
    ...
    //       oss.clear();oss.str("");
    //       pflow::PrintCIF(oss,vcif.at(j),1);//aurostd::string2utype<int>(data.spacegroup_relax));
    //       if(j==0) cout << MESSAGE << " CIF bands: " << directory_LIB << " doing normal_corner" << endl;
    //       if(j==1) cout << MESSAGE << " CIF bands: " << directory_LIB << " doing sprim_corner" << endl;
    //       if(j==2) cout << MESSAGE << " CIF bands: " << directory_LIB << " doing sconv_corner" << endl;
    ...
    namespace aflowlib {
      bool LIB2RAW_Loop_Thermodynamics(string& directory_LIB,string& directory_RAW,vector<string> &vfiles,aflowlib::_aflowlib_entry& data,string MESSAGE,bool LOCAL) {
        bool LDEBUG=(FALSE || XHOST.DEBUG);
        ...
        if(flag_EDATA_ORIG_ && flag_EDATA_RELAX_) {
          fileA_LIB=directory_LIB+"/POSCAR.bands.bz2"; fileA_RAW=directory_RAW+"/POSCAR.bands.bz2"; 
          if(aurostd::FileExist(fileA_LIB)) {flag_EDATA_BANDS_=TRUE;aurostd::CopyFile(fileA_LIB,fileA_RAW);aurostd::BunzipFile(fileA_RAW);vfiles.push_back("POSCAR.bands");}
        }
        // check for flag_DATA_BANDS_
        if(flag_DATA_ORIG_ && flag_DATA_RELAX_) {
          fileA_LIB=directory_LIB+"/POSCAR.bands.bz2";fileA_RAW=directory_RAW+"/POSCAR.bands.bz2"; 
          if(aurostd::FileExist(fileA_LIB)) {flag_DATA_BANDS_=TRUE;aurostd::CopyFile(fileA_LIB,fileA_RAW);aurostd::BunzipFile(fileA_RAW);vfiles.push_back("POSCAR.bands");}
        }
        ...
        //DX - START
        str_orig.directory = str_relax1.directory = str_relax.directory = aflags.Directory;
        //DX - END

        vector<string> tokens,vcmd;
        ...
        if(TRUE || flag_DATA_ORIG_ || flag_EDATA_ORIG_ || flag_SG1 || flag_SG2) {  // POSCAR.orig.bz2
          // if(flag_ORIG==FALSE) {
          bool found=FALSE;
          string file_LIB=directory_LIB+"/POSCAR.orig.bz2"; 
          if(!found && aurostd::FileExist(file_LIB)) { found=TRUE;//      cerr << "BUILDING POSCAR.orig from POSCAR.orig" << endl;
            aflowlib::LIB2RAW_FileNeeded(directory_LIB,"POSCAR.orig",directory_RAW,"POSCAR.orig",vfiles,MESSAGE);  // POSCAR.orig    
          }
          file_LIB=directory_LIB+"/POSCAR.relax1.bz2"; 
          if(!found && aurostd::FileExist(file_LIB)) { found=TRUE;//      cerr << "BUILDING POSCAR.orig from POSCAR.relax1" << endl;
            aflowlib::LIB2RAW_FileNeeded(directory_LIB,"POSCAR.relax1",directory_RAW,"POSCAR.orig",vfiles,MESSAGE);  // POSCAR.orig 
          }
        ...
        if(TRUE || flag_DATA_RELAX_ || flag_EDATA_RELAX_ || TRUE || flag_SG1 || flag_SG2) {  // CONTCAR.relax.bz2
          for(uint i=1;i<=relax_max;i++) {
            fileX_LIB=aurostd::CleanFileName(directory_LIB+"/CONTCAR.relax"+aurostd::utype2string(i)+".bz2");fileX_RAW=aurostd::CleanFileName(directory_RAW+"/CONTCAR.relax.bz2");
            fileE_LIB=aurostd::CleanFileName(directory_LIB+"/OUTCAR.relax"+aurostd::utype2string(i)+".bz2");fileE_RAW=aurostd::CleanFileName(directory_RAW+"/OUTCAR.relax.bz2");
            fileK_LIB=aurostd::CleanFileName(directory_LIB+"/KPOINTS.relax"+aurostd::utype2string(i)+".bz2");fileK_RAW=aurostd::CleanFileName(directory_RAW+"/KPOINTS.relax.bz2");
            fileI_LIB=aurostd::CleanFileName(directory_LIB+"/INCAR.relax"+aurostd::utype2string(i)+".bz2");fileI_RAW=aurostd::CleanFileName(directory_RAW+"/INCAR.relax.bz2");
            if(aurostd::FileExist(fileX_LIB)) {aurostd::CopyFile(fileX_LIB,fileX_RAW);aurostd::BunzipFile(fileX_RAW);vfiles.push_back("CONTCAR.relax");}
            if(aurostd::FileExist(fileE_LIB)) {aurostd::CopyFile(fileE_LIB,fileE_RAW);aurostd::BunzipFile(fileE_RAW);vfiles.push_back("OUTCAR.relax");}
            // nocopy if(aurostd::FileExist(fileK_LIB)) {aurostd::CopyFile(fileK_LIB,fileK_RAW);aurostd::BunzipFile(fileK_RAW);vfiles.push_back("KPOINTS.relax");}
            // nocopy if(aurostd::FileExist(fileI_LIB)) {aurostd::CopyFile(fileI_LIB,fileI_RAW);aurostd::BunzipFile(fileI_RAW);vfiles.push_back("INCAR.relax");}
          }
          if(!aurostd::FileExist(directory_RAW+"/CONTCAR.relax") && aurostd::FileExist(directory_LIB+"/CONTCAR.static.bz2") &&
              !aurostd::FileExist(directory_RAW+"/OUTCAR.relax") && aurostd::FileExist(directory_LIB+"/OUTCAR.static.bz2")) {
            fileX_LIB=aurostd::CleanFileName(directory_LIB+"/CONTCAR.static.bz2");fileX_RAW=aurostd::CleanFileName(directory_RAW+"/CONTCAR.relax.bz2");
            fileE_LIB=aurostd::CleanFileName(directory_LIB+"/OUTCAR.static.bz2");fileE_RAW=aurostd::CleanFileName(directory_RAW+"/OUTCAR.relax.bz2");
            if(AFLOWLIB_VERBOSE) cout << MESSAGE << " WARNING - PATCHING CONTCAR.relax with CONTCAR.static " << fileX_LIB << endl;
            if(AFLOWLIB_VERBOSE) cout << MESSAGE << " WARNING - PATCHING OUTCAR.relax with OUTCAR.static " << fileE_LIB << endl;
            if(aurostd::FileExist(fileX_LIB)) {aurostd::CopyFile(fileX_LIB,fileX_RAW);aurostd::BunzipFile(fileX_RAW);vfiles.push_back("CONTCAR.relax");}
            if(aurostd::FileExist(fileE_LIB)) {aurostd::CopyFile(fileE_LIB,fileE_RAW);aurostd::BunzipFile(fileE_RAW);vfiles.push_back("OUTCAR.relax");}
          }
        ...
        if(flag_SG1 || flag_SG2) {  // CONTCAR.relax1
          fileX_LIB=aurostd::CleanFileName(directory_LIB+"/CONTCAR.relax1.bz2");fileX_RAW=aurostd::CleanFileName(directory_RAW+"/CONTCAR.relax1.bz2");
          fileE_LIB=aurostd::CleanFileName(directory_LIB+"/OUTCAR.relax1.bz2");fileE_RAW=aurostd::CleanFileName(directory_RAW+"/OUTCAR.relax1.bz2");
          if(aurostd::FileExist(fileX_LIB)) {aflowlib::LIB2RAW_FileNeeded(directory_LIB,"CONTCAR.relax1",directory_RAW,"CONTCAR.relax1",vfiles,MESSAGE);}
          if(aurostd::FileExist(fileE_LIB)) {aflowlib::LIB2RAW_FileNeeded(directory_LIB,"OUTCAR.relax1",directory_RAW,"OUTCAR.relax1",vfiles,MESSAGE);}
          if(!aurostd::FileExist(directory_RAW+"/CONTCAR.relax1") && aurostd::FileExist(directory_LIB+"/CONTCAR.static.bz2") &&
              !aurostd::FileExist(directory_RAW+"/OUTCAR.relax1") && aurostd::FileExist(directory_LIB+"/OUTCAR.static.bz2")) {
            fileX_LIB=aurostd::CleanFileName(directory_LIB+"/CONTCAR.static.bz2");fileX_RAW=aurostd::CleanFileName(directory_RAW+"/CONTCAR.relax1.bz2");
            fileE_LIB=aurostd::CleanFileName(directory_LIB+"/OUTCAR.static.bz2");fileE_RAW=aurostd::CleanFileName(directory_RAW+"/OUTCAR.relax1.bz2");
            if(AFLOWLIB_VERBOSE) cout << MESSAGE << " WARNING - PATCHING CONTCAR.relax1 with CONTCAR.static " << fileX_LIB << endl;
            if(AFLOWLIB_VERBOSE) cout << MESSAGE << " WARNING - PATCHING OUTCAR.relax1 with OUTCAR.static " << fileE_LIB << endl;
            if(aurostd::FileExist(fileX_LIB)) {aurostd::CopyFile(fileX_LIB,fileX_RAW);aurostd::BunzipFile(fileX_RAW);vfiles.push_back("CONTCAR.relax1");}
            if(aurostd::FileExist(fileE_LIB)) {aurostd::CopyFile(fileE_LIB,fileE_RAW);aurostd::BunzipFile(fileE_RAW);vfiles.push_back("OUTCAR.relax1");}
          }
          if(aurostd::FileExist(fileX_RAW)) {cout << MESSAGE << " ERROR - aflowlib::LIB2RAW_Loop_Thermodynamics:[4] - file not prepared " << fileX_LIB << endl;exit(0);}
          if(aurostd::FileExist(fileE_RAW)) {cout << MESSAGE << " ERROR - aflowlib::LIB2RAW_Loop_Thermodynamics:[5] - file not prepared " << fileE_LIB << endl;exit(0);}
        }
        if(LDEBUG) cerr << "aflowlib::LIB2RAW_Loop_Thermodynamics [6]" << endl;
        if(flag_DATA_RELAX_ || flag_EDATA_RELAX_ || TRUE) {  // OSZICAR.relax.bz2
          // trying to get an OUTCAR
          if(aurostd::FileExist(directory_LIB+"/OUTCAR.relax1.bz2")) aflowlib::LIB2RAW_FileNeeded(directory_LIB,"OUTCAR.relax1",directory_RAW,"OUTCAR.relax1",vfiles,MESSAGE);  // _AFLOWIN_
          if(aurostd::FileExist(directory_LIB+"/CONTCAR.relax1.bz2")) aflowlib::LIB2RAW_FileNeeded(directory_LIB,"CONTCAR.relax1",directory_RAW,"CONTCAR.relax1",vfiles,MESSAGE);  // _AFLOWIN_
          //     if(aurostd::FileExist(directory_LIB+"/KPOINTS.relax1.bz2")) aflowlib::LIB2RAW_FileNeeded(directory_LIB,"KPOINTS.relax1",directory_RAW,"KPOINTS.relax1",vfiles,MESSAGE);  // _AFLOWIN_
          fileE_LIB=aurostd::CleanFileName(directory_LIB+"/OUTCAR.static.bz2");fileE_RAW=aurostd::CleanFileName(directory_RAW+"/OUTCAR.relax.bz2");
          fileX_LIB=aurostd::CleanFileName(directory_LIB+"/CONTCAR.static.bz2");fileX_RAW=aurostd::CleanFileName(directory_RAW+"/CONTCAR.relax.bz2");
          //  fileK_LIB=aurostd::CleanFileName(directory_LIB+"/KPOINTS.static.bz2");fileK_RAW=aurostd::CleanFileName(directory_RAW+"/KPOINTS.relax.bz2");
          if(aurostd::FileExist(fileE_LIB) && aurostd::FileExist(fileX_LIB) && aurostd::FileExist(fileK_LIB)) {
            aurostd::CopyFile(fileE_LIB,fileE_RAW);aurostd::BunzipFile(fileE_RAW);vfiles.push_back("OUTCAR.relax");
            //	aurostd::CopyFile(fileX_LIB,fileX_RAW);aurostd::BunzipFile(fileX_RAW);vfiles.push_back("CONTCAR.relax");
            aurostd::CopyFile(fileK_LIB,fileK_RAW);aurostd::BunzipFile(fileK_RAW);vfiles.push_back("KPOINTS.relax");
          } else {
            fileE_LIB=aurostd::CleanFileName(directory_LIB+"/OUTCAR.relax2.bz2");fileE_RAW=aurostd::CleanFileName(directory_RAW+"/OUTCAR.relax.bz2");
            fileX_LIB=aurostd::CleanFileName(directory_LIB+"/CONTCAR.relax2.bz2");fileX_RAW=aurostd::CleanFileName(directory_RAW+"/CONTCAR.relax.bz2");
            fileK_LIB=aurostd::CleanFileName(directory_LIB+"/KPOINTS.relax2.bz2");fileK_RAW=aurostd::CleanFileName(directory_RAW+"/KPOINTS.relax.bz2");
            fileI_LIB=aurostd::CleanFileName(directory_LIB+"/INCAR.relax2.bz2");fileI_RAW=aurostd::CleanFileName(directory_RAW+"/INCAR.relax.bz2");
            if(aurostd::FileExist(fileE_LIB) && aurostd::FileExist(fileX_LIB) && aurostd::FileExist(fileK_LIB) && aurostd::FileExist(fileI_LIB)) {
              aurostd::CopyFile(fileE_LIB,fileE_RAW);aurostd::BunzipFile(fileE_RAW);vfiles.push_back("OUTCAR.relax");
              aurostd::CopyFile(fileX_LIB,fileX_RAW);aurostd::BunzipFile(fileX_RAW);vfiles.push_back("CONTCAR.relax");
              aurostd::CopyFile(fileK_LIB,fileK_RAW);aurostd::BunzipFile(fileK_RAW);vfiles.push_back("KPOINTS.relax");
              // nocopy aurostd::CopyFile(fileI_LIB,fileI_RAW);aurostd::BunzipFile(fileI_RAW);vfiles.push_back("INCAR.relax");
            } else {
              fileE_LIB=aurostd::CleanFileName(directory_LIB+"/OUTCAR.relax1.bz2");fileE_RAW=aurostd::CleanFileName(directory_RAW+"/OUTCAR.relax.bz2");
              fileX_LIB=aurostd::CleanFileName(directory_LIB+"/CONTCAR.relax1.bz2");fileX_RAW=aurostd::CleanFileName(directory_RAW+"/CONTCAR.relax.bz2");
              fileK_LIB=aurostd::CleanFileName(directory_LIB+"/KPOINTS.relax1.bz2");fileK_RAW=aurostd::CleanFileName(directory_RAW+"/KPOINTS.relax.bz2");
              fileI_LIB=aurostd::CleanFileName(directory_LIB+"/INCAR.relax1.bz2");fileI_RAW=aurostd::CleanFileName(directory_RAW+"/INCAR.relax.bz2");
              if(aurostd::FileExist(fileE_LIB) && aurostd::FileExist(fileX_LIB) && aurostd::FileExist(fileK_LIB) && aurostd::FileExist(fileI_LIB)) {
                aurostd::CopyFile(fileE_LIB,fileE_RAW);aurostd::BunzipFile(fileE_RAW);vfiles.push_back("OUTCAR.relax");
                aurostd::CopyFile(fileX_LIB,fileX_RAW);aurostd::BunzipFile(fileX_RAW);vfiles.push_back("CONTCAR.relax");
                aurostd::CopyFile(fileK_LIB,fileK_RAW);aurostd::BunzipFile(fileK_RAW);vfiles.push_back("KPOINTS.relax");
        ...
        if(str_orig.num_each_type.size()==0 && aurostd::FileExist(directory_RAW+"/POSCAR.orig")) {xstructure _str_orig(directory_RAW+"/POSCAR.orig",IOVASP_AUTO);str_orig=_str_orig;str_orig.ReScale(1.0);/*xstr_js.str("");xstr_js << xstructure2json(str_orig);aurostd::stringstream2file(xstr_js,directory_RAW+"/"+system_name+"_structure_orig.json");*/} //CO 171025
        if(str_relax.num_each_type.size()==0 && aurostd::FileExist(directory_RAW+"/CONTCAR.relax")) {xstructure _str_relax(directory_RAW+"/CONTCAR.relax",IOVASP_AUTO);str_relax=_str_relax;str_relax.ReScale(1.0);xstr_js.str("");xstr_js << xstructure2json(str_orig);aurostd::stringstream2file(xstr_js,directory_RAW+"/"+system_name+"_structure_relax.json");} //CO 171025
        if(str_relax.num_each_type.size()==0 && aurostd::FileExist(directory_RAW+"/CONTCAR.static")) {xstructure _str_relax(directory_RAW+"/CONTCAR.static",IOVASP_AUTO);str_relax=_str_relax;str_relax.ReScale(1.0);xstr_js.str("");xstr_js << xstructure2json(str_orig);aurostd::stringstream2file(xstr_js,directory_RAW+"/"+system_name+"_structure_relax.json");} //CO 171025
        if(aurostd::FileExist(directory_RAW+"/CONTCAR.relax1")) {xstructure _str_relax1(directory_RAW+"/CONTCAR.relax1",IOVASP_AUTO);str_relax1=_str_relax1;str_relax1.ReScale(1.0);xstr_js.str("");xstr_js << xstructure2json(str_orig);aurostd::stringstream2file(xstr_js,directory_RAW+"/"+system_name+"_structure_relax1.json");} //CO 171025
        // do the extractions
        ...
        for(uint itry=1;itry<=5&&str_relax.species.size()==0;itry++) {  // ONLY OUTCAR POTCARS ARE OBSOLETE
          string stry="";
          if(itry==1) stry="OUTCAR.relax1.bz2";
          if(itry==2) stry="OUTCAR.relax2.bz2";
          if(itry==3) stry="OUTCAR.relax3.bz2";
          if(itry==4) stry="OUTCAR.static.bz2";
          if(itry==5) stry="OUTCAR.bands.bz2";
          fileE_LIB=directory_LIB+"/"+stry;
          if(aurostd::FileExist(fileE_LIB)) {
            // [OBSOLETE]	stringstream stream_outcar(aurostd::execute2string(XHOST.command("bzcat")+" "+fileE_LIB));
        ...
        if(FORMATION_CALC==TRUE) {
          ...
          if(flag_TIMING) {  // OUTCAR.relax.bz2
            for(uint i=1;i<=relax_max+2;i++) {
              fileE_LIB=directory_LIB+"/OUTCAR.relax"+aurostd::utype2string(i)+".bz2";
              if(i==relax_max+1) fileE_LIB=directory_LIB+"/OUTCAR.static.bz2";  // do the static
              if(i==relax_max+2) fileE_LIB=directory_LIB+"/OUTCAR.bands.bz2";  // do the bands
              // cerr << fileE_LIB << endl;
              if(aurostd::FileExist(fileE_LIB)) {
                ...
                xOUTCAR outcar;
                if(i==relax_max+2) {outcar.GetPropertiesFile(directory_LIB+"/OUTCAR.bands.bz2");} // cerr << "xOUTCAR.Efermi=" << outcar.Efermi << endl;}
              xEIGENVAL eigenval;
              if(i==relax_max+2) {eigenval.GetPropertiesFile(directory_LIB+"/EIGENVAL.bands.bz2");}
              xDOSCAR doscar;
              if(i==relax_max+2) {doscar.GetPropertiesFile(directory_LIB+"/DOSCAR.bands.bz2");} // cerr << "xDOSCAR.Efermi=" << doscar.Efermi << " " << outcar.Efermi-doscar.Efermi<< endl;}
          }
        }
        if(_APENNSY_STYLE_OLD_) aurostd::string2file(aurostd::utype2string((long(100*data.calculation_time))/100)+"\n",directory_RAW+"/"+_FILE_TIME_OUT_);
        ...
        if(flag_SG1 || flag_SG2) {  // POSCAR.orig CONTCAR.relax1 CONTCAR.relax
          ...
          if(flag_SG1) {
            ...
            //      aurostd::stringstream2file(ssfile,directory_RAW+"/"+_FILE_SPACEGROUP1_OUT_);
          ...
          if(flag_SG2) {
            ...
            // aurostd::stringstream2file(ssfile,directory_RAW+"/"+_FILE_SPACEGROUP2_OUT_);
        ...
        if(flag_VOLDISTPARAMS) {  // CONTCAR.relax
          ...
          for(uint i=0;i<vdata1.size();i++) {
            if(aurostd::substring2bool(vdata1.at(i),"Stoich Str1")) {data.stoich=aurostd::substring2string(vdata1.at(i),"Stoich Str1: ",FALSE);ssfile << "STOICH " << data.stoich << endl;}
            if(aurostd::substring2bool(vdata1.at(i),"Vol Str1")) {data_v_atom=aurostd::substring2string(vdata1.at(i),"Vol Str1: ",FALSE);ssfile << "V_ATOM " << data_v_atom << endl;}
            if(aurostd::substring2bool(vdata1.at(i),"Cell Str1")) {data_ucelld=aurostd::substring2string(vdata1.at(i),"Cell Str1: ",FALSE);ssfile << "UCELLD " << data_ucelld << endl;}
          }
          ...
          // aurostd::stringstream2file(ssfile,directory_RAW+"/"+_FILE_VOLDISTPARAMS_OUT_);
        }
        ...
        if(flag_VOLDISTEVOLUTION) {  // CONTCAR.relax
          ...
          //   aurostd::stringstream2file(ssfile,directory_RAW+"/"+_FILE_VOLDISTEVOLUTION_OUT_);
        ...
        if(flag_EDATA_ORIG_) { // ORIG
          if(!aurostd::FileExist(directory_RAW+"/"+_FILE_EDATA_ORIG_OUT_+"") && aurostd::FileExist(directory_RAW+"/POSCAR.orig")) {
            ...
            // [OBSOLETE] aurostd::execute("cd "+directory_RAW+" && cat POSCAR.orig | aflow --edata > "+_FILE_EDATA_ORIG_OUT_);
            str=xstructure(directory_RAW+"/POSCAR.orig",IOAFLOW_AUTO);str_sp.Clear();str_sc.Clear();
            //DX - START
            str.directory = str_sp.directory = str_sc.directory = aflags.Directory;
            //DX - END
            stringstream sss; sss << aflow::Banner("BANNER_TINY") << endl; 
            ...
            aurostd::stringstream2file(jjj,directory_RAW+"/"+_FILE_EDATA_ORIG_JSON_); //CO 171025
            vcif.clear();vcif.push_back(str);vcif.push_back(str_sp);vcif.push_back(str_sc);
            //CO and DX START 170713 - adding symmetry output to RAW
            string new_sym_file;
            //txt variants
            KBIN_SymmetryWrite(FileMESSAGE,str_sp,aflags,_PGROUP_,false,message,"txt"); if(aurostd::FileExist(directory_RAW+"/"+_AFLOW_PGROUP_FILE_)){AddFileNameBeforeExtension(directory_RAW+"/"+_AFLOW_PGROUP_FILE_,"orig",new_sym_file);aurostd::RenameFile(directory_RAW+"/"+_AFLOW_PGROUP_FILE_,new_sym_file);} //CO 171025
            KBIN_SymmetryWrite(FileMESSAGE,str_sp,aflags,_PGROUPK_,false,message,"txt"); if(aurostd::FileExist(directory_RAW+"/"+_AFLOW_PGROUPK_FILE_)){AddFileNameBeforeExtension(directory_RAW+"/"+_AFLOW_PGROUPK_FILE_,"orig",new_sym_file);aurostd::RenameFile(directory_RAW+"/"+_AFLOW_PGROUPK_FILE_,new_sym_file);} //CO 171025
            KBIN_SymmetryWrite(FileMESSAGE,str_sp,aflags,_FGROUP_,false,message,"txt"); if(aurostd::FileExist(directory_RAW+"/"+_AFLOW_FGROUP_FILE_)){AddFileNameBeforeExtension(directory_RAW+"/"+_AFLOW_FGROUP_FILE_,"orig",new_sym_file);aurostd::RenameFile(directory_RAW+"/"+_AFLOW_FGROUP_FILE_,new_sym_file);} //CO 171025
            KBIN_SymmetryWrite(FileMESSAGE,str_sp,aflags,_PGROUP_XTAL_,false,message,"txt"); if(aurostd::FileExist(directory_RAW+"/"+_AFLOW_PGROUP_XTAL_FILE_)){AddFileNameBeforeExtension(directory_RAW+"/"+_AFLOW_PGROUP_XTAL_FILE_,"orig",new_sym_file);aurostd::RenameFile(directory_RAW+"/"+_AFLOW_PGROUP_XTAL_FILE_,new_sym_file);} //CO 171025
            KBIN_SymmetryWrite(FileMESSAGE,str_sp,aflags,_PGROUPK_XTAL_,false,message,"txt"); if(aurostd::FileExist(directory_RAW+"/"+_AFLOW_PGROUPK_XTAL_FILE_)){AddFileNameBeforeExtension(directory_RAW+"/"+_AFLOW_PGROUPK_XTAL_FILE_,"orig",new_sym_file);aurostd::RenameFile(directory_RAW+"/"+_AFLOW_PGROUPK_XTAL_FILE_,new_sym_file);} //DX 12/7/17 - added pgroupk_xtal
            KBIN_SymmetryWrite(FileMESSAGE,str_sp,aflags,_IATOMS_,false,message,"txt"); if(aurostd::FileExist(directory_RAW+"/"+_AFLOW_IATOMS_FILE_)){AddFileNameBeforeExtension(directory_RAW+"/"+_AFLOW_IATOMS_FILE_,"orig",new_sym_file);aurostd::RenameFile(directory_RAW+"/"+_AFLOW_IATOMS_FILE_,new_sym_file);} //CO 171025
            KBIN_SymmetryWrite(FileMESSAGE,str_sp,aflags,_AGROUP_,false,message,"txt"); if(aurostd::FileExist(directory_RAW+"/"+_AFLOW_AGROUP_FILE_)){AddFileNameBeforeExtension(directory_RAW+"/"+_AFLOW_AGROUP_FILE_,"orig",new_sym_file);aurostd::RenameFile(directory_RAW+"/"+_AFLOW_AGROUP_FILE_,new_sym_file);} //CO 171025
            //json variants
            KBIN_SymmetryWrite(FileMESSAGE,str_sp,aflags,_PGROUP_,false,message,"json"); if(aurostd::FileExist(directory_RAW+"/"+_AFLOW_PGROUP_JSON_)){AddFileNameBeforeExtension(directory_RAW+"/"+_AFLOW_PGROUP_JSON_,"orig",new_sym_file);aurostd::RenameFile(directory_RAW+"/"+_AFLOW_PGROUP_JSON_,new_sym_file);} //CO 171025
            KBIN_SymmetryWrite(FileMESSAGE,str_sp,aflags,_PGROUPK_,false,message,"json"); if(aurostd::FileExist(directory_RAW+"/"+_AFLOW_PGROUPK_JSON_)){AddFileNameBeforeExtension(directory_RAW+"/"+_AFLOW_PGROUPK_JSON_,"orig",new_sym_file);aurostd::RenameFile(directory_RAW+"/"+_AFLOW_PGROUPK_JSON_,new_sym_file);} //CO 171025
            KBIN_SymmetryWrite(FileMESSAGE,str_sp,aflags,_FGROUP_,false,message,"json"); if(aurostd::FileExist(directory_RAW+"/"+_AFLOW_FGROUP_JSON_)){AddFileNameBeforeExtension(directory_RAW+"/"+_AFLOW_FGROUP_JSON_,"orig",new_sym_file);aurostd::RenameFile(directory_RAW+"/"+_AFLOW_FGROUP_JSON_,new_sym_file);} //CO 171025
            KBIN_SymmetryWrite(FileMESSAGE,str_sp,aflags,_PGROUP_XTAL_,false,message,"json"); if(aurostd::FileExist(directory_RAW+"/"+_AFLOW_PGROUP_XTAL_JSON_)){AddFileNameBeforeExtension(directory_RAW+"/"+_AFLOW_PGROUP_XTAL_JSON_,"orig",new_sym_file);aurostd::RenameFile(directory_RAW+"/"+_AFLOW_PGROUP_XTAL_JSON_,new_sym_file);} //CO 171025
            KBIN_SymmetryWrite(FileMESSAGE,str_sp,aflags,_PGROUPK_XTAL_,false,message,"json"); if(aurostd::FileExist(directory_RAW+"/"+_AFLOW_PGROUPK_XTAL_JSON_)){AddFileNameBeforeExtension(directory_RAW+"/"+_AFLOW_PGROUPK_XTAL_JSON_,"orig",new_sym_file);aurostd::RenameFile(directory_RAW+"/"+_AFLOW_PGROUPK_XTAL_JSON_,new_sym_file);} //DX 12/7/17 - added pgroupk_xtal
            KBIN_SymmetryWrite(FileMESSAGE,str_sp,aflags,_IATOMS_,false,message,"json"); if(aurostd::FileExist(directory_RAW+"/"+_AFLOW_IATOMS_JSON_)){AddFileNameBeforeExtension(directory_RAW+"/"+_AFLOW_IATOMS_JSON_,"orig",new_sym_file);aurostd::RenameFile(directory_RAW+"/"+_AFLOW_IATOMS_JSON_,new_sym_file);} //CO 171025
            KBIN_SymmetryWrite(FileMESSAGE,str_sp,aflags,_AGROUP_,false,message,"json"); if(aurostd::FileExist(directory_RAW+"/"+_AFLOW_AGROUP_JSON_)){AddFileNameBeforeExtension(directory_RAW+"/"+_AFLOW_AGROUP_JSON_,"orig",new_sym_file);aurostd::RenameFile(directory_RAW+"/"+_AFLOW_AGROUP_JSON_,new_sym_file);} //CO 171025
            //cout << message;
            //CO and DX STOP 170713 - adding symmetry output to RAW
            // now extract info
        ...
        if(flag_EDATA_RELAX_) { // RELAX
          if(!aurostd::FileExist(directory_RAW+"/"+_FILE_EDATA_RELAX_OUT_+"") && aurostd::FileExist(directory_RAW+"/CONTCAR.relax")) {
            ...
            // [OBSOLETE] aurostd::execute("cd "+directory_RAW+" && cat CONTCAR.relax | aflow --edata > "+_FILE_EDATA_RELAX_OUT_);
            str=xstructure(directory_RAW+"/CONTCAR.relax",IOAFLOW_AUTO);str_sp.Clear();str_sc.Clear();
            stringstream sss; sss << aflow::Banner("BANNER_TINY") << endl; 
            xstructure str_sym=str; //CO171027 //DX 2/26/18 - set equal str
            ...
            aurostd::stringstream2file(jjj,directory_RAW+"/"+_FILE_EDATA_RELAX_JSON_); //CO 171025
            vcif.clear();vcif.push_back(str);vcif.push_back(str_sp);vcif.push_back(str_sc);
            //CO and DX START 170713 - adding symmetry output to RAW
            string new_sym_file;
            //txt variants
            KBIN_SymmetryWrite(FileMESSAGE,str_sp,aflags,_PGROUP_,false,message,"txt"); if(aurostd::FileExist(directory_RAW+"/"+_AFLOW_PGROUP_FILE_)){AddFileNameBeforeExtension(directory_RAW+"/"+_AFLOW_PGROUP_FILE_,"relax",new_sym_file);aurostd::RenameFile(directory_RAW+"/"+_AFLOW_PGROUP_FILE_,new_sym_file);} //CO 171025
            KBIN_SymmetryWrite(FileMESSAGE,str_sp,aflags,_PGROUPK_,false,message,"txt"); if(aurostd::FileExist(directory_RAW+"/"+_AFLOW_PGROUPK_FILE_)){AddFileNameBeforeExtension(directory_RAW+"/"+_AFLOW_PGROUPK_FILE_,"relax",new_sym_file);aurostd::RenameFile(directory_RAW+"/"+_AFLOW_PGROUPK_FILE_,new_sym_file);} //CO 171025
            KBIN_SymmetryWrite(FileMESSAGE,str_sp,aflags,_FGROUP_,false,message,"txt"); if(aurostd::FileExist(directory_RAW+"/"+_AFLOW_FGROUP_FILE_)){AddFileNameBeforeExtension(directory_RAW+"/"+_AFLOW_FGROUP_FILE_,"relax",new_sym_file);aurostd::RenameFile(directory_RAW+"/"+_AFLOW_FGROUP_FILE_,new_sym_file);} //CO 171025
            KBIN_SymmetryWrite(FileMESSAGE,str_sp,aflags,_PGROUP_XTAL_,false,message,"txt"); if(aurostd::FileExist(directory_RAW+"/"+_AFLOW_PGROUP_XTAL_FILE_)){AddFileNameBeforeExtension(directory_RAW+"/"+_AFLOW_PGROUP_XTAL_FILE_,"relax",new_sym_file);aurostd::RenameFile(directory_RAW+"/"+_AFLOW_PGROUP_XTAL_FILE_,new_sym_file);} //CO 171025
            KBIN_SymmetryWrite(FileMESSAGE,str_sp,aflags,_PGROUPK_XTAL_,false,message,"txt"); if(aurostd::FileExist(directory_RAW+"/"+_AFLOW_PGROUPK_XTAL_FILE_)){AddFileNameBeforeExtension(directory_RAW+"/"+_AFLOW_PGROUPK_XTAL_FILE_,"relax",new_sym_file);aurostd::RenameFile(directory_RAW+"/"+_AFLOW_PGROUPK_XTAL_FILE_,new_sym_file);} //DX 12/7/17 - added pgroupk_xtal
            KBIN_SymmetryWrite(FileMESSAGE,str_sp,aflags,_IATOMS_,false,message,"txt"); if(aurostd::FileExist(directory_RAW+"/"+_AFLOW_IATOMS_FILE_)){AddFileNameBeforeExtension(directory_RAW+"/"+_AFLOW_IATOMS_FILE_,"relax",new_sym_file);aurostd::RenameFile(directory_RAW+"/"+_AFLOW_IATOMS_FILE_,new_sym_file);} //CO 171025
            KBIN_SymmetryWrite(FileMESSAGE,str_sp,aflags,_AGROUP_,false,message,"txt"); if(aurostd::FileExist(directory_RAW+"/"+_AFLOW_AGROUP_FILE_)){AddFileNameBeforeExtension(directory_RAW+"/"+_AFLOW_AGROUP_FILE_,"relax",new_sym_file);aurostd::RenameFile(directory_RAW+"/"+_AFLOW_AGROUP_FILE_,new_sym_file);} //CO 171025
            //json variants
            KBIN_SymmetryWrite(FileMESSAGE,str_sp,aflags,_PGROUP_,false,message,"json"); if(aurostd::FileExist(directory_RAW+"/"+_AFLOW_PGROUP_JSON_)){AddFileNameBeforeExtension(directory_RAW+"/"+_AFLOW_PGROUP_JSON_,"relax",new_sym_file);aurostd::RenameFile(directory_RAW+"/"+_AFLOW_PGROUP_JSON_,new_sym_file);} //CO 171025
            KBIN_SymmetryWrite(FileMESSAGE,str_sp,aflags,_PGROUPK_,false,message,"json"); if(aurostd::FileExist(directory_RAW+"/"+_AFLOW_PGROUPK_JSON_)){AddFileNameBeforeExtension(directory_RAW+"/"+_AFLOW_PGROUPK_JSON_,"relax",new_sym_file);aurostd::RenameFile(directory_RAW+"/"+_AFLOW_PGROUPK_JSON_,new_sym_file);} //CO 171025
            KBIN_SymmetryWrite(FileMESSAGE,str_sp,aflags,_FGROUP_,false,message,"json"); if(aurostd::FileExist(directory_RAW+"/"+_AFLOW_FGROUP_JSON_)){AddFileNameBeforeExtension(directory_RAW+"/"+_AFLOW_FGROUP_JSON_,"relax",new_sym_file);aurostd::RenameFile(directory_RAW+"/"+_AFLOW_FGROUP_JSON_,new_sym_file);} //CO 171025
            KBIN_SymmetryWrite(FileMESSAGE,str_sp,aflags,_PGROUP_XTAL_,false,message,"json"); if(aurostd::FileExist(directory_RAW+"/"+_AFLOW_PGROUP_XTAL_JSON_)){AddFileNameBeforeExtension(directory_RAW+"/"+_AFLOW_PGROUP_XTAL_JSON_,"relax",new_sym_file);aurostd::RenameFile(directory_RAW+"/"+_AFLOW_PGROUP_XTAL_JSON_,new_sym_file);} //CO 171025
            KBIN_SymmetryWrite(FileMESSAGE,str_sp,aflags,_PGROUPK_XTAL_,false,message,"json"); if(aurostd::FileExist(directory_RAW+"/"+_AFLOW_PGROUPK_XTAL_JSON_)){AddFileNameBeforeExtension(directory_RAW+"/"+_AFLOW_PGROUPK_XTAL_JSON_,"relax",new_sym_file);aurostd::RenameFile(directory_RAW+"/"+_AFLOW_PGROUPK_XTAL_JSON_,new_sym_file);} //DX 12/7/17 - added pgroupk_xtal
            KBIN_SymmetryWrite(FileMESSAGE,str_sp,aflags,_IATOMS_,false,message,"json"); if(aurostd::FileExist(directory_RAW+"/"+_AFLOW_IATOMS_JSON_)){AddFileNameBeforeExtension(directory_RAW+"/"+_AFLOW_IATOMS_JSON_,"relax",new_sym_file);aurostd::RenameFile(directory_RAW+"/"+_AFLOW_IATOMS_JSON_,new_sym_file);} //CO 171025
            KBIN_SymmetryWrite(FileMESSAGE,str_sp,aflags,_AGROUP_,false,message,"json"); if(aurostd::FileExist(directory_RAW+"/"+_AFLOW_AGROUP_JSON_)){AddFileNameBeforeExtension(directory_RAW+"/"+_AFLOW_AGROUP_JSON_,"relax",new_sym_file);aurostd::RenameFile(directory_RAW+"/"+_AFLOW_AGROUP_JSON_,new_sym_file);} //CO 171025
            //cout << message;
            //CO and DX STOP 170713 - adding symmetry output to RAW
            // now extract info
            vector<string> vline_edata;
        ...
        if(flag_EDATA_BANDS_) { // BANDS
          if(!aurostd::FileExist(directory_RAW+"/"+_FILE_EDATA_BANDS_OUT_+"") && aurostd::FileExist(directory_RAW+"/POSCAR.bands")) {
            ...
            // [OBSOLETE] aurostd::execute("cd "+directory_RAW+" && cat POSCAR.bands | aflow --edata > "+_FILE_EDATA_BANDS_OUT_);
            str=xstructure(directory_RAW+"/POSCAR.bands",IOAFLOW_AUTO);str_sp.Clear();str_sc.Clear();
            stringstream sss; sss << aflow::Banner("BANNER_TINY") << endl; 
            xstructure str_sym=str; //CO171027 //DX 2/26/18 - set equal str
            pflow::PrintData(str,str_sym,str_sp,str_sc,sss,"EDATA","txt",false); // EDATA //CO 171025 //CO171027
            ...
            stringstream jjj; //CO 171025
            pflow::PrintData(str_sym,str_sym,str_sp,str_sc,jjj,"EDATA","json",true); // EDATA already_calculated! //CO 171025 //CO171027
            aurostd::stringstream2file(jjj,directory_RAW+"/"+_FILE_EDATA_BANDS_JSON_); //CO 171025
            vcif.clear();vcif.push_back(str);vcif.push_back(str_sp);vcif.push_back(str_sc);
            //CO and DX START 170713 - adding symmetry output to RAW
            string new_sym_file;
            //txt variants
            KBIN_SymmetryWrite(FileMESSAGE,str_sp,aflags,_PGROUP_,false,message,"txt"); if(aurostd::FileExist(directory_RAW+"/"+_AFLOW_PGROUP_FILE_)){AddFileNameBeforeExtension(directory_RAW+"/"+_AFLOW_PGROUP_FILE_,"bands",new_sym_file);aurostd::RenameFile(directory_RAW+"/"+_AFLOW_PGROUP_FILE_,new_sym_file);} //CO 171025
            KBIN_SymmetryWrite(FileMESSAGE,str_sp,aflags,_PGROUPK_,false,message,"txt"); if(aurostd::FileExist(directory_RAW+"/"+_AFLOW_PGROUPK_FILE_)){AddFileNameBeforeExtension(directory_RAW+"/"+_AFLOW_PGROUPK_FILE_,"bands",new_sym_file);aurostd::RenameFile(directory_RAW+"/"+_AFLOW_PGROUPK_FILE_,new_sym_file);} //CO 171025
            KBIN_SymmetryWrite(FileMESSAGE,str_sp,aflags,_FGROUP_,false,message,"txt"); if(aurostd::FileExist(directory_RAW+"/"+_AFLOW_FGROUP_FILE_)){AddFileNameBeforeExtension(directory_RAW+"/"+_AFLOW_FGROUP_FILE_,"bands",new_sym_file);aurostd::RenameFile(directory_RAW+"/"+_AFLOW_FGROUP_FILE_,new_sym_file);} //CO 171025
            KBIN_SymmetryWrite(FileMESSAGE,str_sp,aflags,_PGROUP_XTAL_,false,message,"txt"); if(aurostd::FileExist(directory_RAW+"/"+_AFLOW_PGROUP_XTAL_FILE_)){AddFileNameBeforeExtension(directory_RAW+"/"+_AFLOW_PGROUP_XTAL_FILE_,"bands",new_sym_file);aurostd::RenameFile(directory_RAW+"/"+_AFLOW_PGROUP_XTAL_FILE_,new_sym_file);} //CO 171025
            KBIN_SymmetryWrite(FileMESSAGE,str_sp,aflags,_PGROUPK_XTAL_,false,message,"txt"); if(aurostd::FileExist(directory_RAW+"/"+_AFLOW_PGROUPK_XTAL_FILE_)){AddFileNameBeforeExtension(directory_RAW+"/"+_AFLOW_PGROUPK_XTAL_FILE_,"bands",new_sym_file);aurostd::RenameFile(directory_RAW+"/"+_AFLOW_PGROUPK_XTAL_FILE_,new_sym_file);} //DX 12/7/17 - added pgroupk_xtal
            KBIN_SymmetryWrite(FileMESSAGE,str_sp,aflags,_IATOMS_,false,message,"txt"); if(aurostd::FileExist(directory_RAW+"/"+_AFLOW_IATOMS_FILE_)){AddFileNameBeforeExtension(directory_RAW+"/"+_AFLOW_IATOMS_FILE_,"bands",new_sym_file);aurostd::RenameFile(directory_RAW+"/"+_AFLOW_IATOMS_FILE_,new_sym_file);} //CO 171025
            KBIN_SymmetryWrite(FileMESSAGE,str_sp,aflags,_AGROUP_,false,message,"txt"); if(aurostd::FileExist(directory_RAW+"/"+_AFLOW_AGROUP_FILE_)){AddFileNameBeforeExtension(directory_RAW+"/"+_AFLOW_AGROUP_FILE_,"bands",new_sym_file);aurostd::RenameFile(directory_RAW+"/"+_AFLOW_AGROUP_FILE_,new_sym_file);} //CO 171025
            //json variants
            KBIN_SymmetryWrite(FileMESSAGE,str_sp,aflags,_PGROUP_,false,message,"json"); if(aurostd::FileExist(directory_RAW+"/"+_AFLOW_PGROUP_JSON_)){AddFileNameBeforeExtension(directory_RAW+"/"+_AFLOW_PGROUP_JSON_,"bands",new_sym_file);aurostd::RenameFile(directory_RAW+"/"+_AFLOW_PGROUP_JSON_,new_sym_file);} //CO 171025
            KBIN_SymmetryWrite(FileMESSAGE,str_sp,aflags,_PGROUPK_,false,message,"json"); if(aurostd::FileExist(directory_RAW+"/"+_AFLOW_PGROUPK_JSON_)){AddFileNameBeforeExtension(directory_RAW+"/"+_AFLOW_PGROUPK_JSON_,"bands",new_sym_file);aurostd::RenameFile(directory_RAW+"/"+_AFLOW_PGROUPK_JSON_,new_sym_file);} //CO 171025
            KBIN_SymmetryWrite(FileMESSAGE,str_sp,aflags,_FGROUP_,false,message,"json"); if(aurostd::FileExist(directory_RAW+"/"+_AFLOW_FGROUP_JSON_)){AddFileNameBeforeExtension(directory_RAW+"/"+_AFLOW_FGROUP_JSON_,"bands",new_sym_file);aurostd::RenameFile(directory_RAW+"/"+_AFLOW_FGROUP_JSON_,new_sym_file);} //CO 171025
            KBIN_SymmetryWrite(FileMESSAGE,str_sp,aflags,_PGROUP_XTAL_,false,message,"json"); if(aurostd::FileExist(directory_RAW+"/"+_AFLOW_PGROUP_XTAL_JSON_)){AddFileNameBeforeExtension(directory_RAW+"/"+_AFLOW_PGROUP_XTAL_JSON_,"bands",new_sym_file);aurostd::RenameFile(directory_RAW+"/"+_AFLOW_PGROUP_XTAL_JSON_,new_sym_file);} //CO 171025
            KBIN_SymmetryWrite(FileMESSAGE,str_sp,aflags,_PGROUPK_XTAL_,false,message,"json"); if(aurostd::FileExist(directory_RAW+"/"+_AFLOW_PGROUPK_XTAL_JSON_)){AddFileNameBeforeExtension(directory_RAW+"/"+_AFLOW_PGROUPK_XTAL_JSON_,"bands",new_sym_file);aurostd::RenameFile(directory_RAW+"/"+_AFLOW_PGROUPK_XTAL_JSON_,new_sym_file);} //DX 12/7/17 - added pgroupk_xtal
            KBIN_SymmetryWrite(FileMESSAGE,str_sp,aflags,_IATOMS_,false,message,"json"); if(aurostd::FileExist(directory_RAW+"/"+_AFLOW_IATOMS_JSON_)){AddFileNameBeforeExtension(directory_RAW+"/"+_AFLOW_IATOMS_JSON_,"bands",new_sym_file);aurostd::RenameFile(directory_RAW+"/"+_AFLOW_IATOMS_JSON_,new_sym_file);} //CO 171025
            KBIN_SymmetryWrite(FileMESSAGE,str_sp,aflags,_AGROUP_,false,message,"json"); if(aurostd::FileExist(directory_RAW+"/"+_AFLOW_AGROUP_JSON_)){AddFileNameBeforeExtension(directory_RAW+"/"+_AFLOW_AGROUP_JSON_,"bands",new_sym_file);aurostd::RenameFile(directory_RAW+"/"+_AFLOW_AGROUP_JSON_,new_sym_file);} //CO 171025
            //cout << message;
            //CO and DX STOP 170713 - adding symmetry output to RAW
            // now extract info
            vector<string> vline_edata;
        ...
        if(flag_DATA_ORIG_) { // ORIG
          if(!aurostd::FileExist(directory_RAW+"/"+_FILE_DATA_ORIG_OUT_+"") && aurostd::FileExist(directory_RAW+"/POSCAR.orig")) {
            ...
            aurostd::stringstream2file(jjj,directory_RAW+"/"+_FILE_DATA_ORIG_JSON_); //CO 171025
          }
        }
        ...
        if(flag_DATA_RELAX_) { // RELAX
          if(!aurostd::FileExist(directory_RAW+"/"+_FILE_DATA_RELAX_OUT_+"") && aurostd::FileExist(directory_RAW+"/CONTCAR.relax")) {
            ...
            stringstream jjj; //CO 171025
            pflow::PrintData(str_sym,str_sym,str_sp,str_sc,jjj,"DATA","json",true); // DATA already_calculated! //CO 171025 //CO171027
            aurostd::stringstream2file(jjj,directory_RAW+"/"+_FILE_DATA_RELAX_JSON_); //CO 171025
          }
        }
        ...
        if(flag_DATA_BANDS_) { // BANDS
          if(!aurostd::FileExist(directory_RAW+"/"+_FILE_DATA_BANDS_OUT_+"") && aurostd::FileExist(directory_RAW+"/POSCAR.bands")) {
            ...
            stringstream jjj; //CO 171025
            pflow::PrintData(str_sym,str_sym,str_sp,str_sc,jjj,"DATA","json",true); // DATA already_calculated! //CO 171025 //CO171027
            aurostd::stringstream2file(jjj,directory_RAW+"/"+_FILE_DATA_BANDS_JSON_); //CO 171025
          }
        }
        ...
        if(flag_LIB2==TRUE) {
          ...
          if(_APENNSY_STYLE_OLD_) {fileA_LIB=directory_LIB+"/aflow.agroup.out.bz2";fileA_RAW=directory_RAW+"/aflow.agroup.out.bz2";if(aurostd::FileExist(fileA_LIB)) {aflowlib::LIB2RAW_FileNeeded(directory_LIB,"aflow.agroup.out.bz2",directory_RAW,"aflow.agroup.out.bz2",vfiles,MESSAGE);}}
          if(_APENNSY_STYLE_OLD_) {fileA_LIB=directory_LIB+"/aflow.fgroup.out.bz2";fileA_RAW=directory_RAW+"/aflow.fgroup.out.bz2";if(aurostd::FileExist(fileA_LIB)) {aflowlib::LIB2RAW_FileNeeded(directory_LIB,"aflow.fgroup.out.bz2",directory_RAW,"aflow.fgroup.out.bz2",vfiles,MESSAGE);}}
          if(_APENNSY_STYLE_OLD_) {fileA_LIB=directory_LIB+"/aflow.pgroup.out.bz2";fileA_RAW=directory_RAW+"/aflow.pgroup.out.bz2";if(aurostd::FileExist(fileA_LIB)) {aflowlib::LIB2RAW_FileNeeded(directory_LIB,"aflow.pgroup.out.bz2",directory_RAW,"aflow.pgroup.out.bz2",vfiles,MESSAGE);}}
          if(_APENNSY_STYLE_OLD_) {fileA_LIB=directory_LIB+"/aflow.iatoms.out.bz2";fileA_RAW=directory_RAW+"/aflow.iatoms.out.bz2";if(aurostd::FileExist(fileA_LIB)) {aflowlib::LIB2RAW_FileNeeded(directory_LIB,"aflow.iatoms.out.bz2",directory_RAW,"aflow.iatoms.out.bz2",vfiles,MESSAGE);}}
          fileA_LIB=directory_LIB+"/aflow.qmvasp.out.bz2";fileA_RAW=directory_RAW+"/aflow.qmvasp.out.bz2";if(aurostd::FileExist(fileA_LIB)) {aflowlib::LIB2RAW_FileNeeded(directory_LIB,"aflow.qmvasp.out.bz2",directory_RAW,"aflow.qmvasp.out.bz2",vfiles,MESSAGE);}
          // no vasp.out.relax1  fileA_LIB=directory_LIB+"/vasp.out.relax1.bz2";fileA_RAW=directory_RAW+"/vasp.out.relax1.bz2";if(aurostd::FileExist(fileA_LIB)) {aflowlib::LIB2RAW_FileNeeded(directory_LIB,"vasp.out.relax1.bz2",directory_RAW,"vasp.out.relax1.bz2",vfiles,MESSAGE);}
          // no vasp.out.relax2  fileA_LIB=directory_LIB+"/vasp.out.relax2.bz2";fileA_RAW=directory_RAW+"/vasp.out.relax2.bz2";if(aurostd::FileExist(fileA_LIB)) {aflowlib::LIB2RAW_FileNeeded(directory_LIB,"vasp.out.relax2.bz2",directory_RAW,"vasp.out.relax2.bz2",vfiles,MESSAGE);}
          if(_APENNSY_STYLE_OLD_) {fileX_LIB=directory_LIB+"/POSCAR.relax1.bz2";fileX_RAW=directory_RAW+"/POSCAR.relax1.bz2";if(aurostd::FileExist(fileX_LIB)) {aflowlib::LIB2RAW_FileNeeded(directory_LIB,"POSCAR.relax1.bz2",directory_RAW,"POSCAR.relax1.bz2",vfiles,MESSAGE);}}
          fileX_LIB=directory_LIB+"/POSCAR.relax2.bz2";fileX_RAW=directory_RAW+"/POSCAR.relax2.bz2";if(aurostd::FileExist(fileX_LIB)) {aflowlib::LIB2RAW_FileNeeded(directory_LIB,"POSCAR.relax2.bz2",directory_RAW,"POSCAR.relax2.bz2",vfiles,MESSAGE);}
          aurostd::CopyFile(directory_RAW+"/CONTCAR.relax",directory_RAW+"/CONTCAR.relax2");vfiles.push_back("CONTCAR.relax2");
          // aurostd::CopyFile(directory_LIB+"/OSZICAR.relax1.bz2",directory_RAW+"/OSZICAR.relax1.bz2");vfiles.push_back("OSZICAR.relax1");
          // aurostd::CopyFile(directory_LIB+"/OSZICAR.relax2.bz2",directory_RAW+"/OSZICAR.relax2.bz2");vfiles.push_back("OSZICAR.relax2");
          if(_APENNSY_STYLE_OLD_) {aurostd::CopyFile(directory_RAW+"/OSZICAR.relax",directory_RAW+"/OSZICAR.relax2");vfiles.push_back("OSZICAR.relax2");}
        ...
        // COMPRESS
        // aurostd::BzipFile(directory_RAW+"/OSZICAR.relax");
    ...
    // ***************************************************************************
    namespace aflowlib {
      bool LIB2RAW_Loop_Magnetic(string& directory_LIB,string& directory_RAW,vector<string> &vfiles,aflowlib::_aflowlib_entry& data,string MESSAGE) {
        ...
        outcar_static.clear();
        if(aurostd::FileExist(directory_LIB+"/"+"OUTCAR.static.bz2")) {
          aflowlib::LIB2RAW_FileNeeded(directory_LIB,"OUTCAR.static",directory_RAW,"OUTCAR.static",vfiles,MESSAGE);  // OUTCAR.static
        ...
        outcar_bands.clear();
        doscar.clear();
        if(aurostd::FileExist(directory_LIB+"/"+"OUTCAR.bands.bz2")) {
          ...
          if(outcar_bands.GetPropertiesFile(directory_RAW+"/"+"OUTCAR.bands")){
            ...
            if(data.Egap<MAG_EPS && aurostd::abs(data.spin_cell)>MAG_EPS) { //must be metal and magnetic
              if(doscar.content=="" && aurostd::FileExist(directory_LIB+"/"+"DOSCAR.static.bz2")) {
                aflowlib::LIB2RAW_FileNeeded(directory_LIB,"DOSCAR.static",directory_RAW,"DOSCAR.static",vfiles,MESSAGE);  // DOSCAR.static
              ...
              if(doscar.content=="" && aurostd::FileExist(directory_LIB+"/"+"DOSCAR.relax2.bz2")) {
              ...
              if(!doscar.content.empty()) {data.spinF=doscar.spinF;}
              else{cout << MESSAGE << " MISSING DOSCAR.static and DOSCAR.relax[2]" << endl;}
            }
          }else{cout << MESSAGE << " ERROR OUTCAR.static properties cannot be extracted: " << outcar_static.ERROR << endl;}
        }else{cout << MESSAGE << " MISSING OUTCAR.static" << endl;}
    ...
    // ***************************************************************************
    namespace aflowlib {
      bool LIB2RAW_Loop_Bader(string& directory_LIB,string& directory_RAW,vector<string> &vfiles,aflowlib::_aflowlib_entry& data,string MESSAGE) {
        bool LDEBUG=(FALSE || XHOST.DEBUG);
        ...
        // [OBSOLETE]aflowlib::LIB2RAW_FileNeeded(directory_LIB,"OUTCAR.static",directory_RAW,"OUTCAR.static",vfiles,MESSAGE);  // OUTCAR.static
        ...
        if(aurostd::FileExist(directory_LIB+"/"+"CHGCAR.static.bz2")) {
          aflowlib::LIB2RAW_FileNeeded(directory_LIB,"CHGCAR.static",directory_RAW,"CHGCAR.static",vfiles,MESSAGE);  // OUTCAR.static
          if(AFLOWLIB_VERBOSE) cout << MESSAGE << " loading " << string(directory_RAW+"/"+"CHGCAR.static") << endl;
        } else {
          cout << MESSAGE << " MISSING CHGCAR.static" << endl;
          return FALSE;
        }
        if(aurostd::FileExist(directory_LIB+"/"+"AECCAR0.static.bz2")) {
          aflowlib::LIB2RAW_FileNeeded(directory_LIB,"AECCAR0.static",directory_RAW,"AECCAR0.static",vfiles,MESSAGE);  // OUTCAR.static
        ...
        if(aurostd::FileExist(directory_LIB+"/"+"AECCAR2.static.bz2")) {
          aflowlib::LIB2RAW_FileNeeded(directory_LIB,"AECCAR2.static",directory_RAW,"AECCAR2.static",vfiles,MESSAGE);  // OUTCAR.static
    ...
    // ***************************************************************************
    namespace aflowlib {
      bool LIB2RAW_Loop_AGL(string& directory_LIB,string& directory_RAW,vector<string> &vfiles,aflowlib::_aflowlib_entry& data,string MESSAGE) {
        bool LDEBUG=(FALSE || XHOST.DEBUG);
        ...
        // AFLOW AGL 
        aflow_agl_out.clear();
        if(aurostd::FileExist(directory_LIB+"/"+"aflow.agl.out.bz2")) {
          aflowlib::LIB2RAW_FileNeeded(directory_LIB,"aflow.agl.out.bz2",directory_RAW,"aflow.agl.out",vfiles,MESSAGE);  // aflow.agl.out
          aflowlib::LIB2RAW_FileNeeded(directory_LIB,"AGL.out.bz2",directory_RAW,"AGL.out",vfiles,MESSAGE);  // AGL.out
          aflowlib::LIB2RAW_FileNeeded(directory_LIB,"AGL_energies_temperature.out.bz2",directory_RAW,"AGL_energies_temperature.out",vfiles,MESSAGE);  // AGL.out
          aflowlib::LIB2RAW_FileNeeded(directory_LIB,"AGL_thermal_properties_temperature.out.bz2",directory_RAW,"AGL_thermal_properties_temperature.out",vfiles,MESSAGE);  // AGL.out
    ...
    // ***************************************************************************
    namespace aflowlib {
      bool LIB2RAW_Loop_AEL(string& directory_LIB,string& directory_RAW,vector<string> &vfiles,aflowlib::_aflowlib_entry& data,string MESSAGE) {
        bool LDEBUG=(FALSE || XHOST.DEBUG);
        ...
        // AFLOW AEL 
        aflow_ael_out.clear();
        if(aurostd::FileExist(directory_LIB+"/"+"aflow.ael.out.bz2")) {
          aflowlib::LIB2RAW_FileNeeded(directory_LIB,"aflow.ael.out.bz2",directory_RAW,"aflow.ael.out",vfiles,MESSAGE);  // aflow.ael.out
          aflowlib::LIB2RAW_FileNeeded(directory_LIB,"AEL_Elastic_constants.out.bz2",directory_RAW,"AEL_Elastic_constants.out",vfiles,MESSAGE);  // AEL_Elastic_constants.out
          aflowlib::LIB2RAW_FileNeeded(directory_LIB,"AEL_Compliance_tensor.out.bz2",directory_RAW,"AEL_Compliance_tensor.out",vfiles,MESSAGE);  // AEL_Compliance_tensor.out
    ...
    namespace aflowlib {
      bool LIB2RAW_Loop_LOCK(string& directory_LIB,string& directory_RAW,vector<string> &vfiles,aflowlib::_aflowlib_entry& data,string MESSAGE) {
        ...
        aurostd::string2tokens("POTCAR.relax1.bz2,POTCAR.relax2.bz2,POTCAR.relax3.bz2,POTCAR.static.bz2,POTCAR.bands.bz2,AECCAR1.static.bz2,AECCAR0.bands.bz2,AECCAR1.bands.bz2,AECCAR2.bands.bz2",vremove,",");
        for(uint i=0;i<vremove.size();i++) {
          vremovej.clear();
          aurostd::string2vectorstring(aurostd::execute2string("find "+directory_LIB+" -name \""+vremove.at(i)+"\""),vremovej);
          for(uint j=0;j<vremovej.size();j++) {
            if(aurostd::FileExist(vremovej.at(j))) {
              aurostd::RemoveFile(vremovej.at(j));
              if(AFLOWLIB_VERBOSE) cout << MESSAGE << " REMOVED = " << aurostd::CleanFileName(vremovej.at(j)) << endl;
            }
          }
        }
        // [OBSOLETE]  for(uint i=0;i<vremove.size();i++) {
        // [OBSOLETE]  if(aurostd::FileExist(directory_LIB+"/"+vremove.at(i))) {
        // [OBSOLETE]	aurostd::RemoveFile(directory_LIB+"/"+vremove.at(i));
        // [OBSOLETE]	if(AFLOWLIB_VERBOSE) cout << MESSAGE << " REMOVED = " << aurostd::CleanFileName(directory_LIB+"/"+vremove.at(i)) << endl;
        // [OBSOLETE]   }
        // [OBSOLETE]  }
        // done   ---------------------------------------------------------------
...
#define _XPLUG_FLUSH_THREAD_SLEEP_ 1
    ...
    namespace aflowlib {
      bool XPLUG_Directory_ok(const string &directory,const uint &i,const uint &j,const int& ithread,stringstream &oss) {
        ...
        int answer=0;
        stringstream oss_buffer;

        // clean
        aurostd::RemoveSubString(dir,"/"+_AFLOWIN_);
        aurostd::RemoveSubString(dir,"/aflow.end.out");
        vector<string> vrem_name; vrem_name.push_back("OUTCAR");vrem_name.push_back("OSZICAR");vrem_name.push_back("CONTCAR");vrem_name.push_back("EIGENVAL");
        vector<string> vrem_extn; vrem_extn.push_back(".relax1.bz2");vrem_extn.push_back(".relax2.bz2");vrem_extn.push_back(".relax3.bz2");vrem_extn.push_back(".static.bz2");vrem_extn.push_back(".bands.bz2");
        for(uint i1=0;i1<vrem_name.size();i1++) 
          for(uint i2=0;i2<vrem_extn.size();i2++) 
            aurostd::RemoveSubString(dir,"/"+vrem_name.at(i1)+vrem_extn.at(i2));

        if(!aurostd::FileExist(dir+"/"+_AFLOWLOCK_)) {
          return FALSE;
        }
        if(!aurostd::FileExist(dir+"/OUTCAR.relax1.bz2") &&
            !aurostd::FileExist(dir+"/OUTCAR.relax2.bz2") &&
            !aurostd::FileExist(dir+"/OUTCAR.relax3.bz2") &&
            !aurostd::FileExist(dir+"/OUTCAR.static.bz2") &&
            !aurostd::FileExist(dir+"/OUTCAR.bands.bz2")) {
          return FALSE;
        }

        oss_buffer << "[" << i+1 << "/" << j << "] ";

        if(ithread<0)  oss_buffer << "DIR=" << dir << " ";
        if(ithread>=0) oss_buffer << "DIR(" << ithread << ")="<< dir << " ";
        // CHECK ALL FILES
        print=TRUE;
        ok=TRUE;
        // TEST INCOMPLETE
        if(ok) {oss_buffer << ".";if(!aurostd::FileExist(dir+"/"+_AFLOWLOCK_)) {ok=FALSE;oss_buffer << " no=LOCK";}}
        if(ok) {oss_buffer << ".";if(aurostd::FileExist(dir+"/EIGENVAL")) {ok=FALSE;oss_buffer << " yes=EIGENVAL";}}
        if(ok) {oss_buffer << ".";if(aurostd::FileExist(dir+"/vasp.out")) {ok=FALSE;oss_buffer << " yes=vasp.out";}}
        if(ok) {oss_buffer << ".";if(aurostd::FileExist(dir+"/AECCAR0")) {ok=FALSE;oss_buffer << " yes=AECCAR0";}}
        if(ok) {oss_buffer << ".";if(aurostd::FileExist(dir+"/AECCAR1")) {ok=FALSE;oss_buffer << " yes=AECCAR1";}}
        if(ok) {oss_buffer << ".";if(aurostd::FileExist(dir+"/AECCAR2")) {ok=FALSE;oss_buffer << " yes=AECCAR2";}}
        if(ok) {oss_buffer << ".";if(aurostd::FileExist(dir+"/CHGCAR")) {ok=FALSE;oss_buffer << " yes=CHGCAR";}}
        if(ok) {oss_buffer << ".";if(aurostd::FileExist(dir+"/DOSCAR")) {ok=FALSE;oss_buffer << " yes=DOSCAR";}}
        if(ok) {oss_buffer << ".";if(aurostd::FileExist(dir+"/OUTCAR")) {ok=FALSE;oss_buffer << " yes=OUTCAR";}}
        if(ok) {oss_buffer << ".";if(aurostd::FileExist(dir+"/OUTCAR.relax1")) {ok=FALSE;oss_buffer << " yes=OUTCAR.relax1";}}
        if(ok) {oss_buffer << ".";if(aurostd::FileExist(dir+"/OUTCAR.relax2")) {ok=FALSE;oss_buffer << " yes=OUTCAR.relax2";}}
        if(ok) {oss_buffer << ".";if(aurostd::FileExist(dir+"/OUTCAR.relax3")) {ok=FALSE;oss_buffer << " yes=OUTCAR.relax3";}}
        if(ok) {oss_buffer << ".";if(aurostd::FileExist(dir+"/OSZICAR")) {ok=FALSE;oss_buffer << " yes=OSZICAR";}}
        if(ok) {oss_buffer << ".";if(aurostd::FileExist(dir+"/OSZICAR.relax1")) {ok=FALSE;oss_buffer << " yes=OSZICAR.relax1";}}
        if(ok) {oss_buffer << ".";if(aurostd::FileExist(dir+"/OSZICAR.relax2")) {ok=FALSE;oss_buffer << " yes=OSZICAR.relax2";}}
        if(ok) {oss_buffer << ".";if(aurostd::FileExist(dir+"/OSZICAR.relax3")) {ok=FALSE;oss_buffer << " yes=OSZICAR.relax3";}}
        if(ok) {oss_buffer << ".";if(aurostd::FileExist(dir+"/POTCAR")) {ok=FALSE;oss_buffer << " yes=POTCAR";}}
        if(ok) {oss_buffer << ".";if(aurostd::FileExist(dir+"/POTCAR.relax1")) {ok=FALSE;oss_buffer << " yes=POTCAR.relax1";}}
        if(ok) {oss_buffer << ".";if(aurostd::FileExist(dir+"/POTCAR.relax2")) {ok=FALSE;oss_buffer << " yes=POTCAR.relax2";}}
        if(ok) {oss_buffer << ".";if(aurostd::FileExist(dir+"/POTCAR.relax3")) {ok=FALSE;oss_buffer << " yes=POTCAR.relax3";}}

        // TEST RELAX1
        if(ok && aurostd::FileExist(dir+"/OUTCAR.relax1.bz2")) { // relax1 etc etc...
          if(ok) {oss_buffer << ".";if(!aurostd::FileExist(dir+"/OUTCAR.relax1.bz2")) {ok=FALSE;print=FALSE;oss_buffer << " no=OUTCAR.relax1.bz2";}}
          if(ok) {oss_buffer << ".";if(!aurostd::FileExist(dir+"/OSZICAR.relax1.bz2")) {ok=FALSE;oss_buffer << " no=OSZICAR.relax1.bz2";}}
          if(ok) {oss_buffer << ".";if(!aurostd::FileExist(dir+"/vasp.out.relax1.bz2")) {ok=FALSE;oss_buffer << " no=vasp.out.relax1.bz2";}}
          if(ok) {oss_buffer << ".";if(!aurostd::FileExist(dir+"/EIGENVAL.relax1.bz2")) {ok=FALSE;oss_buffer << " no=EIGENVAL.relax1.bz2";}}
          // OUTCAR.relax1.bz2
          if(ok) {oss_buffer << ".";  // check Answer 4 or 5 in OUTCAR.relax1.bz2
            answer=aurostd::execute2utype<int>(XHOST.command("bzgrep")+" -c \"(sec)\" "+dir+"/OUTCAR.relax1.bz2");
            if(answer!=4 && answer!=5) {ok=FALSE;oss_buffer << " error(" << answer << ")=OUTCAR.relax1.bz2";}}
          // vasp.out.relax1.bz2
          if(ok) {oss_buffer << ".";
            answer=aurostd::execute2utype<int>(XHOST.command("bzgrep")+" -c \"The distance between some ions is very small\" "+dir+"/vasp.out.relax1.bz2");
            if(answer!=0) {ok=FALSE;oss_buffer << " ions=vasp.out.relax1.bz2";}}
        }
        // TEST RELAX2
        if(ok && aurostd::FileExist(dir+"/OUTCAR.relax2.bz2")) { // relax2 etc etc...
          if(ok) {oss_buffer << ".";if(!aurostd::FileExist(dir+"/OUTCAR.relax2.bz2")) {ok=FALSE;print=FALSE;oss_buffer << " no=OUTCAR.relax2.bz2";}}
          if(ok) {oss_buffer << ".";if(!aurostd::FileExist(dir+"/OSZICAR.relax2.bz2")) {ok=FALSE;oss_buffer << " no=OSZICAR.relax2.bz2";}}
          if(ok) {oss_buffer << ".";if(!aurostd::FileExist(dir+"/vasp.out.relax2.bz2")) {ok=FALSE;oss_buffer << " no=vasp.out.relax2.bz2";}}
          if(ok) {oss_buffer << ".";if(!aurostd::FileExist(dir+"/EIGENVAL.relax2.bz2")) {ok=FALSE;oss_buffer << " no=EIGENVAL.relax2.bz2";}}
          // OUTCAR.relax2.bz2
          if(ok) {oss_buffer << ".";  // check Answer 4 or 5 in OUTCAR.relax2.bz2
            answer=aurostd::execute2utype<int>(XHOST.command("bzgrep")+" -c \"(sec)\" "+dir+"/OUTCAR.relax2.bz2");
            if(answer!=4 && answer!=5) {ok=FALSE;oss_buffer << " error(" << answer << ")=OUTCAR.relax2.bz2";}}
          // vasp.out.relax2.bz2
          if(ok) {oss_buffer << ".";
            answer=aurostd::execute2utype<int>(XHOST.command("bzgrep")+" -c \"The distance between some ions is very small\" "+dir+"/vasp.out.relax2.bz2");
            if(answer!=0) {ok=FALSE;oss_buffer << " ions=vasp.out.relax2.bz2";}}
        }
        // TEST RELAX3
        if(ok && aurostd::FileExist(dir+"/OUTCAR.relax3.bz2")) { // relax3 etc etc...
          if(ok) {oss_buffer << ".";if(!aurostd::FileExist(dir+"/OUTCAR.relax3.bz2")) {ok=FALSE;print=FALSE;oss_buffer << " no=OUTCAR.relax3.bz2";}}
          if(ok) {oss_buffer << ".";if(!aurostd::FileExist(dir+"/OSZICAR.relax3.bz2")) {ok=FALSE;oss_buffer << " no=OSZICAR.relax3.bz2";}}
          if(ok) {oss_buffer << ".";if(!aurostd::FileExist(dir+"/vasp.out.relax3.bz2")) {ok=FALSE;oss_buffer << " no=vasp.out.relax3.bz2";}}
          if(ok) {oss_buffer << ".";if(!aurostd::FileExist(dir+"/EIGENVAL.relax3.bz2")) {ok=FALSE;oss_buffer << " no=EIGENVAL.relax3.bz2";}}
          // OUTCAR.relax3.bz2
          if(ok) {oss_buffer << ".";  // check Answer 4 or 5 in OUTCAR.relax3.bz2
            answer=aurostd::execute2utype<int>(XHOST.command("bzgrep")+" -c \"(sec)\" "+dir+"/OUTCAR.relax3.bz2");
            if(answer!=4 && answer!=5) {ok=FALSE;oss_buffer << " error(" << answer << ")=OUTCAR.relax3.bz2";}}
          // vasp.out.relax3.bz2
          if(ok) {oss_buffer << ".";
            answer=aurostd::execute2utype<int>(XHOST.command("bzgrep")+" -c \"The distance between some ions is very small\" "+dir+"/vasp.out.relax3.bz2");
            if(answer!=0) {ok=FALSE;oss_buffer << " ions=vasp.out.relax3.bz2";}}
        }
        // TEST STATIC
        if(ok && aurostd::FileExist(dir+"/OUTCAR.static.bz2")) { // static etc etc...
          // OUTCAR.static.bz2
          if(ok && aurostd::FileExist(dir+"/OUTCAR.static.bz2")) {oss_buffer << ".";  // check Answer 4 or 5 in OUTCAR.static.bz2
            answer=aurostd::execute2utype<int>(XHOST.command("bzgrep")+" -c \"(sec)\" "+dir+"/OUTCAR.static.bz2");
            if(answer!=4 && answer!=5) {ok=FALSE;oss_buffer << " error(" << answer << ")=OUTCAR.static.bz2";}}
          // vasp.out.static.bz2
          if(ok && aurostd::FileExist(dir+"/vasp.out.static.bz2")) {oss_buffer << ".";
            answer=aurostd::execute2utype<int>(XHOST.command("bzgrep")+" -c \"The distance between some ions is very small\" "+dir+"/vasp.out.static.bz2");
            if(answer!=0) {ok=FALSE;oss_buffer << " ions=vasp.out.static.bz2";}}
          // EIGENVAL.static.bz2
          if(ok && aurostd::FileExist(dir+"/OUTCAR.static.bz2")) {oss_buffer << ".";  // check existence EIGENVAL.static.bz2
            if(!aurostd::FileExist(dir+"/EIGENVAL.static.bz2")) {ok=FALSE;oss_buffer << " no=EIGENVAL.static.bz2";}}
          // PROCAR.static.bz2
          //      if(ok && aurostd::FileExist(dir+"/OUTCAR.static.bz2")) {oss_buffer << ".";  // check existence PROCAR.static.bz2
          //	if(!aurostd::FileExist(dir+"/PROCAR.static.bz2")) {ok=FALSE;oss_buffer << " no=PROCAR.static.bz2";}}
          // DOSCAR.static.bz2
          if(ok && aurostd::FileExist(dir+"/OUTCAR.static.bz2")) {oss_buffer << ".";  // check existence DOSCAR.static.bz2
            if(!aurostd::FileExist(dir+"/DOSCAR.static.bz2")) {ok=FALSE;oss_buffer << " no=DOSCAR.static.bz2";}}
        }
        // TEST BANDS
        if(ok && aurostd::FileExist(dir+"/OUTCAR.bands.bz2")) { // bands etc etc...
          // OUTCAR.bands.bz2
          if(ok && aurostd::FileExist(dir+"/OUTCAR.bands.bz2")) {oss_buffer << ".";  // check Answer 4 or 5 in OUTCAR.bands.bz2
            answer=aurostd::execute2utype<int>(XHOST.command("bzgrep")+" -c \"(sec)\" "+dir+"/OUTCAR.bands.bz2");
            if(answer!=4 && answer!=5) {ok=FALSE;oss_buffer << " error(" << answer << ")=OUTCAR.bands.bz2";}}
          // vasp.out.bands.bz2
          if(ok && aurostd::FileExist(dir+"/vasp.out.bands.bz2")) {oss_buffer << ".";
            answer=aurostd::execute2utype<int>(XHOST.command("bzgrep")+" -c \"The distance between some ions is very small\" "+dir+"/vasp.out.bands.bz2");
            if(answer!=0) {ok=FALSE;oss_buffer << " ions=vasp.out.bands.bz2";}}
          // EIGENVAL.bands.bz2
          if(ok && aurostd::FileExist(dir+"/OUTCAR.bands.bz2")) {oss_buffer << ".";  // check existence EIGENVAL.bands.bz2
            if(!aurostd::FileExist(dir+"/EIGENVAL.bands.bz2")) {ok=FALSE;oss_buffer << " no=EIGENVAL.bands.bz2";}}
        }

        // DONE
        if(ok==TRUE) oss_buffer << " good";
        if(ok==FALSE) oss_buffer << " bad";
        oss_buffer << endl;
        if(print==TRUE) oss << oss_buffer.str();
        oss.flush();
        return ok;
      }
    }
    ...
    namespace aflowlib {
      bool XPLUG(vector<string> argv) {
        ...
        deque<bool> vok;
        deque<uint> vrun;
        vector<string> vrem_name; vrem_name.push_back("OUTCAR");vrem_name.push_back("OSZICAR");vrem_name.push_back("EIGENVAL");
        vector<string> vrem_extn; vrem_extn.push_back(".relax1.bz2");vrem_extn.push_back(".relax2.bz2");vrem_extn.push_back(".relax3.bz2");vrem_extn.push_back(".static.bz2");vrem_extn.push_back(".bands.bz2");
        for(uint i=0;i<argv.size();i++) {
          ...
          for(uint i1=0;i1<vrem_name.size();i1++) 
            for(uint i2=0;i2<vrem_extn.size();i2++) 
              if(aurostd::substring2bool(argv.at(i),vrem_name.at(i1)+vrem_extn.at(i2)))
                vdirs.push_back(aurostd::RemoveSubString(argv.at(i),"/"+vrem_name.at(i1)+vrem_extn.at(i2)));
        }
        ...
        if(vzips.size()>0) {
          ...
          for(uint i=0;i<vzips.size();i+=AFLOW_MAX_ARGV) {
            command << "aflow --multizip " << (FLAG_DO_ADD?"--add ":"") << "--np=" << NUM_ZIP 
    ...
    namespace aflowlib {
      bool VaspFileExist(const string& str_dir, const string& FILE) {
        ...
        if(!RUN_FLAG && aurostd::FileExist(str_dir+"/"+FILE+".bz2")) RUN_FLAG=TRUE;
        if(!RUN_FLAG && aurostd::FileExist(str_dir+"/"+FILE+".gz")) RUN_FLAG=TRUE;
        if(!RUN_FLAG && aurostd::FileExist(str_dir+"/"+FILE+".static")) RUN_FLAG=TRUE;
        if(!RUN_FLAG && aurostd::FileExist(str_dir+"/"+FILE+".static.bz2")) RUN_FLAG=TRUE;
        if(!RUN_FLAG && aurostd::FileExist(str_dir+"/"+FILE+".static.gz")) RUN_FLAG=TRUE;
        if(!RUN_FLAG && aurostd::FileExist(str_dir+"/"+FILE+".bands")) RUN_FLAG=TRUE;
        if(!RUN_FLAG && aurostd::FileExist(str_dir+"/"+FILE+".bands.bz2")) RUN_FLAG=TRUE;
        if(!RUN_FLAG && aurostd::FileExist(str_dir+"/"+FILE+".bands.gz")) RUN_FLAG=TRUE;
        if(!RUN_FLAG) {
          cerr<< FILE+" or "+FILE+".static/bands or "+FILE+".static/bands.bz2 not found in the directory!"<<endl;
        }
        return RUN_FLAG;
      }
    }
    ...
    namespace aflowlib {
      uint _OLD_vaspfile2stringstream(const string& str_dir, const string& FILE, stringstream& sss) {
        ...
        if(!gfound && (FILE=="EIGENVAL" || FILE=="KPOINTS")) {
          ...
          if(aurostd::FileExist(str_dir+"/"+FILE)) {sss << aurostd::execute2string(XHOST.command("cat")+" "+ str_dir+"/"+FILE);return sss.str().length();}
          if(aurostd::FileExist(str_dir+"/"+FILE+".bz2")) {sss << aurostd::execute2string(XHOST.command("bzcat")+" "+ str_dir+"/"+FILE+".bz2");return sss.str().length();}
          if(aurostd::FileExist(str_dir+"/"+FILE+".gz")) {sss << aurostd::execute2string(XHOST.command("zcat")+" "+ str_dir+"/"+FILE+".gz");return sss.str().length();}
          if(aurostd::FileExist(str_dir+"/"+FILE+".zip")) {sss << aurostd::execute2string(XHOST.command("zcat")+" "+ str_dir+"/"+FILE+".zip");return sss.str().length();}
          if(aurostd::FileExist(str_dir+"/"+FILE+".bands")) {sss << aurostd::execute2string(XHOST.command("cat")+" "+ str_dir+"/"+FILE+".bands");return sss.str().length();}
          if(aurostd::FileExist(str_dir+"/"+FILE+".bands.bz2")) {sss << aurostd::execute2string(XHOST.command("bzcat")+" "+ str_dir+"/"+FILE+".bands.bz2");return sss.str().length();}
          if(aurostd::FileExist(str_dir+"/"+FILE+".bands.gz")) {sss << aurostd::execute2string(XHOST.command("zcat")+" "+ str_dir+"/"+FILE+".bands.gz");return sss.str().length();}
          if(aurostd::FileExist(str_dir+"/"+FILE+".bands.zip")) {sss << aurostd::execute2string(XHOST.command("zcat")+" "+ str_dir+"/"+FILE+".bands.zip");return sss.str().length();}
          cerr<< FILE+" or "+FILE+".bands or "+FILE+".bands.bz2 not found in the directory, aborting!"<<endl;
          exit(1);
        }
        if(!gfound && (FILE=="DOSCAR")) { 
          ...
          if(aurostd::FileExist(str_dir+"/"+FILE+".gz")) {sss << aurostd::execute2string(XHOST.command("zcat")+" "+ str_dir+"/"+FILE+".gz");return sss.str().length();}
          if(aurostd::FileExist(str_dir+"/"+FILE+".zip")) {sss << aurostd::execute2string(XHOST.command("zcat")+" "+ str_dir+"/"+FILE+".zip");return sss.str().length();}
          if(aurostd::FileExist(str_dir+"/"+FILE+".static")) {sss << aurostd::execute2string(XHOST.command("cat")+" "+ str_dir+"/"+FILE+".static");return sss.str().length();}
          if(aurostd::FileExist(str_dir+"/"+FILE+".static.bz2")) {sss << aurostd::execute2string(XHOST.command("bzcat")+" "+ str_dir+"/"+FILE+".static.bz2");return sss.str().length();}
          if(aurostd::FileExist(str_dir+"/"+FILE+".static.gz")) {sss << aurostd::execute2string(XHOST.command("zcat")+" "+ str_dir+"/"+FILE+".static.gz");return sss.str().length();}
          if(aurostd::FileExist(str_dir+"/"+FILE+".static.zip")) {sss << aurostd::execute2string(XHOST.command("zcat")+" "+ str_dir+"/"+FILE+".static.zip");return sss.str().length();}
          cerr<< FILE+" or "+FILE+".static or "+FILE+".static.bz2 not found in the directory, aborting!" << endl;
          exit(1);
        }
        if(!gfound && (FILE=="POSCAR")) {
          ...
          if(aurostd::FileExist(str_dir+"/"+FILE+".static.gz")) {sss << aurostd::execute2string(XHOST.command("zcat")+" "+ str_dir+"/"+FILE+".static.gz");return sss.str().length();}
          if(aurostd::FileExist(str_dir+"/"+FILE+".static.zip")) {sss << aurostd::execute2string(XHOST.command("zcat")+" "+ str_dir+"/"+FILE+".static.zip");return sss.str().length();}
          if(aurostd::FileExist(str_dir+"/"+FILE+".relax1")) {sss << aurostd::execute2string(XHOST.command("cat")+" "+ str_dir+"/"+FILE+".relax1");return sss.str().length();}
          if(aurostd::FileExist(str_dir+"/"+FILE+".relax1.bz2")) {sss << aurostd::execute2string(XHOST.command("bzcat")+" "+ str_dir+"/"+FILE+".relax1.bz2");return sss.str().length();}
          if(aurostd::FileExist(str_dir+"/"+FILE+".relax1.gz")) {sss << aurostd::execute2string(XHOST.command("zcat")+" "+ str_dir+"/"+FILE+".relax1.gz");return sss.str().length();}
          if(aurostd::FileExist(str_dir+"/"+FILE+".relax1.zip")) {sss << aurostd::execute2string(XHOST.command("zcat")+" "+ str_dir+"/"+FILE+".relax1.zip");return sss.str().length();}
          cerr<< FILE+" or "+FILE+".bands/static/relax or "+FILE+".bands./static/relax bz2 not found in the directory, aborting!"<<endl;
          exit(1);
        }
        if(!gfound) {
          ...
          if(aurostd::FileExist(str_dir+"/"+FILE+".bands.gz")) {sss << aurostd::execute2string(XHOST.command("zcat")+" "+ str_dir+"/"+FILE+".bands.gz");return sss.str().length();}
          if(aurostd::FileExist(str_dir+"/"+FILE+".bands.zip")) {sss << aurostd::execute2string(XHOST.command("zcat")+" "+ str_dir+"/"+FILE+".bands.zip");return sss.str().length();}
          cerr<< FILE+" or "+FILE+".static/relax1/bands or "+FILE+".static/relax1/bands.bz2 not found in the directory, aborting!" << endl;
          exit(1);
        }
    ...
    namespace aflowlib {
      string vaspfile2stringstream(const string& str_dir, const string& FILE, stringstream& sss) {
        ...
        if(!gfound && (FILE=="EIGENVAL" || FILE=="KPOINTS")) {
          ...
          string postfix=".bz2,.gz,.bands,.bands.bz2,.bands.gz";
          deque<string> tokens;aurostd::string2tokens(postfix,tokens,",");tokens.push_front(""); // have to add emptyness to tokens at the beginning
          //     if(LDEBUG) aurostd::execute("ls -las "+str_dir);
          for(uint i=0;i<tokens.size();i++) {
            if(LDEBUG) cerr << "aflowlib::vaspfile2stringstream: TESTING FILE=[" << str_dir+"/"+FILE+tokens.at(i) << "]" << endl;
            if(aurostd::FileExist(str_dir+"/"+FILE+tokens.at(i))) {
              aurostd::efile2stringstream(str_dir+"/"+FILE+tokens.at(i),sss);
              if(LDEBUG) cerr << "aflowlib::vaspfile2stringstream: FOUND FILE=[" << str_dir+"/"+FILE+tokens.at(i) << "]" << endl;
              return aurostd::CleanFileName(str_dir+"/"+FILE+tokens.at(i));
            }
          }
          cerr<< FILE+" or "+FILE+".bands or "+FILE+".bands.bz2 not found in the directory, aborting!"<<endl;
          exit(1);
        ...
        if(!gfound && (FILE=="DOSCAR")) { 
          gfound=TRUE;
          if(LDEBUG) cerr << "aflowlib::vaspfile2stringstream: FILE=" << FILE << endl;
          string postfix=".bz2,.gz,.static,.static.bz2,.static.gz";
          deque<string> tokens;aurostd::string2tokens(postfix,tokens,",");tokens.push_front("");  // have to add emptyness to tokens at the beginning
          //     if(LDEBUG) aurostd::execute("ls -las "+str_dir);
          for(uint i=0;i<tokens.size();i++) {
            if(LDEBUG) cerr << "aflowlib::vaspfile2stringstream: TESTING FILE=[" << str_dir+"/"+FILE+tokens.at(i) << "]" << endl;
        ...
        if(!gfound && (FILE=="POSCAR")) {
          gfound=TRUE;
          if(LDEBUG) cerr << "aflowlib::vaspfile2stringstream: FILE=" << FILE << endl;
          string postfix=",.bz2,.gz,.bands,.bands.bz2,.bands.gz,.static,.static.bz2,.static.gz,.relax1,.relax1.bz2,.relax1.gz";
          deque<string> tokens;aurostd::string2tokens(postfix,tokens,",");tokens.push_front(""); // have to add emptyness to tokens at the beginning
          //     if(LDEBUG) aurostd::execute("ls -las "+str_dir);
          for(uint i=0;i<tokens.size();i++) {
            if(LDEBUG) cerr << "aflowlib::vaspfile2stringstream: TESTING FILE=[" << str_dir+"/"+FILE+tokens.at(i) << "]" << endl;
        ...
        if(!gfound) {
          if(LDEBUG) cerr << "aflowlib::vaspfile2stringstream: FILE=" << FILE << endl;
          string postfix=".bz2,.gz,.static,.static.bz2,.static.gz,.relax1,.relax1.bz2,.relax1.gz,.bands,.bands.bz2,.bands.gz";
          deque<string> tokens;aurostd::string2tokens(postfix,tokens,",");tokens.push_front(""); // have to add emptyness to tokens at the beginning
          //     if(LDEBUG) aurostd::execute("ls -las "+str_dir);
          for(uint i=0;i<tokens.size();i++) {
            if(LDEBUG) cerr << "aflowlib::vaspfile2stringstream: TESTING FILE=[" << str_dir+"/"+FILE+tokens.at(i) << "]" << endl;
