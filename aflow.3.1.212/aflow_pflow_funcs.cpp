...
xvector<double> balanceChemicalEquation(const xmatrix<double>& _composition_matrix,bool normalize,double tol){
  ...
  for(int i=1;i<coef.rows+1;i++){
    if(coef[i]<-_ZERO_TOL_){
      cerr << soliloquy << " ERROR! found negative coef[" << i << "], invalid chemical equation (run with --debug to see): coef=" << coef << endl;exit(1);
    }
  }
  ...
  for(int i=1;i<_composition_matrix.cols+1;i++){
    ...
    if(abs(sum)>_ZERO_TOL_){cerr << soliloquy << " ERROR! chemical equation was not balanced (run with --debug to see): coef=" << coef << endl;exit(1);}
  }
  return coef;
}
