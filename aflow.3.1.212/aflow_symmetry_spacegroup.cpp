  ...
  xstructure ConventionalCell(xstructure& xstr, int& IT, int& cell_choice, bool& last_orientation, string& crystalsystem_prev,
	...
	// =============== HEXAGONAL =============== //
	  ...
	  if(!findTrigonalLattice(rot_lattice_vectors, rot_ops_vec, big_expanded, candidate_lattice_vectors, candidate_lattice_chars)) {
	    ...
	    skew = SYM::isLatticeSkewed(candidate_lattice_vectors[0], xstr_out.dist_nn_min, _SYM_TOL_);
	    ...
	    if(bravais_count == 3){ // if three lattice points in the cell, then the system is rhl
	      ...
	      if(!findRhombohedralSetting(big_expanded, candidate_lattice_vectors, candidate_lattice_chars)) {
		symmetryfound = false;
		return xstr_out;
	      }
	    } 
	...
	//else if(((mcount == 1 || mcount == 2 || mcount == 3) && three_six_count==0) || crystalsystem == "MONOCLINIC") { //DX 20180613 - can be orthogonal and still be monoclinic 
	  ...
	  if(!findMonoclinicLattice(mirror_lattice_vectors, twofold_lattice_vectors, big_expanded,
				     candidate_lattice_vectors, candidate_lattice_chars, cell_choice)) { //DX 20180816 - added cell choice
	    symmetryfound = false;
	    return xstr_out;
	  }
	}
...
namespace SYM {
  bool findMonoclinicLattice(vector<xvector<double> >& mirror_lattice_vectors, vector<xvector<double> >& twofold_lattice_vectors,
			     ...
			     vector<char>& candidate_lattice_chars, int& cell_choice) { //DX 20180816 - added setting_choice
	...
	for (uint j = i + 1; j < possible_efg_vectors.size(); j++) {
	  for (uint k = j + 1; k < possible_efg_vectors.size(); k++) {
		...
		if(!SYM::checkAngle(tranvec_e, tranvec_f, Pi_r, _SYM_TOL_) &&
		  ...
		  int consistent = 0;
		  if((cell_choice == 0 || cell_choice == 1) && aurostd::det(xvec2xmat(tranvec_f, h_lat, tranvec_e)) > 0.0 && aurostd::det(xvec2xmat(tranvec_g, h_lat, tranvec_f)) > 0.0 &&
		      aurostd::det(xvec2xmat(tranvec_e, h_lat, tranvec_g)) > 0.0) { //DX 20180816 - added cell choice; check axis b
		    consistent = 3;
		  }
		  else if(cell_choice == 2 && aurostd::det(xvec2xmat(tranvec_f, h_lat, tranvec_e)) > 0.0 && aurostd::det(xvec2xmat(tranvec_g, h_lat, tranvec_f)) > 0.0 &&
    ...
    if(cell_choice == 0 || cell_choice == 1){
...
namespace SYM {
  bool findRhombohedralSetting(vector<xvector<double> >& big_expanded, vector<xmatrix<double> >& candidate_lattice_vectors,
    ...
    for (uint i = 0; i < big_expanded.size(); i++) {
      if(aurostd::abs(aurostd::modulus(big_expanded[i]) - mod_aprime) < _SYM_TOL_){
    ...
    for (uint i = 0; i < candidate_rhl_setting_vectors.size(); i++) {
      ...
      for (uint j = 0; j < candidate_rhl_setting_vectors.size(); j++) {
        if(j!=i){
	      ...
	      if(aurostd::abs(aurostd::scalar_product(tmpa,aurostd::vector_product(tmpb,tmpc)))>_SYM_TOL_){
		...
		if(SYM::checkAngle(modb, modc, tmpalpha, alpha_prime, _SYM_TOL_) &&
		  SYM::checkAngle(moda, modc, tmpbeta, alpha_prime, _SYM_TOL_) &&
		  SYM::checkAngle(moda, modb, tmpgamma, alpha_prime, _SYM_TOL_) && 
		  tmpalpha>_ZERO_TOL_ && tmpbeta>_ZERO_TOL_ && tmpgamma>_ZERO_TOL_ &&
                  aurostd::scalar_product(tmpa,hex_c)>_SYM_TOL_ &&                    // we want to orient the rhl cell to be in the positive direction of hex-c
                  aurostd::scalar_product(tmpb,hex_c)>_SYM_TOL_ &&                    // we want to orient the rhl cell to be in the positive direction of hex-c
                  aurostd::scalar_product(tmpc,hex_c)>_SYM_TOL_) {                    // we want to orient the rhl cell to be in the positive direction of hex-c
		  count++;
		  if(count > 1) {
		    if(aurostd::modulus(tmpa) <= min_norm) {
		      min_norm = aurostd::modulus(tmpa);
...
//  bool first_wyckoff = true;
  ...
  while (foundspacegroup == false) {
    ...
    if(cell_choice == 1 && CCell.lattice_label_ITC == 'r'){ //DX 20180807
    ...
    for (uint j = 0; j < sgindex.size(); j++) {
      if(SYM::invec<int>(sg_search, sgindex[j])) {
        //DEBUGGER:
        if(LDEBUG){
          cerr << "SYM::SpaceGroup_ITC: Checking generators for space group: " << sgindex[j] << " (with cell choice = " << axis_cell.str() << ")." <<  endl;
        }
        //cerr << "////////////////////////////////////////" << endl;
        ...
        if(match_count == generators[j].size() * (centeringgroups)) {
          if(LDEBUG){
            cerr << "SYM::SpaceGroup_ITC: Generator reduction matches for space group: " << sgindex[j] << endl;
          }
          ...
          for (uint ix = 0; ix < CG.size(); ix++) {
            ...
            if(SYM::solve_overdetermined_system(LHS, RHS, OriginShift, CCell.lattice, CCell.dist_nn_min)) {
              ...
              if(LDEBUG){
                cerr << "SYM::SpaceGroup_ITC: Generators and origin shift of space group #" << sgindex[j] << " COULD NOT be matched to ITC [dir=" << xstr.directory << "]." << endl;
              }
            }
