...
#define DOD_CONRAD_DEFAULT_KILL_MEM_CUTOFF 1.50 //DX 5/2/18 - DoD CONRAD
...
namespace KBIN {
  int KBIN_Main(vector<string> argv) {        // AFLOW_FUNCTION_IMPLEMENTATION
    ...
    // "MACHINE::CONRAD"
    aflags.AFLOW_MACHINE_GLOBAL.flag("MACHINE::DOD_CONRAD",aurostd::args2flag(argv,"--machine=conrad|--machine=dod_conrad"));
    if(aflags.AFLOW_MACHINE_GLOBAL.flag("MACHINE::DOD_CONRAD")) XHOST.maxmem=DOD_CONRAD_DEFAULT_KILL_MEM_CUTOFF;
	...
	if(!FileAFLOWIN) {                                                                                      // ******* _AFLOWIN_ does not exist
	  ...
	  if(aflags.AFLOW_MACHINE_LOCAL.flag("MACHINE::MPCDF_HYDRA")) {
	    ...
	    kflags.KBIN_MPI=TRUE; // overrides the MPI for machines
          ...
          //DX 5/2/18 - DoD CONRAD - START
	  // dod_conrad	
	  if(aflags.AFLOW_MACHINE_GLOBAL.flag("MACHINE::DOD_CONRAD") ||
	     aurostd::substring2bool(AflowIn,"[AFLOW_HOST]DOD_CONRAD") || 
	     aurostd::substring2bool(AflowIn,"[AFLOW_HOST]DOD_CONRAD"))   // check DOD_CONRAD
	    aflags.AFLOW_MACHINE_LOCAL=aflags.AFLOW_MACHINE_GLOBAL;
