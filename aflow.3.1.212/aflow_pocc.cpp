...
//[moved aflow.h]//tolerances
//[moved aflow.h]const double ZERO_TOL = pow(10,-PRECISION);
...
namespace pocc {
  ...
  void generateStructures(const _xvasp& in_xvasp,const string& AflowIn,const _aflags& in_aflags,const _kflags& in_kflags,const _vflags& in_vflags, ofstream& FileMESSAGE, ostream& oss) {
    ...
    if(!pcalc.calculate()){throw aurostd::xerror(soliloquy,"POccCalculator failed to calculate");}
    ...
    for(unsigned long long int i=0;i<pcalc.getUniqueDerivativeStructuresCount();i++) {
      ...
      xvasp.aopts.flag("FLAG::AVASP_AUTO_PSEUDOPOTENTIALS",TRUE);
      //xvasp.POTCAR_TYPE_DATE_PRINT_flag=true; //CO 180622
      //xvasp.AVASP_prototype_mode=LIBRARY_MODE_ARUN; //special mode for ARUN's
      xvasp.Directory=arun_directory;
      
      //if(LDEBUG){cerr << soliloquy << " producing/modifying input for structure[" << i << "]" << endl;}
      XHOST.QUIET=true;
      //not needed, we are simply creating aflow.in, not separate inputs (INCAR, POTCAR, etc.)
      //if(!KBIN::VASP_Produce_and_Modify_INPUT(xvasp,AflowIn,FileMESSAGE,aflags,kflags,vflags,true)){  //load from xvasp.str
      //  throw aurostd::xerror(soliloquy,"Cannot produce/modify input");
      //}
      if(LDEBUG){
      ...
      //CO 180622 - load in some options from aflow.in - START
      ...
      //CO 180622 - load in some options from aflow.in - STOP
...
void POccCalculator::setPOccStructure(const xstructure& xstr_pocc) {
  p_str.initialize(xstr_pocc,m_aflags,m_kflags,*p_FileMESSAGE,*p_oss); //p_str.setPOccStructure(xstr_pocc);
  ...
  for(uint i=starting_index;i<vprds.size();i++){
    ...
    for(uint j=0;j<unique_structure_bins.size();j++){
      if(unique_structure_bins[j].size()==0){
        throw aurostd::xerror(soliloquy,"No structure groups found");
      }
...
xstructure POccCalculator::getUniqueDerivativeStructure(unsigned long long int i) {
  ...
  if(i>l_derivative_structures_sets.size()-1){
    throw aurostd::xerror(soliloquy,"Invalid derivative structure index");
  }
...
void POccStructure::preparePOccStructure() {
  ...
  for(uint i=0;i<types2pc_map.size();i++){
    ...
    if(!getUFFParameters(element,uff_parameters_string)){throw aurostd::xerror(soliloquy,"Unable to fetch UFF parameters (requested "+element+")");}
...
void POccStructure::partitionPOccSites() {
  ...
  for(uint i=0;i<xstr_pocc.atoms.size();i++){
    ...
    for(uint j=0;j<v_sites.size() && new_site;j++){
      if(v_sites[j].v_occupants.size() && aurostd::modulus(xstr_pocc.atoms[i].cpos-xstr_pocc.atoms[v_sites[j].v_occupants[0]].cpos)<_AFLOW_POCC_ZERO_TOL_){
    ...
    if(new_site){
      ...
      v_sites.back().partial_occupation_flag=!aurostd::isequal(xstr_pocc.atoms[i].partial_occupation_value,1.0,_AFLOW_POCC_ZERO_TOL_); //if the only other occupant is a vacancy
    }
  }
  pocc_atoms_total=0;
    ...
    for(uint occ=0;occ<v_sites[site].v_occupants.size();occ++){
      ...
      for(uint k=0;k<vv_pocc_groups[site].size() && new_site;k++){
        if(aurostd::isequal(xstr_pocc.atoms[v_sites[site].v_occupants[occ]].partial_occupation_value,\
              vv_pocc_groups[site][k].partial_occupation_value,_AFLOW_POCC_ZERO_TOL_)){
...
string POccStructure::hnfTableHeader(){
  ...
  ss_header << " | " << header_max_site_error;
  ss_header << " | " << header_max_stoich_error << endl;
...
string POccStructure::hnfTableLineOutput(int i_hnf,int str_config){
      ...
      for(int pocc_group=0;pocc_group<site_config.xv_occupation_count_input.rows;pocc_group++){
        for(int pocc_atom=0;pocc_atom<site_config.xv_occupation_count_input[pocc_group];pocc_atom++){
          ...
          if(!aurostd::isequal(site_config.xv_occupation_multiple[pocc_group]/((double)i_hnf),
                site_config.xv_partial_occupation_value[pocc_group],_ZERO_TOL_)){ //hard zero
  ...
  //line_output << " | " << ( site_error < _AFLOW_POCC_ZERO_TOL_ ? 0.0 : stoich_error ) << endl;
  //line_output << " | " << ( site_error < _AFLOW_POCC_ZERO_TOL_ ? 0.0 : site_error ) << endl;
  ausf.str(""); ausf << site_error;
  line_output << " | " << aurostd::PaddedPOST(ausf.str(),header_max_site_error.size()); //hnf_table_error_padding);
...
bool POccStructure::calculateNHNF(){
  ...
  //setHNFTablePadding(_AFLOW_POCC_PRECISION_);
  ...
  /*for(uint iatom=0;iatom<xstr_pocc.atoms.size();iatom++) {
    if(!aurostd::isequal(xstr_pocc.atoms[iatom].partial_occupation_value,1.0,_AFLOW_POCC_ZERO_TOL_)) {
