...
namespace pflow {
  int main(vector<string> &argv,vector<string> &cmds) {
	...
	_PROGRAMRUN=true;
      ...
      if(vpflow.flag("CIF")) {pflow::CIF(cin,vpflow); _PROGRAMRUN=true;} //DX 20180806 - added vpflow
...
namespace pflow {
  ...
  "<< x<<" --convex_hull=|--chull --alloy=MnPdPt[,AlCuZn,...] [chull_options] [--destination=[DIRECTORY]] \n\
    chull_options: \n\
                 ...
                 GENERAL OPTIONS: \n\
                 --usage \n\
                 --output=|--o=|--print=|--p=latex|pdf|png|json|text \n\
                 --screen_only \n\
...
namespace pflow {
  ...
  string BZDirectionsSTRUCTURE(istream& input, aurostd::xoption& vpflow) {
    ...
    if(!vpflow.flag("BZDIRECTION::TRANSFORM2ORIGINAL")){
      a.transform_coordinates_original2new=aurostd::eyes<double>();
    }
    ...
    return LATTICE::KPOINTS_Directions(a.bravais_lattice_variation_type,a.lattice,a.transform_coordinates_original2new,grid,a.iomode,foundBZ); //DX 20181101
  }
} // namespace pflow
...
namespace pflow {
  bool PROTO_AFLOW(aurostd::xoption vpflow,bool flag_REVERSE) { // too many options
    ...
    // check bands
    if(LDEBUG) cerr << soliloquy << " CHECK BANDS" << endl; 
    PARAMS.vparams.flag("AFLOWIN_FLAG::BANDS",vpflow.flag("PROTO_AFLOW::BANDS_CALCULATION"));
    if(LDEBUG) cerr << soliloquy << " PARAMS.vparams.flag(\"AFLOWIN_FLAG::BANDS\")=" << PARAMS.vparams.flag("AFLOWIN_FLAG::BANDS") << endl;
    ...
    if(LDEBUG) cerr << soliloquy << " CHECK NO_VOLUME_ADJUSTMENT" << endl;
    PARAMS.vparams.flag("AFLOWIN_FLAG::NO_VOLUME_ADJUSTMENT",vpflow.flag("PROTO_AFLOW::NO_VOLUME_ADJUSTMENT"));
    if(LDEBUG) cerr << soliloquy << " PARAMS.vparams.flag(\"AFLOWIN_FLAG::NO_VOLUME_ADJUSTMENT\")=" << PARAMS.vparams.flag("AFLOWIN_FLAG::NO_VOLUME_ADJUSTMENT") << endl;
    ...
    if(LDEBUG) cerr << soliloquy << " CHECK VASP/QE/ABINIT/AIMS" << endl; 
    ...
    if(LDEBUG) cerr << soliloquy << " PARAMS.vparams.flag(\"AFLOWIN_FLAG::NO_VOLUME_ADJUSTMENT\")=" << PARAMS.vparams.flag("AFLOWIN_FLAG::NO_VOLUME_ADJUSTMENT") << endl; // CO 180214
    if(LDEBUG) cerr << soliloquy << " PARAMS.vparams.flag(\"AFLOWIN_FLAG::EDIFFG\")=" << PARAMS.vparams.flag("AFLOWIN_FLAG::EDIFFG") << endl;
...
namespace pflow {
  bool QMVASP(aurostd::xoption& vpflow) { //vector<string> argv) {  //CO 180703
    ...
    if(found_run){
      for(uint i=0;i<vCARs.size();i++) {aurostd::RemoveFile(directory+"/"+vCARs[i]);} //aurostd::execute("rm -f "+directory+"/"+vCARS.at(i)); }
    }
    for(uint i=0;i<vCARs.size();i++){
...
namespace pflow {
  string WYCCAR(aurostd::xoption& vpflow,istream& input) {
    ...
    str.SpaceGroup_ITC(tolerance,-1,setting,no_scan);
    ofstream File("/dev/null");
    bool verbose=TRUE;
