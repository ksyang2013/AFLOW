...
void PhononDispersionCalculator::writePDIS() {
  _logger << "Writing dispersion curves into file " << DEFAULT_APL_PDIS_FILE << "." << apl::endl;
  ...
  //CO - START
  aurostd::stringstream2file(outfile, DEFAULT_APL_PDIS_FILE);
  if (!aurostd::FileExist(DEFAULT_APL_PDIS_FILE)) {
    string function = "PhononDispersionCalculator::writePDIS()";
    string message = "Cannot open output file " + DEFAULT_APL_PDIS_FILE + ".";
...
//    throw apl::APLRuntimeError("Cannot open output PDIS file.");
  ...
  //PINKU //PN180705
  aurostd::stringstream2file(ouths, DEFAULT_APL_HSKPTS_FILE);
  if (!aurostd::FileExist(DEFAULT_APL_HSKPTS_FILE)) {
    string function = "PhononDispersionCalculator::writePDIS()";
    string message = "Cannot open output file " + DEFAULT_APL_HSKPTS_FILE + ".";
    throw aurostd::xerror(function, message, _FILE_ERROR_);
