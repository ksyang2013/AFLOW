...
namespace apl {
    /////////////////////////////////////////////////////////////////////////////
    //Kesong YANG fixes warnning message
    //Constructor
    ShellData::ShellData()
        : occupation(0), 
        ...
        radius(0.0), 
        stdevRadius(0.0) {
            index = std::vector<xvector<int>>();
            atoms = std::vector<std::deque<_atom>>();
            ratoms = std::vector<std::deque<_atom>>();
        }
    ...
    // Copy constructor
    ShellData::ShellData(const ShellData& that) {
        copyFrom(that);
    }
    ...
    // Helper function to copy data from another ShellData
    void ShellData::copyFrom(const ShellData& that) {
