...
void MonkhorstPackMesh::setDensity(int na, int nb, int nc)
{
    ...
    // Set up map //ME180705
    aurostd::xtensor<int> temp(_n); //ME180705
    _allToIrrPointMap = temp;

    //[OBSOLETE ME180705]// TODO: Be aware of the bug in aflow_xtensor.cpp in operator= function...
    //[OBSOLETE ME180705]if( _allToIrrPointMap.uindex[1] == 3 ||
    //[OBSOLETE ME180705]    _allToIrrPointMap.uindex[2] == 3 ||
    //[OBSOLETE ME180705]    _allToIrrPointMap.uindex[3] == 3 )
    //[OBSOLETE ME180705]{
    //[OBSOLETE ME180705]    throw APLRuntimeError("Be aware of the bug in aflow_xtensor.cpp in operator= function...");
    //[OBSOLETE ME180705]}
}
...
void MonkhorstPackMesh::generateAllGridPoints()
{
    ...
    */
    ...
    for(int p = 1; p <= _n(1); p++)
    {
        ...
        _kpoints.push_back(kpoint);
        _weights.push_back(1.0);
        _allToIrrPointMap[p][r][s] = id++; //ME180705
    }
}
...
void MonkhorstPackMesh::makeIrreducible(const vector<_sym_op>& pgroupk_xtal)
{
    ...
    for(int r = 1; r <= _n(2); r++)
    for(int p = 1; p <= _n(1); p++)
        _allToIrrPointMap[p][r][s] = map[id++]; //ME180705
    map.clear();
...
aurostd::xtensor<int> MonkhorstPackMesh::getAllToIrrPointMap() //ME180705
{
