...
namespace apl {
  ...
  void LinearTetrahedronMethod::generateTetrahedras() {
    ...
    // We need this map because we are loking for irreducible tetrahedra //ME180705
    aurostd::xtensor<int> allToIrrPointMap = _rg.getAllToIrrPointMap(); //ME180705
	...
	for(int i1 = 1; i1 <= _rg.getN(1); i1++) {
	  ...
	  aurostd::xtensor<int> tetraCornerToIrrPoint(2, 0); //ME180705
	  for(int k3 = 0; k3 <= 1; k3++) {
		...
		tetraCornerToIrrPoint[k1][k2][k3] = allToIrrPointMap[j1][j2][j3]; //ME180705
		//cout << k1 << k2 << k3 << " -> " << j1 << " " << j2 << " " << j3 << " -> " << tetraCornerToIrrPoint(k1,k2,k3) << std::endl;
	      }
	    }
	  }

	  // Get all 6 tetrahedra now... //ME180705
	  vector<int> tetraIrrPointID, tetrahedron(3); //ME180705
	  for(int i = 0; i < 6; i++) {
            tetraIrrPointID.clear();
            for(int j = 0; j < 4; j++) {
              for (int t = 1; t < 4; t++) { //ME180705
                tetrahedron[t-1] = (int)tetrahedra[i][j](t);
              }
	      tetraIrrPointID.push_back(tetraCornerToIrrPoint(tetrahedron)); //ME180705
	    }
