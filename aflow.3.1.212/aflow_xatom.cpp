...
// *                                                                         *
// ***************************************************************************
// Written by Stefano Curtarolo - 2007-2018
...
xstructure::xstructure(string structure_title) {
    ...
    scale=1.0;
    neg_scale=FALSE;
    scale_second=DEFAULT_PARTIAL_OCCUPATION_TOLERANCE; // CO 180409
    neg_scale_second=FALSE; // CO 180409
    scale_third.isentry=false;  // CO 170803 - site tol
    scale_third.content_double=DEFAULT_PARTIAL_OCCUPATION_TOLERANCE;  // CO 170803 - site tol
    ...
    // PARTIAL OCCUPATION -------------------
    partial_occupation_flag=FALSE;
    partial_occupation_site_tol=DEFAULT_PARTIAL_OCCUPATION_TOLERANCE;     // DEFAULT // CO 180409
    partial_occupation_stoich_tol=DEFAULT_PARTIAL_OCCUPATION_TOLERANCE;   // DEFAULT // CO 180409
...
bool sortAtomsTypes(const _atom& a1,const _atom& a2) {
    ...
    return a1.partial_occupation_value>a2.partial_occupation_value; //reverse, we actually want lowest type but highest occupation first
    //if(a1.type!=a2.type){return a1.type<a2.type;}
...
bool sortAtomsDist(const _atom& a1,const _atom& a2) {
    ...
    if(dist1!=dist2){return dist1<dist2;} //CO 180705 - maybe we need to consider adding tol here
    return sortAtomsTypes(a1,a2); //CO 180705
}
...
#define oss cout
                ...
                for(uint i=0;i<vinput.size();i++){
                    ...
                    if(tokens_line.size() && tokens_line[0]=="lattice_vector"){
                        ...
                        if(lat_count==1){lat1_found=true;}
                            ...
                            cerr << soliloquy << " AIMS input error, too many lattice vectors found" << endl;
                            exit(1);
                        }
                    }
                }
                if(!lat1_found || !lat2_found || !lat3_found){
                    cerr << soliloquy << " AIMS input error, incomplete lattice vector specification (needed if atom_frac found)" << endl;
                    exit(1);
                }
            }
        }
    ...
    if(a.iomode==IOVASP_AUTO || a.iomode==IOVASP_POSCAR || a.iomode==IOVASP_ABCCAR || a.iomode==IOVASP_WYCKCAR) { // VASP POSCAR
        ...
        if(a.iomode==IOVASP_POSCAR) {
            // cerr << soliloquy << " Chosen IOVASP_POSCAR" << endl;
        ...
        if(a.partial_occupation_flag){ //&& a.neg_scale_second) 
    ...
    if(a.iomode==IOQE_AUTO || a.iomode==IOQE_GEOM) {
        ...
        for(uint i=iline;i<vinput.size()&&i<jline;i++) {
            ...
            if(stmp_tokens.size()<4) {
                oss << "ERROR:  Insufficient number of atom entries in atom=" << i << "" << endl; // CO 180409
                exit(0);
            }  
            int id=0;
            string name;
    ...
    if(a.iomode==IOCIF) { // CIF
        ...
        for(uint setting_number=1;setting_number<=2;setting_number++){
            ...
            if(general_wyckoff_position.size()==spacegroup_symop_xyz.size()){
                ...
                if(match_count!=general_wyckoff_position.size()){
                    //oss << "ERROR: Symmetry operations do not match between input operations and space group number/option." << endl; 
                ...
                oss << "ERROR: Number of symmetry operations do not match between input operations and space group number (aflow=" 
        ...
        for(uint i=0;i<vinput.size();i++) {
            if(aurostd::substring2bool(vinput.at(i),"_atom_site")){
                ...
                if(tokens.size()==atom_site_fields.size()){
                    ...
                    cerr << "ERROR: Unexpected number of input fields based on _atom_site_[] information (tokens=" << tokens.size() << ", atom_sites_[]=" << atom_site_fields.size() << ")." <<  endl; 
                    exit(1);
                }
            }
        }
    ...
    if(a.is_vasp5_poscar_format==TRUE) {
        ...
        if(a.num_each_type.size()!=tokens.size()) {
            cerr << "ERROR in aflow_xatom.cpp you need to specify the same number of species and atoms types" << endl;
...
deque<_atom> foldAtomsInCell(deque<_atom>& atoms, xmatrix<double>& c2f_new, xmatrix<double>& f2c_new, bool& skew) {
    ...
    return foldAtomsInCell(atoms, c2f_new, f2c_new, skew, tol);
}

deque<_atom> foldAtomsInCell(xstructure& a, xmatrix<double>& lattice_new, bool& skew, double& tol, bool fold_in_only) {
    bool LDEBUG=(FALSE || XHOST.DEBUG);
...
void xstructure::buildGenericTitle(bool vasp_input,bool force_fix){
    ...
    for(uint i=0;i<atoms.size()&&atom_names;i++){if(atoms[i].cleanname.empty()){atom_names=false;}} // CO 180316
    for(uint itype=0;itype<num_each_type.size();itype++){
        for(uint j=0;j<(uint) num_each_type.at(itype);j++) {
            if(j==0){
                if(atom_names){title+=atoms.at(iat).cleanname;} // CO 180316
