...
namespace LATTICE {
    ...
    {//**************************JUNKAI EDITED*****************
        ...
        str_sp.transform_coordinates_original2new = aurostd::eyes<double>(); //DX 20181024 
        str_sp.rotate_lattice_original2new = aurostd::eyes<double>(); //DX 20181024 
        ...
        xmatrix<double> orig_lattice = str_sp.lattice; //DX 20181024 - save original lattice to find transformation
        xmatrix<double> original_metric_tensor = MetricTensor(orig_lattice);
        ...
        if(!aurostd::identical(original_metric_tensor,minkowski_metric_tensor)){
            str_sp.transform_coordinates_original2new=str_sp.lattice*aurostd::inverse(orig_lattice); 
        }
        else{
            str_sp.rotate_lattice_original2new=aurostd::inverse(orig_lattice)*str_sp.lattice; 
        }
    ...
    */  
    ...
    if(found){
        ...
        if(aurostd::abs(aurostd::abs(det(str_sp.scale*str_sp.lattice))-aurostd::abs(det(str_in.scale*str_in.lattice))) < eps_volume){ 
            ...
            str_sp.transform_coordinates_new2original = aurostd::eyes<double>();
            str_sp.transform_coordinates_original2new = aurostd::eyes<double>();
            str_sp.rotate_lattice_new2original = aurostd::eyes<double>();
            str_sp.rotate_lattice_original2new = aurostd::eyes<double>();
        }
    }
...
namespace LATTICE {
    ...
    bool Standard_Lattice_Structure_20170101(const xstructure& str_in,xstructure& str_sp,xstructure& str_sc,double eps,double epsang,int& time,double symeps,bool histogram) {//**************************JUNKAI EDITED*****************
        if(0) { cerr << "REMAKING LATTICE" << symeps << endl; exit(0); }
        bool LDEBUG=(FALSE || XHOST.DEBUG);
        if(LDEBUG) cerr << "LATTICE::Standard_Lattice_Structure: BEGIN" << endl;
...
namespace LATTICE {
    string KPOINTS_Directions(string lattice_type,xmatrix<double> sp, double _grid,int iomode,bool &foundBZ) {
        xmatrix<double> transformation_matrix = aurostd::eyes<double>();
