...
namespace AEL_functions {
  ...
  uint RunElastic_AEL(_xvasp& xvasp, string AflowIn, _aflags& aflags, _kflags& kflags, _vflags& vflags, _AEL_data& AEL_data, ofstream& FileMESSAGE) {
    ...
    aurostd::xoption USER_USESTABLEPRESSURE;             USER_USESTABLEPRESSURE.option = false;        
    ...
    if( aurostd::substring2bool(AflowIn,_AELSTROPT_+"USESTABLEPRESSURE=",TRUE) ) {
      USER_USESTABLEPRESSURE.options2entry(AflowIn,_AELSTROPT_+"USESTABLEPRESSURE=",TRUE);
      aurostd::StringstreamClean(aus);
      aus << _AELSTR_MESSAGE_ + "Use elastic constants at pressure where material is stable = " << USER_USESTABLEPRESSURE.option << endl;  
      aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
    } 	        
	...
	for (uint k = 0; k < vaspRunsPressures.size(); k++) {
      ...
      if(USER_PRESSURECALC.option && USER_USESTABLEPRESSURE.option) {
